...
namespace oomph
{
  ...
  public:
    ...
    double smart_time_step(double dt, const double& tol)
    {
      ...
      oomph_info << "Time for step " << Total_step_time << std::endl;
    ...
    void get_solver_parameters(SolverParameters& sp)
    {
      ...
      // Explicit solver
      sp.mass_matrix_solver_for_explicit_timestepper_pt =
    ...
    virtual void actions_before_newton_step()
    {
      ...
      if (to_lower(Doc_info.output_jacobian) == "always")
      {
        ...
        std::string label = to_string(Doc_info.number()) + "_" +
    ...
    virtual void actions_after_explicit_stage()
    {
      ...
      if (its_pt != 0)
      {
        ...
        Preconditioner_setup_times.push_back(
          its_pt->preconditioner_setup_time());
      }
      else
      {
        // Fill in dummy data
      ...
      for (unsigned msh = 0, nmsh = nsub_mesh(); msh < nmsh; msh++)
      {
        ...
        for (unsigned nd = 0, nnd = mesh_pt->nnode(); nd < nnd; nd++)
        {
          ...
          for (unsigned j = 0; j < indices.size(); j++)
          {
            ...
            {
              nd_pt->eqn_number(indices[j]) = Data::Is_segregated_solve_pinned;
            }
          }
        }
      }
    ...
    void check_norm_limits()
    {
      ...
      if (Error_norm_limit != -1.0)
      {
        ...
        if ((error_norm != Dummy_doc_data) && (error_norm > Error_norm_limit))
        {
    ...
    void final_doc()
    {
      ...
      std::ofstream pvd_file((Doc_info.directory() + "/" + "soln.pvd").c_str(),
                             std::ios::app);
      pvd_file << "</Collection>" << std::endl << "</VTKFile>" << std::endl;
      pvd_file.close();
      ...
      if (doc_exact())
      {
        std::ofstream pvd_file(
          (Doc_info.directory() + "/" + "exact.pvd").c_str(), std::ios::app);
        pvd_file << "</Collection>" << std::endl << "</VTKFile>" << std::endl;
        pvd_file.close();
      }
    ...
    void output_ltes(const unsigned& t_hist, std::ostream& output) const
    {
      ...
      for (unsigned i = 0, ni = mesh_pt()->nnode(); i < ni; i++)
      {
        ...
        for (unsigned j = 0; j < nd_pt->nvalue(); j++)
        {
          ...
          // at this point.
          double error =
            nd_pt->time_stepper_pt()->temporal_error_in_value(nd_pt, j);
    ...
    /// Should we write output for initial conditions as though they are time
    /// steps as well?
  ...
  void MyProblem::build(Vector<Mesh*>& bulk_mesh_pt)
  {
    ...
    if (fele_pt != 0)
    {
      ...
      {
        if (bulk_mesh_pt[j]->node_pt(0)->time_stepper_pt() != ts_pt)
        {
          std::string err =
    ...
    else
    {
      ...
      // ??ds again assumed any timesteppers hiding somewhere else are added
      // elsewhere
    ...
    {
      std::string warning = "Requested doc'ing exact solution, but we don't "
                            "have an exact solution function pointer.";
      OomphLibWarning(
        warning, OOMPH_CURRENT_FUNCTION, OOMPH_EXCEPTION_LOCATION);
    }
  }
  ...
  void MyProblem::initial_doc()
  {
    ...
    std::ofstream pvd_file((dir + "/" + "soln.pvd").c_str(), std::ios::out);
    ...
    if (doc_exact())
    {
      std::ofstream pvd_file((dir + "/" + "exact.pvd").c_str(), std::ios::out);
      pvd_file << "<?xml version=\"1.0\"?>" << std::endl
               << "<VTKFile type=\"Collection\" version=\"0.1\" "
                  "byte_order=\"LittleEndian\">"
               << std::endl
    ...
    trace_file << "DocInfo_numbers" << Trace_seperator << "times"
               << Trace_seperator << "dts" << Trace_seperator << "error_norms"

               << Trace_seperator << "n_newton_iters" << Trace_seperator
               << "n_solver_iters"

               << Trace_seperator << "solver_times" << Trace_seperator
               << "jacobian_setup_times" << Trace_seperator
               << "preconditioner_setup_times"

               << Trace_seperator << "LTE_norms" << Trace_seperator
               << "trace_values"

               << Trace_seperator << "unix_timestamp" << Trace_seperator
               << "newton_residuals" << Trace_seperator << "solution_norms"
               << Trace_seperator
               << "total_step_time"


               // Reserved slots in case I think of more things to add later
               << Trace_seperator << "dummy" << Trace_seperator << "dummy"
               << Trace_seperator << "dummy" << Trace_seperator << "dummy"
               << Trace_seperator << "dummy" << Trace_seperator << "dummy";
    ...
    info_file << "real_time " << real_date_time() << std::endl
                               ...
                               const std::string& prefix)
  {
    ...
    {
      doc_this_step =
        should_doc_this_step(time_pt()->dt(t_hist), time_pt()->time(t_hist));
    }
    ...
    if (doc_this_step)
    {
      ...
      std::ofstream soln_file(
        (dir + "/" + prefix + "soln" + num + ".dat").c_str(), std::ios::out);
      ...
      if (doc_exact())
      {
        std::ofstream exact_file(
          (dir + "/" + prefix + "exact" + num + ".dat").c_str(), std::ios::out);
  ...
  double MyProblem::min_element_size()
  {
    ...
    {
      double min_size =
        mesh_pt(0)->finite_element_pt(0)->compute_physical_size();
  ...
  void MyProblem::write_trace(const unsigned& t_hist)
  {
    ...
    // values instead.
    double nnewton_iter_taken = Dummy_doc_data, lte_norm = Dummy_doc_data,
           real_time = Dummy_doc_data, total_step_time = Dummy_doc_data;

    Vector<double> solver_iterations(1, Dummy_doc_data),
      solver_times(1, Dummy_doc_data), jacobian_setup_times(1, Dummy_doc_data),
      preconditioner_setup_times(1, Dummy_doc_data), max_res(1, Dummy_doc_data);
    ...
    trace_file << Doc_info.number() << Trace_seperator << time
               << Trace_seperator << dt << Trace_seperator
               << get_error_norm(t_hist)
    ...
    unsigned preconditioner_setup_times_length =
      preconditioner_setup_times.size();
    ...
    trace_file << Trace_seperator << get_solution_norm(t_hist)
               ...
               // Reserved slots in case I think of more things to add later
               << Trace_seperator << Dummy_doc_data << Trace_seperator
               << Dummy_doc_data << Trace_seperator << Dummy_doc_data
               << Trace_seperator << Dummy_doc_data << Trace_seperator
               << Dummy_doc_data << Trace_seperator << Dummy_doc_data;
    ...
    const std::string& boundary_file_basename) const
  {
    ...
    for (unsigned b = 0; b < n_boundary; b++)
    {
      std::ofstream boundary_file(
        (boundary_file_basename + to_string(b) + ".csv").c_str(),
        std::ios::out);
