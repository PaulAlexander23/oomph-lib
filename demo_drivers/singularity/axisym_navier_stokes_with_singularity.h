...
#ifndef AXISYM_NAVIER_STOKES_ELEMENTS_WITH_SINGULARITY_HEADER
#define AXISYM_NAVIER_STOKES_ELEMENTS_WITH_SINGULARITY_HEADER
...
namespace oomph
{
  ...
  class AxisymNavierStokesElementWithSingularity
  ...
  public:
    /// Constructor
    AxisymNavierStokesElementWithSingularity() : IsAugmented(false)
    {
                                        ...
                                        const unsigned& i) const
    {
      ...
      {
        // index = BASIC_AXISYM_NAVIER_STOKES_ELEMENT::u_index_nst(n, i);
        index = i;
      }
      return index;
  ...
  public:
    ...
    Vector<SingularNavierStokesSolutionElement<
      AxisymNavierStokesElementWithSingularity<
    ...
    void add_c_equation_element_pt(SingularNavierStokesSolutionElement<
                                   AxisymNavierStokesElementWithSingularity<
                                     BASIC_AXISYM_NAVIER_STOKES_ELEMENT>>* c_pt)
    {
      // Add the element
    ...
    {
      return nodal_value(n, this->p_nodal_index_axi_nst());
    }
    ...
    inline Vector<double> interpolated_u_nst_sum(const Vector<double>& s) const
    {
      ...
      for (unsigned j = 0; j < n_node; j++)
      {
        ...
        {
          interpolated_u[d] +=
            this->nodal_value(j, u_index_nst(j, d)) * psif[j];
        }
      }
    ...
    DenseMatrix<double> interpolated_stress_tensor(const Vector<double>& s)
    {
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        for (unsigned i = 0; i < cached_dim; i++)
        {
          const double u_value = this->nodal_value(l, u_index_nst(l, i));
          for (unsigned j = 0; j < cached_dim; j++)
          {
      ...
      if (scaled_re > 0.0)
      {
        ...
        for (unsigned ipt = 0; ipt < n_intpt; ipt++)
        {
          ...
          for (unsigned l = 0; l < n_node; l++)
          {
            ...
            for (unsigned i = 0; i < this->n_u_nst(); i++)
            {
              double u_value = this->nodal_value(l, u_index_nst(l, i));
            ...
            if (local_eqn >= 0)
            {
              ...
              if (not(Node_is_subject_to_velocity_dirichlet_bcs[l][0]))
              {
                ...
                if (flag)
                {
                  ...
                  {
                    local_unknown = this->u_local_unknown(l2, 0);
                    ...
                    local_unknown = this->u_local_unknown(l2, 1);
                    ...
                    local_unknown = this->u_local_unknown(l2, 2);
        ...
        for (unsigned d = 0; d < this->n_u_nst(); d++)
        {
          ...
          if (local_eqn >= 0)
          {
            ...
            if (Node_is_subject_to_velocity_dirichlet_bcs[l][d])
            {
              ...
              residuals[local_eqn] += this->nodal_value(l, u_index_nst(l, d));
  ...
  class FaceGeometry<AxisymNavierStokesElementWithSingularity<ELEMENT>>
  ...
  template<class ELEMENT>
  class FaceGeometry<
    FaceGeometry<AxisymNavierStokesElementWithSingularity<ELEMENT>>>
