...
namespace oomph
{
  ...
  protected:
    ...
    {
      NavierStokesTractionElement<ELEMENT>::get_traction(
        time, x, n, result);
  ...
  template<class ELEMENT>
  class SingularFarFieldElement : public FarFieldElement<ELEMENT>
  {
  private:
    // Singular velocity gradient contribution
    double (*Singular_velocity_gradient_fct_pt)(const Vector<double>& s,
                                                const unsigned& i,
                                                const unsigned& j);

    // Singularity scaling external data index
    unsigned Scaling_data_index;

  public:
    SingularFarFieldElement<ELEMENT>()
      : FarFieldElement<ELEMENT>(),
        Singular_velocity_gradient_fct_pt(0),
        Scaling_data_index(0)
    {
    }
    ...
    void build(FiniteElement* const& element_pt,
               ...
               const unsigned& id = 0)
    {
      FarFieldElement<ELEMENT>::build(element_pt, face_index, id);

      this->add_external_data(singular_scaling_data_pt);
    }


    // Access function for the singular velocity gradient function
    double (*&singular_velocity_gradient_fct_pt())(const Vector<double>& s,
                                                   const unsigned& i,
                                                   const unsigned& j)
    {
      return Singular_velocity_gradient_fct_pt;
    }
    ...
    // {
    //   // Call the generic routine with the flag set to 1
    //   FarFieldElement<ELEMENT>::fill_in_contribution_to_jacobian(residuals,
    //                                                              jacobian);
  ...
  protected:
    ...
    double interpolated_dudx_bulk(const Vector<double>& s,
                                  const unsigned& i,
                                  const unsigned& j) const
    {
      const double bulk_velocity_gradient =
        FarFieldElement<ELEMENT>::interpolated_dudx_bulk(s, i, j);

      Vector<double> x(this->nodal_dimension());
      this->interpolated_x(s, x);

      const double singular_velocity_gradient =
        (*Singular_velocity_gradient_fct_pt)(x, i, j) *
        this->external_data_pt(Scaling_data_index)->value(0);

      return bulk_velocity_gradient + singular_velocity_gradient;
    }
  };
