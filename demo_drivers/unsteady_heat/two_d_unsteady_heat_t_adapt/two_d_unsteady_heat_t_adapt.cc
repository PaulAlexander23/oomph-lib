...
namespace ExactSolnForUnsteadyHeat
{
                   ...
                   Vector<double>& u)
  {
    ...
    u[0] = sin(K * zeta) * 0.5 *
           (1.0 + tanh(Gamma * cos(2.0 * MathematicalConstants::Pi * time)));
  }
  ...
  void get_source(const double& time, const Vector<double>& x, double& source)
  {
    source =
      ...
      0.1E1 * sin(K * (cos(Phi) * x[0] + sin(Phi) * x[1])) *
        (1.0 -
         pow(tanh(Gamma * cos(0.2E1 * 0.3141592653589793E1 * time)), 2.0)) *
                                                ...
                                                ofstream& trace_file)
{
  ...
  trace_file << time_pt()->time() << " " << time_pt()->dt() << " "
             << Control_node_pt->value(0) << " " << u_exact << " " << error
             << " " << norm;
  trace_file << " " << global_temporal_error_norm();
  trace_file << std::endl;
...
double UnsteadyHeatProblem<ELEMENT>::global_temporal_error_norm()
{
  ...
  for (unsigned i = 0; i < n_node; i++)
  {
    ...
    double error =
      mesh_pt()->node_pt(i)->time_stepper_pt()->temporal_error_in_value(
        mesh_pt()->node_pt(i), 0);
