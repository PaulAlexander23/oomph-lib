...
void LinearWaveProblem<ELEMENT, TIMESTEPPER>::set_initial_condition()
{
  ...
  Vector<typename TIMESTEPPER::NodeInitialConditionFctPt> initial_value_fct(1);
  Vector<typename TIMESTEPPER::NodeInitialConditionFctPt> initial_veloc_fct(1);
  Vector<typename TIMESTEPPER::NodeInitialConditionFctPt> initial_accel_fct(1);
...
void LinearWaveProblem<ELEMENT, TIMESTEPPER>::doc_solution(DocInfo& doc_info)
{
  ...
  Trace_file << time_pt()->time() << " " << time_pt()->dt() << " "
             << Bulk_mesh_pt->nelement() << " " << error << " " << norm
             << std::endl;
