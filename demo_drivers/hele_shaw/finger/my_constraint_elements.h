...
namespace oomph
{
  ...
  {
  private:
    typedef void (*TimeDependentFctPt)(const double& t, double& y);

    /// Pointer to the desired value of the volume
    TimeDependentFctPt Prescribed_volume_fct_pt;
  ...
  public:
    ...
    MyConstraintElement(TimeDependentFctPt prescribed_volume_fct_pt,
                        double* actual_volume_pt,
                        Data* p_traded_data_pt,
                        const unsigned& index_of_traded_pressure)
    {
      // Store pointer to prescribed volume
      Prescribed_volume_fct_pt = prescribed_volume_fct_pt;
      ...
      bool compute_jacobian)
    {
      ...
      if (local_eqn >= 0)
      {
        residuals[local_eqn] += -prescribed_volume + *Actual_volume_pt;
      }
