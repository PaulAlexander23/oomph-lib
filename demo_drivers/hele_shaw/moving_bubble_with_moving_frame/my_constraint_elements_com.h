#ifndef MY_CONSTRAINT_ELEMENTS_COM_HEADER
#define MY_CONSTRAINT_ELEMENTS_COM_HEADER

#include "generic.h"

namespace oomph
{
  class MyConstraintElementCoM : public GeneralisedElement
  {
  private:
    /// Pointer to the desired value of the volume
    double* Prescribed_com_pt;

    /// Pointer to the actual value of the moment
    double* Actual_moment_pt;
  ...
  public:
    ...
    MyConstraintElementCoM(double* prescribed_com_pt,
                           double* actual_moment_pt,
                           double* actual_volume_pt,
                           Data* p_traded_data_pt,
                           const unsigned& index_of_traded_pressure)
    {
      // Store pointer to prescribed com
      Prescribed_com_pt = prescribed_com_pt;

      // Store pointer to actual moment
      Actual_moment_pt = actual_moment_pt;
      ...
      bool compute_jacobian)
    {
      ...
      {
        residuals[local_eqn] +=
          -*Prescribed_com_pt * (*Actual_volume_pt) + *Actual_moment_pt;
      }

      if (compute_jacobian)
      {
        int external_unknown;
        external_unknown = this->external_local_eqn(0, 0);
        if (external_unknown >= 0)
        {
          jacobian(local_eqn, external_unknown) += 1;
        }

        external_unknown = this->external_local_eqn(1, 0);
        if (external_unknown >= 0)
        {
          jacobian(local_eqn, external_unknown) += -1;
        }
