...
void HeleShawChannelProblem<ELEMENT>::generate_info_mesh()
{
  ...
  Data* x_mom_integral_data_pt = new Data(number_of_values);
  this->Info_mesh_pt->add_element_pt(new InfoElement(x_mom_integral_data_pt));
  Data* y_mom_integral_data_pt = new Data(number_of_values);
  this->Info_mesh_pt->add_element_pt(new InfoElement(y_mom_integral_data_pt));
}
...
void HeleShawChannelProblem<ELEMENT>::upcast_and_finalise_elements()
{
  ...
  for (unsigned i = 0; i < n_element; i++)
  {
    ...
    el_pt->add_x_mom_data_pt(Info_mesh_pt->element_pt(3)->internal_data_pt(0));
    el_pt->add_y_mom_data_pt(Info_mesh_pt->element_pt(4)->internal_data_pt(0));
  }
  ...
  ofstream& output_stream, string filename)
{
  ...
  for (unsigned index = 3; index < 5; index++)
  {
    my_integral =
      this->Info_mesh_pt->element_pt(index)->internal_data_pt(0)->value(0)/
      this->Info_mesh_pt->element_pt(2)->internal_data_pt(0)->value(0);
    output_stream << "CoM " << index << "= " << setprecision(17)
                  << my_integral << endl;
  }
  output_stream.close();
}
