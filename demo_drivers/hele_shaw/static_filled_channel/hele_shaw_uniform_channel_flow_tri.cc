...
private:
  ...
  /// Generate inlet surface
  void generate_inlet_surface();
...
void HeleShawChannelProblem<ELEMENT>::doc_solution(DocInfo& doc_info)
{
  ...
  unsigned n_points = 5;
  save_solution_to_file(output_stream, data_directory + "/soln.dat", n_points);
}
...
void HeleShawChannelProblem<ELEMENT>::generate_mesh()
{
  ...
  this->generate_inlet_surface();
}

template<class ELEMENT>
void HeleShawChannelProblem<ELEMENT>::generate_boundary()
{
  double l_x = 2.0;
  double l_y = 1.0;
...
template<class ELEMENT>
void HeleShawChannelProblem<ELEMENT>::generate_bulk()
{
  cout << "Generate bulk" << endl;
...
template<class ELEMENT>
void HeleShawChannelProblem<ELEMENT>::generate_inlet_surface()
{
  cout << "Generate inlet surface" << endl;
