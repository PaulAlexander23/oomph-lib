#ifndef OOMPH_INJECT_AIR_PROBLEM_HEADER
#define OOMPH_INJECT_AIR_PROBLEM_HEADER
...
namespace oomph
{
  ...
  void RelaxingBubbleProblem<ELEMENT>::create_outer_boundary_polygon()
  {
    ...
    // Vertex coordinates
    Vector<Vector<double>> boundary_vertex(npoints);

    for (unsigned ipoint = 0; ipoint < npoints; ipoint++)
    {
      boundary_vertex[ipoint].resize(2);
      ...
      // Shift
      boundary_vertex[ipoint][0] = coord[0] + x_center;
      boundary_vertex[ipoint][1] = coord[1] + y_center;
    }

    // Build the 1st bubble polyline
    boundary_polyline_pt[0] =
      new TriangleMeshPolyLine(boundary_vertex, Bottom_boundary_id);
    ...
    boundary_polyline_pt[1] =
      new TriangleMeshPolyLine(boundary_vertex, Top_boundary_id);
  ...
  void RelaxingBubbleProblem<ELEMENT>::create_volume_constraint_mesh()
  {
    MyConstraintElement* vol_constraint_element =
      new MyConstraintElement(parameters::target_fluid_volume_pt,
  ...
  void RelaxingBubbleProblem<ELEMENT>::set_variable_and_function_pointers()
  {
    ...
    parameters::bubble_pressure_pt = Bubble_pressure_data_pt->value_pt(0);
    ...
    for (unsigned e = 0; e < n_element; e++)
    {
      ...
      interface_element_pt->bubble_pressure_fct_pt() =
        parameters::bubble_pressure_fct;
  ...
  void RelaxingBubbleProblem<ELEMENT>::set_boundary_conditions()
  {
    ...
    for (unsigned ibound = 0; ibound < nbound; ibound++)
    {
      ...
      for (unsigned inod = 0; inod < num_nod; inod++)
      {
        ...
        if (ibound == Top_boundary_id || ibound == Bottom_boundary_id)
        {
    ...
    Volume_data_pt->set_value(0, (*parameters::target_fluid_volume_pt));
  ...
  {
    parameters::volume_fct(time(), *parameters::target_bubble_volume_pt);
