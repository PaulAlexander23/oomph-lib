...
namespace problem_parameter
{
  ...
  void upper_wall_fct(const Vector<double>& x, double& b, double& dbdt)
  {
    ...
    b = 1.0 -
        0.2 * std::exp(-0.0 * pow(local_x - 1.5, 2.0) / (2.0 * pow(0.1, 2.0)) -
                       pow(local_y - 0.5, 2.0) / (2.0 * pow(0.1, 2.0)));
    dbdt = 0.0;
  }
                           ...
                           Vector<double>& d_dbdt_dx)
  {
    ...
    dbdx[0] = 0.0 * 0.2 * (local_x - 1.5) / pow(0.1, 2.0) *
              std::exp(-pow(local_x - 1.5, 2.0) / (2.0 * pow(0.1, 2.0)) -
                       pow(local_y - 0.5, 2.0) / (2.0 * pow(0.1, 2.0)));
    dbdx[1] = 0.2 * (local_y - 0.5) / pow(0.1, 2.0) *
              std::exp(-0.0 * pow(local_x - 1.5, 2.0) / (2.0 * pow(0.1, 2.0)) -
                       pow(local_y - 0.5, 2.0) / (2.0 * pow(0.1, 2.0)));
  ...
  void prescribed_moving_frame_speed(const double& t, double& frame_speed)
  {
    frame_speed = 1.0;
    // 0.5 * MathematicalConstants::Pi * sin(MathematicalConstants::Pi * t);
  }

  void wall_speed_fct_pt(const Vector<double>& x, Vector<double>& velocity)
  {
    /// This function should have obstacle width and height, asymmetry and
    /// possibly sharpness.

    velocity[0] = *global_frame_speed_pt;
    velocity[1] = 0.0;
  }
  ...
  void get_inlet_bc(const Vector<double>& x, double& flux)
  {
    ...
    double dpdx = (*global_flux_pt - *global_frame_speed_pt * *inlet_area_pt) /
                  *inlet_b3_pt;
  ...
  void get_outlet_bc(const Vector<double>& x, double& flux)
  {
    ...
    double dpdx =
      -(*global_flux_pt - *global_frame_speed_pt * *outlet_area_pt) /
      *outlet_b3_pt;
...
private:
  ...
  void save_integral_data_to_file(ofstream& output_stream, string filename);
  ...
  Data* Inlet_area_data_pt;
  Data* Inlet_b3_data_pt;
  Data* Outlet_area_data_pt;
  Data* Outlet_b3_data_pt;
};
...
void HeleShawChannelProblem<ELEMENT>::doc_solution(DocInfo& doc_info)
{
  ...
  filename = data_directory + "/integral_data.dat";
  this->save_integral_data_to_file(output_stream, filename);
}
...
void HeleShawChannelProblem<ELEMENT>::generate_data()
{
  ...
  double initial_frame_speed;
  problem_parameter::prescribed_moving_frame_speed(0.0, initial_frame_speed);
...
void HeleShawChannelProblem<ELEMENT>::generate_mesh()
{
  ...
  Inlet_area_data_pt = new Data(number_of_values);
  Inlet_b3_data_pt = new Data(number_of_values);
  Outlet_area_data_pt = new Data(number_of_values);
  Outlet_b3_data_pt = new Data(number_of_values);
  unsigned index = 0;
  Inlet_area_data_pt->set_value(index, 1.0);
  Inlet_b3_data_pt->set_value(index, 1.0);
  Outlet_area_data_pt->set_value(index, 1.0);
  Outlet_b3_data_pt->set_value(index, 1.0);
  Inlet_area_data_pt->unpin(index);
  Inlet_b3_data_pt->unpin(index);
  Outlet_area_data_pt->unpin(index);
  Outlet_b3_data_pt->unpin(index);
  problem_parameter::inlet_area_pt = Inlet_area_data_pt->value_pt(index);
  problem_parameter::inlet_b3_pt = Inlet_b3_data_pt->value_pt(index);
  problem_parameter::outlet_area_pt = Outlet_area_data_pt->value_pt(index);
  problem_parameter::outlet_b3_pt = Outlet_b3_data_pt->value_pt(index);
  this->Info_mesh_pt->add_element_pt(new InfoElement(Inlet_area_data_pt));
  this->Info_mesh_pt->add_element_pt(new InfoElement(Inlet_b3_data_pt));
  this->Info_mesh_pt->add_element_pt(new InfoElement(Outlet_area_data_pt));
  this->Info_mesh_pt->add_element_pt(new InfoElement(Outlet_b3_data_pt));
  ...
  Frame_travel_data_pt =
    this->ODE_mesh_pt->element_pt(el_index)->internal_data_pt(index);
  ...
  const unsigned& boundary)
{
  if (boundary == 3)
  {
    ...
    for (unsigned n = 0; n < n_element; n++)
    {
      ...
      flux_element_pt->add_area_data_pt(Inlet_area_data_pt);
      flux_element_pt->add_b3_data_pt(Inlet_b3_data_pt);
  ...
  else if (boundary == 1)
  {
    ...
    for (unsigned n = 0; n < n_element; n++)
    {
      ...
      flux_element_pt->add_area_data_pt(Outlet_area_data_pt);
      flux_element_pt->add_b3_data_pt(Outlet_b3_data_pt);
...
void HeleShawChannelProblem<ELEMENT>::setup_elements()
{
  ...
  for (unsigned i = 0; i < n_element; i++)
  {
    ...
    el_pt->wall_speed_fct_pt() = problem_parameter::wall_speed_fct_pt;
  }
...
{
  problem_parameter::prescribed_moving_frame_speed(
    time(), *problem_parameter::global_frame_speed_pt);
}
  ...
  ofstream& output_stream, string filename)
{
  ...
  unsigned value_index = 0;
  double x = Frame_travel_data_pt->value(value_index);
  output_stream << "x: " << x << ", ";
...
template<class ELEMENT>
void HeleShawChannelProblem<ELEMENT>::save_integral_data_to_file(
  ofstream& output_stream, string filename)
{
  /// Create and open the file stream
  output_stream.open(filename.c_str(), ofstream::out | ofstream::app);
  ...
  /// Write the current time
  output_stream << "t: " << this->time() << ", ";
  /// Write integral data
  output_stream << "Inlet area: " << Inlet_area_data_pt->value(0) << ", ";
  output_stream << "Inlet b3: " << Inlet_b3_data_pt->value(0) << ", ";
  output_stream << "Outlet area: " << Outlet_area_data_pt->value(0) << ", ";
  output_stream << "Outlet b3: " << Outlet_b3_data_pt->value(0) << endl;
