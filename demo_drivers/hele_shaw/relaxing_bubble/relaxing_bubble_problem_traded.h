...
RelaxingBubbleProblem<ELEMENT>::RelaxingBubbleProblem()
{
  ...
  Bubble_pressure_data_pt->set_value(0, 1.0);
  ...
  //  dynamic_cast<InfoElement*>(Volume_mesh_pt->element_pt(0));
  // cout << volume_element_pt->internal_data_pt(0)->value(0) << endl;
}
...
void RelaxingBubbleProblem<ELEMENT>::generate_mesh()
{
  ...
  VolumeConstraintElement* vol_constraint_element = new VolumeConstraintElement(
    relaxing_bubble::target_fluid_volume_pt, Bubble_pressure_data_pt, 0);
  cout<<*relaxing_bubble::target_fluid_volume_pt<<endl;
  cout<<Bubble_pressure_data_pt->value(0)<<endl;
  ...
  ELEMENT>::set_adaptable_variable_and_function_pointers()
{
  ...
  for (unsigned e = 0; e < n_element; e++)
  {
    ...
    VolumeConstraintElement* volume_element_pt =
      dynamic_cast<VolumeConstraintElement*>(
        Volume_constraint_mesh_pt->element_pt(i_element));

    el_pt->add_external_data(volume_element_pt->p_traded_data_pt(),
                             fd_jacobian);
  }
