...
public:
  ...
  ~BubbleInChannelProblem()
  {
    // std::cout << "In the revised destructor" << std::endl;
  ...
  void create_inflow_elements()
  {
    ...
    cout << n_inflow_element
         << " Poisson Flux Elements created on main inflow boundary" << endl;
    std::cout << "G is " << G_data_pt->value(0) << std::endl;
  }
    ...
    unsigned i_bubble, double x_center, double y_center, double a, double b)
  {
    ...
    unsigned ibound = First_bubble_boundary_id;
  ...
  void output_ordered_polygon(ofstream& Boundary_output_file)
  {
    ...
    {
      unsigned num_nodes =
        Fluid_mesh_pt->nboundary_node(First_bubble_boundary_id);
...
BubbleInChannelProblem<ELEMENT>::BubbleInChannelProblem()
{
  ...
  double Bubble_x1 = -Problem_Parameter::All_the_bubbles[0][0][0];
  double Bubble_x2 = Problem_Parameter::All_the_bubbles[0][0][0];
  ...
  std::cout << "(" << vertex_coord[0][0] << "," << vertex_coord[0][1]
            << ") to (" << vertex_coord[1][0] << "," << vertex_coord[1][1]
            << ")" << std::endl;
  ...
  boundary_polyline_pt[3] =
    new TriangleMeshPolyLine(vertex_coord, Symmetry_one_id);

  vertex_coord[0][0] = Bubble_x2;
  vertex_coord[0][1] = 0;
  vertex_coord[1][0] = Bubble_x1;
  vertex_coord[1][1] = 0;
  ...
  boundary_polyline_pt[4] =
    new TriangleMeshPolyLine(vertex_coord, Symmetry_two_id);


  vertex_coord[0][0] = Bubble_x1;
  vertex_coord[0][1] = 0;
  vertex_coord[1][0] = Problem_Parameter::Channel_start;
  vertex_coord[1][1] = 0;
  ...
  boundary_polyline_pt[5] =
    new TriangleMeshPolyLine(vertex_coord, Symmetry_three_id);
  ...
  Vector<TriangleMeshCurveSection*> bubble_polyline_pt(1);
  ...
  std::cout << "Connect vertices" << std::endl;

  // Now we first want to connect the interface boundary to the outer boundary
  bubble_polyline_pt[0]->connect_initial_vertex_to_polyline(
    dynamic_cast<TriangleMeshPolyLine*>(boundary_polyline_pt[4]), 0);
  bubble_polyline_pt[0]->connect_final_vertex_to_polyline(
    dynamic_cast<TriangleMeshPolyLine*>(boundary_polyline_pt[5]), 0);

  std::cout << "Done connecting" << std::endl;

  Vector<TriangleMeshCurveSection*> internal_curve_section_pt(1);
  internal_curve_section_pt[0] = bubble_polyline_pt[0];

  internal_curve_section_pt[0]->set_refinement_tolerance(0.005);
  internal_curve_section_pt[0]->set_unrefinement_tolerance(0.01);

  // Interface boundary is an open curve
  Vector<TriangleMeshOpenCurve*> interface_open_curve_pt(1);
  interface_open_curve_pt[0] =
  ...
  triangle_mesh_parameters.internal_open_curves_pt() = interface_open_curve_pt;
  ...
  std::cout << "Try to construct the mesh" << std::endl;
  // Create the mesh
  ...
  Fluid_mesh_pt->max_permitted_error() = 2e-4;
  Fluid_mesh_pt->min_permitted_error() = 5e-6;
  Fluid_mesh_pt->max_element_size() = 0.01;
  Fluid_mesh_pt->min_element_size() = 5e-5;
...
void BubbleInChannelProblem<ELEMENT>::create_free_surface_elements()
{
  ...
  for (unsigned bubble = 0; bubble < Problem_Parameter::N_Bubble; bubble++)
  {
         ...
         b++)
    {
      ...
      std::cout << "This is : " << n_element << " bulk elements" << std::endl;
      for (unsigned e = 0; e < n_element; e++)
      {
        ELEMENT* bulk_elem_pt = dynamic_cast<ELEMENT*>(
          Fluid_mesh_pt->boundary_element_in_region_pt(b, region, e));

        //		std::cout << "Got bulk element" << std::endl;
...
void BubbleInChannelProblem<ELEMENT>::create_CoM_Y_constraint_elements()
{
  ...
  for (unsigned b = First_bubble_boundary_id; b < First_bubble_boundary_id + 1;
       b++)
  {
...
void BubbleInChannelProblem<ELEMENT>::complete_problem_setup()
{
  ...
  is_on_bubble_bound[0] = false;
  is_on_bubble_bound[1] = false;
  is_on_bubble_bound[2] = false;
  is_on_bubble_bound[3] = false;
  is_on_bubble_bound[4] = false;
  is_on_bubble_bound[5] = false;
  is_on_bubble_bound[6] = true;


  std::cout << "Loop over all nodes in fluid mesh, pinning both pressure nodes"
            << std::endl;
  ...
  {
    unsigned num_nod = Fluid_mesh_pt->nboundary_node(ibound);
    std::cout << ibound << " " << num_nod << std::endl;
    for (unsigned inod = 0; inod < num_nod; inod++)
    {
      Node* nod_pt = Fluid_mesh_pt->boundary_node_pt(ibound, inod);
      SolidNode* solid_node_pt = dynamic_cast<SolidNode*>(nod_pt);
      ...
      {
        //		std::cout << "Do nothing, on curved boundary" <<std::endl;
        //			solid_node_pt->unpin_position(0);
        //			solid_node_pt->unpin_position(1);
      }
      else if (ibound == 3 || ibound == 4 || ibound == 5)
      {
        solid_node_pt->pin_position(1);
        //			std::cout << "Pin y only, on symmetry" << std::endl;
      }
      else
      {
        solid_node_pt->pin_position(0);
        solid_node_pt->pin_position(1);
        //			std::cout << "Both components" <<std::endl;
      }
      if (ibound == Inflow_boundary_id)
      {
        ...
        double y = nod_pt->x(1);
        double p = -x;
        nod_pt->set_value(0, p);
        std::cout << "UnPin at " << x << " " << y << " " << std::endl;
      }
      if (ibound == Outflow_boundary_id)
      {
        //			std::cout << "Pinning pressure at node " <<std::endl;
        nod_pt->pin(0);
        double x = nod_pt->x(0);
        double y = nod_pt->x(1);
        double p = -x;
        nod_pt->set_value(0, p);
        std::cout << "Pin at " << x << " " << y << " " << std::endl;
      }

      if (is_on_bubble_bound[ibound])
      {
        if (nod_pt->is_on_boundary(Symmetry_two_id) ||
            nod_pt->is_on_boundary(Symmetry_one_id) ||
            nod_pt->is_on_boundary(Symmetry_three_id))
        {
          std::cout << "Found a corner node!" << std::endl;
          //	      std::cout << "ibound " << ibound << std::endl;
          std::cout << "x :" << nod_pt->x(0) << " y: " << nod_pt->x(1)
                    << std::endl;
          //	      std::cout << nod_pt->is_on_boundary(Symmetry_one_id) << " "
          //<< nod_pt->is_on_boundary(Symmetry_two_id) << " " <<
          //nod_pt->is_on_boundary(Symmetry_three_id)  << " "
          //	      nod_pt->is_on_boundary(<< std::endl;
          // nod_pt->pin(3);
          nod_pt->pin(4);
        }
      }
...
void BubbleInChannelProblem<ELEMENT>::solve_for_eigenproblem()
{
  bool doc_eigenvector =
    true; /// Will doc perturbed solutions for 10 eigenvectors.
