...
TetmeshPoissonProblem<ELEMENT>::TetmeshPoissonProblem()
{
  ...
  Exact_volume_of_hole =
    2.0 * cube_half_width * 2.0 * cube_half_width * 2.0 * cube_half_width;
  ...
  // For sanity check:
  Exact_volume_of_internal_region = 2.0 * cube_region_half_width * 2.0 *
                                    cube_region_half_width * 2.0 *
                                    cube_region_half_width;
  ...
  // Look, we can visualise the faceted surface!
  embedded_rectanglar_facet_pt->output(
    "embedded_rectangular_faceted_surface.dat");
  ...
  oomph_info << "Mesh volume " << av_el_size << " " << " nel: " << nel
...
void TetmeshPoissonProblem<ELEMENT>::apply_boundary_conditions()
{
  ...
  for (unsigned ibound = First_hollow_cube_boundary_id;
       ibound < First_hollow_cube_boundary_id + 6;
       ibound++)
  {
                                                  ...
                                                  DocInfo& doc_info)
{
  ...
  // Check volume:
  oomph_info << "Volume of region 1 [in mesh, exact, diff (%)]: "
             << volume_in_internal_region << " "
             << Exact_volume_of_internal_region << " "
             << abs(volume_in_internal_region -
                    Exact_volume_of_internal_region) /
                  Exact_volume_of_internal_region * 100.0
             << std::endl;
  ...
  double exact_volume = Exact_bounded_volume - Exact_volume_of_hole -
                        Exact_volume_of_internal_region;
  oomph_info << "Volume of region 0 [in mesh, exact, diff (%)]: "
             << volume_in_region0 << " " << exact_volume << " "
             << abs(volume_in_region0 - exact_volume) / exact_volume * 100.0
             << std::endl;
  ...
  oomph_info << "Total volume      [in mesh, exact, diff (%)]: "
             << total_mesh_volume << " " << exact_volume << " "
             << abs(total_mesh_volume - exact_volume) / exact_volume * 100.0
             << std::endl;

  // Doc volumes
  volume_file << volume_in_internal_region << " " << total_mesh_volume << " "
  ...
  //-----------------------------
  sprintf(
    filename, "%s/soln%i.vtu", doc_info.directory().c_str(), doc_info.number());
  some_file.open(filename);
  if (do_bulk_output)
  {
