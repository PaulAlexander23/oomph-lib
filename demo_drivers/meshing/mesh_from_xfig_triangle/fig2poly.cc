...
int main(int argc, char* argv[])
{
  ...
  if (version_string != "#FIG 3.2")
  {
    std::cout << "*.fig file is in the wrong format!\n"
              ...
              << "but the version string is: " << version_string << std::endl;
    exit(1);
  }
  ...
  while (!buffer2.eof())
  {
    ...
    if (object_id == 1)
    {
      cout << "I've read a circle -- it will be used as a hole identifier."
           << std::endl;
    ...
    {
      cout << "I've read a polyline -- it defines a boundary." << std::endl;
      ...
      {
        std::cout << "Can't handle this sub-type of polygon: " << sub_type
                  << std::endl
                  << "Can only do open polylines -- no closed boxes, etc. "
                  << std::endl;
        exit(1);
      }
    }
    ...
    {
      std::cout << "Can't handle this object id: " << object_id
  ...
  for (unsigned b = 0; b < nbound; b++)
  {
    ...
    for (unsigned i = 0; i < npoints; i++)
    {
      ...
      poly_file << count << " " << boundary_point_coords[b][i][0] << " "
                << boundary_point_coords[b][i][1] << " #  [located on boundary "
                << b << "]" << std::endl;
  ...
  poly_file << total_npoints << " 1 " //<< nbound
            ...
            << " flag for using boundary markers here]" << std::endl;
  ...
  {
    poly_file << hole_count << " " << hole_coords[i][0] << " "
