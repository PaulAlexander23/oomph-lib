...
int main()
{
  ...
  Vector<Vector<Vector<unsigned>>> fluid_faces(
    4, Vector<Vector<unsigned>>(nbound_cond, Vector<unsigned>(3)));
  ...
  Vector<Vector<Vector<unsigned>>> solid_linking_faces(
    3, Vector<Vector<unsigned>>(nbound_cond, Vector<unsigned>(3)));
  ...
  for (unsigned i = 0; i < nbound_cond; i++)
  {
    ...
    if (bound_id != 1 && bound_id != 2 && bound_id != 3 && bound_id != 4)
    {
      ...
      error_stream << "bound_id=" << bound_id
                   ...
                   << " input file contains a boundary with id : " << bound_id
                   << ".\n"
    ...
    else
    {
      ...
      for (unsigned ii = 0; ii < 3; ii++)
      {
        fluid_faces[bound_id - 1][nfluid_face[bound_id - 1] - 1][ii] =
          side_node[ii];
      }
    }
  }
  ...
  for (std::set<unsigned>::iterator it = quadratic_fsi_boundary_nodes.begin();
       it != quadratic_fsi_boundary_nodes.end();
       it++)
  {
    quadratic_fsi_boundary_file << x_node[*it] << " " << y_node[*it] << " "
                                << z_node[*it] << " " << *it << std::endl;
  }


  for (Vector<Vector<unsigned>>::iterator it =
         quadratic_fsi_boundary_face_element_nodes.begin();
       it != quadratic_fsi_boundary_face_element_nodes.end();
       it++)
  {
    quadratic_fsi_boundary_file
      << (*it)[0] << " " << (*it)[1] << " " << (*it)[2] << " " << (*it)[3]
  ...
  // Output quadratic representation of outer solid boundary (six node
  // triangles)
  //-----------------------------------------------------------------------------
  ...
  fluid_output_stream << n_fluid_node << ' ' << 3 << ' ' << 0 << ' ' << 1
                      << '\n'
                      << '\n';
    ...
    for (unsigned iface = 0; iface < nface; iface++)
    {
      ...
      for (unsigned ii = 0; ii < 3; ii++)
      {
        ...
        if (!done_fluid_node[inod])
        {
          ...
          fluid_output_stream << counter << ' ' << x_node[inod] << ' '
                              << y_node[inod] << ' ' << z_node[inod] << ' ' << 0
                              << '\n';
        }
      }
    }
  ...
  // Number of nodes, dimension =3 , no attributes, with boundary markers
  solid_output_stream << n_solid_node << ' ' << 3 << ' ' << 0 << ' ' << 1
                      << '\n'
                      << '\n';
  ...
  for (unsigned iface = 0; iface < nface; iface++)
  {
    ...
    for (unsigned ii = 0; ii < 3; ii++)
    {
      ...
      if (!done_solid_node[inod])
      {
        ...
        solid_output_stream << counter << ' ' << x_node[inod] << ' '
                            << y_node[inod] << ' ' << z_node[inod] << ' ' << 0
                            << '\n';
        ...
        solid_output_stream << counter << " "
                            << x_outer_node[inod] +
                                 wall_thickness * normal_for_outer_wall[inod][0]
                            << " "
                            << y_outer_node[inod] +
                                 wall_thickness * normal_for_outer_wall[inod][1]
                            << " "
                            << z_outer_node[inod] +
                                 wall_thickness * normal_for_outer_wall[inod][2]
                            << 0 << '\n';
      }
    }
  }
  ...
  for (unsigned i = 0; i < 4; i++)
  {
    fluid_output_stream << '\n'
                        << "#============================="
                        << "=====================================" << '\n'
                        << "# Faces originally on boundary " << i + 1 << '\n'
                        << "# --------------------------------" << '\n';
  ...
  for (unsigned i = 0; i < 4; i++)
  {
    fluid_output_stream << "# Boundary " << i + 1 << " : from boundary id ";
    unsigned id = 0;
    ...
    fluid_output_stream << id << " until boundary id "
                        << id + nfluid_face[i] - 1 << '\n';
    boundary_enumeration << id << " " << id + nfluid_face[i] - 1 << " "
                         << "# (fluid boundary" << i + 1 << ") \n";
  }
  ...
  solid_output_stream << "# the inner surface : from boundary id " << 0
                      << " until boundary id ";
  ...
  solid_output_stream << "# the front inflow face : from boundary id " << id + 1
                      << " until boundary id ";
  ...
  solid_output_stream << "# the right bifurcation : from boundary id " << id + 1
                      << " until boundary id ";
  boundary_enumeration << id + 1 << " ";
  ...
  solid_output_stream << "# the outer surface : from boundary id " << id + 1
                      << " until boundary id ";
  boundary_enumeration << id + 1 << " ";
  id += nfluid_face[0];
  solid_output_stream << id << "\n";
