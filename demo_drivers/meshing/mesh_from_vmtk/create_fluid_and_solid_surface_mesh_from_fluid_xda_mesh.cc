  ...
  const bool& do_multi_boundary_ids = true)
{
  ...
  std::vector<std::vector<std::vector<unsigned>>> fluid_faces(
    4,
    std::vector<std::vector<unsigned>>(nbound_cond, std::vector<unsigned>(3)));
  ...
  std::vector<std::vector<std::vector<unsigned>>> solid_linking_faces(
    3,
    std::vector<std::vector<unsigned>>(nbound_cond, std::vector<unsigned>(3)));
  ...
  for (unsigned iface = 0; iface < nface; iface++)
  {
    ...
    for (unsigned ii = 0; ii < 3; ii++)
    {
      ...
      if (!done_solid_node[inod])
      {
        ...
        solid_output_stream << counter << ' ' << x_outer_node[inod] << ' '
                            << y_outer_node[inod] << ' ' << z_outer_node[inod]
                            << ' ' << 0 << '\n';
      }
    }
  }
