...
public:
  ...
  void output(ostream& outfile, const unsigned& nplot)
  {
    ...
    for (unsigned iplot = 0; iplot < num_plot_points; iplot++)
    {
      ...
      outfile << interpolated_p(s, 0) << " " << interpolated_p(s, 1) << " ";
      outfile << std::endl;
    }
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        if (local_eqn >= 0)
        {
          ...
          for (unsigned l2 = 0; l2 < n_pres; l2++)
          {
            ...
            if (local_unknown >= 0)
            {
              jacobian(local_eqn, local_unknown) += psip(l2) * dpsidx(l, 0) * W;
            }
          }
        }
...
public:
  ...
  ZeroResidualGenElement()
  {
    ...
    eigenvalue_problem_pt =
      new OrrSommerfeldProblem<QOrrSommerfeldElement<1, 3>>(
