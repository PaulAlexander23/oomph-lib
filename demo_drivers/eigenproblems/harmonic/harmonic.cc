...
void HarmonicProblem<ELEMENT, EIGEN_SOLVER>::solve(const unsigned& label)
{
  ...
  // Normalise the eigenvector
  {
    ...
    length = sqrt(length);
    // Fix the sign
    if (eigenvector_real[second_smallest_index][0] < 0)
    {
      length *= -1.0;
    }
    // Finally normalise
  ...
  {
    // Print to screen
    cout << sorted_eigenvalue[i].real() << " " << sorted_eigenvalue[i].imag()
         << std::endl;
    // Send to file
    evalues << sorted_eigenvalue[i].real() << " " << sorted_eigenvalue[i].imag()
            << std::endl;
  }
