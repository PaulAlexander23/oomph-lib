...
void ElasticRingProblem<ELEMENT, TIMESTEPPER>::doc_solution(DocInfo& doc_info)
{
  ...
  Trace_file << time_pt()->time() << " "
             ...
             << global_pot + global_kin << " " << posn_ctrl[1] << std::endl;
  ...
  for (unsigned iplot = 0; iplot < nplot; iplot++)
  {
    ...
    some_file << posn[0] << " " << posn[1] << " " << xi[0] << " " << veloc[0]
              << " " << veloc[1] << " " << accel[0] << " " << accel[1] << " "
              << sqrt(pow(posn[0], 2) + pow(posn[1], 2)) << " "
              << sqrt(pow(veloc[0], 2) + pow(veloc[1], 2)) << " "
              << sqrt(pow(accel[0], 2) + pow(accel[1], 2)) << " " << std::endl;
  }
...
void ElasticRingProblem<ELEMENT, TIMESTEPPER>::unsteady_run()
{
  ...
  Trace_file << ",\"<sub>exact</sub>R<sub>ctrl</sub>\"" << std::endl;

  // Number of steps
  unsigned nstep = 600;
  if (Global_Physical_Variables::Long_run_flag == 0)
  {
    nstep = 10;
  }
  ...
  if (Global_Physical_Variables::Fixed_timestep_flag == 0)
  {
    timestep_ratio = 0.995;
  }
  ...
  for (unsigned i = 1; i <= nstep; i++)
  {
    ...
    // Reduce timestep
    if (time_pt()->time() < 100.0)
    {
      dt = timestep_ratio * dt;
    }
  }
...
int main(int argc, char* argv[])
{
  ...
  cout << "Long run flag: " << Global_Physical_Variables::Long_run_flag
       << std::endl;
  ...
  // Set up the problem
  ElasticRingProblem<HermiteBeamElement, Newmark<3>> problem(nelem, L);
