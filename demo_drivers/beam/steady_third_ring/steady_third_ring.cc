...
namespace Global_Physical_Variables
{
                  ...
                  Vector<double>& load)
  {
    ...
    {
      load[i] =
        (Pext_data_pt->value(0) + Pcos * cos(double(Nbuckl) * xi[0])) * N[i];
    }
  }
...
ElasticRingProblem<ELEMENT>::ElasticRingProblem(const unsigned& n_element)
{
  ...
  // Angle
  double phi =
    MathematicalConstants::Pi / double(Global_Physical_Variables::Nbuckl);
  ...
  bc_el_pt->set_symmetry_line(vector_to_symmetry_line, normal_to_symmetry_line);
