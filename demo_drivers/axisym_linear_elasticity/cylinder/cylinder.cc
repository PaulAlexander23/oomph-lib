...
namespace Global_Parameters
{
                         ...
                         Vector<double>& result)
  {
    ...
    result[1] =
      cos(time) * (-Mu * (3.0 * pow(x[0], 2) - pow(x[0], 3)) * sin(x[1]));
                  ...
                  Vector<double>& result)
  {
    ...
    result[1] = cos(time) * (x[0] * sin(x[1]) *
                             (Mu * (-9.0) + 4.0 * x[0] * (Lambda + Mu) +
                              pow(x[0], 2) * (Lambda + 2.0 * Mu - Omega_sq)));
    result[2] =
      cos(time) *
      (-x[0] * (8.0 * Mu * pow(x[1], 3) +
  ...
  AxisymmetricLinearElasticityProblem()
{
  ...
  // Now create the mesh
  Bulk_mesh_pt = new RectangularQuadMesh<ELEMENT>(Global_Parameters::Nr,
...
//===================================================================
template<class ELEMENT, class TIMESTEPPER>
void AxisymmetricLinearElasticityProblem<ELEMENT,
                                         TIMESTEPPER>::set_initial_conditions()
{
  ...
  else // Smooth start
  {
    ...
    for (unsigned jnod = 0; jnod < n_node; jnod++)
    {
      ...
      double u_r_exact = Global_Parameters::u_r(time_pt()->time(), x);
      double u_z_exact = Global_Parameters::u_z(time_pt()->time(), x);
      double u_theta_exact = Global_Parameters::u_theta(time_pt()->time(), x);

      // Get exact velocities
      double d_u_r_dt_exact = Global_Parameters::d_u_r_dt(time_pt()->time(), x);
      double d_u_z_dt_exact = Global_Parameters::d_u_z_dt(time_pt()->time(), x);
      ...
      double error =
        sqrt(pow(u_r_exact - u_r_fe, 2) + pow(u_z_exact - u_z_fe, 2) +
    ...
    std::cout << "Max error in assignment of initial conditions " << err_max
              << std::endl;
  }
                                         ...
                                         TIMESTEPPER>::set_boundary_conditions()
{
  ...
  // Storage for pointers to the functions defining the displacement,
  // velocity and acceleration components
  Vector<typename TIMESTEPPER::NodeInitialConditionFctPt> initial_value_fct(3);
  Vector<typename TIMESTEPPER::NodeInitialConditionFctPt> initial_veloc_fct(3);
  Vector<typename TIMESTEPPER::NodeInitialConditionFctPt> initial_accel_fct(3);
  ...
  DocInfo& doc_info)
{
  ...
  Bulk_mesh_pt->output_fct(
    some_file, npts, time_pt()->time(), Global_Parameters::exact_solution);
  some_file.close();
...
int main(int argc, char* argv[])
{
  ...
  AxisymmetricLinearElasticityProblem<QAxisymmetricLinearElasticityElement<3>,
                                      Newmark<1>>
    problem;
