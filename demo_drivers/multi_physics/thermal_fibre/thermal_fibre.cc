...
// LIC//
// LIC//====================================================================
// Driver for a multi-physics problem that couples the Axisymmetric
// Navier--Stokes equations to the axisymmetric advection diffusion equations,
...
private:
  ...
  /// Delete Axisymmetric Advection Diffusion flux elements and wipe the surface
  /// mesh
    ...
    for (unsigned i = 0; i < n_spine; i++)
    {
      ...
      if (z_value <= (Height / 1.1))
      {
        ...
        Bulk_mesh_pt->spine_pt(i)->height() =
          sqrt(exp(-log(Dr) * (1.0 - (1.1 * z_value / Height))));
      }
  ...
  : Lr(l_r), Height(h)
{
  ...
  Bulk_mesh_pt = new HorizontalSingleLayerSpineMesh<ELEMENT>(n_r, n_z, l_r, h);
...
//========================================================================
template<class ELEMENT>
void AxisymFreeSurfaceNozzleAdvDiffRobinProblem<
  ELEMENT>::actions_before_newton_solve()
{
  ...
  for (unsigned n = 0; n < n_node; n++)
  {
    ...
    double ur_value = -0.5 * r_value * (1.0 / Height) * log(Dr) *
                      exp(log(Dr) * (1.0 - (1.1 * z_value / Height)));
...
void AxisymFreeSurfaceNozzleAdvDiffRobinProblem<ELEMENT>::steady_run()
{
  ...
  {
    cout << "Solving for Re = " << Global_Physical_Variables::Re
         << " Ca = " << Global_Physical_Variables::Ca
         << " Pe = " << Global_Physical_Variables::Pe
         << " Bi = " << Global_Physical_Variables::Bi << std::endl;
...
int main(int argc, char** argv)
{
  ...
  AxisymFreeSurfaceNozzleAdvDiffRobinProblem<
    SpineElement<NonIsothermalAxisymmetricQCrouzeixRaviartElement>>
