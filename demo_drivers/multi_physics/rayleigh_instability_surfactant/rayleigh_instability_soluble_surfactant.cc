...
protected:
    ...
    const double& J)
  {
    ...
    ElasticAxisymmetricSurfactantTransportInterfaceElement<
      ELEMENT>::add_additional_residual_contributions_interface(residuals,
                                                                jacobian,
                                                                flag,
                                                                psif,
                                                                dpsifds,
...
void InterfaceProblem<ELEMENT, TIMESTEPPER>::doc_solution(DocInfo& doc_info)
{
  ...
  Trace_file << time_pt()->time() << " " << Document_node_pt->x(0) << " "
...
int main(int argc, char* argv[])
{
  ...
  InterfaceProblem<
    PseudoSolidNodeUpdateElement<AxisymmetricQAdvectionCrouzeixRaviartElement,
                                 QPVDElement<2, 3>>,
    BDF<2>>
    problem(n_r, n_z, l_z);
