...
namespace oomph
{
  ...
  {
  public:
    AnnularSpineMesh(const unsigned& n_r,
                     const unsigned& n_y,
      ...
      : // This will make a cubic mesh with n_r in the x-direction
        ...
        SingleLayerCubicSpineMesh<ELEMENT>(
          n_theta, n_y, n_r, r_max, l_y, theta_max, time_stepper_pt)
    {
      ...
      for (unsigned n = 0; n < n_node; n++)
      {
        ...
        {
          spine_node_pt->h() = Global_Physical_Variables::Film_Thickness;
...
void InterfaceProblem<ELEMENT, TIMESTEPPER>::unsteady_run(const unsigned& nstep)
{
  ...
  for (unsigned i = 0; i < Nspine; i++)
  {
    ...
    Bulk_mesh_pt->spine_pt(i)->height() =
      ...
      (1.0 + +epsilon * cos(Global_Physical_Variables::Alpha * y_value));
  }
...
int main(int argc, char* argv[])
{
...

  {
    ...
    InterfaceProblem<SpineElement<QCrouzeixRaviartElement<3>>, BDF<2>> problem(
      n_r, n_y, n_theta, r_min, r_max, l_y, theta_max);
