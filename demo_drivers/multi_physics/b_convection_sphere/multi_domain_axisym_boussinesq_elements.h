...
public:
  ...
  void further_build()
  {
    ...
    // element type
    RefineableQAxisymCrouzeixRaviartBoussinesqElement* cast_father_element_pt =
                              ...
                              Vector<double>& body_force)
  {
    ...
    const double interpolated_t = adv_diff_el_pt->interpolated_u_axi_adv_diff(
...
public:
                                        ...
                                        DenseMatrix<double>& jacobian)
  {
    ...
    RefineableQAxisymAdvectionDiffusionElement<
      3>::fill_in_contribution_to_jacobian(residuals, jacobian);
                                           ...
                                           DenseMatrix<double>& jacobian)
  {
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      // Call the derivatives of the shape and test functions
      double J = this->dshape_and_dtest_eulerian_at_knot_axi_adv_diff(
...
public:
  ...
  QAxisymCrouzeixRaviartElementWithExternalElement()
    : AxisymmetricQCrouzeixRaviartElement(), ElementWithExternalElement()
  {
    Ra_pt = &Default_Physical_Constant_Value;
                                        ...
                                        DenseMatrix<double>& jacobian)
  {
    ...
    // Get the contribution from the basic Navier--Stokes element
    AxisymmetricQCrouzeixRaviartElement::fill_in_contribution_to_jacobian(
      residuals, jacobian);
...
public:
  ...
  QAxisymAdvectionDiffusionElementWithExternalElement()
    : QAxisymAdvectionDiffusionElement<3>(), ElementWithExternalElement()
  {
    // There is only one interaction
    this->set_ninteraction(1);
  }
                                        ...
                                        DenseMatrix<double>& jacobian)
  {
    ...
    QAxisymAdvectionDiffusionElement<3>::fill_in_contribution_to_jacobian(
      residuals, jacobian);
    // Get the off-diagonal terms analytically
    this->fill_in_off_diagonal_block_analytic(residuals, jacobian);
...
/// from the "source" AdvectionDiffusion element via current integration point
//========================================================
void QAxisymCrouzeixRaviartElementWithExternalElement::get_body_force_axi_nst(
  const double& time,
  const unsigned& ipt,
  const Vector<double>& s,
  const Vector<double>& x,
...
void QAxisymAdvectionDiffusionElementWithExternalElement::
  get_dwind_axi_adv_diff_dexternal_element_data(
    const unsigned& ipt,
    const unsigned& i,
    Vector<double>& result,
    Vector<unsigned>& global_eqn_number)
...
//=========================================================================
double QAxisymCrouzeixRaviartElementWithExternalElement::
  Default_Physical_Constant_Value = 0.0;
