...
public:
  ...
  QCrouzeixRaviartElementWithTwoExternalElement()
    : QCrouzeixRaviartElement<DIM>(), ElementWithExternalElement()
  {
                                        ...
                                        DenseMatrix<double>& jacobian)
  {
    ...
    QCrouzeixRaviartElement<DIM>::fill_in_contribution_to_jacobian(residuals,
                                                                   jacobian);
...
public:
                                        ...
                                        DenseMatrix<double>& jacobian)
  {
    ...
    QAdvectionDiffusionElement<DIM, 3>::fill_in_contribution_to_jacobian(
      residuals, jacobian);
    // Get the off-diagonal terms analytically
    this->fill_in_off_diagonal_block_analytic(residuals, jacobian);
#endif
...
//========================================================
template<unsigned DIM>
void QCrouzeixRaviartElementWithTwoExternalElement<DIM>::get_body_force_nst(
  const double& time,
  const unsigned& ipt,
  const Vector<double>& s,
  const Vector<double>& x,
  ...
  const double interpolated_t =
    dynamic_cast<AdvectionDiffusionEquations<DIM>*>(
      ...
      ->interpolated_u_adv_diff(
        external_element_local_coord(t_interaction, ipt));


  // Get the interpolated concentration field from the external element
  const double interpolated_c =
    dynamic_cast<AdvectionDiffusionEquations<DIM>*>(
      external_element_pt(c_interaction, ipt))
      ->interpolated_u_adv_diff(
        external_element_local_coord(c_interaction, ipt));
...
//=========================================================================
template<unsigned DIM>
void QCrouzeixRaviartElementWithTwoExternalElement<
                                                     ...
                                                     global_eqn_number)
{
  ...
  for (unsigned i = 0; i < DIM; i++)
  {
    ...
    {
      result(i, n_external_temp_data + n) =
        -gravity[i] * du_conc_ddata[n] * ra_s();
    }
  }
...
//=========================================================================
template<>
double QCrouzeixRaviartElementWithTwoExternalElement<
...
int main(int argc, char** argv)
{
  ...
  trace << problem.time_pt()->time() << " "
        << problem.nst_mesh_pt()->boundary_node_pt(1, 8)->value(1) << " " << E
        << " " << Edot << std::endl;
