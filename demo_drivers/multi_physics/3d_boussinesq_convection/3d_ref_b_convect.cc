...
/// diagonal blocks.
//=============================================================================
class BoussinesqPreconditioner : public BlockPreconditioner<CRDoubleMatrix>
{
public:
...
// only use HYPRE if it's been initialised
...
#endif
    {
      ...
      Temperature_preconditioner_pt = 0;
    ...
    if (Retain_temperature_onto_fluid_terms)
    {
      oomph_info
        ...
        << double(Block_matrix_0_1_pt->nnz()) /
             double(Block_matrix_0_1_pt->nrow() * Block_matrix_0_1_pt->ncol()) *
             100.0
        << "%  " << std::endl;
    }
    ...
    if (Retain_fluid_onto_temperature_terms)
    {
      oomph_info
        ...
        << double(Block_matrix_1_0_pt->nnz()) /
             double(Block_matrix_1_0_pt->nrow() * Block_matrix_1_0_pt->ncol()) *
             100.0
        << "%  " << std::endl;
    }
  }
...
public:
  ...
  void get_interpolated_values(const Vector<double>& s, Vector<double>& values)
  {
    ...
    RefineableQCrouzeixRaviartElement<DIM>::get_interpolated_values(s,
                                                                    nst_values);
                ...
                for (unsigned m2 = 0; m2 < n_master2; m2++)
                {
                  ...
                  if (local_unknown >= 0)
                  {
                    ...
                    jacobian(local_eqn, local_unknown) +=
                      -gravity[i] * psif(l2) * Ra * testf(l) * W * hang_weight *
                      hang_weight2;
                  }
                }
              }
...
template<>
double
  RefineableBuoyantQCrouzeixRaviartElement<3>::Default_Physical_Constant_Value =
    0.0;
...
int main(int argc, char** argv)
{
  ...
  RefineableConvectionProblem<RefineableBuoyantQCrouzeixRaviartElement<3>>
    problem;
