...
namespace oomph
{
        ...
        if (local_eqn >= 0)
        {
          ...
          if (flag)
          {
            ...
            for (unsigned l2 = 0; l2 < n_node; l2++)
            {
              ...
              {
                jacobian(local_eqn, local_unknown) -=
                  Beta_b * dflux_dC * psif(l2) * psif(l) * W * J;
              }
            }
          }
        }
  ...
  public:
    ...
    double l2_norm_of_height(const double& h0)
    {
      ...
      // which it is
      double scale =
        (this->node_pt(n_node - 1)->x(0) - this->node_pt(0)->x(0)) / 2.0;
  ...
  {
  public:
    FluidInterfaceAdditionalValues<SolubleSurfactantTransportInterfaceElement>()
    {
    }
  ...
  class BoundingElementType<ElasticUpdateFluidInterfaceElement<
    SolubleSurfactantTransportInterfaceElement,
    LineDerivatives,
    ELEMENT>> : public ElasticPointFluidInterfaceBoundingElement<ELEMENT>
  {
  public:
  ...
  public:
    ...
    SpinePointMarangoniSurfactantFluidInterfaceBoundingElement()
      : SpinePointFluidInterfaceBoundingElement<ELEMENT>(), C_index(0)
    {
    }
                                          ...
                                          DenseMatrix<double>& jacobian)
    {
      ...
      // Make sure to include concentration variations
      {
        ...
        // Use finite differences to handle the interfacial concentration
        // variations Loop over the nodes again
        for (unsigned n = 0; n < n_node; n++)
        {
