...
namespace oomph
{
  ...
  public:
    ...
    unsigned required_nvalue(const unsigned& n) const
    {
      return (RefineableQAdvectionDiffusionReactionElement<2, DIM, 3>::
                required_nvalue(n) +
                                 ...
                                 Vector<double>& values)
    {
      ...
      for (unsigned i = 0; i < 2; i++)
      {
        values.push_back(advection_values[i]);
      }
    }
    ...
    void further_build()
    {
      ...
      // element type
      RefineableDoubleBuoyantQCrouzeixRaviartElement<DIM>*
    ...
    void get_Z2_flux(const Vector<double>& s, Vector<double>& flux)
    {
      ...
      RefineableQAdvectionDiffusionReactionElement<2, DIM, 3>::get_Z2_flux(
        s, conc_flux);
        ...
        for (unsigned i = 0; i < DIM; i++)
        {
              ...
              *value_pt += fd_step;
              ...
              for (unsigned l = 0; l < n_node; l++)
              {
                for (unsigned r = 0; r < 2; r++)
                {
                  // Local equation for temperature or solute
                  local_eqn =
                    this->nodal_local_eqn(l, C_nodal_adv_diff_react[r]);
          ...
          else
          {
            ...
            for (unsigned m = 0; m < n_master; m++)
            {
              ...
              local_unknown =
                this->local_hang_eqn(master_node_pt, u_nodal_nst[i]);
              // If the variable is free
              if (local_unknown >= 0)
              {
                // Get a pointer to the nodal value stored at the master node
                double* const value_pt =
                  master_node_pt->value_pt(u_nodal_nst[i]);
                // Save the old value
                double old_var = *value_pt;
        ...
        for (unsigned r = 0; r < 2; r++)
        {
            ...
            if (local_unknown >= 0)
            {
              ...
              *value_pt += fd_step;
              ...
              RefineableNavierStokesEquations<
                DIM>::fill_in_contribution_to_residuals(newres);
          ...
          else
          {
            ...
            HangInfo* hang_info_pt =
              local_node_pt->hanging_pt(C_nodal_adv_diff_react[r]);
            // Loop over the master nodes
            const unsigned n_master = hang_info_pt->nmaster();
            for (unsigned m = 0; m < n_master; m++)
            {
              ...
              // Get the number of the unknown
              local_unknown =
                this->local_hang_eqn(master_node_pt, C_nodal_adv_diff_react[r]);
              // If the variable is free
              if (local_unknown >= 0)
              {
                // Get a pointer to the nodal value stored at the master node
                double* const value_pt =
                  master_node_pt->value_pt(C_nodal_adv_diff_react[r]);
  ...
  //=========================================================================
  template<>
  double RefineableDoubleBuoyantQCrouzeixRaviartElement<
    2>::Default_Physical_Constant_Value = 1.0;
  ...
  template<>
  class FaceGeometry<
    FaceGeometry<RefineableDoubleBuoyantQCrouzeixRaviartElement<2>>>
    : public virtual PointElement
  {
  public:
    FaceGeometry() : PointElement() {}
