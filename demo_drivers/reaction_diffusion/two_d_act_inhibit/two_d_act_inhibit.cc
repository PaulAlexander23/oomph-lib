  ...
  const unsigned& nx, const unsigned& ny)
{
  ...
  Problem::mesh_pt() = new RectangularQuadMesh<ELEMENT>(
    nx, ny, 2.0, 2.0, Problem::time_stepper_pt());
  ...
  {
    mesh_pt()->boundary_node_pt(1, n)->make_periodic(
      mesh_pt()->boundary_node_pt(3, n));
  }
...
int main()
{
  ...
  ActivatorInhibitorProblem<QAdvectionDiffusionReactionElement<2, 2, 3>>
    problem(20, 20);
