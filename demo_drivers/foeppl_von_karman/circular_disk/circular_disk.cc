...
public:
  ...
  void actions_after_adapt()
  {
    Volume_constraint_element_pt =
      new FoepplvonKarmanVolumeConstraintElement<ELEMENT,
                                                 RefineableTriangleMesh>(
        My_mesh_pt, Bubble_regions, *Temp_pressure_pt);
    ...
    Element_area(element_area)
{
  ...
  nsegment =
    (int)(TestSoln::r_b * MathematicalConstants::Pi / sqrt(element_area));
  ...
  Volume_constraint_element_pt =
    new FoepplvonKarmanVolumeConstraintElement<ELEMENT, RefineableTriangleMesh>(
      My_mesh_pt, Bubble_regions);
  Volume_constraint_element_pt->set_prescribed_volume(
    &TestSoln::prescribed_volume);
...
void UnstructuredFvKProblem<ELEMENT>::doc_solution(const std::string& comment)
{
  ...
  //------------------
  sprintf(
    filename, "RESLT/boundaries%i-%f.dat", Doc_info.number(), Element_area);
  some_file.open(filename);
  ...
  // ----------------------------
  sprintf(
    filename, "RESLT/radial_soln%i-%f.dat", Doc_info.number(), Element_area);
  some_file.open(filename);
  ...
  My_mesh_pt->compute_error(some_file, TestSoln::zero, dummy_error, zero_norm);
  some_file.close();

  // Doc L2 error and norm of solution
  oomph_info << "Norm of computed solution: " << sqrt(dummy_error) << std::endl;

  Trace_file << TestSoln::p_b_pt->value(0) << " " << TestSoln::p_0_pt->value(0)
             << " " << w_0 << " " << bubble_volume << '\n';
