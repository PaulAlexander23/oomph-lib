...
YoungLaplaceProblem<ELEMENT>::YoungLaplaceProblem()
{
  ...
  if (GlobalParameters::Use_height_control)
  {
    ...
    height_control_element_pt->kappa_pt()->set_value(
      0, GlobalParameters::Kappa_initial);
  }
  ...
  for (unsigned b = 0; b < n_bound; b++)
  {
    ...
    if ((GlobalParameters::Case == GlobalParameters::All_pinned) || (b == 0) ||
        (b == 2))
    {
      ...
      GlobalParameters::T_junction_with_nonzero_contact_angle)
  {
    ...
    for (unsigned e = Last_element_on_boundary1; e < Last_element_on_boundary3;
         e++)
    {
      // Upcast from GeneralisedElement to YoungLaplace contact angle element
      YoungLaplaceContactAngleElement<ELEMENT>* el_pt =
    ...
    for (unsigned bnd = 0; bnd < 2; bnd++)
    {
      ...
      for (unsigned e = el_lo; e < el_hi; e++)
      {
        ...
        for (unsigned i = 0; i < npts; i++)
        {
          ...
          tangent_file << r_contact[0] << " " << r_contact[1] << " "
                       << r_contact[2] << " " << tangent[0] << " " << tangent[1]
                       << " " << tangent[2] << " " << std::endl;

          normal_file << r_contact[0] << " " << r_contact[1] << " "
                      << r_contact[2] << " " << normal[0] << " " << normal[1]
                      << " " << normal[2] << " " << std::endl;
