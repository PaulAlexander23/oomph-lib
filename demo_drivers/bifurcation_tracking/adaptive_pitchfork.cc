  ...
  int operator()(Node* const& node1_pt, Node* const& node2_pt)
  {
    ...
    // Check that the dimensions of both nodes are the same
    {
      ...
      if (n_dim != n_dim2)
      {
        ...
        warn_message << "Warning: Two nodess do not have the same dimension"
                     << n_dim << " and " << n_dim2
                     << ". They cannot be compared\n";
        OomphLibWarning(
          warn_message.str(), "CompareNodes::()", OOMPH_EXCEPTION_LOCATION);
      }
    }
    ...
    // to within a given tolerance
    {
      unsigned i = 0;
      double tol = 1.0e-14;
      while (i != n_dim && (std::abs(x1[i] - x2[i]) < tol))
      {
        ++i;
      }
...
public:
  ...
  void symmetrise_eigenfunction_for_adaptive_pitchfork_tracking()
  {
    ...
    std::sort(local_nodes_pt.begin(), local_nodes_pt.end(), CompareNodes());
      ...
      if (std::abs(local_nodes_pt[n]->x(0) - x) > 1.0e-14)
      {
        ...
        for (unsigned i = 0; i < n_value; i++)
        {
          ...
          if (symm[i] == -1)
          {
            ...
            if (odd)
            {
              local_nodes_pt[column_start + n_half]->set_value(i, 0.0);
            }
...
int main()
{
  ...
  RefineablePorousChannelProblem<RefineableQTaylorHoodElement<2>> problem;
  ...
  problem.solve_eigenproblem(
    4, eigenvalues, eigenvector_real, eigenvector_imag);

  // Find the eigenvalue with greatest real part
  unsigned ev_crit_index = 0;
