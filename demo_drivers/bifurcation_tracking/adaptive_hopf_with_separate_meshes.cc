...
public:
  ...
  RefineableRectangleWithHoleMesh(
    ...
    : RectangleWithHoleMesh<ELEMENT>(
        cylinder_pt, length, height, time_stepper_pt)
  {
...
public:
  ...
  void actions_after_adapt()
  {
    ...
    {
      // Unpin all pressure dofs
      RefineableLinearisedNavierStokesEquations::unpin_all_pressure_dofs(
        Eigenproblem_mesh_pt->element_pt());

      // Pin redudant pressure dofs
      RefineableLinearisedNavierStokesEquations::pin_redundant_nodal_pressures(
        Eigenproblem_mesh_pt->element_pt());
  ...
  void add_eigenproblem()
  {
    ...
    Eigenproblem_mesh_pt =
      new RefineableRectangleWithHoleMesh<PERTURBED_ELEMENT>(
    ...
    *local_re_pt = Re;
    ...
    // Loop over all the (fluid) elements
    {
      ...
      for (unsigned e = 0; e < n_element; e++)
      {
        ...
        // Set the eigenfunction normalisation element
        el_pt->set_eigenfunction_normalisation_element(
          normalisation_element_pt);
...

{
  ...
  // Build mesh
  Base_flow_mesh_pt = new RefineableRectangleWithHoleMesh<BASE_ELEMENT>(
    cylinder_pt, length, height);
...
//=========================================================================
template<class BASE_ELEMENT, class PERTURBED_ELEMENT>
FlowAroundCylinderProblem<BASE_ELEMENT,
                          PERTURBED_ELEMENT>::~FlowAroundCylinderProblem()
{
...
int main()
{
  ...
  else
  {
    ...
    problem.solve_eigenproblem(
      6, eigenvalues, eigenvector_real, eigenvector_imag);
