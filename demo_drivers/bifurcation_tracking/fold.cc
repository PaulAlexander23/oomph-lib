...
public:
    ...
    DenseMatrix<double>& djac_dparam)
  {
    // Call the generic routine with the flag set to 1
    fill_in_generic_dresidual_contribution(
      parameter_pt, dres_dparam, djac_dparam, 1);
  }
                                              ...
                                              unsigned flag)
  {
    ...
    if ((parameter_pt != Lambda_pt) && (parameter_pt != Mu_pt))
    {
      ...
      error_stream
        ...
        << parameter_pt << "\n";
      error_stream << "Can only compute derivatives wrt lambda (" << Lambda_pt
                   << ") or mu (" << Mu_pt << ")\n";
      throw OomphLibError(
  ...
  void output(ostream& output)
  {
    ...
    for (unsigned n = 0; n < n_node; n++)
    {
      output << this->nodal_position(n, 0) << " " << this->nodal_value(n, 0)
             << std::endl;
    }
  } // End of function
