...
public:
  ...
  void actions_before_adapt()
  {
    ...
    Bulk_mesh_pt->boundary_polyline_pt(Contact_boundary_id)
      ->set_maximum_length(Maximum_element_length_on_contact_boundary *
...
private:
  ...
  void create_contact_elements()
  {
    ...
    for (unsigned e = 0; e < n_element; e++)
    {
      ...
      LinearSurfaceContactElement<ELEMENT>* contact_element_pt =
        new LinearSurfaceContactElement<ELEMENT>(
...
int main(int argc, char* argv[])
{
  ...
  // Build problem
  ContactProblem<ProjectableLinearElasticityElement<
  ...
  for (unsigned i = 0; i < nstep; i++)
  {
    oomph_info << "Re-solving imposed circle pos for element length factor: "
               << ProblemParameters::Element_length_factor << std::endl;
