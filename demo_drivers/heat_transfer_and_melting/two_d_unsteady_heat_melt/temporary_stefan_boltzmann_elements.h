...
namespace oomph
{
                                                   ...
                                                   const Vector<double>& n)
  {
    ...
    Atmospheric_radiation_fct_pt(time,
    ...
    //-------------------------------------
    double phi_exposed =
      Diffuse_limit_angles[intpt].second - Diffuse_limit_angles[intpt].first;
    if (phi_exposed > 0.0)
    {
      radiation +=
        total_diffuse_radiation * phi_exposed / MathematicalConstants::Pi;
    ...
    double cos_angle =
      -(n[0] * solar_flux_unit_vector[0] + n[1] * solar_flux_unit_vector[1]);
                                               ...
                                               bool& no_problem)
  {
    ...
    else if ((x_prev < 0.0) && (y_prev < 0.0) && (x_next > 0.0) &&
             (y_next > 0.0))
    {
      crossing_quadrants = true;
    ...
    const Vector<Node*>& shielding_node_pt)
  {
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      if (x_prev > 0.0)
      {
        throw OomphLibError("Leftmost point in shielding nodes is not to the "
                            "left of current int pt",
                            OOMPH_CURRENT_FUNCTION,
                            OOMPH_EXCEPTION_LOCATION);
      }
#endif
      ...
      oomph_info << "Vertex point " << 0 << " : " << nod_pt->x(0) << " "
                 << nod_pt->x(1) << " phi_raw: " << phi_prev;
      ...
      for (unsigned j = 1; j <= j_left; j++)
      {
        ...
        oomph_info << "Vertex point " << j << " : " << nod_pt->x(0) << " "
                   << nod_pt->x(1) << " phi_raw: " << phi_next;
        ...
        else
        {
          ...
          error_message << "\n x/y_prev, x/y_next:" << x_prev << " " << y_prev
                        << " " << x_next << " " << y_next << " " << std::endl;
          error_message << "for point at " << x[0] << " " << x[1] << std::endl;
      ...
      if (x_prev < 0.0)
      {
        throw OomphLibError("Rightmost point in shielding nodes is not to the "
                            "right of current int pt",
                            OOMPH_CURRENT_FUNCTION,
                            OOMPH_EXCEPTION_LOCATION);
      }
#endif
    ...
    std::ostream& outfile, const double& radius)
  {
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      for (unsigned j = 0; j < nplot; j++)
      {
        ...
        outfile << x[0] + fract * (1.0 + 0.1 * double(j) / double(nplot - 1)) *
                            radius * cos(phi)
                << " "
                << x[1] + fract * (1.0 + 0.1 * double(j) / double(nplot - 1)) *
                            radius * sin(phi)
                << "\n";
      }
      outfile << x[0] << " " << x[1] << "\n"
  ...
  void SolarRadiationBase::output_limiting_angles(std::ostream& outfile)
  {
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      outfile << x[0] << " " << x[1] << " " << normal[0] << " " << normal[1]
              << " " << phi_min << " " << phi_max << "\n";
    }
  }
  ...
  void SolarRadiationBase::output_atmospheric_radiation(std::ostream& outfile)
  {
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      // Get radiation
      double radiation = atmospheric_radiation(ipt, time, x, normal);
      // output
      outfile << x[0] << " " << x[1] << " " << radiation << " " << normal[0]
              << " " << normal[1] << "\n";
    }
  }
  ...
  public:
      ...
      const bool& add_solid_position_data = true)
    {
      ...
      {
        if (Stefan_boltzmann_illumination_info[ipt][e].first ==
            illuminating_el_pt)
        {
          throw OomphLibError("Element has already been added!",
                              OOMPH_CURRENT_FUNCTION,
                              OOMPH_EXCEPTION_LOCATION);
        }
      }
      ...
      Stefan_boltzmann_illumination_info[ipt].push_back(std::make_pair(
        illuminating_el_pt, illuminating_integration_point_index));
      ...
      std::ofstream& outfile)
  {
    ...
    {
      outfile << "ZONE\n";
      outfile << r_illuminated[0] << " " << r_illuminated[1] << "0 0\n";
    }
    ...
    for (unsigned ii = 0; ii < nint; ii++)
    {
      ...
      {
        // Add to temperature
        interpolated_u += node_pt(l)->value(U_index_ust_heat) * psi[l];
      ...
      // Angles
      double cos_phi =
        inv_length * (n_illuminated[0] * ray[0] + n_illuminated[1] * ray[1]);

      // INTEGRAL IS:
      // \int sigma T^4 1/2 \cos \varphi d \varphi =
      // where
      // d\varphi=1/|{\bf R}| \partial {\bf R}/\partial s \cdot {\bf e}_\varphi
      // ds

      double integrand =
        this->sigma() * pow((interpolated_u + this->theta_0()), 4) * 0.5 *
        cos_phi *
    ...
    std::ostream& outfile, const unsigned& integration_point)
  {
    ...
    for (unsigned ipt = ipt_lo; ipt <= ipt_hi; ipt++)
    {
      ...
      for (unsigned i2 = 0; i2 < n_illuminating; i2++)
      {
        ...
        unsigned n_pts =
          (Stefan_boltzmann_illumination_info[ipt][i2].second).size();
  ...
  public:
    ...
    /// temperature, total incoming flux, incoming s.b. flux, outgoing s.b.
    /// flux, outer unit normal. Note that total incoming flux is simply given
    ...
    void output_stefan_boltzmann_radiation(std::ostream& outfile)
    {
      ...
      for (unsigned ipt = 0; ipt < n_intpt; ipt++)
      {
        ...
        double incoming_sb_radiation =
          incoming_stefan_boltzmann_radiation(ipt, x, unit_normal);
        ...
        {
          std::stringstream error_message;
          error_message << "Difference between incoming ( "
                        << incoming_sb_radiation << " ) and outgoing ( "
                        << outgoing_sb_radiation << " ) is "
                        << (incoming_sb_radiation - outgoing_sb_radiation)
                        << " but total flux is " << flux << " so difference is "
                        << fabs(flux - (incoming_sb_radiation -
                                        outgoing_sb_radiation));
          throw OomphLibError(error_message.str(),
                              OOMPH_CURRENT_FUNCTION,
                              OOMPH_EXCEPTION_LOCATION);
        }
  ...
  protected:
                          ...
                          double& flux)
    {
      ...
      // local temperature
      double outgoing_sb_radiation =
        this->sigma() * pow((u + this->theta_0()), 4);

      // Incoming Stefan Boltzmann radiation
      double incoming_sb_radiation =
        incoming_stefan_boltzmann_radiation(ipt, x, n);
      ...
      // Net flux
      flux =
        incoming_sb_radiation - outgoing_sb_radiation + incoming_atmospheric;
    }
  };
  ...
  namespace StefanBoltzmannHelper
  {
                    ...
                    std::ofstream& outfile)
    {
      ...
      if (outfile.is_open())
      {
        ...
        {
          // Issue a warning
          OomphLibWarning("Not outputting while bin is being populated...\n",
                          OOMPH_CURRENT_FUNCTION,
                          OOMPH_EXCEPTION_LOCATION);
        }
        else
      ...
      unsigned ix_start =
        unsigned((lower_ray_vertex[0] - Min_coord[0]) /
                 (Max_coord[0] - Min_coord[0]) * double(Nx_bin));
      unsigned iy_start =
        unsigned((lower_ray_vertex[1] - Min_coord[1]) /
                 (Max_coord[1] - Min_coord[1]) * double(Ny_bin));
      if (populate_bin)
      {
      ...
      if (lower_ray_vertex[0] == upper_ray_vertex[0])
      {
        ...
        unsigned iy_end =
          unsigned((upper_ray_vertex[1] - Min_coord[1]) /
                   (Max_coord[1] - Min_coord[1]) * double(Ny_bin));
      ...
      else
      {
        ...
        double slope = (upper_ray_vertex[1] - lower_ray_vertex[1]) /
        ...
        while (y_this_bin_min < upper_ray_vertex[1])
        {
          ...
          unsigned ix_intersect =
            unsigned((x_intersect - Min_coord[0]) /
                     (Max_coord[0] - Min_coord[0]) * double(Nx_bin));
          ...
          for (unsigned i = i_lo; i <= i_hi; i++)
          {
            ...
            else
            {
              ...
              else if (x_right_end_bin <
                       std::min(upper_ray_vertex[0], lower_ray_vertex[0]))
              {
                add_it = false;
              }
            }
                           ...
                           const Vector<FiniteElement*>& sb_face_element_pt)
    {
      ...
      for (unsigned e = 0; e < nel; e++)
      {
        ...
        {
          std::stringstream error_message;
          error_message << "Failed to cast possible intersecting element " << e
      ...
      const Vector<FiniteElement*>& sb_face_element_pt)
    {
      ...
      for (unsigned e_illuminated = 0; e_illuminated < nel; e_illuminated++)
      {
        ...
        if (illuminated_el_pt == 0)
        {
          ...
          error_message << "Failed to cast illuminated element "
                        << e_illuminated << " to  StefanBoltzmannRadiationBase";
          throw OomphLibError(error_message.str(),
                              OOMPH_CURRENT_FUNCTION,
                              OOMPH_EXCEPTION_LOCATION);
        }
      ...
      if (use_bins)
      {
        ...
        for (unsigned e_illuminated = 0; e_illuminated < nel; e_illuminated++)
        {
          ...
          for (unsigned ipt_illuminated = 0; ipt_illuminated < nint;
               ipt_illuminated++)
          {
            // Local coordinate of integration point
            s_illuminated[0] =
            ...
            for (unsigned i = 0; i < 2; i++)
            {
              if (r_illuminated[i] > Max_coord[i])
                Max_coord[i] = r_illuminated[i];
              if (r_illuminated[i] < Min_coord[i])
                Min_coord[i] = r_illuminated[i];
            }
          }
        }
        ...
        for (unsigned e = 0; e < nel; e++)
        {
          ...
          if (el_pt == 0)
          {
            ...
            error_message << "Failed to cast possible intersecting element "
                          << e << " to  StefanBoltzmannRadiationBase";
            throw OomphLibError(error_message.str(),
                                OOMPH_CURRENT_FUNCTION,
                                OOMPH_EXCEPTION_LOCATION);
          }
        ...
        oomph_info << "Time for setting up bin: " << t_end_bin - t_start_bin
                   << std::endl;
               ...
               ipt_illuminated++)
          {
                 ...
                 e_illuminating++)
            {
                   ...
                   ipt_illuminating++)
              {
                ...
                // No recycling of shape fcts -- may as well call interpolated_x
                // directly
                illuminating_el_pt->interpolated_x(s_illuminating,
                                                   r_illuminating);

                // Get outer unit normal
                illuminating_face_el_pt->outer_unit_normal(
                  s_illuminating, unit_normal_illuminating);
                  ...
                  if (dot_illuminated < 0.0)
                  {
                    ...
                    if (use_bins)
                    {
                      ...
                      for (unsigned b = 0; b < nbin; b++)
                      {
                             ...
                             it++)
                        {
...
#endif
                          // Skip intersections with illuming/illuminated
                          // element
                              ...
                              (el_pt != illuminating_el_pt))
                          {
                            ...
                            for (unsigned j_sample = 0; j_sample < Nsample - 1;
                                 j_sample++)
                            {
                              ...
                              // Get cector of local coordinates of plot point
                              // j+1 as second vertex
                              ...
                              // Check intersection
                              bool intersection =
                                IntersectionChecker::intersects(segment_vertex,
                                                                ray_vertex);
                      ...
                      if (!have_intersection)
                      {
                        ...
                        illuminating_integration_point_index.push_back(
                          ipt_illuminating);
                      }
                    }
                           ...
                           e_intersect++)
                      {
                              ...
                              (e_intersect == e_illuminating)))
                        {
                          ...
                          for (unsigned j = 0; j < Nsample - 1; j++)
                          {
                            ...
                            // as first vertex
                            Vector<double> s(1);
                            sb_face_element_pt[e_intersect]->get_s_plot(
                              j, Nsample, s);

                            // Get coordinate of first vertex
                            sb_face_element_pt[e_intersect]->interpolated_x(
                              s, segment_vertex[0]);
                    ...
                    } // end if for brute force (rather than bin-based) search
                      // loop
                  }
                }
              }
