...
ContactProblem<ELEMENT>::ContactProblem()
{
  ...
  for (unsigned j = 1; j < npt_right; j++)
  {
    ...
    double x =
      X_ur - (X_ur - x_contact_end_right) * double(j) / double(npt_right - 1);
    double y = y_c;
    if (ProblemParameters::Radius_of_elastic_body > 0.0)
    {
      y -= sqrt(radius * radius -
                (x - 0.5 * (X_ll + X_ur)) * (x - 0.5 * (X_ll + X_ur)));
    }
  ...
  for (unsigned j = 1; j < npt_contact; j++)
  {
    ...
    double x =
      x_contact_end_right - (x_contact_end_right - x_contact_end_left) *
  ...
  for (unsigned j = 1; j < npt_left - 1; j++)
  {
    ...
    double x = x_contact_end_left -
               (x_contact_end_left - X_ll) * double(j) / double(npt_left - 1);
    double y = y_c;
    if (ProblemParameters::Radius_of_elastic_body > 0.0)
    {
      y -= sqrt(radius * radius -
...
int main(int argc, char* argv[])
{
  ...
  ProblemParameters::Centre[1] =
    ProblemParameters::Y_c + ProblemParameters::Radius;
  ...
  for (unsigned i = 0; i < nstep; i++)
  {
    ...
    oomph_info << "Re-solving for weight="
               ...
               << dynamic_cast<CircularPenetratorElement*>(
                    ...
                    ->target_horizontal_force()
               << " and body force ampl: "
               << ProblemParameters::Body_force_amplitude << std::endl;
