...
namespace oomph
{
  ...
  public:
    ...
    inline unsigned u_index_ust_heat() const
    {
      return DIM;
    }
                                          ...
                                          DenseMatrix<double>& jacobian)
    {
      ...
      // residuals)
      LinearElasticityEquations<DIM>::fill_in_contribution_to_jacobian(
        residuals, jacobian);

      // Calculate the unsteady heat contributions (diagonal block and
      // residuals)
      UnsteadyHeatEquations<DIM>::fill_in_contribution_to_jacobian(residuals,
                                                                   jacobian);
  ...
  public:
    ...
    unsigned nhistory_values_for_projection(const unsigned& fld)
    {
      ...
      if (fld > 3)
      {
        ...
        error_stream << "Elements only store three fields so fld can't be"
                     << " " << fld << std::endl;
        throw OomphLibError(
  ...
  template<class ELEMENT>
  class FaceGeometry<
    FaceGeometry<ProjectableLinearHeatAndElasticityElement<ELEMENT>>>
  ...
  public:
        ...
        Use_collocated_heat_flux_flag_pt(0)
    {
      ...
      this->Use_collocated_penetration_flag_pt =
        &activate_iso_colloc_for_contact;
      this->Use_collocated_contact_pressure_flag_pt =
        &activate_iso_colloc_for_contact;
    ...
    double heat_flux(const Vector<double>& s) const
    {
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        interpolated_lambda_q +=
          this->node_pt(l)->value(first_index_q) * psi_p[l];
      }
    ...
    /// Determine which method to use to discretise heat_flux, collocation
    /// (true)
...

    {
      ...
      for (unsigned ipt = 0; ipt < n_intpt; ipt++)
      {
        ...
        for (unsigned l = 0; l < n_node; l++)
        {
          ...
          interpolated_temp +=
            this->node_pt(l)->value(temperature_index) * psi[l];
        ...
        else
        {
          ...
          // throw error here? Although it may not always be a problem,
          // depending on penetrator/contact surface geometry
        ...
        for (unsigned l = 0; l < n_node; l++)
        {
          ...
          temp_deviation_integral[l] +=
            (interpolated_temp - penetrator_temperature) * psi_i[l] * W;
        }
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        // IF it's not a boundary condition for both heat_flux and
        // contact_pressure
...
public:
  ...
  void position(const Vector<double>& zeta, Vector<double>& r) const
  {
    ...
    // Lift off
    r[1] -=
      Lift_off_amplitude * exp(-Lift_off_alpha * (zetaa - 0.5) * (zetaa - 0.5));
  }
...
public:
  ...
  void fill_in_contribution_to_residuals(Vector<double>& residuals)
  {
    ...
    // node
    int local_eqn =
      external_local_eqn(External_data_index_of_traded_contact_pressure,
                         Index_of_contact_pressure);
    if (local_eqn >= 0)
    {
...
#endif
          residuals[local_eqn] +=
            internal_data_pt(0)->value(0) - (*Target_rotation_angle_pt);
#ifdef PARANOID
        }
        else
        {
  ...
  void output(std::ostream& outfile, const unsigned& nplot) const
  {
    for (unsigned j = 0; j < nplot; j++)
    {
      ...
      outfile << centre(0) + (*Radius_pt) * cos(phi) << " "
              << centre(1) + (*Radius_pt) * sin(phi) << std::endl;
    }
  }
...
namespace ProblemParameters
{
  ...
  /// GeomObject specifying the shape of the boundary: Initially it's
  /// almost flat.
  WarpedLine Boundary_geom_object_contact(1.0e-10,
                                          X_contact_end_left,
                                          X_contact_end_right);
...
public:
  ...
  void actions_before_adapt()
  {
    ...
    // Impose max_el_length from el_area, ie derive average element length from
    // El_area then apply multiplier
...
private:
  ...
  void complete_problem_setup()
  {
    ...
    //------------------------------------------------------------
    {
      ...
      oomph_info << "Control node located at: " << Control_node_pt->x(0) << " "
      ...
      if (ProblemParameters::Impose_position_of_centre)
      {
        ...
        // Make new one
        ProblemParameters::Penetrator_pt = new HeatedCircularPenetrator(
    ...
    //  // hierher setup interaction el_pt->flux_fct_pt() =
    //  &ProblemParameters::unit_flux;
...
ContactProblem<ELEMENT>::ContactProblem()
{
  ...
  boulder_outer_curvilinear_boundary_pt[0] =
    new TriangleMeshCurviLine(boulder_boundary_circle_pt,
                              zeta_start,
                              zeta_end,
                              n_seg_boulder,
                              Boulder_top_boundary_id);
  ...
  boulder_outer_curvilinear_boundary_pt[1] =
    new TriangleMeshCurviLine(boulder_boundary_circle_pt,
                              zeta_start,
                              zeta_end,
                              n_seg_boulder,
  ...
  boulder_triangle_mesh_parameters.element_area() = ProblemParameters::El_area;
  ...
  for (unsigned j = 1; j < npt_right; j++)
  {
    ...
    double x = X_ur - (X_ur - ProblemParameters::X_contact_end_right) *
                        double(j) / double(npt_right - 1);
    double y = Y_ur;
  ...
  for (unsigned j = 1; j < npt_contact; j++)
  {
    ...
    double x = ProblemParameters::X_contact_end_right -
               (ProblemParameters::X_contact_end_right -
                ProblemParameters::X_contact_end_left) *
                 double(j) / double(npt_contact - 1);
    double y = Y_ur;
  ...
  for (unsigned j = 1; j < npt_left - 1; j++)
  {
    ...
    double x = ProblemParameters::X_contact_end_left -
               (ProblemParameters::X_contact_end_left - X_ll) * double(j) /
                 double(npt_left - 1);
    double y = Y_ur;
  ...
  Bulk_mesh_pt->spatial_error_estimator_pt() = new DummyErrorEstimator(
    Bulk_mesh_pt, lower_left, upper_right, central_node_number);
...
void ContactProblem<ELEMENT>::doc_solution()
{
  ...
  sprintf(filename,
          "%s/contact_integration_points%i.dat",
          Doc_info.directory().c_str(),
          Doc_info.number());
  some_file.open(filename);
  Vector<double> s(1);
...
#ifdef STRUCTURED_MESH
  radius_of_elastic_body = ProblemParameters::Boundary_geom_object.radius();
#else
  ...
  {
    b_hertz =
      sqrt(4.0 * (1.0 - ProblemParameters::Nu * ProblemParameters::Nu) /
           (MathematicalConstants::Pi *
            (-1.0 / radius_of_elastic_body + 1.0 / ProblemParameters::Radius)) *
  ...
  {
    p_max_hertz =
      2.0 * total_contact_force[1] / (MathematicalConstants::Pi * b_hertz);
  }
  ...
  // Write trace file
  Trace_file << total_contact_force[0] << " " << total_contact_force[1] << " "
             << centre[0] << " " << centre[1] << " " << target_weight
             << " "
             // ALH/hierher Suppressed so that validation tests pass
             // For small changes in loading a different contact point can be
             // selected which means that different angles are computed
             //  << angle << " "
             << std::endl;
...
int main(int argc, char* argv[])
{
  ...
  // Build problem
  ContactProblem<ProjectableLinearHeatAndElasticityElement<
  ...
  //---------------------------------------------------------
  {
    ...
    for (unsigned i = 0; i < nstep; i++)
    {
      ...
      oomph_info << "Re-solving for prescr displ amplitude = " << ampl
                 << std::endl;
  ...
  for (unsigned i = 0; i < nstep; i++)
  {
    ...
    oomph_info << "Re-solving for yc=" << ProblemParameters::Y_c << std::endl;
