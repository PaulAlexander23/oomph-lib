...
namespace ProblemParameters
{
  ...
  double Direct_radiation =
    1.382370394; // peak of 250W/m^2 gives: 0.3455925986;
                             ...
                             double& total_diffuse_radiation)
  {
    ...
    solar_flux_unit_vector[0] =
      cos(2.0 * MathematicalConstants::Pi * time / One_day);
    solar_flux_unit_vector[1] =
      -sin(2.0 * MathematicalConstants::Pi * time / One_day);
  }
...
void SolarRadiationProblem<ELEMENT>::doc_solution()
{
  ...
  sprintf(filename,
          "%s/solar_radiation%i.dat",
          Doc_info.directory().c_str(),
          Doc_info.number());
  some_file.open(filename);
  some_file << "0.0 3.5 " << solar_flux_magnitude << " "
            << solar_flux_unit_vector[0] << " " << solar_flux_unit_vector[1]
            << " "
            << "\n";
  some_file.close();
  ...
  if (plot_individual)
  {
    ...
    {
      sprintf(filename,
              "%s/diffuse_radiation_cone%i_%i.dat",
              Doc_info.directory().c_str(),
              Doc_info.number(),
              count++);
      some_file.open(filename);
    ...
    {
      sprintf(filename,
              "%s/diffuse_radiation_cone_max_angle%i_%i.dat",
              Doc_info.directory().c_str(),
              Doc_info.number(),
              count++);
      some_file.open(filename);
    ...
    {
      sprintf(filename,
              "%s/diffuse_radiation_cone_min_angle%i_%i.dat",
              Doc_info.directory().c_str(),
              Doc_info.number(),
              count++);
      some_file.open(filename);
