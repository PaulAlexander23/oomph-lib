              ...
              {
                {
                  ...
                  if (local_unknown >= 0)
                  {
                    jacobian(local_eqn, local_unknown) -=
                      Viscosity_Ratio *
                      (dpsifdx_parent(bulk_node_number(l2), 1) *
                       dtestfdx_parent(bulk_node_number(l), 1)) *
                      W;
                  }
                }
              ...
              {
                {
                  ...
                  if (local_unknown >= 0)
                  {
                    jacobian(local_eqn, local_unknown) -=
                      Viscosity_Ratio *
                      (dpsifdx_parent(External_node[l2], 1) *
                       dtestfdx_parent(bulk_node_number(l), 1)) *
                      W;
                  }
                }
  ...
  else
  {
    ...
    if (external_data_pt(0)->nvalue() != 1)
    {
      cout << "The external Data item that stores the traded pressure in "
           << std::endl;
      cout << "SpineVolumeConstraintPointElement should only have a single"
           << std::endl;
      cout << "value but is has " << external_data_pt(0)->nvalue() << std::endl;
      exit(1); // assert(false);
    }
#endif
...
AirwayReopeningProblem<ELEMENT>::AirwayReopeningProblem()
{
  ...
  Upper_wall_mesh_pt = new OneDLagrangianMesh<FSIHermiteBeamElement>(
    nwall, xi0, xi2, Undeformed_upper_wall_geom_pt);
  ...
  // Create the Lagrangian Mesh
  Lower_wall_mesh_pt = new OneDLagrangianMesh<FSIHermiteBeamElement>(
    nwall, xi0, xi2, Undeformed_lower_wall_geom_pt);
  ...
  // Now create the mesh
  Bulk_mesh_pt =
    new BrethertonSpineMesh<ELEMENT, SpineLineFluidInterfaceElement<ELEMENT>>(
      nx1,
      nx2,
      nx3,
      nh,
  ...
  connect_walls(
    Lower_wall_mesh_pt, upper_wall_pt, Global_Physical_Variables::upper_map);

  connect_walls(
    Upper_wall_mesh_pt, lower_wall_pt, Global_Physical_Variables::lower_map);
...
void AirwayReopeningProblem<ELEMENT>::doc_solution(DocInfo& doc_info)
{
  ...
  Trace_file << " " << Upper_wall_mesh_pt->boundary_node_pt(0, 0)->x(1);
  Trace_file << " " << Upper_wall_mesh_pt->boundary_node_pt(1, 0)->x(1);
  Trace_file << " " << Lower_wall_mesh_pt->boundary_node_pt(0, 0)->x(1);
  Trace_file << " " << Lower_wall_mesh_pt->boundary_node_pt(1, 0)->x(1);
  ...
  Trace_file << " "
             << (Bubble_pressure_data_pt->value(0) -
                                                      ...
                                                      const bool& restart)
{
  ...
  for (unsigned step = 1; step <= nsteps; step++)
  {
    ...
    // Solve
    cout << "Solving for capillary number: " << Global_Physical_Variables::Ca
         << " Bo = " << Global_Physical_Variables::Bo << std::endl;
    ...
    cout << "Bubble pressure is " << Bubble_pressure_data_pt->value(0)
         << std::endl;
    ...
    if (step > 1)
    {
      ...
      sprintf(dumpfile,
              ...
              H,
              Tube_width,
              Mesh_fraction_at_transition_pt->value(0),
              Kstiff);
    ...
    else
    {
      ...
      {
        flux -= 0.05;
        Flux_constraint_pt->set_flux(flux);
      }
      else
      {
        if (flux < -2.0)
