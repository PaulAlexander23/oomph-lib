...
UnstructuredFSIProblem<FLUID_ELEMENT, SOLID_ELEMENT>::UnstructuredFSIProblem()
{
  ...
  Fluid_mesh_pt = new FluidTriangleMesh<FLUID_ELEMENT>(
    fluid_node_file_name, fluid_element_file_name, fluid_poly_file_name);
  ...
  for (unsigned ibound = 0; ibound < n_boundary; ibound++)
  {
    ...
    for (unsigned inod = 0; inod < num_nod; inod++)
    {
      ...
      {
        double y = fluid_mesh_pt()->boundary_node_pt(ibound, inod)->x(1);
        double veloc = 1.5 / (y_max - y_min) * (y - y_min) * (y_max - y) /
                       ((y_mid - y_min) * (y_max - y_mid));
  ...
  Solid_mesh_pt = new MySolidTriangleMesh<SOLID_ELEMENT>(
    solid_node_file_name, solid_element_file_name, solid_poly_file_name);
...
void UnstructuredFSIProblem<FLUID_ELEMENT,
                            SOLID_ELEMENT>::doc_solid_boundary_coordinates()
{
  ...
  create_lagrange_multiplier_elements()
{
  ...
  Solid_fsi_boundary_pt = new MeshAsGeomObject(Traction_mesh_pt);
