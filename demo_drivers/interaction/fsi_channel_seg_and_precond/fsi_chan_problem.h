  ...
  const bool& steady_flag)
{
  ...
  Wall_geom_object_pt->locate_zeta(
    zeta_displ_ctrl, Ctrl_geom_obj_pt, S_displ_ctrl);
  ...
  const unsigned& niter)
{
  ...
  // Write trace file
  trace_file << Left_node_pt->value(0) << " " << Right_node_pt->value(0) << " "
             << cpu << " " << Newton_iter << " " << std::endl;
...
void FSICollapsibleChannelProblem<ELEMENT>::restart(ifstream& restart_file)
{
  ...
  {
    std::cout << "Increasing external pressure from "
  ...
  Global_Physical_Variables::P_ext_data_pt->set_value(
    0, double(atof(input_string.c_str())) + Global_Physical_Variables::P_step);
...
void FSICollapsibleChannelProblem<ELEMENT>::set_initial_condition()
{
  ...
  if (Flags::Restart_file_name != "")
  {
    // Open restart file
    restart_file_pt =
      new ifstream(Flags::Restart_file_name.c_str(), ios_base::in);
    if (restart_file_pt != 0)
    {
      cout << "Have opened " << Flags::Restart_file_name << " for restart. "
           << std::endl;
    ...
    {
      std::ostringstream error_stream;
      error_stream << "ERROR while trying to open " << Flags::Restart_file_name
...
void FSICollapsibleChannelProblem<ELEMENT>::steady_run()
{
  ...
  trace_file << "\"u_1\","
             << "\"u_2\","
             << "\"CPU time for solve\","
             << "\"Number of Newton iterations\"," << std::endl;
  ...
  // Steady run
  double delta_y = (Global_Physical_Variables::Yprescr_min -
                    Global_Physical_Variables::Yprescr) /
                   double(Flags::Nsteps - 1);
