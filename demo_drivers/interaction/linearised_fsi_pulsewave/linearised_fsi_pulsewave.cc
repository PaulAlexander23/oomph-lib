...
namespace Global_Physical_Variables
{
                                      ...
                                      Vector<double>& result)
  {
    ...
    if (T_cos != 0.0)
    {
      ...
      {
        ramp_factor =
          0.5 * (1.0 - cos(2.0 * MathematicalConstants::Pi * time / T_cos));
      }
    }
  ...
  void doc_dependent_parameters()
  {
    ...
    oomph_info << "==================" << std::endl;
    oomph_info << "Womersley number (ReSt)           : " << Wo << std::endl;
    oomph_info << "Wall inertia parameter (Lambda^2) : " << Lambda_sq
               << std::endl;
    oomph_info << "Moens-Korteweg wavespeed          : " << Wavespeed
               << std::endl;
    oomph_info << "Pressure wavespeed in solid       : " << Pressure_wavespeed
               << std::endl;
    oomph_info << std::endl;
  }
...
PressureWaveFSIProblem<FLUID_ELEMENT, SOLID_ELEMENT>::PressureWaveFSIProblem()
{
  ...
  fluid_triangle_mesh_parameters.element_area() = uniform_element_area;
  ...
  // Take the maximum element area
  solid_triangle_mesh_parameters.element_area() = uniform_element_area;
...
void PressureWaveFSIProblem<FLUID_ELEMENT,
                            SOLID_ELEMENT>::create_solid_traction_elements()
{
...
template<class FLUID_ELEMENT, class SOLID_ELEMENT>
void PressureWaveFSIProblem<FLUID_ELEMENT,
                            SOLID_ELEMENT>::create_fluid_traction_elements()
{
  // Inflow:
  unsigned bound = 2;
...
void PressureWaveFSIProblem<FLUID_ELEMENT, SOLID_ELEMENT>::setup_fsi()
{
  ...
  Multi_domain_functions::
    setup_bulk_elements_adjacent_to_face_mesh<FLUID_ELEMENT, 2>(
  ...
  Multi_domain_functions::
    setup_bulk_elements_adjacent_to_face_mesh<SOLID_ELEMENT, 2>(
  ...
  DocInfo& doc_info)
{
  ...
  // Solid traction
  sprintf(filename,
          "%s/outside_solid_traction%i.dat",
          doc_info.directory().c_str(),
          doc_info.number());
  file.open(filename);
  ...
  sprintf(filename,
          ...
          doc_info.number());
  file.open(filename);
  Fluid_mesh_pt->output_fct(
    file, 5, time_pt()->time(), Global_Physical_Variables::pulse_wave_solution);
  file.close();
...
int main(int argc, char** argv)
{
  ...
  PressureWaveFSIProblem<AxisymmetricTTaylorHoodElement,
                         TAxisymmetricLinearElasticityElement<3>>
    problem;
