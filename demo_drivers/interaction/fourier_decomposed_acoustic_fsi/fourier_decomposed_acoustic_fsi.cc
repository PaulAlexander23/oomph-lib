                    ...
                    HELMHOLTZ_ELEMENT>::CoatedSphereProblem()
{
  ...
  // Build mesh
  Helmholtz_mesh_pt =
    new TwoDAnnularMesh<HELMHOLTZ_ELEMENT>(periodic,
                                           azimuthal_fraction_of_coating,
                                           ntheta_helmholtz,
                                           nr_helmholtz,
                                           a,
  ...
  DocInfo& doc_info)
{
  ...
  Trace_file << Global_Parameters::Q << " " << Global_Parameters::K_squared
             ...
             << Global_Parameters::Omega_sq.real() << " " << power << " "
             << std::endl;
...
int main(int argc, char** argv)
{
  ...
  // Wavenumber "zenith"-variation of imposed displacement of coating
  // on inner boundary
  CommandLineArgs::specify_command_line_flag("--M", &Global_Parameters::M);
  ...
  CoatedSphereProblem<QTimeHarmonicFourierDecomposedLinearElasticityElement<3>,
                      QFourierDecomposedHelmholtzElement<3>>
    problem;
