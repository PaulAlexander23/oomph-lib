...
public:
                            ...
                            unsigned ynode)
  {
    ...
    if (xelement < Nx1)
    {
      return (x1init + x1step * ((Np - 1) * xelement + xnode));
    }
    else
    {
      return (x2init + x2step * ((Np - 1) * (xelement - Nx1) + xnode));
    }
  }
                            ...
                            unsigned ynode)
  {
    ...
    if (yelement < Ny1)
    {
      return (y1init + y1step * ((Np - 1) * yelement + ynode));
    }
    else
    {
      return (y2init + y2step * ((Np - 1) * (yelement - Ny1) + ynode));
    }
  }
  ...
  void assign_centreline_nodes()
  {
    ...
    for (unsigned e = 0; e < Nx; e++)
    {
      ...
      for (unsigned l = 0; l < (Nnode - 1); l++)
      {
        ...
        Centreline_node_pt.push_back(
          this->finite_element_pt(Ny1 * Nx + e)->node_pt(l));
      }
    }
    // Add the final node to the centreline Vector
    Centreline_node_pt.push_back(
      this->finite_element_pt(Ny1 * Nx + Nx - 1)->node_pt(Nnode - 1));
  }
  ...
  : Re(0.0), Length(12.0) // Initialise value of Re to zero
{
  ...
  Problem::mesh_pt() = new CylinderMesh<ELEMENT>(Nr1, Nr2, Nz1, Nz2, Length);
...
void RotatingProblem<ELEMENT>::solve_system()
{
  ...
  for (unsigned l = 0; l < 2; l++)
  {
    ...
    if (this->communicator_pt()->my_rank() == 0)
    {
      trace << Re << " " << Length << " "
            ...
            << mesh_pt()->centreline_node_pt(Central_node)->x(0) << std::endl;
    }
    else
    {
      trace << Re << " " << Length << " "
...

  {
    ...
    for (unsigned n = 0; n < n_node; n++)
    {
      ...
      {
        nod_pt->set_value(0, 0.01 * (y - 0.5));
      }
      // Symmetric in second and third
      if (!nod_pt->is_pinned(1))
      {
        nod_pt->set_value(1, 0.01 * (y - 0.5) * (y - 0.5));
      }
      if (!nod_pt->is_pinned(2))
      {
        nod_pt->set_value(2, 0.01 * (y - 0.5) * (y - 0.5));
      }
    }
...
// Ignore halo elements
  ...
  // Pitchfork found at
  oomph_info << "Pitchfork found at " << Re << " " << std::endl;
  ...
  if (this->communicator_pt()->my_rank() == 0)
  {
    trace << Re << " " << Length << " " << dof(2 * n_dof_local + 1) << " "
          << mesh_pt()->centreline_node_pt(0)->value(1) << " "
          << mesh_pt()->centreline_node_pt(Central_node)->value(1) << " "
          << mesh_pt()->centreline_node_pt(Central_node)->x(0) << std::endl;
  }
