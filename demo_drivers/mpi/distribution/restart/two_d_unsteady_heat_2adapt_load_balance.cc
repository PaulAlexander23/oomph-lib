...
void RefineableUnsteadyHeatProblem<ELEMENT>::restart()
{
  ...
  sprintf(filename,
          "%s_on_proc%i.dat",
          GlobalParameters::Restart_file.c_str(),
          this->communicator_pt()->my_rank());
  restart_file_pt = new ifstream(filename, ios_base::in);
  if (restart_file_pt != 0)
  {
    oomph_info << "Have opened " << filename << " for restart. " << std::endl;
  }
  else
  {
    std::ostringstream error_stream;
    error_stream << "ERROR while trying to open " << filename << " for restart."
                 << std::endl;
  ...
  const std::string& comment)
{
  ...
  oomph_info << "================================================="
             << std::endl;
  oomph_info << "Docing solution " << Doc_info.number()
             << " for t=" << time_pt()->time() << " [ndof=" << ndof() << "]\n";
  oomph_info << "================================================="
             << std::endl;
  ...
  if (Communicator_pt->my_rank() == 0)
  {
    ...
    Trace_file << time_pt()->time() << " " << u_exact << " " << r_wall[0] << " "
               << time_pt()->dt() << " " << temp_error_norm << " "
               << GlobalParameters::step_position(time_pt()->time()) << " "
               << global_ref_stats[0] << " " << global_ref_stats[1] << " "
               << global_ref_stats[2] << " " << error << " " << norm
               << std::endl;
...
double RefineableUnsteadyHeatProblem<ELEMENT>::global_temporal_error_norm()
{
  ...
  for (unsigned i = 0; i < n_node; i++)
  {
    ...
    if (!(nod_pt->is_halo()))
    {
      ...
      double error =
        nod_pt->time_stepper_pt()->temporal_error_in_value(nod_pt, 0);
  ...
  for (unsigned i = 0; i < n_node; i++)
  {
    ...
    // value for nodal value 0
    double error =
      Bulk_mesh_pt->node_pt(i)->time_stepper_pt()->temporal_error_in_value(
        Bulk_mesh_pt->node_pt(i), 0);
...
void RefineableUnsteadyHeatProblem<ELEMENT>::restart(ifstream& restart_file)
{
  ...
  MPI_Allreduce(&local_doc_info_number,
                &doc_info_number,
                1,
                MPI_UNSIGNED,
                MPI_MAX,
  ...
  comment_stream << "Restarted; Step " << doc_info().number();
...
int main(int argc, char* argv[])
{
  ...
  if (CommandLineArgs::command_line_flag_has_been_set("--validation_run"))
  {
    ...
    if (CommandLineArgs::command_line_flag_has_been_set("--load_balance_first"))
    {
      ...
      else
      {
        ...
        sprintf(doc_info_directory,
                "RESLT_load_balance_first_restarted_from_step_%s",
                step);
    ...
    else
    {
      ...
      else
      {
        ...
        step = strtok(NULL, "/");
        sprintf(
          doc_info_directory, "RESLT_prune_first_restarted_from_step_%s", step);
      }
    }
  }
  else
  ...
  if (!CommandLineArgs::command_line_flag_has_been_set("--partitioning_file"))
  {
    ...
    char filename[100];
    sprintf(
      filename, "%s/partitioning.dat", problem.doc_info().directory().c_str());
  ...
  comment_stream0 << "Distributed; Step " << problem.doc_info().number();
  ...
  while (problem.doc_info().number() < 22)
  {
    ...
    double dt_new = problem.doubly_adaptive_unsteady_newton_solve(
      dt, problem.epsilon_t(), max_adapt, first);
        ...
        (problem.doc_info().number() == second_prune))
    {
             ...
             "--load_balance_first"))))
      {
        ...
        comment_stream2 << "Pruned; Step " << problem.doc_info().number();
      }
      else
      {
        ...
        (problem.doc_info().number() == second_load_balance))
    {
      ...
      comment_stream3 << "Load balanced; Step " << problem.doc_info().number();
