  ...
  const std::string& header)
{
  ...
  some_file << "TEXT X=25,Y=93,F=HELV,HU=POINT,C=BLUE,H=26,T=\"" << header
            << "\"" << std::endl;
  some_file.close();
  ...
  // Output solution
  sprintf(filename,
          "%s/soln_with_haloes%i_on_proc%i.dat",
          Doc_info.directory().c_str(),
          Doc_info.number(),
          my_rank);
  some_file.open(filename);
...

  {
    ...
    oomph_info << "Number of non halo elements: " << total_count_non_halo << " "
               << count_non_halo << "\n";
  }

  {
    sprintf(filename,
            "%s/soln_non_halo_nodes%i_on_proc%i.dat",
            Doc_info.directory().c_str(),
            Doc_info.number(),
            my_rank);
    some_file.open(filename);
    ...
    oomph_info << "Number of non halo nodes: " << total_count_non_halo << " "
               << count_non_halo << "\n";
  }
