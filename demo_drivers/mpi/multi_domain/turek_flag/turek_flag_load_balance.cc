...
public:
  ...
  ~TurekProblem()
  {
    ...
    delete dynamic_cast<CylinderWithFlagDomain*>(
      this->fluid_mesh_pt()->domain_pt())
      ->cylinder_pt();
...
void TurekProblem<FLUID_ELEMENT, SOLID_ELEMENT>::build_mesh()
{
  ...
  std::cout << "Coordinates of fluid control point "
            << Fluid_control_node_pt->x(0) << " " << Fluid_control_node_pt->x(1)
            << " " << std::endl;
  ...
  const bool& called_from_adapt)
{
  ...
  for (unsigned j = 0; j < n; j++)
  {
    // Get the (algebraic) node
    AlgebraicNode* alg_nod_pt =
      dynamic_cast<AlgebraicNode*>(external_halo_node_pt[j]);
  ...
  DocInfo& doc_info, ofstream& trace_file)
{
  ...
  oomph_info << "Doced solution for step " << doc_info.number() << std::endl
             << std::endl
             << std::endl;
