  ...
  : Htot(htot)
{
  ...
  // from the wall mesh
  MeshAsGeomObject* wall_geom_object_pt = new MeshAsGeomObject(Solid_mesh_pt);
...
class SimpleFSIPreconditioner : public virtual BlockPreconditioner<MATRIX>
{
public:
  ...
  SimpleFSIPreconditioner(Problem* problem_pt)
    : BlockPreconditioner<MATRIX>(),
      ...
      Fluid_solid_coupling_matvec_pt(0),
...
void SimpleFSIPreconditioner<MATRIX>::setup()
{
  ...
  this->setup_matrix_vector_product(
    Fluid_solid_coupling_matvec_pt, fluid_onto_solid_matrix_pt, 0);
  // Clean up
  delete fluid_onto_solid_matrix_pt;
  fluid_onto_solid_matrix_pt = 0;
}
