...
namespace oomph
{
  ...
  double DecomposedLinearElasticityEquationsBase<DIM>::Default_lambda_sq_value =
    1.0;
  ...
  //======================================================================
  template<unsigned DIM>
  void DecomposedLinearElasticityEquationsBase<DIM>::get_strain(
    ...
    for (unsigned l = 0; l < n_node; l++)
    {
      ...
      for (unsigned i = 0; i < DIM; i++)
      {
        ...
        const double u_value =
          this->nodal_value(l, u_index_linear_elasticity(l, i, jth_solution));
  ...
  //======================================================================
  template<unsigned DIM>
  void DecomposedLinearElasticityEquations<DIM>::get_stress(
    const Vector<double>& s,
    const unsigned& jth_solution,
    DenseMatrix<double>& stress) const
  {
    ...
    this->get_strain(s, jth_solution, strain);
  ...
  //=======================================================================
  template<unsigned DIM>
  void DecomposedLinearElasticityEquations<DIM>::
      ...
      Vector<double>& residuals, DenseMatrix<double>& jacobian, unsigned flag)
  {
    ...
    for (unsigned jth_solution = 0; jth_solution < 2; jth_solution++)
    {
      ...
      for (unsigned ipt = 0; ipt < n_intpt; ipt++)
      {
        ...
        this->interpolated_u_linear_elasticity(s, jth_solution, u);
        ...
        for (unsigned l = 0; l < n_node; l++)
        {
          ...
          for (unsigned i = 0; i < DIM; i++)
          {
            ...
            {
              accel[i] +=
                this->d2u_dt2_linear_elasticity(l, i, jth_solution) * psi(l);
            }

            // Get the nodal displacements
            const double u_value = this->raw_nodal_value(
              l, this->u_index_linear_elasticity(l, i, jth_solution));
            // Loop over derivative directions
            for (unsigned j = 0; j < DIM; j++)
            {
              interpolated_dudx(i, j) += u_value * dpsidx(l, j);
        ...
        for (unsigned l = 0; l < n_node; l++)
        {
          ...
          for (unsigned a = 0; a < DIM; a++)
          {
            ...
            local_eqn = this->nodal_local_eqn(
              l, this->u_index_linear_elasticity(l, a, jth_solution));
                ...
                for (unsigned l2 = 0; l2 < n_node; l2++)
                {
                  ...
                  for (unsigned c = 0; c < DIM; c++)
                  {
                    local_unknown = this->nodal_local_eqn(
                      l2, this->u_index_linear_elasticity(l2, c, jth_solution));
                    // If it's not pinned
                    if (local_unknown >= 0)
                    {
                      if (this->Unsteady)
  ...
  //=======================================================================
  template<unsigned DIM>
  void DecomposedLinearElasticityEquations<DIM>::output_fct(
    std::ostream& outfile,
  ...
  //=======================================================================
  template<unsigned DIM>
  void DecomposedLinearElasticityEquations<DIM>::output(std::ostream& outfile,
                                                        const unsigned& nplot)
  {
    // Set output Vector
    Vector<double> s(DIM);
    ...
    for (unsigned iplot = 0; iplot < num_plot_points; iplot++)
    {
      ...
      this->interpolated_x(s, x);
      this->interpolated_u_linear_elasticity(s, 0, u1);
      this->interpolated_u_linear_elasticity(s, 1, u2);
  ...
  //=======================================================================
  template<unsigned DIM>
  void DecomposedLinearElasticityEquations<DIM>::output(FILE* file_pt,
                                                        const unsigned& nplot)
  {
    // Vector of local coordinates
    Vector<double> s(DIM);
    ...
    for (unsigned iplot = 0; iplot < num_plot_points; iplot++)
    {
      ...
      for (unsigned j = 0; j < 2; j++)
      {
        ...
        {
          fprintf(
            file_pt, "%g ", this->interpolated_u_linear_elasticity(s, i, j));
        }
      }
    }
    fprintf(file_pt, "\n");
  ...
  template class DecomposedLinearElasticityEquationsBase<1>;
  template class DecomposedLinearElasticityEquations<1>;

  template class DecomposedLinearElasticityEquationsBase<2>;
  template class DecomposedLinearElasticityEquations<2>;

  // template class QLinearElasticityElement<3, 3>;
  template class DecomposedLinearElasticityEquationsBase<3>;
  template class DecomposedLinearElasticityEquations<3>;
