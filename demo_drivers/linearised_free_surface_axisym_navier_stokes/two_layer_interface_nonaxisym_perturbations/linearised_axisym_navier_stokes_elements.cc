...
#ifndef OOMPH_LINEARISED_AXISYM_NAVIER_STOKES_ELEMENTS_CC
#define OOMPH_LINEARISED_AXISYM_NAVIER_STOKES_ELEMENTS_CC
      ...
      unsigned flag)
  {
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        for (unsigned i = 0; i < 4; i++)
        {
          ...
          const double xhat_value =
            this->raw_nodal_value(l, xhat_nodal_index[i]);
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        {
          throw OomphLibError(
            "Incorrect node_update_fct_id",
            "LinearisedAxisymmetricNavierStokesEquations::fill_in_generic_"
            "residual_contribution_lin_axi_nst()",
            OOMPH_EXCEPTION_LOCATION);
        }
      }
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
            ...
            for (unsigned l2 = 0; l2 < n_p; l2++)
            {
              ...
              if (local_unknown >= 0)
              {
                for (unsigned a = 0; a < 3; a++)
                {
                  ...
                  sum -= scaled_re * r * base_flow_ur * group_C[l2 + (3 * a)] *
                         testf_ * w;
                  sum += scaled_re_st * r * mesh_velocity[0] *
                         group_C[l2 + (3 * a)] * testf_ * w;
                  ...
                  sum -= visc_ratio * (1.0 + Gamma[0]) * r * base_flow_durdr *
                         group_F(l, (l2 + (3 * a))) * w;
                  sum -= visc_ratio * (1.0 + Gamma[0]) * r *
                         group_C[l2 + (3 * a)] * dtestfdr * w;
                  sum += visc_ratio * (1.0 + Gamma[0]) * r * base_flow_durdr *
                         group_A[l2 + (3 * a)] * dtestfdr * w;
                  sum += visc_ratio * k * k * base_flow_durdz *
                         psif[l2 + (3 * a)] * testf_ * Jbar * w / r;
                  sum -= visc_ratio * Gamma[0] * r * group_D[l2 + (3 * a)] *
                         dtestfdz * w;
                  sum += visc_ratio * Gamma[0] * r * base_flow_duzdr *
                         group_A[l2 + (3 * a)] * dtestfdz * w;
                  sum += visc_ratio * r * base_flow_durdz *
                         group_A[l2 + (3 * a)] * dtestfdz * w;
                  ...
                  sum += scaled_re_inv_fr * r * G[0] * testf_ *
                         group_A[l2 + (3 * a)] * w;
              ...
              if (local_unknown >= 0)
              {
                for (unsigned a = 0; a < 3; a++)
                {
                  ...
                  sum += k * visc_ratio * Gamma[0] * base_flow_duthetadr *
                         dtestfdz * psif[l2 + (3 * a)] * Jbar * w;
                  sum += k * visc_ratio * Gamma[0] * group_E[l2 + (3 * a)] *
                         testf_ * w;
                  sum -= visc_ratio * k * base_flow_utheta * dtestfdz *
                         psif[l2 + (3 * a)] * Jbar * w / r;
                  sum += visc_ratio * (1.0 + Gamma[0]) * k *
                         base_flow_duthetadz * psif[l2 + (3 * a)] * testf_ *
                         Jbar * w / r;
            ...
            for (unsigned l2 = 0; l2 < n_p; l2++)
            {
              ...
              if (local_unknown >= 0)
              {
                for (unsigned a = 0; a < 3; a++)
                {
                  ...
                  sum -= scaled_re * r * base_flow_ur * group_D[l2 + (3 * a)] *
                         testf_ * w;
                  sum += scaled_re_st * r * mesh_velocity[0] *
                         group_D[l2 + (3 * a)] * testf_ * w;
                  sum += scaled_re_st * r * psif[l2 + (3 * a)] *
                         node_pt(l2 + (3 * a))
                           ->position_time_stepper_pt()
                           ->weight(1, 0) *
                         base_flow_duzdz * testf_ * Jbar * w;
                  sum -= visc_ratio * r * base_flow_duzdr *
                         group_F(l, (l2 + (3 * a))) * w;
                  sum -= visc_ratio * r * group_D[l2 + (3 * a)] * dtestfdr * w;
                  sum += visc_ratio * r * base_flow_duzdr *
                         group_A[l2 + (3 * a)] * dtestfdr * w;
                  sum -= visc_ratio * Gamma[1] * r * base_flow_durdz *
                         group_F(l, (l2 + (3 * a))) * w;
                  sum += visc_ratio * Gamma[1] * r * base_flow_durdz *
                         group_A[l2 + (3 * a)] * dtestfdr * w;
                  sum += visc_ratio * k * k * base_flow_duzdz *
                         psif[l2 + (3 * a)] * testf_ * Jbar * w / r;
                  sum += visc_ratio * (1.0 + Gamma[1]) * r * base_flow_duzdz *
                         group_A[l2 + (3 * a)] * dtestfdz * w;
                  sum -= scaled_re_st * r * base_flow_duzdt * testf_ *
                         group_A[l2 + (3 * a)] * w;
                  sum += r * body_force[1] * testf_ * group_A[l2 + (3 * a)] * w;
                  sum += scaled_re_inv_fr * r * G[1] * testf_ *
                         group_A[l2 + (3 * a)] * w;
              ...
              if (local_unknown >= 0)
              {
                for (unsigned a = 0; a < 3; a++)
                {
                  ...
                  sum += k * scaled_re * base_flow_utheta * base_flow_duzdz *
                         psif[l2 + (3 * a)] * testf_ * Jbar * w;
                  sum += k * visc_ratio * Gamma[1] * base_flow_duthetadz *
                         dtestfdz * psif[l2 + (3 * a)] * Jbar * w;
        ...
        // if(offensive_eqn>=0) cout<<"residuals[offensive_eqn] =
        // "<<residuals[offensive_eqn]<<endl;
            ...
            for (unsigned l2 = 0; l2 < n_p; l2++)
            {
              ...
              if (local_unknown >= 0)
              {
                for (unsigned a = 0; a < 3; a++)
                {
                  ...
                  sum -= scaled_re * r * base_flow_ur * group_E[l2 + (3 * a)] *
                         testf_ * w;
                  sum += scaled_re_st * r * mesh_velocity[0] *
                         group_E[l2 + (3 * a)] * testf_ * w;
                  sum += scaled_re_st * r * psif[l2 + (3 * a)] *
                         node_pt(l2 + (3 * a))
                           ->position_time_stepper_pt()
                           ->weight(1, 0) *
                         base_flow_duthetadz * testf_ * Jbar * w;
                  sum -= visc_ratio * r * base_flow_duthetadr *
                         group_F(l, (l2 + (3 * a))) * w;
                  sum -= visc_ratio * r * group_E[l2 + (3 * a)] * dtestfdr * w;
                  sum += visc_ratio * r * base_flow_duthetadr *
                         group_A[l2 + (3 * a)] * dtestfdr * w;
                  sum += visc_ratio * Gamma[0] * base_flow_utheta *
                         group_F(l, (l2 + (3 * a))) * w;
                  sum += visc_ratio * (1.0 + Gamma[0]) * k * k *
                         base_flow_duthetadz * psif[l2 + (3 * a)] * testf_ *
                         Jbar * w / r;
                  sum += visc_ratio * r * base_flow_duthetadz *
                         group_A[l2 + (3 * a)] * dtestfdz * w;
                  sum +=
                    visc_ratio * Gamma[0] * group_E[l2 + (3 * a)] * testf_ * w;
                  sum -= scaled_re_st * r * base_flow_duthetadt * testf_ *
                         group_A[l2 + (3 * a)] * w;
                  sum -= scaled_re * base_flow_utheta * base_flow_ur * testf_ *
                         group_A[l2 + (3 * a)] * w;
                  sum += r * body_force[2] * testf_ * group_A[l2 + (3 * a)] * w;
                  sum += scaled_re_inv_fr * r * G[2] * testf_ *
                         group_A[l2 + (3 * a)] * w;
                  sum -= visc_ratio * base_flow_utheta * testf_ *
                         group_A[l2 + (3 * a)] * w / r;
              ...
              if (local_unknown >= 0)
              {
                for (unsigned a = 0; a < 3; a++)
                {
                  ...
                  sum += k * scaled_re * base_flow_utheta *
                         base_flow_duthetadz * psif[l2 + (3 * a)] * testf_ *
                         Jbar * w;
                  sum += k * visc_ratio * Gamma[0] * base_flow_durdz *
                         psif[l2 + (3 * a)] * dtestfdr * Jbar * w;
                  sum -=
                    k * base_flow_p * dtestfdz * psif[l2 + (3 * a)] * Jbar * w;
                  sum += visc_ratio * (1.0 + Gamma[0]) * k * base_flow_ur *
                         dtestfdz * psif[l2 + (3 * a)] * Jbar * w / r;
                  sum += k * visc_ratio * Gamma[0] * base_flow_duzdz *
                         psif[l2 + (3 * a)] * dtestfdz * Jbar * w;
                  sum -= visc_ratio * k * base_flow_durdz * psif[l2 + (3 * a)] *
                         testf_ * Jbar * w / r;
                  sum -= k * base_flow_p * testf_ * group_A[l2 + (3 * a)] * w;
                  sum += k * visc_ratio * (1.0 + Gamma[0]) * base_flow_ur *
                         testf_ * group_A[l2 + (3 * a)] * w / r;
            ...
            for (unsigned l2 = 0; l2 < n_p; l2++)
            {
              ...
              if (local_unknown >= 0)
              {
                for (unsigned a = 0; a < 3; a++)
                {
                  ...
                  sum -= k * base_flow_duthetadz * psif[l2 + (3 * a)] * testp_ *
                         Jbar * w;
