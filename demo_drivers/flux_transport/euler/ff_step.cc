             ...
             TimeStepper* time_stepper_pt = &Mesh::Default_TimeStepper)
  {
    ...
    for (unsigned ex = 0; ex < Nx1; ex++)
    {
      ...
      for (unsigned ey = 0; ey < Ny1; ey++)
      {
        ...
        double ll_corner[2] = {llx1 + ex * el_length[0],
                               lly1 + ey * el_length[1]};
    ...
    // Top
    {
      ...
      {
        FiniteElement* const elem_pt =
          this->finite_element_pt(Ny1 - 1 + Ny1 * e);
        unsigned n_p = elem_pt->nnode_1d();
        for (unsigned n = 0; n < n_p; n++)
        {
    ...
    // Bottom
    {
      ...
      {
        FiniteElement* const elem_pt =
          this->finite_element_pt(Ny1 * (Nx1 - 1) + e);
        unsigned n_p = elem_pt->nnode_1d();
        for (unsigned n = 0; n < n_p; n++)
        {
    ...
    switch (face_index)
    {
      ...
      case 1:
        ...
        else
        {
          ...
          if (cast_bulk_element_pt->face_element_pt(1)
                ->node_pt(0)
                ->is_on_boundary(1))
          {
            // std::cout <<
            // cast_bulk_element_pt->face_element_pt(1)->node_pt(0)->x(0) << " "
            //           <<
          ...
          else
          {
            ...
            // Face_element_pt.push_back(face_element_pt);
            face_element_pt =
              Face_element_pt[std::make_pair(bulk_element_pt, 1)];
            if (face_element_pt == 0)
            {
              throw OomphLibError("Something wrong",
                                  OOMPH_CURRENT_FUNCTION,
...
public:
  ...
  void limit()
  {
    ...
    for (unsigned e = 0; e < n_element; e++)
    {
      ...
      for (unsigned f = 0; f < 4; f++)
      {
        ...
        for (unsigned n = 0; n < n_face_node; n++)
        {
          ...
          cast_element_pt->get_neighbouring_face_and_local_coordinate(
          ...
          {
            centre_x(n, i) =
              neighbour_bulk_element_pt->interpolated_x(s_bulk, i);
          }
        ...
        for (unsigned i = 0; i < n_flux; i++)
        {
          grad(f, i, 0) = ((prim_centre(1, i) - prim_centre(0, i)) * Y_len -
                           (prim_nodal(1, i) - prim_nodal(0, i)) * y_len) /
                          A;
          grad(f, i, 1) = ((prim_centre(1, i) - prim_centre(0, i)) * X_len -
                           (prim_nodal(1, i) - prim_nodal(0, i)) * x_len) /
                          A;
        }
      }
    ...
    for (unsigned e = 0; e < n_element; e++)
    {
      ...
      {
        limited_value(n, 1) =
          cast_element_pt->average_value(1) +
  ...
  void parameter_study(std::ostream& trace, const bool& disc)
  {
    ...
    for (unsigned t = 0; t < 5000; t++)
    {
      ...
      trace << mesh_pt()->nelement() << " " << this->time() << std::endl;
      ++count;
    }
  }
};
