...
namespace Global
{
                      ...
                      Vector<double>& u)
  {
    ...
    double p =
      1.0 + (1.0 / (75.0 * 75.0)) * (0.5 * r * r - 32 * log(r) - 128 / (r * r));
...
public:
             ...
             TimeStepper* time_stepper_pt = &Mesh::Default_TimeStepper)
  {
    ...
    double el_length[2] = {(R_max - R_min) / (double)Nx,
                           (2.0 * pi) / (double)Ny};
...
int main()
{
  ...
  for (unsigned i = 0; i < 2; i++)
  {
    TwoDDGProblem<DGSpectralEulerElement<2, 3>> problem(n_element, n_element);
