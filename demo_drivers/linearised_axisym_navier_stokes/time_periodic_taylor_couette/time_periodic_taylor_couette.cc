...
public:
  ...
  void create_trace_file(DocInfo& doc_info)
  {
    ...
    char filename1[256];
    sprintf(filename1,
            "%s/base_trace_epsilon%2.1f_Re%4.2f.dat",
  ...
  const double& l_z)
{
  ...
  Problem::mesh_pt() =
    new RectangularQuadMesh<BASE_ELEMENT>(n_r,
                                          n_z,
                                          radius_inner_cylinder,
                                          radius_outer_cylinder,
                                          0.0,
                                          l_z,
                                          time_stepper_pt());
...
//=======================================================================
template<class BASE_ELEMENT>
void BaseStateProblem<BASE_ELEMENT>::set_boundary_conditions(const double& time)
{
  // Cache radius ratio
    ...
    for (unsigned n = 0; n < n_node; n++)
    {
      ...
      for (unsigned i = 0; i < 3; i++)
      {
        ...
        if (b == 3)
        {
          ...
          double azi_vel =
            eta * GlobalPhysicalVariables::MMC_Re_current / (1.0 - eta);
                                                  ...
                                                  const bool& output_soln)
{
  ...
  if (output_soln)
  {
    ...
    sprintf(filename,
            "%s/base_soln_epsilon%2.1f_Re%4.2f_soln%i.dat",
            doc_info.directory().c_str(),
            GlobalPhysicalVariables::Epsilon,
            GlobalPhysicalVariables::MMC_Re_current,
            doc_info.number());
...
public:
  ...
  {
    return dynamic_cast<RectangularQuadMesh<BASE_ELEMENT>*>(Base_state_mesh_pt);
  }
  ...
  void create_trace_file(DocInfo& doc_info)
  {
    ...
    char filename[256];
    sprintf(filename,
            "%s/perturbed_trace_epsilon%2.1f_Re%4.2f.dat",
            doc_info.directory().c_str(),
            GlobalPhysicalVariables::Epsilon,
            GlobalPhysicalVariables::MMC_Re_current);
    ...
    Domain_height(l_z)
{
  ...
  // Build and assign mesh
  Problem::mesh_pt() =
    new RectangularQuadMesh<PERTURBED_ELEMENT>(n_r,
                                               n_z,
                                               radius_inner_cylinder,
                                               radius_outer_cylinder,
                                               0.0,
  ...
  DocInfo& doc_info, const bool& output_soln)
{
  ...
  if (output_soln)
  {
    ...
    // Open solution output file
    sprintf(filename,
            "%s/perturbed_soln_epsilon%2.1f_Re%4.2f_soln%i.dat",
            doc_info.directory().c_str(),
            GlobalPhysicalVariables::Epsilon,
            GlobalPhysicalVariables::MMC_Re_current,
            doc_info.number());
...
public:
  ...
  void create_trace_files(DocInfo& doc_info)
  {
    ...
    char filename[256];
    sprintf(filename,
            "%s/periodic_base_flow_trace_epsilon%2.1f_Re%4.2f.dat",
            doc_info.directory().c_str(),
            GlobalPhysicalVariables::Epsilon,
            GlobalPhysicalVariables::MMC_Re_current);
    ...
    sprintf(filename,
            "%s/power_method_trace_epsilon%2.1f_Re%4.2f.dat",
            doc_info.directory().c_str(),
            GlobalPhysicalVariables::Epsilon,
            GlobalPhysicalVariables::MMC_Re_current);
    ...
    Vector<DoubleVector>& base_state_dofs_over_period)
  {
    ...
    this->unsteady_run(dt,
                       n_timesteps,
                       doc_info,
                       output_base_soln,
                       perturbed_state_dofs_over_period,
  ...
  Vector<DoubleVector>& base_state_dofs_over_period)
{
  ...
  for (unsigned i = 0; i < n_timesteps; i++)
  {
    ...
    std::cout << "  Timestep " << i + 1 << " of " << n_timesteps << "; Time is "
              << Base_state_problem_pt->time_pt()->time() << " (base), "
              << Perturbed_state_problem_pt->time_pt()->time() << " (perturbed)"
              << std::endl;
    ...
    else if (i == 1)
    {
      ...
      {
        std::cout << "    - nstep/2 loop: Timestep " << j + 1 << " of "
                  << n_timesteps_thisloop << "; Time (before advance) is "
                  << Base_state_problem_pt->time_pt()->time() << " (base), "
        ...
        if (First_power_method_iteration)
        {
          ...
          Base_state_problem_pt->get_dofs(
        ...
        if (j == 0)
        {
          this->initialise_dt(dt_thisloop);
        }
      }
    }
    ...
    else if (i == 3)
    {
      ...
      if (First_power_method_iteration)
      {
        ...
        Base_state_problem_pt->get_dofs(base_state_dofs_over_period[(
          (n_timesteps * 1.5) + (n_timesteps * 0.2))]);
      }
      else
      {
        Base_state_problem_pt->assign_eigenvector_to_dofs(
          base_state_dofs_over_period[((n_timesteps * 1.5) +
                                       (n_timesteps * 0.2))]);
      }
...
unsigned StabilityProblem<BASE_ELEMENT, PERTURBED_ELEMENT>::
  set_up_periodic_base_flow(const double& dt,
                            const unsigned& n_timesteps,
                            DocInfo& doc_info,
                            const double& tolerance)
{
  ...
  while (norm_over_ndof > tolerance)
  {
    ...
    {
      current_dofs[i] -= previous_dofs[i];
    }
  ...
  cout << "\nI have broken out of the base state loop after " << loop_counter
       << " periods.\n"
       << endl;
                       ...
                       Vector<DoubleVector>& base_state_dofs_over_period)
{
  ...
  for (unsigned p = 0; p < max_iter; p++)
  {
    ...
    unsteady_run_for_power_method(dt,
                                  n_timesteps,
                                  doc_info,
                                  input,
                                  output,
                                  output_base_state_solution,
                                  dofs_over_period,
                                  base_state_dofs_over_period);
    ...
    if (convergence_criterion <= tolerance * abs(calc_eigenvalue))
    {
      ...
      for (unsigned i = 0; i < n_timesteps; i++)
      {
        ...
        Perturbed_state_problem_pt->assign_eigenvector_to_dofs(
          dofs_over_period[i]);
  ...
  for (unsigned i = 0; i < n_timesteps; i++)
  {
    // Populate perturbed state problem with dofs which have been stored
    // over the last period
    Perturbed_state_problem_pt->assign_eigenvector_to_dofs(dofs_over_period[i]);
  ...
  // -----------------------------------------
  {
    ...
    for (unsigned param = 0; param < n_param_steps; param++)
    {
      ...
      cout << "Beginning parameter run " << param + 1 << " of " << n_param_steps
           << ": MMC_Re = " << GlobalPhysicalVariables::MMC_Re_current << endl;
      cout << "====================================================" << endl;
      ...
      const unsigned n_periods_to_set_up_periodic_base_flow =
        problem.set_up_periodic_base_flow(
          dt, nstep, doc_info, base_flow_norm_tolerance);
