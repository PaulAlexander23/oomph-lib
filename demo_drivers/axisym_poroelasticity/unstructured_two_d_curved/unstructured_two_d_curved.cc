...
namespace ProblemParameters
{
  ...
  double boundary_flux_1(const double& time, const Vector<double>& x)
  {
    ...
    {
      const double z = x[1];
      return 1.0 / Permeability * Alpha * time * time *
             (-2.0 / 3.0 * pow(z, 3) - z * z);
    }
    else
    {
  ...
  double boundary_dfluxdt_1(const double& time, const Vector<double>& x)
  {
    ...
    {
      const double z = x[1];
      return 1.0 / Permeability * 2.0 * Alpha * time *
             (-2.0 / 3.0 * pow(z, 3) - z * z);
    }
    else
    {
  ...
  double boundary_d2fluxdt2_1(const double& time, const Vector<double>& x)
  {
    ...
    {
      const double z = x[1];
      return 1.0 / Permeability * 2.0 * Alpha *
             (-2.0 / 3.0 * pow(z, 3) - z * z);
    }
    else
    {
                        ...
                        Vector<double>& b)
  {
    if (CommandLineArgs::command_line_flag_has_been_set("--validation"))
    {
      ...
      b[0] =
        (time / 3.0) *
        (6 * r * time * z * Alpha +
         6 * (Rho_f_over_rho * Alpha + r * z * (z + Rho_f_over_rho * Alpha)) *
           Lambda_sq -
      ...
      b[1] = (-time / (3.0 * pow(r, 3))) *
             (-3 * pow(r, 5) * time * Alpha + 6 * r * r * z * Lambda_sq -
              ...
              pow(r, 3) *
                (6 * time * Alpha +
                 2 * Rho_f_over_rho * z * z * (3 + 2 * z) * Alpha * Lambda_sq +
                 4 * time * time * z * (Lambda + Mu)));
    }
    else
    {
      b[0] = 0;
                        ...
                        Vector<double>& f)
  {
    if (CommandLineArgs::command_line_flag_has_been_set("--validation"))
    {
      const double r = x[0], z = x[1];
      f[0] =
        (time * (2 * Rho_f_over_rho * (1 + r * z) * Alpha * Lambda_sq +
                 Porosity * (1.0 / Permeability * time * Alpha +
                             r * z *
      ...
      f[1] =
        (time *
         (-2 * r * Rho_f_over_rho * z * z * (3 + 2 * z) * Alpha * Lambda_sq +
          Porosity *
            (3 * pow(r, 3) * time -
             r * time * (6 + 1.0 / Permeability * z * z * (3 + 2 * z) * Alpha) -
...
AxiPoroProblem<ELEMENT, TIMESTEPPER>::AxiPoroProblem()
{
  ...
  outer_curvilinear_boundary_pt[0] = new TriangleMeshCurviLine(
    outer_boundary_lower_circle_pt, zeta_start, zeta_end, nsegment, 0);

  // Second bit
  zeta_start = -atan(2.0);
  zeta_end = atan(2.0);
  nsegment = (unsigned)max(3.0,
                           (2.0 * MathematicalConstants::Pi * sqrt(5.0) *
                            ProblemParameters::Domain_radius *
  ...
  outer_curvilinear_boundary_pt[1] = new TriangleMeshCurviLine(
    outer_boundary_right_circle_pt, zeta_start, zeta_end, nsegment, 1);
  ...
  outer_curvilinear_boundary_pt[2] = new TriangleMeshCurviLine(
    outer_boundary_upper_circle_pt, zeta_start, zeta_end, nsegment, 2);
  ...
  outer_curvilinear_boundary_pt[3] = new TriangleMeshPolyLine(temp_coord, 3);
  ...
  temp_coord[0][0] =
    ProblemParameters::Inner_radius + ProblemParameters::Domain_radius;
...
void AxiPoroProblem<ELEMENT, TIMESTEPPER>::create_pressure_elements()
{
  ...
  for (unsigned n = 0; n < n_neigh; n++)
  {
    ...
    pressure_element_pt->pressure_fct_pt() =
      &ProblemParameters::boundary_pressure;
    pressure_element_pt->traction_fct_pt() =
      &ProblemParameters::boundary_traction;
  }
...
void AxiPoroProblem<ELEMENT, TIMESTEPPER>::set_boundary_values()
{
  ...
  Vector<typename TIMESTEPPER::NodeInitialConditionFctPt> value_fct(2);
  Vector<typename TIMESTEPPER::NodeInitialConditionFctPt> veloc_fct(2);
  Vector<typename TIMESTEPPER::NodeInitialConditionFctPt> accel_fct(2);
  ...
  Vector<typename TIMESTEPPER::NodeInitialConditionFctPt> flux_fct(2 + n, 0);
  Vector<typename TIMESTEPPER::NodeInitialConditionFctPt> flux_ddt_fct(2 + n,
                                                                       0);
  Vector<typename TIMESTEPPER::NodeInitialConditionFctPt> flux_d2dt2_fct(2 + n,
                                                                         0);
        ...
        if (nod_pt->is_on_boundary(pinned_boundaries[ibound]))
        {
          ...
          for (unsigned g = 0; g < n_flux_interpolation_points; g++)
          {
            ...
            else
            {
              // assign zero
              timestepper_pt->assign_initial_data_values(
                nod_pt, flux_fct, flux_ddt_fct, flux_d2dt2_fct);
            }
...
void AxiPoroProblem<ELEMENT, TIMESTEPPER>::doc_solution(const unsigned& label)
{
  ...
  unsigned npts = 5;
  sprintf(
    filename, "%s/soln%i.dat", ProblemParameters::Directory.c_str(), label);
  ...
  sprintf(filename,
          "%s/coarse_soln%i.dat",
          ProblemParameters::Directory.c_str(),
          label);
  some_file.open(filename);
  ...
  if (CommandLineArgs::command_line_flag_has_been_set("--validation"))
  {
    ...
    Bulk_mesh_pt->output_fct(
      some_file, npts, time(), ProblemParameters::exact_soln);
    some_file.close();
    ...
    sprintf(
      filename, "%s/error%i.dat", ProblemParameters::Directory.c_str(), label);
    some_file.open(filename);
    Bulk_mesh_pt->compute_error(
      some_file, ProblemParameters::exact_soln, time(), error, norm);
    some_file.close();
    ...
    Trace_file << ndof() << " " << ProblemParameters::Element_area << " "
               << ProblemParameters::Dt << " " << sqrt(error[0]) << " "
               << sqrt(error[1]) << " " << sqrt(error[2]) << " "
               << sqrt(norm[0]) << " " << sqrt(norm[1]) << " " << sqrt(norm[2])
               << std::endl;
  }
  else
  {
...
int main(int argc, char** argv)
{
  ...
  // Timestep
  CommandLineArgs::specify_command_line_flag("--dt", &ProblemParameters::Dt);
  ...
  CommandLineArgs::specify_command_line_flag("--n_steps", &n_steps);
  ...
  else
  {
    ...
    for (unsigned i = 1; i <= n_steps; i++)
    {
      ...
      oomph_info << "Solving at time "
                 << problem.time_pt()->time() + ProblemParameters::Dt
                 << std::endl;
