...
FourierDecomposedHelmholtzProblem<ELEMENT>::FourierDecomposedHelmholtzProblem()
{
  ...
  Bulk_mesh_pt = new AnnularQuadMesh<ELEMENT>(
    n_r, n_theta, r_min, r_max, theta_min, theta_max);
...
void FourierDecomposedHelmholtzProblem<ELEMENT>::check_gamma(DocInfo& doc_info)
{
  ...
  for (unsigned e = 0; e < nel; e++)
  {
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      some_file << atan2(x[0], x[1]) << " " << gamma[ipt].real() << " "
                << gamma[ipt].imag() << " " << flux.real() << " " << flux.imag()
                << " " << std::endl;
...
void FourierDecomposedHelmholtzProblem<ELEMENT>::doc_solution(DocInfo& doc_info)
{
  ...
  // Output total power
  oomph_info << "Radiated power: " << ProblemParameters::N_fourier << " "
             << ProblemParameters::K_squared << " " << power << std::endl;
  some_file.open(filename);
...
//==========================================================
template<class ELEMENT>
void FourierDecomposedHelmholtzProblem<
...
int main(int argc, char** argv)
{
  ...
  //-----------------------------
  {
    ...
    for (unsigned i = 0; i < nplot; i++)
    {
      ...
      // jv[3] contains J_{7/2}(x).
      CRBond_Bessel::bessjyv(
        order_max_in, x, order_max_out, &jv[0], &yv[0], &djv[0], &dyv[0]);
      bessely_file << x << " ";
  ...
  //--------------------------
  {
    ...
    {
      double x =
        double(i) / double(nplot - 1) * 2.0 * MathematicalConstants::Pi;
