...
public:
  ...
  void actions_after_adapt()
  {
    ...
    if (Global_Physical_Variables::inlet_traction &&
      ...
      mesh_pt()->make_flux_element();

    // Reattach shear elements
    mesh_pt()->make_shear_elements();

    // Unpin all pressure dofs
    RefineablePolarNavierStokesEquations::unpin_all_pressure_dofs(
      mesh_pt()->fluid_elt_vector());

    // Pin redundant pressure dofs
    RefineablePolarNavierStokesEquations::pin_redundant_nodal_pressures(
      mesh_pt()->fluid_elt_vector());
  ...
  void assign_external_pressure()
  {
    ...
    {
      this->mesh_pt()->inlet_traction_elt_pt(e)->set_external_pressure_data(
                                                    ...
                                                    bool eigen)
{
  ...
  cout << "Number of (streamfunction) equations:"
       << stream_problem.assign_eqn_numbers() << endl;
  ...
  clock_t end = clock();
  cout << "This took  " << (double(end - start)) / CLOCKS_PER_SEC << " seconds."
       << endl;
                                           ...
                                           double pressure_norm)
{
  ...
  for (unsigned e = 0; e < n_fluid; e++)
  {
    ...
    s[0] = -1.;
    double r_min = el_pt->interpolated_x(s, 0);
    s[0] = 1.;
    double r_max = el_pt->interpolated_x(s, 0);
    s[1] = -1.;
    double phi_min = el_pt->interpolated_x(s, 1);
    s[1] = 1.;
    double phi_max = el_pt->interpolated_x(s, 1);
...
void PolarNSProblem<ELEMENT>::header(ofstream& some_file)
{
  ...
  some_file << "# Initial xmesh = " << xmesh << " Initial ymesh = " << ymesh
            << "\n";
  some_file << "# Uniform refinements: " << uniform
            << " Adaptive refinements: " << adaptive << "\n";
  some_file << "# inlet_traction = " << inlet_traction
            << " eta_inlet = " << eta_inlet << "\n";
  some_file << "# outlet_traction = " << outlet_traction
            << " eta_outlet = " << eta_outlet << "\n";
  some_file << "# log_mesh = " << log_mesh
            << " new_outlet_region = " << new_outlet_region << "\n";
  some_file << "# pinv = " << pinv << "\n";
...
int main()
{
  ...
  for (unsigned k = 0; k < n_eval; k++)
    cout << "eigenvalue: " << eigenvalues[k] << endl;
  ...
  out << "# Output { Re, Alpha, Int du_dphi(-1), Int du_dphi(1), "
         "Delta_Shear_Stress, Jacobian sign }"
      << endl;
  out << "# Radius ratio: " << R_l << " xmesh: " << xmesh << " ymesh " << ymesh
      << endl;
  out << "# Alpha = " << Alpha << endl;
  out << "# New outlet region: " << new_outlet_region << endl;
  out << "# Inlet traction: " << inlet_traction
      << " Outlet traction: " << outlet_traction << endl;
  out << "# eta_inlet (Homotopy parameter): " << eta_inlet
      << " eta_outlet: " << eta_outlet << endl;
  ...
  {
    cout << endl
         << "Using symmetry vector from get_symmetry routine.  Re = " << Re
         << endl
         << endl;
  ...
  {
    cout << endl
         << "Using eigenvector for pitchfork detection.  Re = " << Re << endl
         << endl;
    problem.activate_pitchfork_tracking(&Global_Physical_Variables::Re,
                                        eigenvector);
  }
  ...
  unsigned long n_dof = problem.ndof();
  cout << "ndof: " << n_dof << endl;
  DoubleVector soln;
  ...
  cout << "Shear stress is: Lower = " << shear_stress[0]
       << " Upper = " << shear_stress[1]
       << " Delta = " << (shear_stress[0] + shear_stress[1]) << endl;

  cout << endl
       << "Pitchfork detected at Re = " << Re << " and Alpha = " << Alpha
       << endl;
  cout << "========================================================" << endl;

  out << Re << " " << Alpha << " " << shear_stress[0] << " " << shear_stress[1]
      << " " << (shear_stress[0] + shear_stress[1]) << " " << 0 << " " << slack
      << endl;


  cout << "========================================================" << endl;
  cout << endl
       << "Computing eigenvalue at pitchfork.  Re = " << Re << endl
       << endl;
  // Solve the eigenproblem
  problem.solve_eigenproblem(
    n_eval, eigenvalues, eigenvector_real, eigenvector_imag);
  ...
  cout << endl
       << "Adding eigenvector[0] with multiplier " << multiplier
       << " to degrees of freedom" << endl;
  for (unsigned long d = 0; d < n_dof; d++)
  {
    problem.dof(d) =
      soln_at_bifurcation[d] + multiplier * critical_eigenvector[d];
  }

  // Now we increace/decreace the Reynolds number slightly and compute a
  // solution
  Re += epsilon;
  cout << "Adjusting Reynolds number by " << epsilon << " to Re: " << Re
       << " and newton solving" << endl
       << endl;
  problem.newton_solve();
  problem.get_shear_stress(shear_stress);
  ...
  out << Re << " " << Alpha << " " << shear_stress[0] << " " << shear_stress[1]
      << " " << (shear_stress[0] + shear_stress[1]) << " " << sign << endl;
  ...
  cout << "Shear stress is: Lower = " << shear_stress[0]
       ...
       << " Delta = " << (shear_stress[0] + shear_stress[1]) << endl;
  if (inlet_traction && outlet_traction)
    cout << " pext: " << problem.get_pext() << endl;
  out << Re << " " << Alpha << " " << shear_stress[0] << " " << shear_stress[1]
      << " " << (shear_stress[0] + shear_stress[1]) << " " << sign << endl;
  ...
  for (; Re < Rmax;)
  {
    ...
    if (inlet_traction && outlet_traction)
      cout << " pext: " << problem.get_pext() << endl;
    out << Re << " " << Alpha << " " << shear_stress[0] << " "
        << shear_stress[1] << " " << (shear_stress[0] + shear_stress[1]) << " "
        << sign << endl;
  }
