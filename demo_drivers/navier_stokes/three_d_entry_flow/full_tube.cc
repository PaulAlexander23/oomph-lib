...
public:
  ...
  void test_mesh_as_geom_object()
  {
    cout << "test_mesh_as_geom_object" << endl;
    MeshAsGeomObject mesh_as_geom_obj(mesh_pt());

    Vector<double> zeta(3, 0.0);
    GeomObject* sub_geom_object_pt = 0;
    Vector<double> s(3, 0.0);
    mesh_as_geom_obj.locate_zeta(zeta, sub_geom_object_pt, s);
    ...
    ELEMENT* bulk_el_pt = dynamic_cast<ELEMENT*>(sub_geom_object_pt);
    Vector<double> x(3, 0.0);
    bulk_el_pt->position(s, x);
    cout << "x: ";
    for (unsigned k = 0; k < 3; k++)
    {
      cout << x[k] << ", ";
    ...
    Node* node_pt = bulk_el_pt->get_node_at_local_coordinate(s);
    cout << "node_pt: " << node_pt << ", ";

    cout << endl;
    cout << "end of test_mesh_as_geom_object" << endl;
  }
...
int main(int argc, char* argv[])
{
  ...
  //------------------------
  {
    ...
    // Build problem
    SteadyTubeProblem<RefineableQTaylorHoodElement<3>> problem(
      doc_info, min_error_target, max_error_target);

    cout << " Doing Taylor-Hood elements " << std::endl;
    problem.test_mesh_as_geom_object();
  ...
  //------------------------
  {
    ...
    // Build problem
    SteadyTubeProblem<RefineableQCrouzeixRaviartElement<3>> problem(
      doc_info, min_error_target, max_error_target);
