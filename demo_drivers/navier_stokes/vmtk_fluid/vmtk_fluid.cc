...
//=======================================================================
template<class ELEMENT>
void UnstructuredFluidProblem<
  ELEMENT>::create_parallel_outflow_lagrange_elements()
{
  ...
  for (unsigned in_out = 0; in_out < 2; in_out++)
  {
    ...
    for (unsigned i = 0; i < n; i++)
    {
      ...
      for (unsigned e = 0; e < n_element; e++)
      {
        ...
        // Add it to the mesh
        Parallel_outflow_lagrange_multiplier_mesh_pt[count]->add_element_pt(
          el_pt);
...
void UnstructuredFluidProblem<ELEMENT>::doc_solution(DocInfo& doc_info)
{
  ...
  std::cout << Fluid_mesh_pt->nelement() << std::endl;
