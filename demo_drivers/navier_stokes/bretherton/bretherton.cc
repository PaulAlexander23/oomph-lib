...
namespace Global_Physical_Variables
{
  ...
  void inflow(const Vector<double>& x, Vector<double>& veloc)
  {
    ...
    double C =
      6.0 * (2.0 * h_av + y_lo - y_up) /
      (y_up * y_up * y_up - y_lo * y_lo * y_lo - h_av * y_up * y_up * y_up +
       h_av * y_lo * y_lo * y_lo - 3.0 * y_lo * y_up * y_up +
       3.0 * y_lo * y_lo * y_up + 3.0 * y_lo * y_up * y_up * h_av -
       3.0 * y_lo * y_lo * y_up * h_av);
    ...
    for (unsigned j = 0; j < n_nod; j++)
    {
      ...
      if (nod_pt->is_on_boundary(Inflow_boundary))
      {
        ...
        for (unsigned i = 0; i < 2; i++)
        {
          ...
          {
            std::ostringstream error_stream;
            error_stream << "We're assigning a Dirichlet condition for the "
                         << i << "-th "
...
namespace oomph
{
  ...
  template<>
  class FaceGeometry<
    FaceGeometry<BrethertonElement<SpineElement<QCrouzeixRaviartElement<2>>>>>
  ...
  template<>
  class FaceGeometry<
    FaceGeometry<BrethertonElement<SpineElement<QTaylorHoodElement<2>>>>>
    : public virtual PointElement
  {
  public:
    FaceGeometry() : PointElement() {}
...
void BrethertonProblem<ELEMENT>::parameter_study(const unsigned& nsteps)
{
  ...
  for (unsigned step = 1; step <= nsteps; step++)
  {
    ...
    while (true)
    {
      cout << "Checking max. res for Ca = " << Global_Physical_Variables::Ca
           << ".   Max. residual: ";
...
int main(int argc, char* argv[])
{
  ...
  // Set up the problem
  BrethertonProblem<BrethertonElement<SpineElement<QCrouzeixRaviartElement<2>>>>
    problem;
