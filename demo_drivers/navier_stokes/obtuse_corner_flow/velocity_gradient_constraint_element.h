#ifndef VELOCITY_GRADIENT_CONSTRAINT_ELEMENTS_HEADER
#define VELOCITY_GRADIENT_CONSTRAINT_ELEMENTS_HEADER
...
namespace oomph
{
  ...
  class VelocityGradientConstraintElement
  ...
  private:
    ...
    int Traded_data_index;
    Vector<int> Node_index;
    Vector<double> Evalution_point_s;

  public:
    // Constructor
    VelocityGradientConstraintElement(FiniteElement* const& element_pt,
    ...
    void output(std::ostream& outfile)
    {
      ...
      // Output the pressure
      outfile << interpolated_d2udx2(Evalution_point_s) << ",";
  ...
  private:
    void compute_s(Node* const& node_pt)
    {
      ...
      {
        Evalution_point_s[0] = 0.0;
      }
      else
      {
        Evalution_point_s[0] = 1.0;
      }
    }
  ...
  public:
    ...
    void set_traded_data_pt(Data* const& traded_data_pt)
    {
      Traded_data_index = add_external_data(traded_data_pt);
    }
  ...
  protected:
      ...
      Vector<double>& residuals, DenseMatrix<double>& jacobian, unsigned flag)
    {
      ...
      if (el_dim != 1)
      {
        ...
        error_stream
          << "Currently, the VelocityGradientConstraintElement elements\n"
          ...
          << "   "
             "VelocityGradientConstraintElement::fill_in_generic_residual_"
      ...
      const unsigned traded_value_index = 0;
      // Add to singular function scaling residual
      const int local_eqn =
        this->external_local_eqn(Traded_data_index, traded_value_index);

      // If the equation is not pinned
      if (local_eqn >= 0)
      {
        // Add (or subtract) the pressure at the evaluation point
        residuals[local_eqn] += interpolated_d2udx2(Evalution_point_s);
    ...
    double interpolated_d2udx2(const Vector<double>& s) const
    {
      // Find number of nodes
      const unsigned n_node = Cast_bulk_element_pt->nnode();

      // Find the dimension of the problem
      const unsigned cached_dim = Cast_bulk_element_pt->dim();

      // Set up memory for pressure shape and test functions
      Shape psif(n_node);
      DShape dpsifdx(n_node, cached_dim);
      DShape d2psifdx2(n_node, cached_dim);
      ...
      Cast_bulk_element_pt->d2shape_eulerian(s_bulk, psif, dpsifdx, d2psifdx2);

      // Compute the velocity derivative
      double interpolated_d2udx2 = 0.0;
      // Loop over the local nodes and sum
      for (unsigned l = 0; l < n_node; l++)
      {
        // Loop over the spatial directions
        const double u_value = Cast_bulk_element_pt->u_nst(l, 1);
        interpolated_d2udx2 += u_value * d2psifdx2(l, 1);
      }

      return interpolated_d2udx2;
    }
  };
} // namespace oomph
#endif
