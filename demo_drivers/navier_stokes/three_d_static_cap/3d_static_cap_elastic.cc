...
public:
      ...
      SolidMesh()
  {
    ...
    for (unsigned e = 0; e < n_element; e++)
    {
      ...
      {
        // Bottom
        adjacent[0] &= interface_element_pt->node_pt(n)->is_on_boundary(3);
        // Right
        adjacent[1] &=
  ...
  : Volume(atan(1.0)), Doc_info(doc_info)
{
  ...
  Bulk_mesh_pt =
    new AxialSolidQuarterTubeMesh<ELEMENT,
                                  ElasticSurfaceFluidInterfaceElement<ELEMENT>>(
...
/// the volume
template<class ELEMENT>
void SolidFreeSurfaceRotationProblem<
...
int main(int argc, char* argv[])
{
  ...
  //------------------------
  {
    ...
    // Build problem
    SolidFreeSurfaceRotationProblem<Hijacked<
