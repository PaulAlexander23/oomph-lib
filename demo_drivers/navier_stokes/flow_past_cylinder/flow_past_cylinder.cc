  ...
  GeomObject* cylinder_pt, const double& length, const double& height)
{
  ...
  // adapted. Since the velocity is not constant, the interpolation is
  // insufficiently accurate and the boundary conditions must be re-applied in
  // actions_after_adapt(). Setup tangential flow along boundary 0:
  unsigned ibound = 3;
  double ycoord, uy;
  double um = 0.3;
  unsigned num_nod = mesh_pt()->nboundary_node(ibound);
