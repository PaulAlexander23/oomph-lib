    ...
    double square_of_l2_norm()
    {
      ...
      for (unsigned ipt = 0; ipt < n_intpt; ipt++)
      {
        ...
        for (unsigned l = 0; l < n_node; l++)
        {
          ...
          {
            // Get the nodal value
            double u_value = raw_nodal_value(l, u_index_nst(l, i));
...
public:
  ...
  void actions_before_implicit_timestep()
  {
    ...
    double dx =
      Problem_Parameter::Block_x_velocity * ampl * this->time_pt()->dt();

    // Assign the y increment of the hole using velocity and timestep
    double dy =
      Problem_Parameter::Block_y_velocity * ampl * this->time_pt()->dt();

    // Assign the rotation of the hole using velocity and timestep
    double ampl2 = 1.0 - exp(-10.0 * time * time);
    double drotation = Problem_Parameter::Block_rotation_velocity * ampl2 *
                       this->time_pt()->dt();
    ...
    for (unsigned ihole = 0; ihole < nhole; ihole++)
    {
      ...
      // Update values
      Problem_Parameter::Centre_displacement_data_pt[ihole]->set_value(
        1, old_y + dy);
...
void UnstructuredFluidProblem<ELEMENT>::doc_boundary_coordinates()
{
  ...
  for (unsigned b = 0; b < n_boundary; b++)
  {
    ...
    if (boundary_geom_obj_pt != 0)
    {
      ...
      Vector<double> zeta_limits = Fluid_mesh_pt->boundary_coordinate_limits(b);
