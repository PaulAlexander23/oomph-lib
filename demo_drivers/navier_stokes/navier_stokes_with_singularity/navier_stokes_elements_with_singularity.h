    ...
    inline Vector<double> interpolated_u_nst(const Vector<double>& s) const
    {
      ...
      for (unsigned d = 0; d < cached_dim; d++)
      {
        ...
        for (unsigned j = 0; j < n_node; j++)
        {
          ...
          const unsigned u_nodal_index = this->u_index_nst(j, d);
  ...
  private:
      ...
      const unsigned& flag)
    {
          ...
          (!all_singular_functions_satisfy_stokes_equation))
      {
        ...
        for (unsigned ipt = 0; ipt < n_intpt; ipt++)
        {
          ...
          for (unsigned l = 0; l < n_node; l++)
          {
            ...
            for (unsigned i = 0; i < cached_dim; i++)
            {
              double u_value = this->raw_nodal_value(l, this->u_index_nst(l, i));
          ...
          for (unsigned l = 0; l < n_node; l++)
          {
            ...
            for (unsigned i = 0; i < cached_dim; i++)
            {
              ...
              local_eqn = this->nodal_local_eqn(l, this->u_index_nst(l, i));
                    ...
                    if (scaled_re > 0.0)
                    {
                      ...
                      for (unsigned l2 = 0; l2 < n_node; l2++)
                      {
                        ...
                        for (unsigned i2 = 0; i2 < cached_dim; i2++)
                        {
                          ...
                          local_unknown =
                            this->nodal_local_eqn(l2, this->u_index_nst(l2, i2));
                          if (local_unknown >= 0)
                          {
                            double sum = 0.0;
                            if (i == i2)
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        {
          // Find its local equation number
          local_eqn = this->nodal_local_eqn(l, this->u_index_nst(l, d));
