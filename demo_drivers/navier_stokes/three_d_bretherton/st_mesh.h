...
///
/// Remarck, we set the flag of every mesh twice. it is not logical but saves
/// programming time
//======================================================================
...
public:
  ...
  // Acces functions for the fixed coordinates of the bulk outlet and inlet
  // elements
  ...
  virtual void spine_node_update(SpineNode* spine_node_pt)
  {
    ...
    if (flag != 2)
    {
      ...
      if (flag == 0) // Canyon mesh
      {
        ...
        // Set th x coordinate
        spine_node_pt->x(0) =
          origin_pt[0] + H * W * (Xsim - origin_pt[0]) / norm;

        // Set the value of z
        spine_node_pt->x(2) =
          origin_pt[2] + H * W * (Zsim - origin_pt[2]) / norm;
      }
      ...
      if (flag == 1) // Tip mesh
      {
        // set scale norm vector
        double norm = sqrt((Xsim - origin_pt[0]) * (Xsim - origin_pt[0]) +
                           (Ysim - origin_pt[1]) * (Ysim - origin_pt[1]) +
                           (Zsim - origin_pt[2]) * (Zsim - origin_pt[2]));
        ...
        spine_node_pt->x(1) =
          origin_pt[1] + H * W * (Ysim - origin_pt[1]) / norm;
...
STSpineMesh<ELEMENT, INTERFACE_ELEMENT>::STSpineMesh(
  ...
  : Nel_can(nel_can),
    ...
    Nel_liq(nel_liq),
    Alpha(alpha),
    Length_can(length_can),
    Length_tip(length_tip),
    Length_liq(length_liq),
    Height(height),
    Radius(radius)
{
  ...
  TimeStepper* time_stepper_pt)
{
  ...
  std::cout << "The complete mesh consists of  " << nbulk()
  ...
  unsigned spine_flag)
{
  ...
  // Another control. Test that the two shared boundaries have the same number
  // of nodes
  if (addmesh_pt->nboundary_node(bound2) != mesh_pt->nboundary_node(bound1))
  {
    std::ostringstream error_stream;
    error_stream << "Different number of nodes in the shared boundaries:\n"
  ...
  for (unsigned long i = 0; i < addmesh_pt->nboundary_node(bound2); i++)
  {
    ...
    do
    {
      ...
      {
        d += (addmesh_pt->boundary_node_pt(bound2, i)->x(k) -
              mesh_pt->boundary_node_pt(bound1, nodecounter)->x(k)) *
             (addmesh_pt->boundary_node_pt(bound2, i)->x(k) -
              mesh_pt->boundary_node_pt(bound1, nodecounter)->x(k));
      }
    ...
    {
      std::ostringstream error_stream;
      error_stream << "Error doing the mapping between shared boundaries:\n"
  ...
  // Now we loop over the elements of the added mesh on the shared boundary and
  // reasign the node  pointers to the ones
  // of the original mesh.

  // REMARCK!!!!!  the boundary_element scheme was not implemented in the mesh
  // design, so that we have to loop over all the elements
  ...
  for (unsigned int i = 0; i < addmesh_pt->nelement(); i++)
  {
    ...
    {
      // In case the node is in the boundary (and therefore in the map), we
      // reasign the node
  ...
  {
    std::ostringstream error_stream;
    error_stream << "You have added "
                 << (zaehler - addmesh_pt->nboundary_node(bound2))
                 << " nodes too much to the mesh.\n"
                 << "(This control should be removed in case we do not want to "
                    "copy all the nodes of the shared boundaries)\n";
  ...
  // We reset the boundary conditions of the old mesh (the shared boundary is
  // not more in a boundary)
  ...
  for (int i = 0; i < (int)(addmesh_pt->nboundary()); i++)
  {
    ...
    {
      // We do not reset the commom boundary,whose nodes will be deleted  at the
      // end
      if (i != bound2)
      {
        ...
        // if not we have to include to the boundarie (in the case that they
  ...
  // At the end we remove the shared nodes of the second mesh for avoiding
  // problems with memory leaking
  ...
  {
    std::cout << "Boundary " << i << " has " << mesh_pt->nboundary_node(i)
              << " nodes." << std::endl;
  }
  // #endif
}
