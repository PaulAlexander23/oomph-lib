...
namespace oomph
{
  namespace MeshHelper
  {
                            ...
                            unsigned spine_flag)
    {
      ...
      // Another control. Test that the two shared boundaries have the same
      // number of nodes
      ...
      for (unsigned long i = 0; i < addmesh_pt->nboundary_node(bound2); i++)
      {
        ...
        else
        {
          ...
          if (add_spinenode == 0)
          {
            throw OomphLibError("There are nodes on the shared boundary of the "
                                "added mesh which are not Spine Nodes\n",
                                OOMPH_CURRENT_FUNCTION,
                                OOMPH_EXCEPTION_LOCATION);
          }
          ...
          {
            throw OomphLibError("There are nodes on the shared boundary of the "
                                "original mesh which are not Spine Nodes\n",
                                OOMPH_CURRENT_FUNCTION,
                                OOMPH_EXCEPTION_LOCATION);
          } // mesh3_pt
#endif
          ...
          {
// We assure that the nodes that belong to  one spine in one mesh are as well
// belonging to only one spine in the other mesh
#ifdef PARANOID
            if (map_spines[add_spine] != org_spine)
            {
              throw OomphLibError(
                "The spines map could not be performed because the shared "
                "spines are pointing to different nodes\n",
                OOMPH_CURRENT_FUNCTION,
                OOMPH_EXCEPTION_LOCATION);
            }
#endif
      ...
      // Now we loop over the elements of the added mesh on the shared boundary
      // and reasign the node and spine pointers to the ones
      //  of the original mesh. We need to do a distition between ELMENTS and
      //  SURFACE ELEMENT

      // REMARCK!!!!!  the boundary_element scheme was not implemented in the
      // mesh design, so that we have to loop over all the elements
      ...
      if (zaehler != addmesh_pt->nboundary_node(bound2))
      {
        ...
        error_stream
          ...
          << "(This control should be removed in case we do not want to copy "
             "all the nodes of the shared boundaries)\n";
      ...
      // We reset the boundary conditions of the old mesh (the shared boundary
      // is not more in a boundary)
      ...
      for (int i = 0; i < (int)(addmesh_pt->nboundary()); i++)
      {
        ...
        {
          // We do not reset the commom boundary,whose nodes will be deleted  at
          // the end
          if (i != bound2)
          {
