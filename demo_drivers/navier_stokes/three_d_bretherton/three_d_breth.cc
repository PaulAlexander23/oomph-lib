...
public:
  ...
  void multiply_aspect_ratio(const double factor, const bool& mult_width)
  {
    ...
    for (unsigned s = 0; s < n_spine; s++)
    {
      ...
      if (mult_width)
      {
        spine_pt->geom_parameter(0) = x_spine * factor;
      }
...
ThreeDimBethertonProblem<ELEMENT>::ThreeDimBethertonProblem()
{
  ...
  Problem::mesh_pt() =
    new STSpineMesh<Hijacked<ELEMENT>,
                    SpineSurfaceFluidInterfaceElement<ELEMENT>>(ncan,
                                                                ntip,
                                                                nliq,
                                                                alpha,
                                                                length_can,
  ...
  cout << "We have " << n_bulk << " bulk elemens, " << n_interface
       << " interface elements,  " << n_element
       << " generic elements in this mesh" << endl;
  ...
  Trace_file
    << "\"Ca\",\t \"Bo\", \t \"Alpha\",  \t \"m\", \t \"p<sub>bubble</sub>\", "
       "\t \"r<sub>h_top</sub>\", \t \"r<sub>d_top</sub>\", \t "
       "\"r<sub>h_middle</sub>\", \t \"r<sub>d_bottom</sub>\", \t "
       "\"r<sub>h_bottom</sub>\",\t \"lambda\"  ";
...
int main(int argc, char* argv[])
{
  ...
  ThreeDimBethertonProblem<SpineElement<QCrouzeixRaviartElement<3>>> problem;
