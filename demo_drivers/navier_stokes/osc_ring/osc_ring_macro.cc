  ...
  : IC_Fct_pt(IC_fct_pt)
{
  ...
  Wall_pt = new PseudoBucklingRing(
...
void OscRingNStProblem<ELEMENT, TIMESTEPPER>::doc_solution(DocInfo& doc_info)
{
  ...
  Trace_file << time_pt()->time() << " " << r[1] << " " << global_kin << " "
             ...
             << Veloc_trace_node_pt->value(1) << " " << mesh_pt()->nelement()
             << " " << ndof() << " " << min_level << " " << max_level << " "
             << mesh_pt()->nrefinement_overruled() << " "
             << mesh_pt()->max_error() << " " << mesh_pt()->min_error() << " "
             << mesh_pt()->max_permitted_error() << " "
             << mesh_pt()->min_permitted_error() << " "
             << mesh_pt()->max_keep_unrefined() << " " << doc_info.number()
...
int main(int argc, char* argv[])
{
  ...
  OscRingNStProblem<RefineableQCrouzeixRaviartElement<2>, BDF<4>> problem(
    dt, &SarahBL::full_exact_soln);
  ...
  if (CommandLineArgs::Argc == 3)
  {
    ...
    // Set up the problem: Pass timestep and Sarah's asymptotic solution for
    // generation of initial condition
    OscRingNStProblem<RefineableQCrouzeixRaviartElement<2>, BDF<4>>
