...
namespace oomph
{
  ...
  void RefineableTetgenMesh<ELEMENT>::adapt(const Vector<double>& elem_error)
  {
    ...
    double max_edge_ratio = compute_volume_target(elem_error, target_size);
        ...
        (max_edge_ratio > this->max_permitted_edge_ratio()))
    {
      ...
      {
        tmp_new_mesh_pt =
          new RefineableTetgenMesh<ELEMENT>(this->Outer_boundary_pt,
      ...
      while (!done)
      {
        ...
        Multi_domain_functions::setup_multi_domain_interaction<ELEMENT>(
          this->Problem_pt, this, tmp_new_mesh_pt);
        ...
        for (unsigned e = 0; e < nelem; e++)
        {
          ...
          for (unsigned ipt = 0; ipt < nint; ipt++)
          {
            ...
            // of small elements
            target_size_map[ext_el_pt] =
              std::max(target_size_map[ext_el_pt], target_size[e]);
          }
        ...
        {
          new_mesh_pt = new RefineableTetgenMesh<ELEMENT>(new_target_size,
                                                          tmp_new_tetgenio_pt,
                                                          this->Time_stepper_pt,
                                                          this->Use_attributes);
        }
...
RisingBubbleProblem<ELEMENT>::RisingBubbleProblem()
{
  ...
  Vol_constraint_el_pt = new VolumeConstraintElement(&Global_Parameters::Volume,
  ...
  Outer_boundary_pt = new TetgenMeshOLDFacetedSurface(
    box_point, box_facet, box_facet_boundary_id);
  ...
  for (unsigned p = 0; p < 12; p++)
  {
    ...
    for (unsigned i = 0; i < 3; i++)
    {
      r2 += icosa_point[p][i] * icosa_point[p][i];
    }
  ...
  Inner_boundary_pt[0] = new TetgenMeshClosedFacetedSurface(
  ...
  Fluid_mesh_pt = new RefineableSolidTetgenMesh<ELEMENT>(
