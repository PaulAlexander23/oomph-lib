...
public:
                ...
                Vector<double>& r) const
  {
    ...
    // Position
    r[0] = X_0 + D_x * xi[0] * xi[0] / Length / Length *
                   sin(2.0 * Pi * Time_pt->time(t) / T);
    r[1] = xi[0] * (1.0 + D_y / Length * 0.5 *
                            (1.0 - cos(4.0 * Pi * Time_pt->time(t) / T)));
  }
  ...
  const double& period)
{
  ...
  // Build the mesh
  Problem::mesh_pt() =
    new RefineableAlgebraicChannelWithLeafletMesh<ELEMENT>(Leaflet_pt,
                                                           l_left,
                                                           l_right,
                                                           h_leaflet,
                                                           h_tot,
