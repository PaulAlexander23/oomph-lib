  ...
  // Initially set all nodes to the Nusselt flat-film solution
  {
    ...
    for (unsigned n = 0; n < n_node; n++)
    {
      ...
      Bulk_mesh_pt->node_pt(n)->set_value(
        0, 0.5 * ReInvFr * sin(Alpha) * (2.0 * y - y * y));
    }
  }
  ...
  const double& dt, const unsigned& n_tsteps)
{
  ...
  trace << time_pt()->time() << " "
        ...
        << Bulk_mesh_pt
             ->boundary_node_pt(2, Bulk_mesh_pt->nboundary_node(2) - 1)
             ->x(1)
        << " " << std::endl;
...
class SpineInclinedPlaneMesh : public SimpleRectangularQuadMesh<ELEMENT>,
                               public SpineMesh
{
public:
      ...
      for (unsigned l2 = 0; l2 < n_pmax; l2++)
      {
        ...
        for (unsigned long i = 0; i < ny; i++)
        {
          ...
          for (unsigned l1 = 1; l1 < n_p; l1++)
          {
            ...
            // Set the fraction
            nod_pt->fraction() =
              (double(i) + double(l1) / double(n_p - 1)) / double(ny);
            // Pointer to the mesh that implements the update fct
            nod_pt->spine_mesh_pt() = this;
          }
        }
...
//======================================================================
template<class ELEMENT>
class ElasticInclinedPlaneMesh : public SimpleRectangularQuadMesh<ELEMENT>,
                                 public SolidMesh
{
  // Public functions
public:
...
#define FLUID_ELEMENT QTaylorHoodElement<2>
  ...
  {
    // Create the problem
    SpineInclinedPlaneProblem<SpineElement<FLUID_ELEMENT>, BDF<2>> problem(
      30, 4, Global_Physical_Variables::Length);
  ...
  // Elastic problem
  {
    ...
    ElasticInclinedPlaneProblem<
      PseudoSolidNodeUpdateElement<FLUID_ELEMENT, QPVDElement<2, 3>>,
      BDF<2>>
