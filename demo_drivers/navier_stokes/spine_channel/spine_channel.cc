...
public:
                         ...
                         DenseMatrix<double>& drdzeta) const
  {
    ...
    drdzeta(0, 1) = MathematicalConstants::Pi * A *
                    cos(MathematicalConstants::Pi * (zeta[0] - zeta_min) / Lz) /
                    Lz;
  }
                          ...
                          RankThreeTensor<double>& ddrdzeta) const
  {
    ...
    ddrdzeta(0, 0, 0) = 0.0;
    ddrdzeta(0, 0, 1) =
      -MathematicalConstants::Pi * MathematicalConstants::Pi * A *
      sin(MathematicalConstants::Pi * (zeta[0] - zeta_min) / Lz) / (Lz * Lz);
  }
...
int main()
{
  ...
  {
    // Build problem
    ChannelSpineFlowProblem<SpineElement<QCrouzeixRaviartElement<2>>> problem;
...
void doc_sparse_node_update()
{
  ...
  // Output solution
  sprintf(
    filename, "%s/mesh_update%i.dat", doc_info.directory().c_str(), count);
  count++;
