...
public:
  ...
  virtual void spine_node_update(SpineNode* spine_node_pt)
  {
    ...
    spine_node_pt->spine_pt()->geom_object_pt(0)->position(x_spine_origin,
                                                           spine_base);
...

{
  ...
  for (unsigned i = 0; i < ny; i++)
  {
    ...
    for (unsigned l1 = 1; l1 < np; l1++)
    {
      ...
      // Set the fraction
      nod_pt->fraction() =
        (double(i) + double(l1) / double(np - 1)) / double(ny);
...
ChannelSpineFlowProblem<ELEMENT>::ChannelSpineFlowProblem()
{
  ...
  Problem::mesh_pt() =
    new SimpleSpineMesh<ELEMENT>(Nx, Ny, Lx, Ly, substrate_pt);
