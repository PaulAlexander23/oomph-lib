  ...
  const unsigned& nx, const unsigned& ny, const double& lx, const double& ly)
{
  ...
  for (unsigned e = 0; e < n_el; e++)
  {
    ...
    el_pt->traction_fct_pt() = &ExactSoln::prescribed_traction;
  }
...
int main(int argc, char* argv[])
{
  ...
  // Solve with Crouzeix-Raviart elements
  {
    ...
    // Set up problem
    RayleighTractionProblem<QCrouzeixRaviartElement<2>, BDF<2>> problem(
      nx, ny, lx, ly);
  ...
  // Solve with Taylor-Hood elements
  {
    ...
    // Set up problem
    RayleighTractionProblem<QTaylorHoodElement<2>, BDF<2>> problem(
      nx, ny, lx, ly);
