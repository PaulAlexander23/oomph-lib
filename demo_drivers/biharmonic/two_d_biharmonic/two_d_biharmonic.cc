...
public:
  ...
  BiharmonicTestProblem1(const int x_min,
                         const int x_max,
                         const int y_min,
                         const int y_max,
...
namespace BiharmonicTestFunctions2
{
  ...
  void surface_load(const Vector<double>& x, double& f)
  {
    ...
    f = (16 * sinr2 * x[0] * x[0] * x[0] * x[1] - 64 * cosr2 * x[0] * x[1] -
         48 * sinr2 * x[1] / x[0] +
...
public:
  ...
  BiharmonicTestProblem2(const unsigned n_element)
  {
    ...
    // create the domain describing the geometry of the problem
    Domain_pt = new TopologicallyRectangularDomain(
                              ...
                              const Problem* const problem_pt)
{
  ...
  oomph_info << "Elements: 1D: " << n_ele_1d << ", total: " << n_element
             << std::endl;
...
void print_connectivity_matrix(const Problem* const problem_pt)
{
  ...
  filenamestream << RayParam::Connectivity_dir << "/" << RayParam::Matbase_str;
...
int main(int argc, char* argv[])
{
  ...
  CommandLineArgs::specify_command_line_flag("--noel", &RayParam::Noel);
  CommandLineArgs::specify_command_line_flag("--xmin", &RayParam::X_min);
  CommandLineArgs::specify_command_line_flag("--xmax", &RayParam::X_max);
  CommandLineArgs::specify_command_line_flag("--ymin", &RayParam::Y_min);
  CommandLineArgs::specify_command_line_flag("--ymax", &RayParam::Y_max);
  ...
  if (argc > 1)
  {
    ...
    if (RayParam::Print_natural_jacobian)
    {
      ...
      dump_stream << RayParam::Natural_jac_dir << "/" << RayParam::Matbase_str;
    ...
    {
      std::ostringstream dump_stream;
      dump_stream << RayParam::Blocked_dir << "/" << RayParam::Matbase_str;
      std::string dump_str = dump_stream.str();

      //      my_prec.print_subblocks(dump_str);

      std::ostringstream dump_ss;
      dump_ss << RayParam::Natural_jac_dir << "/" << RayParam::Matbase_str;
  ...
  else
  {
    ...
    // Exact Biharmonic Preconditioner
    {
      ...
      oomph_info << "TESTING: Square 2D Biharmonic Problem w/ "
                 << "Exact Preconditioning" << std::endl;
      oomph_info << "//////////////////////////////////////////////////////////"
                    "///////////"
                 << std::endl;
    ...
    // Inexact Biharmonic Preconditioner w/ SuperLU
    {
      ...
      oomph_info << "TESTING: Curved 2D Biharmonic Problem w/ "
                 << "Inexact Preconditioning" << std::endl;
      oomph_info << "//////////////////////////////////////////////////////////"
                    "///////////"
                 << std::endl;
