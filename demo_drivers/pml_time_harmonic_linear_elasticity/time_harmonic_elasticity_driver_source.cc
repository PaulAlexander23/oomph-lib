...
namespace Global_Parameters
{
  ...
  void body_force(const Vector<double>& x, Vector<std::complex<double>>& b)
  {
    ...
    double source_fct =
      magnitude * exp(-alpha * sqrt(x[0] * x[0] + x[1] * x[1]));
...
ElasticAnnulusProblem<ELASTICITY_ELEMENT>::ElasticAnnulusProblem()
{
  ...
  // Build the mesh
  Solid_mesh_pt = new RectangularQuadMesh<ELASTICITY_ELEMENT>(
    n_bulk_x, n_bulk_y, x_bulk_start, x_bulk_end, y_bulk_start, y_bulk_end);
...
int main(int argc, char** argv)
{
  ...
  // Set up the problem
  ElasticAnnulusProblem<QPMLTimeHarmonicLinearElasticityElement<2, 3>> problem;
