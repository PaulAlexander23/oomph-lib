                                         ...
                                         ofstream& trace_file)
{
  ...
  trace_file
    << time_pt()->time() << " " << Trace_node_pt->x(1) << " "
    << Trace_node2_pt->x(0) << " " << rate_of_work << " " << global_pot << " "
    << global_kin << " " << global_kin + global_pot
    << " "
...
void ShellProblem<ELEMENT>::run_it()
{
  ...
  trace_file
    << "VARIABLES=\"time\",\"R_1\",\"R_2\",\"rate of work of "
       "load\",\"E_p_o_t\",\"E_k_i_n\",\"E_k_i_n + E_p_o_t\",\"p_c_o_s\""
    << std::endl;
