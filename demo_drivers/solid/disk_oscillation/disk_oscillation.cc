  ...
  : GeomObject(2, 2, time_stepper_pt), Ampl(ampl), Nu(nu)
{
  ...
  BlackBoxFDNewtonSolver::black_box_fd_newton_solve(
    residual_for_dispersion, param, omega);
...
void DiskOscillationProblem<ELEMENT>::doc_solution(DocInfo& doc_info)
{
  ...
  // Add to trace file
  Trace_file << time_pt()->time() << " " << exact_r << " ";
  ...
  //-----------------
  {
    ...
    for (unsigned e = 0; e < nelem; e++)
    {
      ...
      for (unsigned i = 0; i < npts; i++)
      {
        ...
        some_file << xi[0] << " " << x[0] - xi[0] << " " << dxdt[0]
                  << std::endl;

        some_file2 << xi[0] << " " << r_exact[0] - xi[0] << " " << v_exact[0]
                   << std::endl;
      }
    }
    some_file.close();
    ...
    for (unsigned e = 0; e < nelem; e++)
    {
      ...
      for (unsigned i = 0; i < npts; i++)
      {
        ...
        for (unsigned j = 0; j < npts; j++)
        {
          ...
          some_file << xi[0] << " " << xi[1] << " " << x[0] - xi[0] << " "
                    << x[1] - xi[1] << " " << dxdt[0] << " " << dxdt[1]
                    << std::endl;


          some_file2 << xi[0] << " " << xi[1] << " " << r_exact[0] - xi[0]
                     << " " << r_exact[1] - xi[1] << " " << v_exact[0] << " "
                     << v_exact[1] << std::endl;
        }
      }
    }
...
void DiskOscillationProblem<ELEMENT>::run(const unsigned& nstep)
{
  ...
  IC_geom_object_pt = new AxisymOscillatingDisk(
