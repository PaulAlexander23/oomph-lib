...
public:
    ...
    unsigned* direction_pt)
  {
    ...
    {
      // Add the node as external data in the SingularPoissonSolutionElement
      // class because they affect the slope that we set to zero to determine
      // the value of the amplitude C
  ...
  double singular_function(const Vector<double>& x) const
  {
    ...
    if (Singular_fct_pt == 0)
    {
      ...
      error_stream << "Pointer to singular function hasn't been defined!"
                   << std::endl;
      throw OomphLibError(
    ...
    const unsigned& flag)
  {
    ...
    if (!all_singular_functions_satisfy_laplace_equation)
    {
...

      {
        ...
        for (unsigned i = 0; i < n_sing; i++)
        {
          grad_singular_function_local[i] =
            grad_singular_function(i, interpolated_x);
        }
                    ...
                    if (local_equation_number_C[i] >= 0)
                    {
                      for (unsigned d = 0; d < cached_dim; d++)
                      {
                        jacobian(local_eqn, local_equation_number_C[i]) +=
                          grad_singular_function_local[i][d] * dtestdx(l, d) *
                          W;
                      }
                    }
                  }
      ...
      if (Node_is_subject_to_dirichlet_bc[j])
      {
        ...
        if (local_eqn_number_boundary_node >= 0)
        {
          ...
          if (flag)
          {
            ...
            for (unsigned i = 0; i < n_sing; i++)
            {
              // Find the contribution of the additional unknowns to the
              // jacobian
              int local_unknown = local_equation_number_C[i];
              if (local_unknown >= 0)
              {
                jacobian(local_eqn_number_boundary_node, local_unknown) +=
