...
int main(int argc, char** argv)
{
  ...
  problem.time_stepper_pt()->make_steady();

  // Timestep until the desired final time
  // problem.timestep(parameters.time_step, parameters.final_time);
  const unsigned n_eig = 8;
  Vector<std::complex<double>> eigenvalue =
    problem.solve_and_document_n_most_unstable_eigensolutions(n_eig);
  ofstream file(parameters.output_directory + "/eigenvalue.dat");

  for (unsigned n = 0; n < n_eig; n++)
  {
    file << eigenvalue[n].real() << " " << eigenvalue[n].imag() << std::endl;
  }
  file.close();
