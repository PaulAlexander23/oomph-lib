...
public:
  ...
  ElasticRefineableTwoLayerMesh(
    ...
    : RectangularQuadMesh<ELEMENT>(
        nx, ny1 + ny2, lx, h1 + h2, false, time_stepper_pt),
  ...
  const double& epsilon, const double& k)
{
  ...
  for (unsigned n = 0; n < n_node; n++)
  {
    ...
    CRBond_Bessel::bessjy01a(
      k * current_r_pos, j0, j1, y0, y1, j0p, j1p, y0p, y1p);

    // Determine new vertical position of node
    const double new_z_pos =
      current_z_pos + (1.0 - fabs(1.0 - current_z_pos)) * epsilon * j0;
