#ifndef HIJACK_REGION_AXISYM_SECTOR_PROBLEM_HEADER
#define HIJACK_REGION_AXISYM_SECTOR_PROBLEM_HEADER
...
namespace oomph
{
  ...
  class HijackRegionAxisymSectorProblem
  ...
  public:
      ...
      : RegionAxisymSectorProblem<ELEMENT>(), Contact_line_node_pt(0)
    {
      ...
      this->doc_info_pt()->set_directory("RESLT_hijack");
    ...
    void hijack_boundary()
    {
      // Loop over the inner boundary and hijack the equations

      const unsigned n_element =
        this->bulk_mesh_pt()->nboundary_element(Inner_boundary_id);
      for (unsigned n = 0; n < n_element; n++)
      {
        ELEMENT* bulk_elem_pt = dynamic_cast<ELEMENT*>(
          this->bulk_mesh_pt()->boundary_element_pt(Inner_boundary_id, n));
        bulk_elem_pt->hijack();
      }
    }
    ...
    void setup_and_augment_bulk_elements()
    {
      ...
      for (unsigned e = 0; e < n_bulk; e++)
      {
        ...
        {
          // ... add eigensolution forcing
          el_pt->axi_nst_body_force_fct_pt() = Eigensolution_forcing;
  ...
  void SingularRegionAxisymSectorProblem<ELEMENT>::setup_mesh_interaction()
  {
    ...
    for (unsigned e = 0; e < n_aug_bulk; e++)
    {
      ...
      // of the singular fct
      el_pt->add_external_data_pt(singular_el_pt);
    }
  }
