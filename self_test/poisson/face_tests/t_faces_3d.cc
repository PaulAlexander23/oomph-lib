...
public:
  /// Constructor
  TFaceTestProblem(const unsigned& h_power,
  ...
  : H_power(h_power)
{
  ...
  unsigned perm[4][4] = {
    {0, 1, 2, 3}, {3, 0, 2, 1}, {1, 3, 2, 0}, {2, 1, 3, 0}};
  ...
  if (single_element)
  {
    ...
    for (unsigned b = 0; b < 4; b++)
    {
      ...
      PoissonFluxElement<ELEMENT>* flux_element_pt =
        new PoissonFluxElement<ELEMENT>(local_elem_pt, b);
  ...
  else
  {
    ...
    Problem::mesh_pt() =
      new SimpleCubicTetMesh<ELEMENT>(nx, ny, nz, lx, ly, lz);
