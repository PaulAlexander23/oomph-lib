...
//========================================================================
template<class ELEMENT>
TriangleFaceTestProblem<ELEMENT>::TriangleFaceTestProblem(
  const unsigned& h_power, const bool& permute)
  : H_power(h_power)
{
  ...
  if (permute)
  {
    std::cout << "Permuted Nodes" << std::endl;
  }
  ...
  for (unsigned b = 0; b < 4; b++)
  {
    ...
    Vector<double> N(2);
    char filename[100];
    sprintf(filename, "%s_normals%i_%i.dat", filestem.c_str(), b, permute);
    ofstream output(filename);
    for (std::list<FaceElement*>::iterator it = faces_on_boundary[b].begin();
         it != faces_on_boundary[b].end();
         ++it)
    {
      for (unsigned i = 0; i < n_pts; i++)
      {
        ...
        output << b << " " << x[0] << " " << x[1] << " " << n[0] << " " << n[1]
               << "  " << N[0] << " " << N[1] << "\n";

        error +=
          sqrt((N[0] - n[0]) * (N[0] - n[0]) + (N[1] - n[1]) * (N[1] - n[1]));
      }
      // Delete the storage
      delete *it;
    }
