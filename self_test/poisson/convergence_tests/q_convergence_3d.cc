...
void EighthSpherePoissonProblem<ELEMENT>::doc_solution(
  DocInfo& doc_info, ostream& convergence_file)
{
  ofstream some_file;
  char filename[100];
  ...
  convergence_file << log_h << " " << log(sqrt(error)) << " ";
  ...
  switch (nnode_1d)
  {
    ...
    default:
      throw OomphLibError(
        "Never get here", OOMPH_CURRENT_FUNCTION, OOMPH_EXCEPTION_LOCATION);
  }
...
int main(int argc, char* argv[])
{
  ...
  {
    // Set up the problem
    EighthSpherePoissonProblem<RefineableQPoissonElement<3, 2>> problem(
      &TanhSolnForPoisson::get_source, h_power);
  ...
  {
    // Set up the problem
    EighthSpherePoissonProblem<RefineableQPoissonElement<3, 4>> problem(
      &TanhSolnForPoisson::get_source, h_power);
