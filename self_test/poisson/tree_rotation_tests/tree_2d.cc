...
public:
                 ...
                 GeneralisedElement* const& element2_pt)
  {
    ...
    if (cast_element1_pt == 0)
    {
      ...
      error_message << "Failed to cast element1_pt to an ELEMENT" << std::endl;
      throw OomphLibError(
    ...
    {
      std::ostringstream error_message;
      error_message << "Failed to cast element2_pt to an ELEMENT" << std::endl;
      throw OomphLibError(
        error_message.str(), OOMPH_CURRENT_FUNCTION, OOMPH_EXCEPTION_LOCATION);
    }
#endif
    ...
    // Check that the dimensions of bothelements are the same
    {
      ...
      if (dim != dim2)
      {
        ...
        warn_message << "Warning: Two elements do not have the same dimension"
    ...
    // to within a given tolerance
    {
      unsigned i = 0;
      double tol = 1.0e-14;
      while (i != dim && (std::abs(x1[i] - x2[i]) < tol))
      {
        ++i;
      }
...
public:
  ...
  /// the Mesh.
  TestRefineableRectangularQuadMesh(
  ...
  void sort_elements()
  {
    std::sort(this->element_pt().begin(),
              this->element_pt().end(),
...
//========================================================================
template<class ELEMENT>
TestRefineablePoissonProblem<ELEMENT>::TestRefineablePoissonProblem(
  const unsigned& angle, PoissonEquations<2>::PoissonSourceFctPt source_fct_pt)
  : Source_fct_pt(source_fct_pt)
{
  // Build and assign mesh
  Problem::mesh_pt() = new TestRefineableRectangularQuadMesh<ELEMENT>(angle);
...
void run_test(const unsigned& i)
{
  ...
  for (unsigned i = 0; i < 4; i++)
  {
    ...
    TestRefineablePoissonProblem<ELEMENT> problem(
      i, &TanhSolnForPoisson::get_source);
