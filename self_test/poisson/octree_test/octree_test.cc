...
namespace oomph
{
  ...
  // /// "rational" way to preserve the ordering given rotations of the
  // elements.
  // //===========================================================================
  ...
  //=============================================================
  template<class ELEMENT>
  class Rotateable : public ELEMENT
  {
  private:
  ...
  public:
    ...
    void further_build()
    {
      ...
      this->Rotated =
        dynamic_cast<Rotateable<ELEMENT>*>(this->father_element_pt())->Rotated;
    }
        ...
        case 2:
          for (unsigned i = 0; i < n_p; i++)
          {
            for (unsigned j = 0; j < n_p; j++)
            {
              ...
              {
                this->node_pt(i + j * n_p + k * n_p * n_p) =
                  elemental_nodes_pt[i + n_p * (n_p - 1 - j) +
                                     (n_p - 1 - k) * n_p * n_p];
              }
            }
          }
  ...
  //=================================================================
  template<class ELEMENT>
  class RotatableRefineableCubicMesh : public virtual SimpleCubicMesh<ELEMENT>,
  ...
  public:
      ...
      : SimpleCubicMesh<ELEMENT>(3, 3, 3, 1.0, 1.0, 1.0, time_stepper_pt)
    {
      ...
      if (rotated_element > this->nelement())
      {
        ...
        error_stream << "rotated_element>this->nelement(): " << rotated_element
                     << " " << this->nelement() << std::endl;
        throw OomphLibError(
  ...
  const unsigned& nrefine)
{
  ...
  if (nrefine > 2)
  {
    triply_refined_el_pt.insert(
      dynamic_cast<RefineableElement*>(mesh_pt()->element_pt(7)));
    triply_refined_el_pt.insert(
      dynamic_cast<RefineableElement*>(mesh_pt()->element_pt(8)));
    triply_refined_el_pt.insert(
      dynamic_cast<RefineableElement*>(mesh_pt()->element_pt(16)));
    triply_refined_el_pt.insert(
      dynamic_cast<RefineableElement*>(mesh_pt()->element_pt(17)));
  }
  ...
  if (nrefine > 1)
  {
    doubly_refined_el_pt.insert(
      dynamic_cast<RefineableElement*>(mesh_pt()->element_pt(0)));
    doubly_refined_el_pt.insert(
      dynamic_cast<RefineableElement*>(mesh_pt()->element_pt(1)));
    doubly_refined_el_pt.insert(
      dynamic_cast<RefineableElement*>(mesh_pt()->element_pt(3)));
    doubly_refined_el_pt.insert(
      dynamic_cast<RefineableElement*>(mesh_pt()->element_pt(4)));
  }


  // Next four block elements get singly refined
  set<RefineableElement*> singly_refined_el_pt;
  singly_refined_el_pt.insert(
    dynamic_cast<RefineableElement*>(mesh_pt()->element_pt(2)));
  singly_refined_el_pt.insert(
    dynamic_cast<RefineableElement*>(mesh_pt()->element_pt(5)));
  singly_refined_el_pt.insert(
    dynamic_cast<RefineableElement*>(mesh_pt()->element_pt(11)));
  singly_refined_el_pt.insert(
    dynamic_cast<RefineableElement*>(mesh_pt()->element_pt(14)));
  ...
  for (unsigned i = 0; i < nrefine; i++)
  {
    ...
    // Triple refinement
    for (IT it = triply_refined_el_pt.begin(); it != triply_refined_el_pt.end();
         ++it)
    {
    ...
    if (i == 0)
    {
      ...
      // Add central element
      ref_el_pt.push_back(
        dynamic_cast<RefineableElement*>(mesh_pt()->element_pt(13)));
    }
    ...
    for (unsigned ii = 0; ii < nr; ii++)
    {
      ...
      {
        triply_refined_el_pt.insert(
          aux_el_pt[ii]->octree_pt()->son_pt(s)->object_pt());
      }
    ...
    if (i < 2)
    {
      ...
      doubly_refined_el_pt.insert(
        dynamic_cast<RefineableQElement<3>*>(ref_el_pt[n_total - 1])
          ->octree_pt()
          ->son_pt(7)
          ->object_pt());
    }
...
void TestPoissonProblem<ELEMENT>::doc_solution(DocInfo& doc_info)
{
  ...
  if ((CommandLineArgs::Argc == 1) || (doc_info.number() == 0))
  {
    ...
    {
      RefineableQElement<3>* el_pt = dynamic_cast<RefineableQElement<3>*>(
      ...
      some_file << x_centre[0] << " " << x_centre[1] << " " << x_centre[2]
                << " " << x_end[0] - x_centre[0] << " "
                << x_end[1] - x_centre[1] << " " << x_end[2] - x_centre[2]
                << " " << std::endl;
...
int main(int argc, char* argv[])
{
  ...
  for (unsigned r = 1; r < 24; r++)
  {
    ...
    {
      std::cout << "Case: r, rotated_element" << r << " " << rotated_element
                << std::endl;

      // Create the problem with rotation r for element rotated_element
      TestPoissonProblem<Rotateable<RefineableQPoissonElement<3, 2>>> problem(
        r, rotated_element, &TanhSolnForPoisson::get_source);
