...
public:
  ...
  ThreeDPoissonProblem(PoissonEquations<3>::PoissonSourceFctPt source_fct_pt);
...

{
  ...
  Problem::mesh_pt() =
    new RefineableSimpleCubicMesh<ELEMENT>(2, 2, 2, 5.0, 5.0, 5.0);
                   ...
                   const bool& doc)
{
  oomph_info << std::endl
             ...
             << " subsequent distributed refinements" << std::endl
             << std::endl;
  ...
  if (my_rank == 0)
  {
    ...
    sprintf(filename,
            "%s/"
            "three_d_initial_refinement_nrefinement_%i_ndofbefore_%i_ndofafter_"
            "%i.dat",
            doc_info.directory().c_str(),
            n_refine_first,
            n_dof_orig,
            n_dof);
    some_file.open(filename);
    some_file << n_refine_first << " " << n_dof_orig << " " << n_dof << " "
              << t_tot << " " << std::endl;
    some_file.close();
  }
  ...
  //---------------------------------------
  problem_pt->mesh_pt()->get_halo_node_stats(
    av_number_halo_nodes, max_number_halo_nodes, min_number_halo_nodes);
  ...
  if (my_rank == 0)
  {
    ...
    sprintf(filename,
            ...
            n_dof);
    some_file.open(filename);
    some_file << n_proc << " " << n_dof << " " << t_tot << " "
              << av_number_halo_nodes << " " << max_number_halo_nodes << " "
              << min_number_halo_nodes << " " << av_efficiency << " "
              << max_efficiency << " " << min_efficiency << " " << std::endl;
    some_file.close();
  }
  ...
  for (unsigned i = 0; i < n_refine_once_distributed; i++)
  {
    ...
    if (my_rank == 0)
    {
      ...
      oomph_info << "Time for distributed adaptation [ndof="
                 << problem_pt->ndof() << "]: " << t_tot << std::endl;
      ...
      sprintf(filename,
              "%s/"
              "three_d_distributed_refinement_np_%i_ninitialrefinement_%i_"
              "ntotalrefinement_%i_ndof_%i_with%sredistribution.dat",
              doc_info.directory().c_str(),
              n_proc,
              n_refine_first,
              n_refine_first + i + 1,
              ...
              snippet.c_str());
      some_file.open(filename);
      some_file << n_proc << " " << n_dof_orig << " " << n_dof << " " << t_tot
                << " " << av_number_halo_nodes << " " << max_number_halo_nodes
                << " " << min_number_halo_nodes << " " << av_efficiency << " "
                << max_efficiency << " " << min_efficiency << " " << std::endl;
      some_file.close();
    ...
    if (redistribute)
    {
      ...
      problem_pt->prune_halo_elements_and_nodes(doc_info, report_stats);
      t_end = clock();
      double t_tot = double(t_end - t_start) / CLOCKS_PER_SEC;
      ...
      if (my_rank == 0)
      {
        oomph_info << "Av., min. max. efficiency before redistribution "
                   << av_efficiency << " " << min_efficiency << " "
                   << max_efficiency << " " << std::endl;
        sprintf(filename,
                "%s/"
                "three_d_redistribution_np_%i_ninitialrefinement_%i_"
                "ntotalrefinement_%i_ndof_%i.dat",
                doc_info.directory().c_str(),
                n_proc,
                n_refine_first,
                n_refine_first + i + 1,
      ...
      if (my_rank == 0)
      {
        oomph_info << "Time for redistribution  [ndof=" << problem_pt->ndof()
                   << "]: " << t_tot << std::endl;
  ...
  if (!redistribute)
  {
    ...
    if (my_rank == 0)
    {
      ...
      sprintf(filename,
              "%s/"
              "three_d_distributed_unrefinement_np_%i_ninitialrefinement_%i_"
              "ntotalrefinement_%i_ndof_%i.dat",
              doc_info.directory().c_str(),
              n_proc,
              n_refine_first,
...
int main(int argc, char** argv)
{
  ...
  oomph_info
    << "Run with  n_refine_first_min/max, n_uniform_min/max, redistribute: "
    << n_refine_first_min << " " << n_refine_first_max << " " << n_uniform_min
    << " " << n_uniform_max << " " << redistribute << " " << std::endl;
  ...
  some_file << "VARIABLES= \"n<SUB>refine first</SUB>\","
            << "\"n<SUB>dof orig</SUB>\","
            << "\"n<SUB>dof</SUB>\","
            << "\"t<SUB>serial adapt</SUB>\"" << std::endl;
  some_file.close();
