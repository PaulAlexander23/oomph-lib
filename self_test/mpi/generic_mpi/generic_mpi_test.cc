...
int main(int argc, char* argv[])
{
  ...
  // Tell us who you are...
  oomph_info << "I'm rank " << my_rank << " on a total of " << nproc
             << " processors" << std::endl;
  ...
  LinearAlgebraDistribution distributed_distribution(comm_pt, nrow_global);
  ...
  oomph_info << "distributed distribution: first_row and nrow_local: "
             ...
             << distributed_distribution.nrow_local() << std::endl;
  ...
  oomph_info << "locally replicated distribution: first_row and nrow_local: "
             ...
             << locally_replicated_distribution.nrow_local() << std::endl;
  ...
  for (unsigned i = 0; i < nrow_local; i++)
  {
    oomph_info << "local row " << i << " is global row " << first_row + i
  ...
  CRDoubleMatrix my_matrix2(
    &distributed_distribution, ncol, values, column_indices, row_start);
  ...
  oomph_info
    ...
    << my_vector.first_row() << " " << my_vector.nrow_local() << std::endl
    << "Uniformly distributed jacobian matrix: first_row, nrow_local and nnz: "
    << my_matrix.first_row() << " " << my_matrix.nrow_local() << " "
    << my_matrix.nnz() << " " << std::endl;
  ...
  oomph_info
    ...
    << locally_replicated_distribution_for_jac.nrow_local() << std::endl;
  ...
  oomph_info << "Replicated residual vector: first_row and nrow_local: "
             << my_vector.first_row() << " " << my_vector.nrow_local()
             << std::endl
