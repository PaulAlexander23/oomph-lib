...
void check_locate_zeta(Mesh* mesh_pt)
{
  ...
  // Check elemental locate zeta:
  {
    ...
    el_pt->get_centre_of_gravity_and_max_radius_in_terms_of_zeta(cog,
                                                                 max_radius);
    ...
    if (n_dim == 2)
    {
      ...
      {
        double phi =
          2.0 * MathematicalConstants::Pi * double(i) / double(nplot - 1);
    ...
    else if (n_dim == 3)
    {
      ...
      for (unsigned i = 0; i < nplot; i++)
      {
        ...
        {
          double theta =
            MathematicalConstants::Pi * (-0.5 + double(j) / double(nplot - 1));
          outfile << cog[0] + max_radius * cos(phi) * cos(theta) << " "
                  << cog[1] + max_radius * sin(phi) * cos(theta) << " "
                  << cog[2] + max_radius * sin(theta) << " " << std::endl;
        }
      }
      outfile.close();
    }
...
#endif
  for (unsigned ref_flag = 0; ref_flag < n_sample_point_container_type;
       ref_flag++)
  {
    ...
    //-------------------------------------
    {
      ...
      for (unsigned e = 0; e < nelem; e++)
      {
        ...
        for (unsigned j = 0; j < nint; j++)
        {
          ...
          if (geom_obj_pt == 0)
          {
            oomph_info << "Search for zeta = ( " << zeta[0] << " " << zeta[0]
                       << " "
                       << " failed " << std::endl;
            abort();
          }
      ...
      oomph_info << "done/tested " << count
                 << " located zetas. Max. error = " << max_error << std::endl
                 << "Total time for locate zeta: " << t_locate_zeta << " sec "
                 << std::endl;
    }
    ...
    //----------------------------------------------------------
    {
      ...
      {
        geom_obj_pt->interpolated_zeta(s_from_locate_zeta, zeta_test);
        oomph_info << "find: " << zeta_test[0] << " " << zeta_test[1] << " "
                   << std::endl;
      ...
      {
        oomph_info << "not find: " << zeta_test[0] << " " << zeta_test[1] << " "
                   << std::endl;
    ...
    //---------------------------------------------------------------------
    {
      oomph_info << "\n\n\nEnforced failure for one zeta (spiraling):"
                 << std::endl;
      oomph_info << "------------------------------------------\n\n"
                 << std::endl;
      ...
      unsigned nsample_points_in_bin_array =
        mesh_geom_obj_pt->sample_point_container_pt()
          ->total_number_of_sample_points_computed_recursively();
          ...
          UseRefineableBinArray)
      {
        RefineableBinArray* sample_point_container_pt =
          dynamic_cast<RefineableBinArray*>(
            mesh_geom_obj_pt->sample_point_container_pt());
      ...
      while ((geom_obj_pt == 0) && (has_not_reached_max_level_of_search))
      {
        ...
        filename << "_spiral_level" + StringConversion::to_string(i_level) +
                      ".dat";
        BinArray::Visited_sample_points_file.open(filename.str().c_str());
        Vector<double> s_from_locate_zeta(n_dim);
        Vector<double> zeta_test(n_dim, 0.5);
                 ...
                 UseNonRefineableBinArray)
        {
          ...
          sample_point_container_pt->current_min_spiral_level() +=
            sample_point_container_pt->n_spiral_chunk();
          sample_point_container_pt->current_max_spiral_level() +=
            sample_point_container_pt->n_spiral_chunk();
        }
#ifdef OOMPH_HAS_CGAL
                 ...
                 UseNonRefineableBinArray)
        {
          ...
          if (sample_point_container_pt->current_max_spiral_level() <
              n_max_level)
          {
            has_not_reached_max_level_of_search = true;
          }
      ...
      if (geom_obj_pt != 0)
      {
        oomph_info << "find: " << zeta[0] << " " << zeta[1] << " " << std::endl;
      }
      else
      {
        oomph_info << "not find: " << zeta[0] << " " << zeta[1] << " "
                   << std::endl;
      }
