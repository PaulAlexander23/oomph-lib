...
#ifndef MY_NAVIER_STOKES_ELEMENTS_WITH_SINGULARITY_HEADER
#define MY_NAVIER_STOKES_ELEMENTS_WITH_SINGULARITY_HEADER
...
namespace oomph
{
  ...
  public:
    /// Constructor
    NavierStokesElementWithSingularity() : IsAugmented(false)
    {
  ...
  protected:
    bool is_node_a_pressure_node(const unsigned& n) const
    {
      bool value = false;
  ...
  public:
    ...
    /// is stored.
    virtual inline int p_index_nst() const
    {
      // dim * [u]
      return this->dim();
    }
  ...
  public:
    void pin_extra_velocity(const unsigned& i_node,
                            const unsigned& i_dim,
                            const double& value)
    {
      this->node_pt(i_node)->pin(this->u_reconstructed_index(i_node, i_dim));
      this->node_pt(i_node)->set_value(
        this->u_reconstructed_index(i_node, i_dim), value);
    }

    void pin_velocity(const unsigned& i_node,
                      const unsigned& i_dim,
                      const double& value)
    {
      this->node_pt(i_node)->pin(this->u_index_nst(i_node, i_dim));
    ...
    void output(std::ostream& outfile, const unsigned& nplot)
    {
      ...
      for (unsigned iplot = 0; iplot < num_plot_points; iplot++)
      {
        ...
        // Total velocity
        Vector<double> velocity = interpolated_u_nst_new(s);
        for (unsigned i = 0; i < cached_dim; i++)
        {
          outfile << velocity[i] << " ";
        }
        ...
        // Singular Velocity
        Vector<double> velocity_bar = u_bar(x);
        for (unsigned i = 0; i < cached_dim; i++)
        {
        ...
        DenseMatrix<double> stress_tensor_bar =
          interpolated_stress_tensor_bar(s);
        for (unsigned i = 0; i < cached_dim; i++)
        {
          for (unsigned j = 0; j < cached_dim; j++)
          {
    ...
    inline Vector<double> interpolated_u_nst_sum(const Vector<double>& s) const
    {
      ...
      for (unsigned d = 0; d < cached_dim; d++)
      {
        ...
        {
          interpolated_u[d] +=
            this->nodal_value(j, this->u_index_nst(j, d)) * psif[j];
    ...
    Vector<double> interpolated_u_nst_new(const Vector<double>& s)
    {
      ...
      for (unsigned d = 0; d < cached_dim; d++)
      {
        ...
        {
          interpolated_u[d] +=
            this->raw_nodal_value(j, u_reconstructed_index(j, d)) * psif[j];
        }
      }
      return interpolated_u;
    }
        ...
        for (unsigned i = 0; i < cached_dim; i++)
        {
          for (unsigned j = 0; j < cached_dim; j++)
          {
            const double u_value =
              this->nodal_value(l, this->u_index_nst(l, i));
  ...
  private:
      ...
      const unsigned& flag)
    {
          ...
          (!all_singular_functions_satisfy_stokes_equation))
      {
        ...
        for (unsigned ipt = 0; ipt < n_intpt; ipt++)
        {
          ...
          for (unsigned l = 0; l < n_node; l++)
          {
            ...
            for (unsigned i = 0; i < cached_dim; i++)
            {
              double u_value = this->nodal_value(l, this->u_index_nst(l, i));
              interpolated_u_tilde[i] += u_value * psif[l];
              interpolated_u[i] +=
                this->raw_nodal_value(l, u_reconstructed_index(l, i)) * psif[l];
          ...
          for (unsigned l = 0; l < n_node; l++)
          {
            ...
            for (unsigned i = 0; i < cached_dim; i++)
            {
              ...
              local_eqn = this->nodal_momentum_local_eqn(l, i);
                    ...
                    if (flag)
                    {
                      ...
                      for (unsigned l2 = 0; l2 < n_node; l2++)
                      {
                        ...
                        for (unsigned i2 = 0; i2 < cached_dim; i2++)
                        {
                          ...
                          // entry
                          local_unknown =
                            this->nodal_momentum_local_eqn(l2, i2);
                          if (local_unknown >= 0)
                          {
                            double sum = 0.0;
                            if (i == i2)
              ...
              // Find its local equation number
              local_eqn = this->nodal_velocity_local_eqn(l, i);
              ...
              if (local_eqn >= 0)
              {
                ...
                if (flag)
                {
                  ...
                  for (unsigned l2 = 0; l2 < n_node; l2++)
                  {
                    local_unknown = this->nodal_momentum_local_eqn(l2, i);
                    if (local_unknown >= 0)
                    {
                      jacobian(local_eqn, local_unknown) -=
                        psif(l2) * testf[l] * W;
                  ...
                  for (unsigned l2 = 0; l2 < n_node; l2++)
                  {
                    local_unknown = nodal_velocity_local_eqn(l2, i);
                    if (local_unknown >= 0)
                    {
                      jacobian(local_eqn, local_unknown) +=
                        psif(l2) * testf[l] * W;
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        {
          // Find its local equation number
          local_eqn = this->nodal_momentum_local_eqn(l, d);
          // local_eqn = this->nodal_velocity_local_eqn(l, d);
          ...
          if (local_eqn >= 0)
          {
            ...
            if (Node_is_subject_to_velocity_dirichlet_bcs[l][d])
            {
              ...
              residuals[local_eqn] +=
                this->raw_nodal_value(l, this->u_index_nst(l, d));
    ...
    virtual inline int nodal_velocity_local_eqn(const unsigned& n,
                                                const unsigned& i) const
    {
    ...
    virtual inline int nodal_momentum_local_eqn(const unsigned& n,
                                                const unsigned& i) const
    {
  ...
  class SingularNavierStokesTractionElement
    : public NavierStokesTractionElement<ELEMENT>
  {
  public:
    SingularNavierStokesTractionElement(
  ...
  //=======================================================================
  template<class ELEMENT>
  class FaceGeometry<NavierStokesElementWithSingularity<ELEMENT>>
  ...
  //=======================================================================
  template<class ELEMENT>
  class FaceGeometry<FaceGeometry<NavierStokesElementWithSingularity<ELEMENT>>>
