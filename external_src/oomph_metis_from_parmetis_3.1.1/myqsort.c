...
static void siqst(idxtype* base, idxtype* max)
{
  ...
  do
  {
    ...
    {
      if (lo >= THRESH) siqst(base, j);
      base = i;
      lo = hi;
    }
    else
    {
      if (hi >= THRESH) siqst(i, max);
      max = j;
    }
  } while (lo >= THRESH);
}
...
static void iiqst(int* base, int* max)
{
  ...
  do
  {
    ...
    if ((lo = j - base) <= (hi = max - i))
    {
      if (lo >= THRESH) iiqst(base, j);
      base = i;
      lo = hi;
    }
    else
    {
      if (hi >= THRESH) iiqst(i, max);
      max = j;
    }
  } while (lo >= THRESH);
}
...
void ikeysort(int n, KeyValueType* base)
{
  ...
  /* Sanity check */
  {
    int i;
    for (i = 0; i < n - 1; i++)
      if (base[i].key > base[i + 1].key) printf("Something went wrong!\n");
  }
}
...
static void keyiqst(KeyValueType* base, KeyValueType* max)
{
  ...
  do
  {
    ...
    {
      if (lo >= THRESH) keyiqst(base, j);
      base = i;
      lo = hi;
    }
    else
    {
      if (hi >= THRESH) keyiqst(i, max);
      max = j;
    }
  } while (lo >= THRESH);
}
...
void ikeyvalsort(int n, KeyValueType* base)
{
  ...
  for (j = lo = base; lo++ < hi;)
  {
    if ((j->key > lo->key) || (j->key == lo->key && j->val > lo->val)) j = lo;
  }
  if (j != base)
  { /* swap j into place */
    c = *base;
...
static void keyvaliqst(KeyValueType* base, KeyValueType* max)
{
  ...
  do
  {
    ...
    {
      j = (base->key > mid->key ||
               (base->key == mid->key && base->val > mid->val) ?
             base :
             mid);
      tmp = max - 1;
    ...
    {
      while (i < mid &&
             (i->key < mid->key || (i->key == mid->key && i->val <= mid->val)))
        i++;
      while (j > mid)
      {
    ...
    if ((lo = (j - base) >> 1) <= (hi = (max - i) >> 1))
    {
      if (lo >= THRESH) keyvaliqst(base, j);
      base = i;
      lo = hi;
    }
    else
    {
      if (hi >= THRESH) keyvaliqst(i, max);
      max = j;
    }
  } while (lo >= THRESH);
}
