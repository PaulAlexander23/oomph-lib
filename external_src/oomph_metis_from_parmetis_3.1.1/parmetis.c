                          ...
                          idxtype* part)
{
  ...
  for (i = 0; i < *nparts; i++) tpwgts[i] = 1.0 / (1.0 * (*nparts));

  METIS_WPartGraphKway2(nvtxs,
                        xadj,
                        adjncy,
                        vwgt,
                        adjwgt,
                           ...
                           idxtype* part)
{
  ...
  SetUpGraph(
    &graph, OP_KMETIS, *nvtxs, 1, xadj, adjncy, vwgt, adjwgt, *wgtflag);
  ...
  ctrl.maxvwgt = 1.5 * ((graph.vwgt ? idxsum(*nvtxs, graph.vwgt) : (*nvtxs)) /
                        ctrl.CoarsenTo);
  ...
  *edgecut =
    MlevelKWayPartitioning(&ctrl, &graph, *nparts, part, tpwgts, 1.000);
                             ...
                             idxtype* sizes)
{
  ...
  if (cpos >= npes - 1) ubfactor = ORDER_UNBALANCE_FRACTION;
  else
    ubfactor = 1.05;
  ...
  if (rgraph.nvtxs > MMDSWITCH || 2 * cpos + 1 < npes - 1)
    MlevelNestedDissectionP(
      ctrl, &rgraph, order, lastvtx, npes, 2 * cpos + 1, sizes);
  else
  {
  ...
  if (lgraph.nvtxs > MMDSWITCH || 2 * cpos + 2 < npes - 1)
    MlevelNestedDissectionP(
      ctrl, &lgraph, order, lastvtx - rgraph.nvtxs, npes, 2 * cpos + 2, sizes);
  else
  {
                                 ...
                                 idxtype* part)
{
  ...
  *edgecut = MCMlevelRecursiveBisection2(
    &ctrl, &graph, *nparts, mytpwgts, part, 1.000, 0);
  ...
  wgt[minwgt*(*ncon)+j], wgt[maxwgt*(*ncon)+j], avgwgt,
  (float)wgt[maxwgt*(*ncon)+j] / avgwgt);
  }
  printf("\n");
  }
  */
                                ...
                                int fpart)
{
  ...
  if (nparts > 3)
  {
    cut += MCMlevelRecursiveBisection2(
      ctrl, &lgraph, nparts / 2, tpwgts, part, ubfactor, fpart);
    cut += MCMlevelRecursiveBisection2(ctrl,
                                       &rgraph,
                                       nparts - nparts / 2,
                                       tpwgts + nparts / 2,
                                       part,
