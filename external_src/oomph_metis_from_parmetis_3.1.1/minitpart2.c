...
void MocInit2WayBalance2(CtrlType* ctrl,
                         ...
                         float* ubvec)
{
  int i, ii, j, k, l, kwgt, nvtxs, nbnd, ncon, nswaps, from, to, pass, me, cnum,
    tmp, imin;
  ...
  for (ii = 0; ii < nvtxs; ii++)
  {
    ...
    if (where[i] == from)
    {
      if (ed[i] > 0) PQueueInsert(&parts[qnum[i]][0], i, ed[i] - id[i]);
      else
  ...
  */
  ...
  imin = 0;
  for (i = 1; i < ncon; i++) imin = (ubvec[i] < ubvec[imin] ? i : imin);
  minwgt = .5 / ubvec[imin];
  ...
  for (nswaps = 0; nswaps < nvtxs; nswaps++)
  {
    ...
    if (npwgts[to * ncon + imin] > minwgt) break;

    if ((cnum = SelectQueueOneWay2(ncon, npwgts + to * ncon, parts, ubvec)) ==
        -1)
      break;
    ...
    {
      printf("Moved %6d from %d(%d). [%5d] %5d, NPwgts: ",
             higain,
             from,
             cnum,
             ed[higain] - id[higain],
    ...
    for (j = xadj[higain]; j < xadj[higain + 1]; j++)
    {
      ...
      if (moved[k] == -1 && where[k] == from)
      {
        ...
        { /* It must be in the boundary already */
          if (bndptr[k] == -1) printf("What you thought was wrong!\n");
    ...
    ASSERTP(ComputeCut(graph, where) == mincut,
                       ...
                       float* ubvec)
{
  ...
  {
    if (max < twgt[i] &&
        (PQueueGetSize(&queues[i][0]) > 0 || PQueueGetSize(&queues[i][1]) > 0))
    {
      max = twgt[i];
      cnum = i;
    }
  }
  if (max > 1) return cnum;

  /* optimize of cut */
  maxgain = -10000000;
  for (i = 0; i < ncon; i++)
  {
    if (PQueueGetSize(&queues[i][0]) > 0 &&
        PQueueGetKey(&queues[i][0]) > maxgain)
    {
