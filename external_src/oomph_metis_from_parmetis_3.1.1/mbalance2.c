...
void MocGeneral2WayBalance2(CtrlType* ctrl,
                            ...
                            float* ubvec)
{
  int i, ii, j, k, l, kwgt, nvtxs, ncon, nbnd, nswaps, from, to, pass, me,
    limit, tmp, cnum;
  ...
  for (i = 0; i < ncon; i++) minbal[i] = origbal[i];
  ...
  if (ctrl->dbglvl & DBG_REFINE)
  {
    ...
    printf("] T[%.3f %.3f], Nv-Nb[%5d, %5d]. ICut: %6d, LB: ",
           tpwgts[0],
           tpwgts[1],
           graph->nvtxs,
           graph->nbnd,
           graph->mincut);
    for (i = 0; i < ncon; i++) printf("%.3f ", origbal[i]);
    printf("[B]\n");
  }
  ...
  {
    if (AreAllBelow(ncon, minbal, ubvec)) break;

    SelectQueue3(ncon, npwgts, tpwgts, &from, &cnum, parts, maxwgt);
    to = (from + 1) % 2;

    if (from == -1 || (higain = PQueueGetMax(&parts[cnum][from])) == -1) break;
    ...
    {
      mincut = newcut;
      for (i = 0; i < ncon; i++) minbal[i] = newbal[i];
      mincutorder = nswaps;
    }
    ...
    {
      printf("Moved %6d from %d(%d). Gain: %5d, Cut: %5d, NPwgts: ",
             higain,
             from,
             cnum,
             ed[higain] - id[higain],
                  ...
                  float* maxwgt)
{
  ...
  *cnum = -1;
  ...
  j = *from;
  for (i=0; i<ncon; i++)
    printf("[%5d %5d %.4f %.4f] ", i, PQueueGetSize(&queues[i][j]),
  npwgts[j*ncon+i], maxwgt[j*ncon+i]); printf("***[%5d %5d]\n", *cnum, *from);
  */
  ...
  if (*from == -1)
  {
    ...
    for (j = 0; j < 2; j++)
    {
      for (i = 0; i < ncon; i++)
      {
        if (PQueueGetSize(&queues[i][j]) > 0 &&
            PQueueGetKey(&queues[i][j]) > maxgain)
        {
