                          ...
                          int npasses)
{
  ...
  { /* Do a number of passes */
    ...
    for (i = 0; i < ncon; i++) mindiff[i] = fabs(tpwgts[0] - npwgts[i]);
                 ...
                 PQueueType queues[MAXNCON][2])
{
  ...
  *cnum = -1;
   ...
   * PQueueGetSize(&queues[*cnum][*from])); */
  ...
  if (maxdiff <= 0.0 || *from == -1)
  {
    ...
    for (part = 0; part < 2; part++)
    {
      for (i = 0; i < ncon; i++)
      {
        if (PQueueGetSize(&queues[i][part]) > 0 &&
            PQueueGetKey(&queues[i][part]) > maxgain)
        {
...
int BetterBalance(int ncon, float* npwgts, float* tpwgts, float* diff)
{
  ...
  for (i = 0; i < ncon; i++) ndiff[i] = fabs(tpwgts[0] - npwgts[i]);
