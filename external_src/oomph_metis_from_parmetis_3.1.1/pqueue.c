...
void PQueueInit(CtrlType* ctrl, PQueueType* queue, int maxnodes, int maxgain)
{
  ...
  if (maxgain > PLUS_GAINSPAN || maxnodes < 500) queue->type = 2;
  else
    queue->type = 1;
  ...
  if (queue->type == 1)
  {
    ...
    ncore = 2 + (sizeof(ListNodeType) / sizeof(idxtype)) * maxnodes +
            (sizeof(ListNodeType*) / sizeof(idxtype)) * j;

    if (WspaceAvail(ctrl) > ncore)
    {
      queue->nodes = (ListNodeType*)idxwspacemalloc(
        ctrl, (sizeof(ListNodeType) / sizeof(idxtype)) * maxnodes);
      queue->buckets = (ListNodeType**)idxwspacemalloc(
        ctrl, (sizeof(ListNodeType*) / sizeof(idxtype)) * j);
      queue->mustfree = 0;
    }
    else
    { /* Not enough memory in the wspace, allocate it */
      queue->nodes = (ListNodeType*)idxmalloc(
        (sizeof(ListNodeType) / sizeof(idxtype)) * maxnodes,
        "PQueueInit: queue->nodes");
      queue->buckets =
        (ListNodeType**)idxmalloc((sizeof(ListNodeType*) / sizeof(idxtype)) * j,
                                  "PQueueInit: queue->buckets");
      queue->mustfree = 1;
    }

    for (i = 0; i < maxnodes; i++) queue->nodes[i].id = i;

    for (i = 0; i < j; i++) queue->buckets[i] = NULL;

    queue->buckets +=
      queue->ngainspan; /* Advance buckets by the ngainspan proper indexing */
    queue->maxgain = -queue->ngainspan;
  }
  else
  {
    queue->heap = (KeyValueType*)idxwspacemalloc(
      ctrl, (sizeof(KeyValueType) / sizeof(idxtype)) * maxnodes);
...
void PQueueFree(CtrlType* ctrl, PQueueType* queue)
{
  if (queue->type == 1)
  {
    ...
    else
    {
      idxwspacefree(ctrl,
                    sizeof(ListNodeType*) *
                      (queue->ngainspan + queue->pgainspan + 1) /
                      sizeof(idxtype));
      idxwspacefree(ctrl,
                    sizeof(ListNodeType) * queue->maxnodes / sizeof(idxtype));
    }
  }
  else
  {
    idxwspacefree(ctrl,
                  sizeof(KeyValueType) * queue->maxnodes / sizeof(idxtype));
...
int PQueueInsert(PQueueType* queue, int node, int gain)
{
  ...
  if (queue->type == 1)
  {
    ...
    if (queue->maxgain < gain) queue->maxgain = gain;
  }
  else
  {
...
int PQueueDelete(PQueueType* queue, int node, int gain)
{
  ...
  if (queue->type == 1)
  {
    ...
    if (newnode->prev != NULL) newnode->prev->next = newnode->next;
    else
      buckets[gain] = newnode->next;
    if (newnode->next != NULL) newnode->next->prev = newnode->prev;

    if (buckets[gain] == NULL && gain == queue->maxgain)
    {
      if (queue->nnodes == 0) queue->maxgain = -queue->ngainspan;
      else
...
void PQueueUpdateUp(PQueueType* queue, int node, int oldgain, int newgain)
{
  ...
  if (queue->type == 1)
  {
    ...
    if (queue->maxgain < newgain) queue->maxgain = newgain;
  }
  else
  { /* Heap Priority Queue */
    heap = queue->heap;
    ...
    /* Here we are just filtering up since the newgain is greater than the
     * oldgain */
    i = locator[node];
    while (i > 0)
    {
...
int PQueueSeeMax(PQueueType* queue)
{
  ...
  if (queue->type == 1) vtx = queue->buckets[queue->maxgain]->id;
  else
...
int PQueueGetKey(PQueueType* queue)
{
  ...
  if (queue->type == 1) key = queue->maxgain;
  else
...
int CheckHeap(PQueueType* queue)
{
  ...
  for (i = 1; i < nnodes; i++)
  {
    ASSERTP(locator[heap[i].val] == i,
            ("%d %d %d %d\n", nnodes, i, heap[i].val, locator[heap[i].val]));
    ASSERTP(heap[i].key <= heap[(i - 1) / 2].key,
            ("%d %d %d %d %d\n",
             i,
             (i - 1) / 2,
             nnodes,
  ...
  for (i = 1; i < nnodes; i++) ASSERT(heap[i].key <= heap[0].key);
