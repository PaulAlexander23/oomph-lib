                              ...
                              idxtype* part)
{
  ...
  METIS_WPartGraphRecursive(nvtxs,
                            xadj,
                            adjncy,
                            vwgt,
                            adjwgt,
                               ...
                               idxtype* part)
{
  ...
  SetUpGraph(
    &graph, OP_PMETIS, *nvtxs, 1, xadj, adjncy, vwgt, adjwgt, *wgtflag);
  ...
  for (i = 0; i < *nparts; i++) mytpwgts[i] = tpwgts[i];
  ...
  *edgecut =
    MlevelRecursiveBisection(&ctrl, &graph, *nparts, part, mytpwgts, 1.000, 0);
                             ...
                             int fpart)
{
  ...
  */
  ...
  if (nparts > 3)
  {
    cut += MlevelRecursiveBisection(
      ctrl, &lgraph, nparts / 2, part, tpwgts, ubfactor, fpart);
    cut += MlevelRecursiveBisection(ctrl,
                                    &rgraph,
                                    nparts - nparts / 2,
                                    part,
                                    tpwgts + nparts / 2,
...
void SplitGraphPart(CtrlType* ctrl,
                    ...
                    GraphType* rgraph)
{
  int i, j, k, kk, l, istart, iend, mypart, nvtxs, ncon, snvtxs[2], snedges[2],
    sum;
  ...
  for (i = 0; i < nvtxs; i++)
  {
    ...
    if (ncon == 1) svwgt[mypart][snvtxs[mypart]] = vwgt[i];
    else
    {
      for (kk = 0; kk < ncon; kk++)
        snvwgt[mypart][snvtxs[mypart] * ncon + kk] =
          nvwgt[i * ncon + kk] / npwgts[mypart * ncon + kk];
    }
                     ...
                     int snedges)
{
  ...
  {
    sgraph->gdata =
      idxmalloc(4 * snvtxs + 1 + 2 * snedges, "SetUpSplitGraph: gdata");
  ...
  {
    sgraph->gdata =
      idxmalloc(3 * snvtxs + 1 + 2 * snedges, "SetUpSplitGraph: gdata");
