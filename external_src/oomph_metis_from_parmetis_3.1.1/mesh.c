                      ...
                      idxtype* dadjncy)
{
  ...
  if (*numflag == 1) ChangeMesh2CNumbering((*ne) * esizes[*etype], elmnts);
                  ...
                  idxtype* dadjncy)
{
  ...
  int esize, esizes[] = {-1, 3, 4, 8, 4}, mgcnum, mgcnums[] = {-1, 2, 3, 4, 2};
  ...
  for (j = esize * nelmnts, i = 0; i < j; i++) nptr[elmnts[i]]++;
  MAKECSR(i, nvtxs, nptr);

  nind = idxmalloc(nptr[nvtxs], "GENDUALMETIS: nind");
  for (k = i = 0; i < nelmnts; i++)
  {
    for (j = 0; j < esize; j++, k++) nind[nptr[elmnts[k]]++] = i;
  }
  for (i = nvtxs; i > 0; i--) nptr[i] = nptr[i - 1];
  nptr[0] = 0;

  for (i = 0; i < nelmnts; i++) dxadj[i] = esize * i;
  ...
  {
    for (k = esize * i; k < dxadj[i]; k++, j++) dadjncy[j] = dadjncy[k];
    dxadj[i] = j;
  }
  for (i = nelmnts; i > 0; i--) dxadj[i] = dxadj[i - 1];
  dxadj[0] = 0;
 ...
 ******************************************************************************/
void TRINODALMETIS(
  int nelmnts, int nvtxs, idxtype* elmnts, idxtype* dxadj, idxtype* dadjncy)
{
  ...
  for (j = 3 * nelmnts, i = 0; i < j; i++) nptr[elmnts[i]]++;
  MAKECSR(i, nvtxs, nptr);

  nind = idxmalloc(nptr[nvtxs], "TRINODALMETIS: nind");
  for (k = i = 0; i < nelmnts; i++)
  {
    for (j = 0; j < 3; j++, k++) nind[nptr[elmnts[k]]++] = i;
  }
  for (i = nvtxs; i > 0; i--) nptr[i] = nptr[i - 1];
  nptr[0] = 0;
  ...
  int nelmnts, int nvtxs, idxtype* elmnts, idxtype* dxadj, idxtype* dadjncy)
{
  ...
  for (j = 4 * nelmnts, i = 0; i < j; i++) nptr[elmnts[i]]++;
  MAKECSR(i, nvtxs, nptr);

  nind = idxmalloc(nptr[nvtxs], "TETNODALMETIS: nind");
  for (k = i = 0; i < nelmnts; i++)
  {
    for (j = 0; j < 4; j++, k++) nind[nptr[elmnts[k]]++] = i;
  }
  for (i = nvtxs; i > 0; i--) nptr[i] = nptr[i - 1];
  nptr[0] = 0;
  ...
  int nelmnts, int nvtxs, idxtype* elmnts, idxtype* dxadj, idxtype* dadjncy)
{
  ...
  idxtype *nptr, *nind;
  idxtype* mark;
  int table[8][3] = {1, 3, 4, 0, 2, 5, 1, 3, 6, 0, 2, 7,
                     0, 5, 7, 1, 4, 6, 2, 5, 7, 3, 4, 6};

  /* Construct the node-element list first */
  nptr = idxsmalloc(nvtxs + 1, 0, "HEXNODALMETIS: nptr");
  for (j = 8 * nelmnts, i = 0; i < j; i++) nptr[elmnts[i]]++;
  MAKECSR(i, nvtxs, nptr);

  nind = idxmalloc(nptr[nvtxs], "HEXNODALMETIS: nind");
  for (k = i = 0; i < nelmnts; i++)
  {
    for (j = 0; j < 8; j++, k++) nind[nptr[elmnts[k]]++] = i;
  }
  for (i = nvtxs; i > 0; i--) nptr[i] = nptr[i - 1];
  nptr[0] = 0;
