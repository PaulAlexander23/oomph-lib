...
void ComputePartitionInfo(GraphType* graph, int nparts, idxtype* where)
{
  ...
  if (ncon == 1)
  {
    printf("\tBalance: %5.3f out of %5.3f\n",
           1.0 * nparts * kpwgts[idxamax(nparts, kpwgts)] /
             (1.0 * idxsum(nparts, kpwgts)),
           1.0 * nparts * vwgt[idxamax(nvtxs, vwgt)] /
             (1.0 * idxsum(nparts, kpwgts)));
  }
  else
  {
    ...
    for (j = 0; j < ncon; j++)
      printf(" (%5.3f out of %5.3f)",
             1.0 * nparts *
               kpwgts[ncon * idxamax_strd(nparts, kpwgts + j, ncon) + j] /
               (1.0 * idxsum_strd(nparts, kpwgts + j, ncon)),
             1.0 * nparts *
               vwgt[ncon * idxamax_strd(nvtxs, vwgt + j, ncon) + j] /
               (1.0 * idxsum_strd(nparts, kpwgts + j, ncon)));
    printf("\n");
  }
  ...
  for (i = 0; i < nvtxs; i++)
  {
    ...
    for (j = xadj[i]; j < xadj[i + 1]; j++) kpwgts[where[adjncy[j]]] = 0;
  }

  for (i = 0; i < nparts; i++) kpwgts[i] = idxsum(nparts, padjncy + i * nparts);
  printf("Min/Max/Avg/Bal # of adjacent     subdomains: %5d %5d %5.2f %7.3f\n",
         kpwgts[idxamin(nparts, kpwgts)],
         kpwgts[idxamax(nparts, kpwgts)],
         1.0 * idxsum(nparts, kpwgts) / (1.0 * nparts),
         1.0 * nparts * kpwgts[idxamax(nparts, kpwgts)] /
           (1.0 * idxsum(nparts, kpwgts)));

  for (i = 0; i < nparts; i++) kpwgts[i] = idxsum(nparts, padjcut + i * nparts);
  ...
  for (i = 0; i < nparts; i++) kpwgts[i] = idxsum(nparts, padjwgt + i * nparts);
  printf(
    ...
    1.0 * nparts * kpwgts[idxamax(nparts, kpwgts)] /
      (1.0 * idxsum(nparts, kpwgts)),
    1.0 * idxsum(nparts, kpwgts) / (1.0 * nvtxs));

  tmpptr = graph->where;
  graph->where = where;
  for (i = 0; i < nparts; i++) IsConnectedSubdomain(NULL, graph, i, 1);
  graph->where = tmpptr;
                             ...
                             float* ubvec)
{
  ...
  if (vwgt == NULL && ncon == 1)
  {
    for (i = 0; i < nvtxs; i++) kpwgts[where[i]]++;
  ...
  else
  {
    ...
    {
      idxset(nparts, 0, kpwgts);
      for (i = 0; i < graph->nvtxs; i++) kpwgts[where[i]] += vwgt[i * ncon + j];

      ubvec[j] = 1.0 * nparts * kpwgts[idxamax(nparts, kpwgts)] /
                 (1.0 * idxsum(nparts, kpwgts));
    }
  }
...
float ComputeElementBalance(int ne, int nparts, idxtype* where)
{
  ...
  for (i = 0; i < ne; i++) kpwgts[where[i]]++;

  balance = 1.0 * nparts * kpwgts[idxamax(nparts, kpwgts)] /
            (1.0 * idxsum(nparts, kpwgts));
                                 ...
                                 float* ubvec)
{
  ...
  for (j = 0; j < ncon; j++)
  {
    ...
    for (i = 0; i < graph->nvtxs; i++) kpwgts[where[i]] += nvwgt[i * ncon + j];

    ubvec[j] =
      (float)nparts * kpwgts[samax(nparts, kpwgts)] / ssum(nparts, kpwgts);
  }
