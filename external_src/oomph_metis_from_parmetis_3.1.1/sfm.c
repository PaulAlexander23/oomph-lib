                       ...
                       int npasses)
{
  ...
  for (pass = 0; pass < npasses; pass++)
  {
    ...
    limit = (ctrl->oflags & OFLAG_COMPRESS ? amin(5 * nbnd, 400) :
                                             amin(2 * nbnd, 300));
    ...
    for (nswaps = 0; nswaps < nvtxs; nswaps++)
    {
      ...
      if (u[0] != -1 && u[1] != -1)
      {
        ...
        /* to = (g[0] > g[1] ? 0 : (g[0] < g[1] ? 1 : (pwgts[0] < pwgts[1] ? 0 :
         * 1))); */

        if (pwgts[to] + vwgt[u[to]] > badmaxpwgt) to = (to + 1) % 2;
      }
      else if (u[0] == -1 && u[1] == -1)
      {
        break;
      ...
      if (moved[higain] ==
          -1) /* Delete if it was in the separator originally */
        PQueueDelete(
          &parts[other], higain, vwgt[higain] - rinfo[higain].edegrees[to]);

      ASSERT(bndptr[higain] != -1);

      pwgts[2] -= (vwgt[higain] - rinfo[higain].edegrees[other]);

      newdiff = abs(pwgts[to] + vwgt[higain] -
      ...
      for (j = xadj[higain]; j < xadj[higain + 1]; j++)
      {
        ...
        { /* This vertex is pulled into the separator */
          ...
          {
            kk = adjncy[jj];
            if (where[kk] != 2) edegrees[where[kk]] += vwgt[kk];
            else
            {
    ...
    for (nswaps--; nswaps > mincutorder; nswaps--)
    {
      ...
      {
        k = adjncy[j];
        if (where[k] == 2) rinfo[k].edegrees[to] -= vwgt[higain];
        else
      ...
      for (j = mptr[nswaps]; j < mptr[nswaps + 1]; j++)
      {
        ...
        {
          kk = adjncy[jj];
          if (where[kk] == 2) rinfo[kk].edegrees[other] += vwgt[k];
        }
      }
    }
...
void FM_2WayNodeRefineEqWgt(CtrlType* ctrl, GraphType* graph, int npasses)
{
  ...
  for (pass = 0; pass < npasses; pass++)
  {
    ...
    for (nswaps = 0; nswaps < nvtxs; nswaps++)
    {
      ...
      if ((higain = PQueueGetMax(&parts[to])) == -1) break;
                                ...
                                int npasses)
{
  ...
  PQueueInit(
    ctrl, &parts, nvtxs, ComputeMaxNodeGain(nvtxs, xadj, adjncy, vwgt));
  ...
  for (pass = 0; pass < npasses; pass++)
  {
    ...
    for (nswaps = 0; nswaps < nvtxs; nswaps++)
    {
      if ((higain = PQueueGetMax(&parts)) == -1) break;
        ...
        { /* This vertex is pulled into the separator */
          ...
          for (jj = xadj[k]; jj < xadj[k + 1]; jj++)
          {
            ...
            else
            {
              ...
              /* Since the moves are one-sided this vertex has not been moved
               * yet */
              PQueueUpdateUp(&parts, kk, oldgain, oldgain + vwgt[k]);
            }
          }

          /* Insert the new vertex into the priority queue. Safe due to
           * one-sided moves */
...
void FM_2WayNodeBalance(CtrlType* ctrl, GraphType* graph, float ubfactor)
{
  ...
  if (abs(pwgts[0] - pwgts[1]) <
      (int)((ubfactor - 1.0) * (pwgts[0] + pwgts[1])))
    return;
  if (abs(pwgts[0] - pwgts[1]) < 3 * idxsum(nvtxs, vwgt) / nvtxs) return;
  ...
  for (nswaps = 0; nswaps < nvtxs; nswaps++)
  {
    ...
    if (pwgts[other] - rinfo[higain].edegrees[other] <
        (pwgts[0] + pwgts[1]) / 2)
      continue;
#ifdef XXX
...
int ComputeMaxNodeGain(int nvtxs, idxtype* xadj, idxtype* adjncy, idxtype* vwgt)
{
  ...
  max = 0;
  for (j = xadj[0]; j < xadj[1]; j++) max += vwgt[adjncy[j]];

  for (i = 1; i < nvtxs; i++)
  {
    for (k = 0, j = xadj[i]; j < xadj[i + 1]; j++) k += vwgt[adjncy[j]];
    if (max < k) max = k;
  }
