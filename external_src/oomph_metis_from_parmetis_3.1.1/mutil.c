 ...
 * a given set of values
 **************************************************************************/
int AreAnyVwgtsBelow(
  int ncon, float alpha, float* vwgt1, float beta, float* vwgt2, float limit)
{
  int i;

  for (i = 0; i < ncon; i++)
    if (alpha * vwgt1[i] + beta * vwgt2[i] < limit) return 1;
...
float ComputeLoadImbalance(int ncon, int nparts, float* npwgts, float* tpwgts)
{
  ...
  for (i = 0; i < ncon; i++)
  {
    ...
    if (max * nparts > lb) lb = max * nparts;
  }
