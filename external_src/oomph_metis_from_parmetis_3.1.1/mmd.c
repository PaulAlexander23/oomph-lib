...
void mmdelm(int mdeg_node,
            ...
            int tag)
{
  int element, i, istop, istart, j, jstop, jstart, link, nabor, node, npv,
...
void mmdupd(int ehead,
            ...
            int* tag)
{
  int deg, deg0, element, enode, fnode, i, iq2, istop, istart, j, jstop, jstart,
    link, mdeg0, mtag, nabor, node, q2head, qxhead;
...
n1600:
  ...
  for (i = istart; i <= istop; i++)
  {
    ...
    if (marker[nabor] < *tag)
    {
      marker[nabor] = *tag;
      link = nabor;
      if (forward[nabor] >= 0) /*if uneliminated, include it in deg count.*/
        deg += qsize[nabor];
      else
      {
      n1700:
