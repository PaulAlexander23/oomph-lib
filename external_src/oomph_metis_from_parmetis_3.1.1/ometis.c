                  ...
                  idxtype* iperm)
{
  ...
  MlevelNestedDissection(
    &ctrl, &graph, iperm, ORDER_UNBALANCE_FRACTION, *nvtxs);

  IFSET(ctrl.dbglvl, DBG_TIME, stoptimer(ctrl.TotalTmr));
  IFSET(ctrl.dbglvl, DBG_TIME, PrintTimers(&ctrl));

  for (i = 0; i < *nvtxs; i++) perm[iperm[i]] = i;

  FreeWorkSpace(&ctrl, &graph);

  if (*numflag == 1) Change2FNumberingOrder(*nvtxs, xadj, adjncy, perm, iperm);
}
                  ...
                  idxtype* iperm)
{
  ...
  if (ctrl.pfactor > 0)
  {
    ...
    * Prune the dense columns
    ...
    PruneGraph(
      &ctrl, &graph, *nvtxs, xadj, adjncy, piperm, (float)(0.1 * ctrl.pfactor));
  }
  ...
  * Do the nested dissection ordering
  ...
  if (ctrl.oflags & OFLAG_CCMP)
    MlevelNestedDissectionCC(
      &ctrl, &graph, iperm, ORDER_UNBALANCE_FRACTION, graph.nvtxs);
  else
    MlevelNestedDissection(
      &ctrl, &graph, iperm, ORDER_UNBALANCE_FRACTION, graph.nvtxs);
  ...
  { /* Order any prunned vertices */
    if (graph.nvtxs < *nvtxs)
    {
      ...
      for (i = 0; i < graph.nvtxs; i++) iperm[piperm[i]] = perm[i];
      for (i = graph.nvtxs; i < *nvtxs; i++) iperm[piperm[i]] = i;
    }
  ...
  { /* Uncompress the ordering */
    if (graph.nvtxs < COMPRESSION_FRACTION * (*nvtxs))
    {
      ...
      for (i = 0; i < graph.nvtxs; i++) perm[iperm[i]] = i;
      for (l = ii = 0; ii < graph.nvtxs; ii++)
      {
        i = perm[ii];
        for (j = cptr[i]; j < cptr[i + 1]; j++) iperm[cind[j]] = l++;
      }
    }
 ...
 **************************************************************************/
void MlevelNestedDissection(
  CtrlType* ctrl, GraphType* graph, idxtype* order, float ubfactor, int lastvtx)
{
  ...
  bndind = graph->bndind;
  label = graph->label;
  for (i = 0; i < nbnd; i++) order[label[bndind[i]]] = --lastvtx;
  ...
  if (lgraph.nvtxs > MMDSWITCH)
    MlevelNestedDissection(
      ctrl, &lgraph, order, ubfactor, lastvtx - rgraph.nvtxs);
  else
  {
  ...
  CtrlType* ctrl, GraphType* graph, idxtype* order, float ubfactor, int lastvtx)
{
  ...
  */
  ...
  sgraphs = (GraphType*)GKmalloc(ncmps * sizeof(GraphType),
                                 "MlevelNestedDissectionCC: sgraphs");
  ...
  for (rnvtxs = i = 0; i < nsgraphs; i++)
  {
    ...
    {
      MlevelNestedDissectionCC(
        ctrl, sgraphs + i, order, ubfactor, lastvtx - rnvtxs);
    }
                                 ...
                                 float ubfactor)
{
  ...
  if (ctrl->nseps == 1 ||
      graph->nvtxs < (ctrl->oflags & OFLAG_COMPRESS ? 1000 : 2000))
  {
  ...
  { /* Multiple separators at the original graph */
                         ...
                         float ubfactor)
{
  ...
  if (ctrl->CoarsenTo > 100) ctrl->CoarsenTo = 100;
                     ...
                     GraphType* rgraph)
{
  ...
  idxtype *xadj, *vwgt, *adjncy, *adjwgt, *adjwgtsum, *label, *where, *bndptr,
    *bndind;
  idxtype *sxadj[2], *svwgt[2], *sadjncy[2], *sadjwgt[2], *sadjwgtsum[2],
    *slabel[2];
  idxtype* rename;
  ...
  {
    i = bndind[ii];
    for (j = xadj[i]; j < xadj[i + 1]; j++) bndptr[adjncy[j]] = 1;
  }
  ...
  for (i = 0; i < nvtxs; i++)
  {
    ...
    { /* This is an interior vertex */
      ...
      for (j = istart; j < iend; j++) auxadjncy[j] = adjncy[j];
    ...
    else
    {
      ...
      {
        k = adjncy[j];
        if (where[k] == mypart) auxadjncy[l++] = k;
      }
      snedges[mypart] = l;
    }

    svwgt[mypart][snvtxs[mypart]] = vwgt[i];
    sadjwgtsum[mypart][snvtxs[mypart]] =
      snedges[mypart] - sxadj[mypart][snvtxs[mypart]];
  ...
  for (mypart = 0; mypart < 2; mypart++)
  {
    ...
    auxadjncy = sadjncy[mypart];
    for (i = 0; i < iend; i++) auxadjncy[i] = rename[auxadjncy[i]];
  }
...
void MMDOrder(CtrlType* ctrl, GraphType* graph, idxtype* order, int lastvtx)
{
  ...
  k = xadj[nvtxs];
  for (i = 0; i < k; i++) adjncy[i]++;
  for (i = 0; i < nvtxs + 1; i++) xadj[i]++;
  ...
  for (i = 0; i < nvtxs; i++) order[label[i]] = firstvtx + iperm[i] - 1;
                      ...
                      idxtype* cind)
{
  ...
  for (iii = 0; iii < ncmps; iii++)
  {
    ...
    for (i = 0; i < snedges; i++) sadjncy[i] = rename[sadjncy[i]];
