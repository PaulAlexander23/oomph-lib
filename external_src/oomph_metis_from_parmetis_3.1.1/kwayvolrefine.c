                                    ...
                                    int nparts)
{
  ...
  graph->rdata = idxmalloc(
    3 * nvtxs + nparts + (sizeof(VRInfoType) / sizeof(idxtype)) * nvtxs + pad64,
    "AllocateVolKWayPartitionMemory: rdata");
  ...
  for (i = 0; i < nvtxs; i++)
  {
    ...
    if (myrinfo->ed > 0)
    {
      myedegrees = myrinfo->edegrees =
        ctrl->wspace.vedegrees + ctrl->wspace.cdegree;
  ...
  for (i = 0; i < nvtxs; i++)
  {
    ...
    if (myrinfo->ndegrees > 0)
    {
      ...
      for (j = xadj[i]; j < xadj[i + 1]; j++)
      {
        ...
        for (k = 0; k < orinfo->ndegrees; k++) ophtable[oedegrees[k].pid] = k;
        ...
        else
        {
          ...
          if (oedegrees[ophtable[me]].ned == 1)
...
void ComputeVolKWayBoundary(CtrlType* ctrl, GraphType* graph, int nparts)
{
  ...
  for (i = 0; i < nvtxs; i++)
  {
    if (graph->vrinfo[i].gv >= 0 ||
        graph->vrinfo[i].ed - graph->vrinfo[i].id >= 0)
...
void ComputeVolKWayBalanceBoundary(CtrlType* ctrl, GraphType* graph, int nparts)
{
  ...
  for (i = 0; i < nvtxs; i++)
  {
    if (graph->vrinfo[i].ed > 0) BNDInsert(nbnd, bndind, bndptr, i);
  }
