 ...
 * kvmetis.c
 *
 * This file contains the top level routines for the multilevel k-way
 * partitioning algorithm KMETIS.
 *
 * Started 7/28/97
 * George
 *
                           ...
                           idxtype* part)
{
  ...
  VolSetUpGraph(
    &graph, OP_KVMETIS, *nvtxs, 1, xadj, adjncy, vwgt, vsize, *wgtflag);
  ...
  *volume =
    MlevelVolKWayPartitioning(&ctrl, &graph, *nparts, part, tpwgts, 1.03);
                              ...
                              float ubfactor)
{
  ...
  METIS_WPartGraphRecursive(&cgraph->nvtxs,
                            cgraph->xadj,
                            cgraph->adjncy,
                            cgraph->vwgt,
                            cgraph->adjwgt,
  ...
  IFSET(ctrl->dbglvl,
        DBG_KWAYPINFO,
        ComputePartitionInfo(cgraph, nparts, cgraph->where));
