...
GraphType* MCCoarsen2Way(CtrlType* ctrl, GraphType* graph)
{
  ...
  do
  {
    ...
    {
      printf("%6d %7d %10d [%d] [%6.4f",
             cgraph->nvtxs,
             cgraph->nedges,
             idxsum(cgraph->nvtxs, cgraph->adjwgtsum),
             ctrl->CoarsenTo,
             ctrl->nmaxvwgt);
      for (i = 0; i < graph->ncon; i++)
        printf(" %5.3f",
               ssum_strd(cgraph->nvtxs, cgraph->nvwgt + i, cgraph->ncon));
      printf("]\n");
    }
    ...
    switch (ctrl->CType)
    {
      case MATCH_RM:
        ...
        break;
      case MATCH_HEM:
        if (clevel < 1 || cgraph->nedges == 0) MCMatch_RM(ctrl, cgraph);
        else
