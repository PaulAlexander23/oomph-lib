                   ...
                   idxtype* cind)
{
  ...
  keys = (KeyValueType*)GKmalloc(nvtxs * sizeof(KeyValueType),
                                 "CompressGraph: keys");
  ...
  for (cnvtxs = i = 0; i < nvtxs; i++)
  {
    ...
    if (map[ii] == -1)
    {
      ...
      for (j = xadj[ii]; j < xadj[ii + 1]; j++) mark[adjncy[j]] = i;
  ...
  { /* Ok, form the compressed graph  */
    ...
    graph->gdata =
      idxmalloc(4 * cnvtxs + 1 + 2 * cnedges, "CompressGraph: gdata");
    ...
    idxset(graph->nedges, 1, graph->adjwgt);
    for (i = 0; i < cnvtxs; i++) graph->adjwgtsum[i] = cxadj[i + 1] - cxadj[i];

    graph->label = idxmalloc(cnvtxs, "CompressGraph: label");
    for (i = 0; i < cnvtxs; i++) graph->label[i] = i;
  }
                ...
                float factor)
{
  ...
  { /* Prune the graph */
    ...
    for (i = 0; i < pnvtxs; i++) graph->adjwgtsum[i] = pxadj[i + 1] - pxadj[i];

    graph->label = idxmalloc(pnvtxs, "CompressGraph: label");
    for (i = 0; i < pnvtxs; i++) graph->label[i] = i;
  }
