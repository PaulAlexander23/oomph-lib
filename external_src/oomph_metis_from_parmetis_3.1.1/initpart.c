                       ...
                       float ubfactor)
{
  ...
  {
    case IPART_GGPKL:
      if (graph->nedges == 0) RandomBisection(ctrl, graph, tpwgts, ubfactor);
      else
...
void GrowBisection(CtrlType* ctrl,
                   ...
                   float ubfactor)
{
  int i, j, k, nvtxs, drain, nleft, first, last, pwgts[2], minpwgt[2],
    maxpwgt[2], from, bestcut, icut, mincut, me, pass, nbfs;
  ...
  ASSERTP(tpwgts[0] + tpwgts[1] == idxsum(nvtxs, vwgt),
  ...
  bestcut =
    idxsum(nvtxs, graph->adjwgtsum) + 1; /* The +1 is for the 0 edges case */
  for (; nbfs > 0; nbfs--)
  {
     ...
     * pwgts[1], graph->pwgts[0], graph->pwgts[1], graph->mincut); */

    Balance2Way(ctrl, graph, tpwgts, ubfactor);
    /*printf("BPART: [%5d %5d] %5d\n", graph->pwgts[0], graph->pwgts[1],
     * graph->mincut);*/
...
void GrowBisectionNode(CtrlType* ctrl, GraphType* graph, float ubfactor)
{
  int i, j, k, nvtxs, drain, nleft, first, last, pwgts[2], tpwgts[2],
    minpwgt[2], maxpwgt[2], from, bestcut, icut, mincut, me, pass, nbfs;
  ...
  /* Allocate memory for graph->rdata. Allocate sufficient memory for both edge
   * and node */
  ...
  for (nbfs++; nbfs > 0; nbfs--)
  {
    ...
    for (i = 0; i < graph->nbnd; i++) where[bndind[i]] = 2;

    Compute2WayNodePartitionParams(ctrl, graph);
    FM_2WayNodeRefine(ctrl, graph, ubfactor, 6);

    /* printf("ISep: [%d %d %d] %d\n", graph->pwgts[0], graph->pwgts[1],
     * graph->pwgts[2], bestcut); */
...
void RandomBisection(CtrlType* ctrl,
                     ...
                     float ubfactor)
{
  int i, ii, j, k, nvtxs, pwgts[2], minpwgt[2], maxpwgt[2], from, bestcut, icut,
    mincut, me, pass, nbfs;
  idxtype *xadj, *vwgt, *adjncy, *adjwgt, *where;
