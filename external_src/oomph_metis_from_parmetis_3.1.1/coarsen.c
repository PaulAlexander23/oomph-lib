...
GraphType* Coarsen2Way(CtrlType* ctrl, GraphType* graph)
{
  ...
  /* The following is ahack to allow the multiple bisections to go through with
     correct coarsening */
  if (ctrl->CType > 20)
  {
    clevel = 1;
  ...
  do
  {
    IFSET(ctrl->dbglvl,
          ...
          printf("%6d %7d [%d] [%d %d]\n",
                 ...
                 ctrl->CoarsenTo,
                 ctrl->maxvwgt,
                 (cgraph->vwgt ? idxsum(cgraph->nvtxs, cgraph->vwgt) :
                                 cgraph->nvtxs)));
    ...
    if (cgraph->adjwgt)
    {
      switch (ctrl->CType)
      {
        case MATCH_RM:
          ...
          break;
        case MATCH_HEM:
          if (clevel < 1 || cgraph->nedges == 0) Match_RM(ctrl, cgraph);
          else
        ...
        case MATCH_SHEM:
          ...
          break;
        case MATCH_SHEMKWAY:
          if (cgraph->nedges == 0) Match_RM(ctrl, cgraph);
          else
            Match_SHEM(ctrl, cgraph);
          break;
        default:
    ...
    clevel++;
           ...
           cgraph->nvtxs < COARSEN_FRACTION2 * cgraph->finer->nvtxs &&
           cgraph->nedges > cgraph->nvtxs / 2);
