                         ...
                         idxtype* npart)
{
  ...
  if (*numflag == 1) ChangeMesh2CNumbering((*ne) * esize, elmnts);
  ...
  for (i = 0; i < *ne; i++)
  {
    ...
    for (j = 1; j < esize; j++)
    {
      if (npart[elmnts[i * esize + j]] != me) break;
    }
    if (j == esize)
    {
      epart[i] = me;
  ...
  for (i = 0; i < *ne; i++)
  {
    ...
    { /* Assign the boundary element */
      ...
      else
      {
        ...
        if (j == nnbrs) epart[i] = nbrind[iamax(nnbrs, nbrwgt)];
      }
      pwgts[epart[i]]++;
    }
  }
                        ...
                        idxtype* npart)
{
  ...
  for (j = esize * (*ne), i = 0; i < j; i++) nptr[elmnts[i]]++;
  MAKECSR(i, *nn, nptr);
  ...
  for (i = (*nn); i > 0; i--) nptr[i] = nptr[i - 1];
  nptr[0] = 0;
