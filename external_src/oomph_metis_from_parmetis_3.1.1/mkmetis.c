                           ...
                           idxtype* part)
{
  ...
  SetUpGraph(
    &graph, OP_KMETIS, *nvtxs, *ncon, xadj, adjncy, vwgt, adjwgt, *wgtflag);
 ...
 **************************************************************************/
int MCMlevelKWayPartitioning(
  CtrlType* ctrl, GraphType* graph, int nparts, idxtype* part, float* rubvec)
{
  int i, j, nvtxs;
  GraphType* cgraph;
  ...
  {
    if (rubvec[i] > 1.2) break;
  }
  if (i == graph->ncon)
    METIS_mCPartGraphRecursiveInternal(&cgraph->nvtxs,
                                       &cgraph->ncon,
                                       cgraph->xadj,
                                       cgraph->adjncy,
                                       cgraph->nvwgt,
    ...
    METIS_mCHPartGraphRecursiveInternal(&cgraph->nvtxs,
                                        &cgraph->ncon,
                                        cgraph->xadj,
                                        cgraph->adjncy,
                                        cgraph->nvwgt,
