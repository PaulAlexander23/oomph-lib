...
void MocFM_2WayEdgeRefine2(
  CtrlType* ctrl, GraphType* graph, float* tpwgts, float* orgubvec, int npasses)
{
  ...
  { /* Do a number of passes */
    ...
    for (nswaps = 0; nswaps < nvtxs; nswaps++)
    {
      ...
      {
        mincut = newcut;
        for (i = 0; i < ncon; i++) minbal[i] = tvec[i];
        mincutorder = nswaps;
      }
      ...
      for (j = xadj[higain]; j < xadj[higain + 1]; j++)
      {
        ...
        { /* If k was a boundary vertex */
          ...
          { /* If it has not been moved, update its position in the queue */
            if (moved[k] == -1)
              PQueueUpdate(
                &parts[qnum[k]][where[k]], k, oldgain, ed[k] - id[k]);
          }
        }
        else
        {
    ...
    if (ctrl->dbglvl & DBG_REFINE)
    {
      printf("\tMincut: %6d at %5d, NBND: %6d, NPwgts: [",
             mincut,
             mincutorder,
             nbnd);
      for (l = 0; l < ncon; l++)
