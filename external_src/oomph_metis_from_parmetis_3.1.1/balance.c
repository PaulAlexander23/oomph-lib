...
void Balance2Way(CtrlType* ctrl, GraphType* graph, int* tpwgts, float ubfactor)
{
  ...
  if (mindiff < 3 * (graph->pwgts[0] + graph->pwgts[1]) / graph->nvtxs) return;
  if (graph->pwgts[0] > tpwgts[0] &&
      graph->pwgts[0] < (int)(ubfactor * tpwgts[0]))
    return;
  if (graph->pwgts[1] > tpwgts[1] &&
      graph->pwgts[1] < (int)(ubfactor * tpwgts[1]))
    return;

  if (graph->nbnd > 0) Bnd2WayBalance(ctrl, graph, tpwgts);
  else
...
void Bnd2WayBalance(CtrlType* ctrl, GraphType* graph, int* tpwgts)
{
  ...
  /* Insert the boundary nodes of the proper partition whose size is OK in the
   * priority queue */
  nbnd = graph->nbnd;
  ...
  for (nswaps = 0; nswaps < nvtxs; nswaps++)
  {
    ...
    if (pwgts[to] + vwgt[higain] > tpwgts[to]) break;
      ...
      { /* If k was a boundary vertex */
        ...
        { /* Not a boundary vertex any more */
          ...
          if (moved[k] == -1 && where[k] == from &&
              vwgt[k] <= mindiff) /* Remove it if in the queues */
...
void General2WayBalance(CtrlType* ctrl, GraphType* graph, int* tpwgts)
{
  ...
  /* Insert the nodes of the proper partition whose size is OK in the priority
   * queue */
