                ...
                float ubfactor)
{
  ...
  for (i = 0;; i++)
  {
    ...
    if (ctrl->RType == RTYPE_KWAYRANDOM_MCONN &&
        (i == nlevels / 2 || i == nlevels / 2 + 1))
      EliminateSubDomainEdges(ctrl, graph, nparts, tpwgts);

    IFSET(ctrl->dbglvl, DBG_TIME, starttimer(ctrl->RefTmr));

    if (2 * i >= nlevels &&
        !IsBalanced(graph->pwgts, nparts, tpwgts, 1.04 * ubfactor))
    {
    ...
    switch (ctrl->RType)
    {
      ...
      case RTYPE_KWAYRANDOM_MCONN:
        Random_KWayEdgeRefineMConn(
          ctrl, graph, nparts, tpwgts, ubfactor, 10, 1);
        break;
    }
  ...
  if (mustfree) GKfree(&graph->vwgt, &graph->adjwgt, LTERM);
...
void AllocateKWayPartitionMemory(CtrlType* ctrl, GraphType* graph, int nparts)
{
  ...
  graph->rdata = idxmalloc(
    3 * nvtxs + nparts + (sizeof(RInfoType) / sizeof(idxtype)) * nvtxs + pad64,
    "AllocateKWayPartitionMemory: rdata");
  ...
  /*
    ...
    ctrl->wspace.edegrees = (EDegreeType
    *)GKmalloc(graph->nedges*sizeof(EDegreeType), "AllocateKWayPartitionMemory:
    edegrees");
  */
}
...
void ComputeKWayPartitionParams(CtrlType* ctrl, GraphType* graph, int nparts)
{
  ...
  for (i = 0; i < nvtxs; i++)
  {
    ...
    {
      if (me != where[adjncy[j]]) myrinfo->ed += adjwgt[j];
    }
    myrinfo->id = graph->adjwgtsum[i] - myrinfo->ed;

    if (myrinfo->ed > 0) mincut += myrinfo->ed;

    if (myrinfo->ed - myrinfo->id >= 0) BNDInsert(nbnd, bndind, bndptr, i);

    /* Time to compute the particular external degrees */
    if (myrinfo->ed > 0)
    {
      myedegrees = myrinfo->edegrees =
        ctrl->wspace.edegrees + ctrl->wspace.cdegree;
...
void ProjectKWayPartition(CtrlType* ctrl, GraphType* graph, int nparts)
{
  ...
  for (nbnd = 0, i = 0; i < nvtxs; i++)
  {
    ...
    if (cmap[i] > 0)
    { /* If it is an interface node. Note cmap[i] = crinfo[cmap[i]].ed */
      istart = xadj[i];
      iend = xadj[i + 1];
      ...
      else
      {
        ...
        for (j = 0; j < ndegrees; j++) htable[myedegrees[j].pid] = -1;
      }
    }
  }
...
int IsBalanced(idxtype* pwgts, int nparts, float* tpwgts, float ubfactor)
{
  ...
  for (i = 0; i < nparts; i++)
  {
    if (pwgts[i] > tpwgts[i] * tvwgt * (ubfactor + 0.005)) return 0;
  }
...
void ComputeKWayBalanceBoundary(CtrlType* ctrl, GraphType* graph, int nparts)
{
  ...
  for (i = 0; i < nvtxs; i++)
  {
    if (graph->rinfo[i].ed > 0) BNDInsert(nbnd, bndind, bndptr, i);
  }
