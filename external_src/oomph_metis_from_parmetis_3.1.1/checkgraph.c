...
int CheckGraph(GraphType* graph)
{
  ...
  for (i = 0; i < nvtxs; i++)
  {
    if (ncon > 1)
    {
      for (j = 0; j < ncon; j++)
      {
        ...
        minvwgt[j] =
          (nvwgt[i * ncon + j] < minvwgt[j]) ? nvwgt[i * ncon + j] : minvwgt[j];
        maxvwgt[j] =
          (nvwgt[i * ncon + j] > maxvwgt[j]) ? nvwgt[i * ncon + j] : maxvwgt[j];
      }
    }
    ...
    for (j = xadj[i]; j < xadj[i + 1]; j++)
    {
      ...
      if (i == k)
      {
        printf("Vertex %d contains a self-loop (i.e., diagonal entry in the "
               "matrix)!\n",
               i);
        err++;
      }
      else
      {
        for (l = xadj[k]; l < xadj[k + 1]; l++)
        {
          if (adjncy[l] == i)
          {
            ...
            {
              printf("Edges (%d %d) and (%d %d) do not have the same weight! "
                     "%d %d\n",
                     i,
                     k,
                     k,
      ...
      {
        printf(
          "Edge %d from vertex %d is repeated %d times\n", k, i, htable[k]++);
        err++;
      }
    }
  ...
  */
  ...
  if (err > 0)
  {
    printf("A total of %d errors exist in the input file. Correct them, and "
           "run again!\n",
           err);
  }
