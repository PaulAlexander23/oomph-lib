                       ...
                       int npasses)
{
  ...
  PQueueType parts[2];
  int higain, oldgain, mincut, mindiff, origdiff, initcut, newcut, mincutorder,
    avgvwgt;
  ...
  { /* Do a number of passes */
    ...
    for (ii = 0; ii < nbnd; ii++)
    {
      ...
      PQueueInsert(
        &parts[where[bndind[i]]], bndind[i], ed[bndind[i]] - id[bndind[i]]);
    }
    ...
    for (nswaps = 0; nswaps < nvtxs; nswaps++)
    {
      ...
      if ((higain = PQueueGetMax(&parts[from])) == -1) break;
      ...
      if ((newcut < mincut &&
           abs(tpwgts[0] - pwgts[0]) <= origdiff + avgvwgt) ||
