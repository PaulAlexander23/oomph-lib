                          ...
                          int ffactor)
{
  ...
  IFSET(ctrl->dbglvl,
        DBG_REFINE,
        printf("VolPart: [%5d %5d]-[%5d %5d], Balance: %3.2f, Nv-Nb[%5d %5d]. "
               "Cut: %5d, Vol: %5d\n",
  ...
  for (pass = 0; pass < npasses; pass++)
  {
    ...
    {
      ii = perm[iii];
      if (ii >= graph->nbnd) continue;
      i = bndind[ii];
      myrinfo = graph->vrinfo + i;

      if (myrinfo->gv >= 0)
      { /* Total volume gain is too high */
        from = where[i];
        vwgt = graph->vwgt[i];
        ...
        {
          to = myedegrees[j].pid;
          if (pwgts[to] + vwgt > maxwgt[to]) continue;
          if (myedegrees[j].gv > myedegrees[k].gv ||
              (myedegrees[j].gv == myedegrees[k].gv &&
        ...
        {
          if ((iii & 5) == 0 || pwgts[from] >= maxwgt[from] ||
        ...
        IFSET(ctrl->dbglvl,
              DBG_MOVEINFO,
              printf("\t\tMoving %6d from %3d to %3d. Gain: [%4d %4d]. Cut: "
                     "%6d, Vol: %6d\n",
                     i,
                     from,
                     to,
                     xgain + myedegrees[k].gv,
    ...
    if (graph->minvol == oldvol && graph->mincut == oldcut) break;
  }
  ...
  for (pass = 0; pass < npasses; pass++)
  {
    ...
    for (nmoves = iii = 0; iii < graph->nbnd; iii++)
    {
      ...
      { /* Total volume gain is too high */
        ...
        {
          to = myedegrees[j].pid;
          if (!phtable[to] || pwgts[to] + vwgt > maxwgt[to]) continue;
          if (myedegrees[j].gv > myedegrees[k].gv ||
              (myedegrees[j].gv == myedegrees[k].gv &&
               myedegrees[j].ed > myedegrees[k].ed) ||
              (myedegrees[j].gv == myedegrees[k].gv &&
        ...
        {
          if ((iii & 5) == 0 || phtable[myedegrees[k].pid] == 2 ||
        ...
        for (j = 0; j < myndegrees; j++) phtable[myedegrees[j].pid] = -1;
                           ...
                           int npasses)
{
  ...
  idxtype *xadj, *adjncy, *adjwgt;
  idxtype *where, *pwgts, *perm, *moved, *bndptr, *bndind, *minwgt, *maxwgt,
    *itpwgts, *updind, *marker, *phtable;
  VEDegreeType* myedegrees;
  VRInfoType* myrinfo;
  PQueueType queue;
  ...
  for (pass = 0; pass < npasses; pass++)
  {
    ...
    for (nmoves = 0;;)
    {
      ...
      IFSET(ctrl->dbglvl,
            DBG_MOVEINFO,
            printf("\t\tMoving %6d from %3d to %3d. Gain: [%4d %4d]. Cut: %6d, "
                   "Vol: %6d\n",
                   i,
                   from,
                   to,
                   ...
                   idxtype* updind)
{
   ...
   *=====================================================================*/
  for (k = 0; k < myrinfo->ndegrees; k++) phtable[myedegrees[k].pid] = k;
  phtable[from] = k;

  myidx =
    phtable[to]; /* Keep track of the index in myedegrees of the 'to' domain */
  ...
  for (j = xadj[v]; j < xadj[v + 1]; j++)
  {
    ...
    if (other == from)
    {
      ...
      {
        if (phtable[oedegrees[k].pid] == -1) oedegrees[k].gv += vsize[v];
      }
    }
    else
    {
  ...
  for (k = 0; k < myrinfo->ndegrees; k++) phtable[myedegrees[k].pid] = -1;
  phtable[from] = -1;
                       ...
                       idxtype* phtable)
{
  ...
  for (iii = 0; iii < nupd; iii++)
  {
    ...
    { /* Only complete gain updates go through */
      for (k = 0; k < myrinfo->ndegrees; k++) myedegrees[k].gv = 0;
      ...
      for (j = xadj[i]; j < xadj[i + 1]; j++)
      {
        ...
        if (me == other)
        {
          ...
          for (k = 0; k < myrinfo->ndegrees; k++)
          {
            if (phtable[myedegrees[k].pid] == -1) myedegrees[k].gv -= vsize[ii];
          }
        }
        else
        {
    ...
    for (k = 0; k < myrinfo->ndegrees; k++)
    {
      if (myedegrees[k].gv > myrinfo->gv) myrinfo->gv = myedegrees[k].gv;
    }
    if (myrinfo->ed > 0 && myrinfo->id == 0) myrinfo->gv += vsize[i];
  }
}
...
void CheckVolKWayPartitionParams(CtrlType* ctrl, GraphType* graph, int nparts)
{
  ...
  tmpdegrees = (VEDegreeType*)GKmalloc(
    nparts * sizeof(VEDegreeType), "CheckVolKWayPartitionParams: tmpdegrees");
  ...
  for (i = 0; i < nvtxs; i++)
  {
    ...
    for (k = 0; k < myrinfo->ndegrees; k++) tmpdegrees[k] = myedegrees[k];
    ...
    for (j = xadj[i]; j < xadj[i + 1]; j++)
    {
      ...
      if (me == other)
      {
        /* Find which domains 'i' is connected and 'ii' is not and update their
         * gain */
        for (k = 0; k < myrinfo->ndegrees; k++)
        {
          ...
          for (kk = 0; kk < orinfo->ndegrees; kk++)
          {
            if (oedegrees[kk].pid == pid) break;
          }
          if (kk == orinfo->ndegrees) myedegrees[k].gv -= vsize[ii];
        }
      }
      else
      {
        ...
        for (k = 0; k < orinfo->ndegrees; k++)
        {
          if (oedegrees[k].pid == me) break;
        }

        if (oedegrees[k].ned == 1)
        { /* I'm the only connection of 'ii' in 'me' */
          for (k = 0; k < myrinfo->ndegrees; k++)
          {
          ...
          for (k = 0; k < myrinfo->ndegrees; k++)
          {
            if ((pid = myedegrees[k].pid) == other) continue;
            for (kk = 0; kk < orinfo->ndegrees; kk++)
            {
  ...
  for (;;)
  {
    ...
    for (min = 0; min < ncand2; min++)
    {
      ...
      for (cpwgt = 0, ii = 0; ii < nind; ii++)
      {
        ...
        {
          k = adjncy[j];
          if (where[k] != other) otherpmat[where[k]] += adjwgt[j];
        }
      }
      ...
      for (ii = 0; ii < nind; ii++)
      {
        ...
        for (j = xadj[i]; j < xadj[i + 1]; j++)
        {
          ...
          if (where[k] != other)
          {
            ...
            if (pmat[nparts * other + where[k]] == 0) ndoms[other]--;

            if (pmat[nparts * where[k] + other] == 0)
              printf("Something wrong\n");
            pmat[nparts * where[k] + other] -= adjwgt[j];
            if (pmat[nparts * where[k] + other] == 0) ndoms[where[k]]--;
          }
        }
        ...
        for (j = xadj[i]; j < xadj[i + 1]; j++)
        {
          ...
          if (where[k] != target)
          {
            if (pmat[nparts * target + where[k]] == 0) ndoms[target]++;
            pmat[nparts * target + where[k]] += adjwgt[j];

            if (pmat[nparts * where[k] + target] == 0) ndoms[where[k]]++;
 ...
 * remove some of them
 **************************************************************************/
void EliminateVolComponents(
  CtrlType* ctrl, GraphType* graph, int nparts, float* tpwgts, float ubfactor)
{
  int i, ii, j, jj, k, me, nvtxs, tvwgt, first, last, nleft, ncmps, cwgt, ncand,
    other, target, deltawgt;
  ...
  { /* There are more components than processors */
    ...
    for (i = 0; i < nvtxs; i++) pwgts[where[i]] += vwgt[i];
    tvwgt = idxsum(nparts, pwgts);
    ...
    for (i = 0; i < ncmps; i++)
    {
      ...
      for (cwgt = 0, j = cptr[i]; j < cptr[i + 1]; j++)
      {
        ...
        for (jj = xadj[ii]; jj < xadj[ii + 1]; jj++)
        {
          ...
          if (me != other) cpvec[other] += adjwgt[jj];
        }
      }
      ...
      if (target != -1)
      {
        ...
        for (j = cptr[i]; j < cptr[i + 1]; j++) where[cind[j]] = target;
