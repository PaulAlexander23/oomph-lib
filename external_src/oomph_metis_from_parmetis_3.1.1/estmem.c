                          ...
                          int* nbytes)
{
  ...
  if (*optype == 2) coresize = nedges;
  else
    coresize = 0;
  coresize += nedges + 11 * (*nvtxs) + 4 * 1024 +
              2 * (NEG_GAINSPAN + PLUS_GAINSPAN + 1) *
                (sizeof(ListNodeType*) / sizeof(idxtype));
  ...
  *nbytes = sizeof(idxtype) * (coresize + gdata + rdata + (*nvtxs));
  ...
  if (*numflag == 1) Change2FNumbering2(*nvtxs, xadj, adjncy);
}
 ...
 **************************************************************************/
void EstimateCFraction(
  int nvtxs, idxtype* xadj, idxtype* adjncy, float* vfraction, float* efraction)
{
  ...
  cnedges =
    ComputeCoarseGraphSize(nvtxs, xadj, adjncy, cnvtxs, cmap, match, perm);
