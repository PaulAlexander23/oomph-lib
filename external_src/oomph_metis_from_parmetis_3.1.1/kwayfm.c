  ...
  for (pass = 0; pass < npasses; pass++)
  {
    ...
    for (nmoves = iii = 0; iii < graph->nbnd; iii++)
    {
      ...
      { /* Total ED is too high */
        ...
        {
          if ((iii & 7) == 0 || pwgts[from] >= maxwgt[from] ||
  ...
  for (pass = 0; pass < npasses; pass++)
  {
    ...
    for (iii = 0;; iii++)
    {
      ...
      for (k = 0; k < myndegrees; k++)
      {
        to = myedegrees[k].pid;
        gain =
          myedegrees[k].ed - j; /* j = myrinfo->id. Allow good nodes to move */
        if (pwgts[to] + vwgt <= maxwgt[to] + gain && gain >= 0) break;
      }
      if (k == myndegrees)
        continue; /* break out if you did not find a candidate */
      ...
      if (myrinfo->ed < myrinfo->id) BNDDelete(nbnd, bndind, bndptr, i);
      ...
      for (j = xadj[i]; j < xadj[i + 1]; j++)
      {
        ...
        if (me == to || me == from)
        {
          ...
          if (moved[ii] == 2)
          {
            if (gain >= 0) PQueueUpdate(&queue, ii, oldgain, gain);
            else
            {
