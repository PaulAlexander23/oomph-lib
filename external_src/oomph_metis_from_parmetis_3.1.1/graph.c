                ...
                int wgtflag)
{
  ...
  if (OpType == OP_KMETIS && ncon == 1 && (wgtflag & 2) == 0 &&
      (wgtflag & 1) == 0)
  {
  ...
  if (ncon == 1)
  { /* We are in the non mC mode */
    gsize = 0;
    if ((wgtflag & 2) == 0) gsize += nvtxs;
    if ((wgtflag & 1) == 0) gsize += graph->nedges;
    ...
    {
      sum = 0;
      for (j = xadj[i]; j < xadj[i + 1]; j++) sum += adjwgt[j];
  ...
  { /* Set up the graph in MOC mode */
    ...
    for (i = 0; i < ncon; i++) tvwgt[i] = idxsum_strd(nvtxs, vwgt + i, ncon);
  ...
  if (OpType != OP_KMETIS && OpType != OP_KVMETIS)
  {
    ...
    for (i = 0; i < nvtxs; i++) graph->label[i] = i;
  }
}
...
void SetUpGraphKway(GraphType* graph, int nvtxs, idxtype* xadj, idxtype* adjncy)
{
  ...
  /* Compute the initial values of the adjwgtsum */
  for (i = 0; i < nvtxs; i++) graph->adjwgtsum[i] = xadj[i + 1] - xadj[i];
}
                   ...
                   int wgtflag)
{
  ...
  { /* We are in the non mC mode */
    ...
    if ((wgtflag & 2) == 0) gsize += nvtxs; /* vwgts */
    if ((wgtflag & 1) == 0) gsize += nvtxs; /* vsize */
    ...
    /* Allocate memory for edge weights and initialize them to the sum of the
     * vsize */
  ...
  { /* Set up the graph in MOC mode */
    ...
    if ((wgtflag & 2) == 0) vwgt = idxsmalloc(nvtxs, 1, "SetUpGraph: vwgt");
...
int IsConnectedSubdomain(CtrlType* ctrl, GraphType* graph, int pid, int report)
{
  ...
  for (i = 0; i < nvtxs; i++)
  {
    if (where[i] == pid) nleft++;
  }

  for (i = 0; i < nvtxs; i++)
  {
    if (where[i] == pid) break;
  }
  ...
  while (first != nleft)
  {
    ...
    { /* Find another starting vertex */
      ...
      for (i = 0; i < nvtxs; i++)
      {
        if (where[i] == pid && !touched[i]) break;
      }
      queue[last++] = i;
      touched[i] = 1;
    }
  ...
  if (ncmps > 1 && report)
  {
    ...
    {
      wgt = 0;
      for (j = cptr[i]; j < cptr[i + 1]; j++) wgt += graph->vwgt[queue[j]];
      printf("[%5d %5d] ", cptr[i + 1] - cptr[i], wgt);
      /*
      if (cptr[i+1]-cptr[i] == 1)
        printf("[%d %d] ", queue[cptr[i]],
      xadj[queue[cptr[i]]+1]-xadj[queue[cptr[i]]]);
      */
    }
    printf("\n");
  }
...
int IsConnected2(GraphType* graph, int report)
{
  ...
  if (ncmps > 1 && report)
  {
    ...
    for (i = 0; i < ncmps; i++)
    {
      if (cptr[i + 1] - cptr[i] > 200) printf("[%5d] ", cptr[i + 1] - cptr[i]);
    }
    printf("\n");
  }
                   ...
                   idxtype* cind)
{
  ...
  for (i = 0; i < graph->nbnd; i++) touched[graph->bndind[i]] = 1;
