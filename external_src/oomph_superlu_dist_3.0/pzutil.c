...
/*! \brief Gather A from the distributed compressed row format to global A in
...
)
{
  ...
  zCompRow_to_CompCol_dist(
    m_loc, n, nnz_loc, a, colind, rowptr, &a_loc, &rowind_loc, &colptr_loc);
  ...
  MPI_Allgather(&fst_row, 1, mpi_int_t, fst_rows, 1, mpi_int_t, grid->comm);
...
/*! \brief Permute the distributed dense matrix: B <= perm(X). perm[i] = j means
 * the i-th row of X is in the j-th row of B.
 */
               ...
               SOLVEstruct_t* SOLVEstruct)
{
  ...
  /* ------------------------------------------------------------
     ...
     ------------------------------------------------------------*/
  if (!(itemp = intMalloc_dist(procs + 1))) ABORT("Malloc fails for itemp[]");
  MPI_Allgather(&fst_row, 1, mpi_int_t, itemp, 1, mpi_int_t, grid->comm);
  itemp[procs] = A->nrow;
  for (p = 0; p < procs; ++p)
  {
  ...
  if (!(SOLVEstruct->gsmv_comm =
          (pzgsmv_comm_t*)SUPERLU_MALLOC(sizeof(pzgsmv_comm_t))))
