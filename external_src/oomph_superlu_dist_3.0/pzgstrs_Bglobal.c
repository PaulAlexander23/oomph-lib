...
/*! @file
 * \brief Solves a system of distributed linear equations A*X = B with a general
 * N-by-N matrix A using the LU factorization
 *
 * <pre>
...
_fcd ftcs3;
#endif
static void gather_diag_to_all(int_t,
                               int_t,
                               doublecomplex[],
                               Glu_persist_t*,
                               LocalLU_t*,
                     ...
                     int* info)
{
  ...
  *info = 0;
  ...
  if (!(fmod = intMalloc_dist(nlb))) ABORT("Calloc fails for fmod[].");
  for (i = 0; i < nlb; ++i) fmod[i] = Llu->fmod[i];
  if (!(frecv = intMalloc_dist(nlb))) ABORT("Malloc fails for frecv[].");
  Llu->frecv = frecv;
  ...
  if (!(lsum =
          doublecomplexCalloc_dist(((size_t)ldalsum) * nrhs + nlb * LSUM_H)))
    ABORT("Calloc fails for lsum[].");
  if (!(x = doublecomplexMalloc_dist(((size_t)ldalsum) * nrhs + nlb * XK_H)))
    ...
    for (k = 0; k < nsupers; ++k)
    {
      ...
      if (myrow == krow)
      {
        ...
        if (mycol != kcol && fmod[lk])
          ...
          i = 0;
        MPI_Reduce(&i, &frecv[lk], 1, mpi_int_t, MPI_SUM, kcol, scp->comm);
        if (mycol == kcol)
        { /* Diagonal process. */
          nfrecvmod += frecv[lk];
          if (!frecv[lk] && !fmod[lk]) ++nleaf;
  ...
  /* Save the count to be altered so it can be used by
     subsequent call to PDGSTRS_BGLOBAL. */
  if (!(bmod = intMalloc_dist(nlb))) ABORT("Calloc fails for bmod[].");
  for (i = 0; i < nlb; ++i) bmod[i] = Llu->bmod[i];
  if (!(brecv = intMalloc_dist(nlb))) ABORT("Malloc fails for brecv[].");
  Llu->brecv = brecv;
    ...
    for (k = 0; k < nsupers; ++k)
    {
      ...
      if (myrow == krow)
      {
        ...
        if (mycol != kcol && bmod[lk])
          ...
          i = 0;
        MPI_Reduce(&i, &brecv[lk], 1, mpi_int_t, MPI_SUM, kcol, scp->comm);
        if (mycol == kcol)
        { /* Diagonal process. */
          nbrecvmod += brecv[lk];
          if (!brecv[lk] && !bmod[lk]) ++nroot;
  ...
  for (p = 0; p < Pr * Pc; ++p)
  {
    if (iam == p)
    {
      ...
      for (lb = 0; lb < nub; ++lb)
      {
        ...
        for (i = 0; i < Pr; ++i) printf("%4d", bsendx_plist[lb][i]);
      }
      printf("\n");
    }
    MPI_Barrier(grid->comm);
  ...
  /* Copy the solution X into B (on all processes). */
  {
    ...
    get_diag_procs(
      n, Glu_persist, grid, &num_diag_procs, &diag_procs, &diag_len);
    jj = diag_len[0];
                               ...
                               doublecomplex work[])
{
  ...
  for (p = 0; p < num_diag_procs; ++p)
  {
    ...
    {
      MPI_Bcast(
        work, diag_len[p] * nrhs, SuperLU_MPI_DOUBLE_COMPLEX, pkk, grid->comm);
    }
