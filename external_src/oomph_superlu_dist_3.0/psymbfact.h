 ...
 *
 * (xlsub,lsub): lsub[*] contains the compressed subscript of L, as described
 *above for Pslu_freeable_t.  This structure is used internally in
 *symbfact_dist. (xusub,usub): usub[*] contains the compressed subscript of U,
 *as described above for Pslu_freeable_t.  This structure is used internally in
 *symbfact_dist.
 *
...
typedef struct
{
  ...
  int_t
    nnz_asup_loc; /* Number of nonzeros in asup not yet consumed.  Used during
   symbolic factorization routine to determine how much
   of xusub, usub is still used to store the input matrix AS */
  int_t
    nnz_ainf_loc; /* Number of nonzeros in ainf.  Similar to nnz_asup_loc. */
...
/*! \brief Structure used for redistributing A for the symbolic factorization
 * algorithm */
typedef struct
{
