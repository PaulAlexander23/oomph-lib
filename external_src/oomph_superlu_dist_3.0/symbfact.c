...
static int_t snode_dfs(SuperMatrix*,
                       const int_t,
                       const int_t,
                       int_t*,
                       int_t*,
                       Glu_persist_t*,
                       Glu_freeable_t*);
static int_t column_dfs(SuperMatrix*,
                        const int_t,
                        int_t*,
                        int_t*,
                        int_t*,
                        ...
                        Glu_freeable_t*);
static int_t pivotL(
  const int_t, int_t*, int_t*, Glu_persist_t*, Glu_freeable_t*);
static int_t set_usub(const int_t,
                      const int_t,
                      const int_t,
                      int_t*,
                      int_t*,
  ...
  )
{
  ...
  if (!(desc = intMalloc_dist(n + 1))) ABORT("Malloc fails for desc[]");
  ;
  ...
  for (j = 0; j < min_mn;)
  {
    ...
    else
    {
      ...
      if ((info = set_usub(
             min_mn, j, nseg, segrep, repfnz, Glu_persist, Glu_freeable)) != 0)
        return info;
  ...
  for (i = jcol; i <= kcol; i++)
  {
    ...
    for (k = xa_begin[i]; k < xa_end[i]; ++k)
    {
      ...
      { /* First time visit krow */
        ...
        if (nextl >= nzlmax)
        {
          if (mem_error = symbfact_SubXpand(
                A->ncol, jcol, nextl, LSUB, &nzlmax, Glu_freeable))
            return (mem_error);
          lsub = Glu_freeable->lsub;
        }
      }
   ...
   Glu_freeable_t* Glu_freeable)
{
  ...
  {
    if (mem_error =
          symbfact_SubXpand(n, jcol, nextu, USUB, &nzumax, Glu_freeable))
      return (mem_error);
    usub = Glu_freeable->usub;
  }
