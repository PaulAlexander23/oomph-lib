                      ...
                      gridinfo_t* grid)
{
  ...
  if (!info) ABORT("C main program must explicitly call MPI_Init()");

  MPI_Comm_size(Bcomm, &info);
  if (info < Np) ABORT("Number of processes is smaller than NPROW * NPCOL");
...
/*! \brief All processes in the MPI communicator must call this routine.
 */
void superlu_gridmap(MPI_Comm Bcomm, /* The base communicator upon which
                      the new grid is formed. */
                     int_t nprow,
                     int_t npcol,
                     ...
                     gridinfo_t* grid)
{
  ...
  for (j = 0; j < npcol; ++j)
    for (i = 0; i < nprow; ++i) pranks[i * npcol + j] = usermap[j * ldumap + i];
