...
PRIVATE int init_rows_cols(
  int n_row, int n_col, RowInfo Row[], ColInfo Col[], int A[], int p[]);
...
PRIVATE int garbage_collection(
  int n_row, int n_col, RowInfo Row[], ColInfo Col[], int A[], int* pfree);
...
PRIVATE void debug_mark(int n_row, RowInfo Row[], int tag_mark, int max_mark);

PRIVATE void debug_matrix(
  int n_row, int n_col, RowInfo Row[], ColInfo Col[], int A[]);

PRIVATE void debug_structures(
  int n_row, int n_col, RowInfo Row[], ColInfo Col[], int A[], int n_col2);
...
#define DEBUG1(params) \
  { \
    if (debug_colamd >= 1) (void)printf params; \
  }
#define DEBUG2(params) \
  { \
    if (debug_colamd >= 2) (void)printf params; \
  }
#define DEBUG3(params) \
  { \
    if (debug_colamd >= 3) (void)printf params; \
  }
#define DEBUG4(params) \
  { \
    if (debug_colamd >= 4) (void)printf params; \
  }
  ...
  )
{
  ...
  ngarbage =
    find_ordering(n_row, n_col, Alen, Row, Col, A, p, n_col2, max_deg, 2 * nnz);
  ...
  )
{
  ...
  for (col = 0; col < n_col; col++)
  {
    ...
    while (cp < cp_end)
    {
      ...
      {
        DEBUG0(
          ("colamd error!  col %d row %d last_row %d\n", col, row, last_row));
        return (-1);
      }
  ...
  )
{
  ...
  for (k = 0; k < n_col2; /* 'k' is incremented below */)
  {
    ...
    debug_step++;
    debug_deg_lists(
      n_row, n_col, Row, Col, head, min_score, n_col2 - k, max_deg);
