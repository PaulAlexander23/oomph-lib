...
void get_metis(int_t n, /* dimension of matrix B */
...
#ifdef USEEND
  METIS_EdgeND(
    &nm, b_colptr_int, b_rowind_int, &numflag, metis_options, perm, iperm);
#else
  METIS_NodeND(
    &nm, b_colptr_int, b_rowind_int, &numflag, metis_options, perm, iperm);
 ...
 * </pre>
 */
void getata_dist(const int_t m, /* number of rows in matrix A. */
...
)
{
  ...
  if (!(marker =
          (int_t*)SUPERLU_MALLOC((SUPERLU_MAX(m, n) + 1) * sizeof(int_t))))
  ...
  for (j = 0; j < n; ++j)
  {
    for (i = colptr[j]; i < colptr[j + 1]; ++i) ++marker[rowind[i]];
  }
  t_colptr[0] = 0;
  for (i = 0; i < m; ++i)
  {
