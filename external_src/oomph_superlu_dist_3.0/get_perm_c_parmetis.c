...
static float a_plus_at_CompRow_loc(int,
                                   int_t*,
                                   int,
                                   int_t*,
                                   int_t,
...

{
  NRformat_loc* Astore;
  int iam, p;
  int *b_rowptr_int, *b_colind_int, *l_sizes_int, *dist_order_int,
    *vtxdist_o_int;
  int *options, numflag;
  ...
  float
    apat_mem_l; /* memory used during the computation of the graph of A+A' */
...
#if (PRNTlevel >= 1)
  if (!iam)
    printf(".. Use parMETIS ordering on A'+A with %d sub-domains.\n",
           noDomains);
#endif
  ...
  MPI_Allgather(&fst_row, 1, mpi_int_t, vtxdist_i, 1, mpi_int_t, grid->comm);
  vtxdist_i[nprocs_i] = m;

  if (noDomains == nprocs_i)
  {
    /* keep the same distribution of A */
    for (p = 0; p <= nprocs_i; p++) vtxdist_o[p] = vtxdist_i[p];
  }
  else
  {
    i = n / noDomains;
    ...
    for (p = noDomains; p <= nprocs_i; p++) vtxdist_o[p] = k;
  }

#if (DEBUGlevel >= 2)
  if (!iam) PrintInt10("vtxdist_o", nprocs_i + 1, vtxdist_o);
#endif

  /* Compute distributed A + A' */
  if ((apat_mem_l = a_plus_at_CompRow_loc(iam,
                                          perm_r,
                                          nprocs_i,
                                          vtxdist_i,
                                          n,
  ...
  for (i = 0; i <= m_loc; i++) b_rowptr_int[i] = b_rowptr[i];
  ...
  if (bnz)
  {
    ...
    for (i = 0; i < bnz; i++) b_colind_int[i] = b_colind[i];
    SUPERLU_FREE(b_colind);
  }

  if (!(vtxdist_o_int = (int*)SUPERLU_MALLOC((nprocs_i + 1) * sizeof(int))))
    ABORT("SUPERLU_MALLOC fails for vtxdist_o_int.");
  for (i = 0; i <= nprocs_i; i++) vtxdist_o_int[i] = vtxdist_o[i];
  SUPERLU_FREE(vtxdist_o);

#else /* Default */

  vtxdist_o_int = vtxdist_o;
  b_rowptr_int = b_rowptr;
  b_colind_int = b_colind;
  l_sizes_int = l_sizes;
  ...
  if (iam < noDomains)
  {
    ...
    ParMETIS_V3_NodeND(vtxdist_o_int,
                       b_rowptr_int,
                       b_colind_int,
                       &numflag,
                       options,
  ...
  if (bnz) SUPERLU_FREE(b_colind_int);
  if (iam < noDomains)
  {
  ...
  if (iam < noDomains)
  {
    ...
    for (i = 0; i < m_loc; i++) dist_order[i] = dist_order_int[i];
    SUPERLU_FREE(dist_order_int);

    for (i = 0; i < 2 * noDomains; i++) l_sizes[i] = l_sizes_int[i];
  ...
  displs[0] = 0;
  for (i = 1; i < nprocs_i; i++) displs[i] = displs[i - 1] + recvcnts[i - 1];

  MPI_Allgatherv(dist_order,
                 m_loc,
                 mpi_int_t,
                 perm_c,
                 recvcnts,
  ...
  if (noDomains <= iam && iam < nprocs_i)
    MPI_Recv(l_sizes, 2 * noDomains, mpi_int_t, 0, 0, grid->comm, &status);

  /* Determine the first node in each separator, store it in l_fstVtxSep */
  for (j = 0; j < 2 * noDomains; j++) l_fstVtxSep[j] = 0;
  ...
  while (i > 0)
  {
    for (j = i; j < i + szSep; j++)
    {
      ...
      l_fstVtxSep[k - 1] =
        l_fstVtxSep[k] + l_sizes[k] - noNodes - l_sizes[k - 1];
    }
    szSep *= 2;
    i -= szSep;
  }
...
static float a_plus_at_CompRow_loc(
  ...
  int nprocs_i, /* Input - number of processors the input matrix
       is distributed on */
  int_t* vtxdist_i, /* Input - index of first row on each processor of the input
                       matrix */
  ...
  int nprocs_o, /* Input - number of processors the output matrix
       is distributed on */
  int_t* vtxdist_o, /* Input - index of first row on each processor of the
                       output matrix */
  ...
  int_t**
    p_b_colind, /* Output - output matrix, column indices of size *p_bnz */
...
)
{
  ...
  {
    sendCnts[p] = 0;
    for (i = vtxdist_i[p]; i < vtxdist_i[p + 1]; i++) sendCnts[p] += marker[i];
  }
  /* exchange send/receive counts information in between all processors */
  MPI_Alltoall(sendCnts, 1, mpi_int_t, recvCnts, 1, mpi_int_t, grid->comm);
  sendCnts[iam] = 0;
  ...
  {
    if (!(tcolind_recv =
            (int_t*)SUPERLU_MALLOC(sz_tcolind_recv * sizeof(int_t))))
      ABORT("SUPERLU_MALLOC fails tcolind_recv[]");
    apat_mem += sz_tcolind_recv;
  }
  /* allocate memory to send blocks of local transpose matrix T to other
   * processors */
  if (sz_tcolind_send)
  {
    if (!(tcolind_send =
            (int_t*)SUPERLU_MALLOC((sz_tcolind_send) * sizeof(int_t))))
  ...
  for (p = 0; p < nprocs_i; p++) sdispls[p] = rdispls[p];
  ...
  if (apat_mem > apat_mem_max) apat_mem_max = apat_mem;

  /* Reset marker to EMPTY */
  for (i = 0; i < n; i++) marker[i] = EMPTY;
  /* restore rdispls information */
  for (p = 0; p < nprocs_i; p++) rdispls[p] = sdispls[p];
  ...
  for (p = 0; p <= SUPERLU_MIN(nprocs_i, nprocs_o); p++)
    if (vtxdist_i[p] != vtxdist_o[p]) redist_pra = TRUE;
  ...
  if (redist_pra)
  {
    ...
    for (i = 0; i < m_loc; i++)
    {
      ...
      while (!j)
      {
        if (vtxdist_o[p] <= k && k < vtxdist_o[p + 1]) j = TRUE;
        else
          p++;
      }
      if (p == iam)
    ...
    for (i = 0; i < m_loc; i++)
    {
      ...
      { /* remote */
        ...
        {
          tcolind_send[sdispls[p]] = b_colind_t[j];
          sdispls[p]++;
        }
      }
    }
    ...
    if (bnz_t) SUPERLU_FREE(b_colind_t);
    if (sz_tcolind_recv) SUPERLU_FREE(tcolind_recv);
