  ...
  *info = 0;
  ...
  if (!factored)
  {
    ...
    if (parSymbFact == NO || Fact == SamePattern_SameRowPerm)
    {
      ...
      t = SuperLU_timer_();
      dist_mem_use =
        pddistribute(Fact, n, A, ScalePermstruct, Glu_freeable, LUstruct, grid);
  ...
  if (nrhs)
  {
    ...
    if (options->IterRefine)
    {
      ...
      if (options->RefineInitialized == NO || Fact == DOFACT)
      {
        ...
        if (options->RefineInitialized) pdgsmv_finalize(SOLVEstruct->gsmv_comm);
