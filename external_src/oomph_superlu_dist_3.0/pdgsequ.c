             ...
             gridinfo_t* grid)
{
  ...
  *info = 0;
  if (A->nrow < 0 || A->ncol < 0 || A->Stype != SLU_NR_loc ||
  ...
  tempmax = rcmax;
  tempmin = rcmin;
  MPI_Allreduce(&tempmax, &rcmax, 1, MPI_DOUBLE, MPI_MAX, grid->comm);
  MPI_Allreduce(&tempmin, &rcmin, 1, MPI_DOUBLE, MPI_MIN, grid->comm);
  ...
  *amax = rcmax;
  ...
  if (!(loc_r = doubleMalloc_dist(m_loc))) ABORT("Malloc fails for loc_r[].");
  j = Astore->fst_row;
  ...
  MPI_Allgatherv(
    loc_r, m_loc, MPI_DOUBLE, r, r_sizes, displs, MPI_DOUBLE, grid->comm);
