                         ...
                         gridinfo_t* grid)
{
  ...
  int_t *nnzToSend, *nnzToRecv, *nnzToSend_l, *nnzToSend_u, *tmp_ptrToSend,
    *mem;
  ...
  int_t SendCnt_l, SendCnt_u, nnz_loc_l, nnz_loc_u, nnz_loc, RecvCnt_l,
    RecvCnt_u, ind_loc;
  ...
  if (nvtcs_loc > INT_MAX) ABORT("ERROR in dist_symbLU nvtcs_loc > INT_MAX\n");
  intNvtcs_loc = (int)nvtcs_loc;
  MPI_Gather(&intNvtcs_loc, 1, MPI_INT, nvtcs, 1, MPI_INT, 0, grid->comm);
  ...
  if (!iam)
  {
    ...
    for (j = 1; j < n; j++)
    {
      if (p != (int)OWNER(globToLoc[j]) || szsn >= maxszsn ||
          gb != temp[ptrToRecv[p]])
      {
        nsupers++;
  ...
  for (p = 0; p < 2 * nprocs; p++) nnzToSend[p] = 0;
  ...
  for (gb_n = 0; gb_n < nsupers; gb_n++)
  {
    ...
    if (iam == (int)OWNER(globToLoc[i]))
    {
      ...
      for (p = pr * grid->npcol; p < (pr + 1) * grid->npcol; p++)
        ...
        {
          if (p == p_diag && !send_2[p]) nnzToSend[2 * p + 1] += nsend_2;
          else
  ...
  MPI_Alltoall(nnzToSend, 2, mpi_int_t, nnzToRecv, 2, mpi_int_t, grid->comm);
  ...
  {
    if (!(rcv_luind = intMalloc_dist(
            SUPERLU_MAX(RecvCnt_l + nnz_loc_l, RecvCnt_u + nnz_loc_u))))
    {
      fprintf(stderr, "Malloc fails for rcv_luind[].");
      return (memAux + memRet);
    }
    memAux +=
      (float)SUPERLU_MAX(RecvCnt_l + nnz_loc_l, RecvCnt_u + nnz_loc_u) * iword;
  }
  ...
  while (sendL || sendU)
  {
    ...
    for (gb_n = 0; gb_n < nsupers; gb_n++)
    {
      ...
      if (iam == OWNER(globToLoc[i]))
      {
        ...
        for (j = xsub_s[fst_s_l]; j < xsub_s[fst_s_l + 1]; j++)
        {
          k = sub_s[j];
          if ((sendL && k >= i) ||
              (sendU && k >= i + xsup_n[gb_n + 1] - xsup_n[gb_n]))
          {
            gb = supno_n[k];
            if (sendL) p = PNUM(PROW(gb, grid), pc, grid);
            else
              p = PNUM(pr, PCOL(gb, grid), grid);
            if (send_1[p] == FALSE)
            {
    ...
    if (sendL) nnzToRecv[iam] = nnz_loc_l;
    else
    ...
    for (p = 0; p < nprocs; p++)
    {
      if (p != iam)
      {
        ...
        while (i < k + nnzToRecv[p])
        {
          gb = rcv_luind[i];
          nelts = rcv_luind[i + 1];
          if (sendL) xsub_n[LBj(gb, grid)] = nelts;
          else
 ...
 *        The type of A can be: Stype = SLU_NR_loc; Dtype = SLU_D; Mtype =
 *SLU_GE.
 *
                     ...
                     int_t* ilsum_j)
{
  ...
  /* All-to-all communication */
  MPI_Alltoall(nnzToSend, 1, mpi_int_t, nnzToRecv, 1, mpi_int_t, grid->comm);
  ...
  {
    if (!(send_req =
            (MPI_Request*)SUPERLU_MALLOC(2 * procs * sizeof(MPI_Request))))
    {
  ...
  for (p = 0; p < procs; ++p)
  {
    ...
    {
      it = 2 * nnzToSend[p];
      MPI_Isend(ia_send[p], it, mpi_int_t, p, iam, grid->comm, &send_req[p]);
      it = nnzToSend[p];
      MPI_Isend(aij_send[p],
                it,
                MPI_DOUBLE,
                p,
                iam + procs,
  ...
  for (p = 0; p < procs; ++p)
  {
    if (p != iam)
    {
      ...
      it = nnzToRecv[p];
      MPI_Recv(dtemp, it, MPI_DOUBLE, p, p + procs, grid->comm, &status);
  ...
  if (procs > 1)
  {
    ...
    memAux -= 2 * procs * sizeof(MPI_Request) + procs * sizeof(int_t*) +
              procs * sizeof(double*) + 2 * SendCnt * iword + SendCnt * dword +
              procs * iword + 2 * maxnnzToRecv * iword + maxnnzToRecv * dword;
  }
  ...
  for (j = ilsum_j[nsupers_j]; j > 0; j--) ainf_colptr[j] = ainf_colptr[j - 1];
  for (j = ilsum_i[nsupers_i]; j > 0; j--) asup_rowptr[j] = asup_rowptr[j - 1];
  ainf_colptr[0] = 0;
  asup_rowptr[0] = 0;
                       ...
                       gridinfo_t* grid)
{
  ...
  int_t bnnz, fsupc, i, irow, istart, j, jb, jj, k, len, len1, nsupc, nsupc_gb,
    ii, nprocs;
 ...
 the full array (local)                 */
  ...
  int_t iword, dword;
  float memStrLU, memA, memDist = 0.; /* memory used for redistributing the
                             data, which does not include the memory for the
                             numerical values of L and U (positive number)*/
  ...
  {
    ABORT("ERROR: call of dist_psymbtonum with fact equals "
          "SamePattern_SameRowPerm.");
  }

  if ((memStrLU = dist_symbLU(
         n, Pslu_freeable, Glu_persist, &xlsub, &lsub, &xusub, &usub, grid)) >
      0)
    return (memStrLU);
  memDist += (-memStrLU);
  ...
  if (!(Unzval_br_ptr = (double**)SUPERLU_MALLOC(nsupers_i * sizeof(double*))))
  {
  ...
  if (!(dense =
          doubleCalloc_dist(SUPERLU_MAX(ldaspa, ldaspa_j) * sp_ienv_dist(3))))
  {
  ...
  if (!(Lnzval_bc_ptr = (double**)SUPERLU_MALLOC(nsupers_j * sizeof(double*))))
  {
  ...
  for (jb = 0; jb < nsupers; jb++)
  {
    ...
    { /* Block row jb in my process row */
      ...
      for (j = ilsum[ljb_i], dense_col = dense; j < ilsum[ljb_i] + nsupc; j++)
      {
        ...
        {
          if (i >= asup_rowptr[ilsum[nsupers_i]]) printf("ERR7\n");
          jcol = asup_colind[i];
          if (jcol >= n)
          ...
          if (jcol >= ldaspa_j)
            printf(
              "Pe[%d] ERR1 jb %d gb %d j %d jcol %d\n", iam, jb, gb, j, jcol);
      ...
      for (i = xusub[ljb_i]; i < xusub[ljb_i + 1]; i++)
      {
        ...
        pr = PROW(gb, grid);
        if (pr != jbrow && mycol == pc) bsendx_plist[lb][jbrow] = YES;
        if (mycol == pc)
        {
          len += nsupc;
          LUb_length[lb] += nsupc;
      ...
      if (nrbu)
      {
        ...
        if (!(Unzval_br_ptr[ljb_i] = doubleMalloc_dist(len)))
        {
          fprintf(stderr, "Malloc fails for Unzval_br_ptr[*][]");
          return (memDist + memNLU);
        }
    ...
    { /* Block column jb in my process column */
      ...
      for (j = ilsum_j[ljb_j], dense_col = dense; j < ilsum_j[ljb_j] + nsupc;
           j++)
      {
      ...
      for (i = xlsub[ljb_j]; i < xlsub[ljb_j + 1]; i++)
      {
        ...
        if (pr != jbrow && fsendx_plist[ljb_j][pr] == EMPTY && myrow == jbrow)
        {
      ...
      { /* Do not ensure the blocks are sorted! */
        /* Set up the initial pointers for each block in
           index[] and nzval[]. */
        /* If I am the owner of the diagonal block, order it first in
           LUb_number. Necessary for SuperLU_DIST routines */
        kseen = EMPTY;
        for (j = 0; j < nrbl; j++)
        {
          if (LUb_number[j] == jb) kseen = j;
        }
        ...
        if (!(Lnzval_bc_ptr[ljb_j] = doubleMalloc_dist(len * nsupc)))
        {
          fprintf(
            stderr, "Malloc fails for Lnzval_bc_ptr[*][] col block %d ", jb);
          return (memDist + memNLU);
        }
  ...
  SUPERLU_FREE(xlsub);
  SUPERLU_FREE(xusub);
  if (lsub != NULL) SUPERLU_FREE(lsub);
  if (usub != NULL) SUPERLU_FREE(usub);
  ...
  for (p = 0; p < nprocs; p++) nnzToRecv[p] = 0;
  ...
  for (p = 0; p < nprocs; p++)
  {
    ...
    i += nnzToRecv[p];
    ptrToSend[p] = 0;
    if (p != iam) nnzToSend[p] = nnzToRecv[iam];
    else
      nnzToSend[p] = 0;
  }
  nnzToRecv[iam] = 0;
  ...
  for (jb = 0; jb < nsupers; jb++)
  {
    ...
    {
      ljb_j = LBj(jb, grid); /* Local block number column wise */
      for (j = 0; j < grid->npcol; j++, i++) recvBuf[i] = ToSendR[ljb_j][j];
    }
  }

  MPI_Alltoallv(&(recvBuf[ptrToRecv[iam]]),
                nnzToSend,
                ptrToSend,
                mpi_int_t,
                recvBuf,
  ...
  /* exchange information about bsendx_plist in between column of processors */
  MPI_Allreduce((*bsendx_plist),
                recvBuf,
                nsupers_j * grid->nprow,
                mpi_int_t,
                MPI_MAX,
                grid->cscp.comm);
    ...
    if (mycol == jbcol)
    {
      ...
      if (myrow == jbrow)
      {
        for (k = ljb_j * grid->nprow; k < (ljb_j + 1) * grid->nprow; k++)
        {
          ...
          if ((*bsendx_plist)[k] != EMPTY) nbsendx++;
        }
      }
      else
      {
  ...
  if (!(Llu->mod_bit = intMalloc_dist(k))) ABORT("Malloc fails for mod_bit[].");

  /* Find the maximum buffer size. */
  MPI_Allreduce(
    mybufmax, Llu->bufmax, NBUFFERS, mpi_int_t, MPI_MAX, grid->comm);
