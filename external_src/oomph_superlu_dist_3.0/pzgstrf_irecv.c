...
static void pzgstrs2(
  int_t, int_t, Glu_persist_t*, gridinfo_t*, LocalLU_t*, SuperLUStat_t*);
#endif
  ...
  for (k = 0; k < nsupers; ++k)
  {
    ...
    else
    {
      ...
      { /* Recv block column L(:,k). */
        ...
        /*probe_recv(iam, kcol, (4*k+1)%NTAGS, SuperLU_MPI_DOUBLE_COMPLEX,
          scp->comm, Llu->bufmax[1]);*/
...
#endif
            MPI_Send(
              usub, msgcnt[2], mpi_int_t, pi, (4 * k + 2) % NTAGS, scp->comm);
            MPI_Send(uval,
                     msgcnt[3],
                     SuperLU_MPI_DOUBLE_COMPLEX,
                     pi,
    ...
    { /* myrow != krow */
      ...
      { /* Recv block row U(k,:). */
        ...
        /*probe_recv(iam, krow, (4*k+3)%NTAGS, SuperLU_MPI_DOUBLE_COMPLEX,
          scp->comm, Llu->bufmax[3]);*/
        MPI_Recv(Uval_buf,
                 Llu->bufmax[3],
                 SuperLU_MPI_DOUBLE_COMPLEX,
                 krow,
...
#endif
        stat->ops[FACT] +=
          4 * segsize * (segsize + 1) + 10 * segsize; /* complex division */
        rukp += segsize;
      }
    }
  } /* for b ... */
