   ...
   SuperLUStat_t* stat)
{
  ...
  for (lb = 0; lb < nlb; ++lb)
  {
    ...
    if ((--fmod[lk]) == 0)
    { /* Local accumulation done. */
      ikcol = PCOL(ik, grid);
      p = PNUM(myrow, ikcol, grid);
      if (iam != p)
      {
#ifdef ISEND_IRECV
        MPI_Isend(&lsum[il - LSUM_H],
                  iknsupc * nrhs + LSUM_H,
   ...
   SuperLUStat_t* stat)
{
  ...
  int_t fnz, gik, gikcol, i, ii, ik, ikfrow, iklrow, il, irow, j, jj, lk, lk1,
    nub, ub, uptr;
  int_t* usub;
      ...
      for (jj = 0; jj < knsupc; ++jj)
      {
        ...
        { /* Nonzero segment. */
          ...
          for (irow = fnz; irow < iklrow; ++irow)
          {
            ...
            z_sub(&dest[irow - ikfrow], &dest[irow - ikfrow], &temp);
            ++uptr;
          }
