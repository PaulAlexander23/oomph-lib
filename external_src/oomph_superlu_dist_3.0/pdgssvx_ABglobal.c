 ...
 *         Stype = SLU_NC; Dtype = SLU_D; Mtype = SLU_GE. That is, A is stored
 *in compressed column format (also known as Harwell-Boeing format). See
 *supermatrix.h for the definition of 'SuperMatrix'. This routine only handles
 *square A, however, the LU factorization routine pdgstrf can factorize
 *rectangular matrices. On exit, A may be overwritten by
 *Pc*Pr*diag(R)*A*diag(C), depending on ScalePermstruct->DiagScale,
 *options->RowPerm and options->colpem: if ScalePermstruct->DiagScale !=
 *NOEQUIL, A is overwritten by diag(R)*A*diag(C). if options->RowPerm !=
 *NATURAL, A is further overwritten by Pr*diag(R)*A*diag(C). if options->ColPerm
 *!= NATURAL, A is further overwritten by Pc*Pr*diag(R)*A*diag(C). If all the
 *above condition are true, the LU decomposition is performed on the matrix
 ...
 *           Elimination tree of Pc*(A'+A)*Pc' or Pc*A'*A*Pc', dimension
 *A->ncol. It is computed in sp_colorder() during the first factorization, and
 *is reused in the subsequent factorizations of the matrices with the same
 *nonzero pattern. On exit of sp_colorder(), the columns of A are permuted so
 *that the etree is in a certain postorder. This postorder is reflected in
 *ScalePermstruct->perm_c. NOTE: Etree is a vector of parent pointers for a
 *forest whose vertices are the integers 0 to A->ncol-1; etree[root]==A->ncol.
 *
...
structures are not used.                                  */
  ...
  *info = 0;
  Fact = options->Fact;
  if (Fact < 0 || Fact > FACTORED) *info = -1;
      ...
      { /* job = 2,3,4 */
        for (j = 0; j < n; ++j)
        {
          for (i = colptr[j]; i < colptr[j + 1]; ++i)
          {
            ...
            if (rowind[i] == j)
            { /* New diagonal */
              if (job == 2 || job == 3) dmin = SUPERLU_MIN(dmin, fabs(a[i]));
              else if (job == 4)
                dsum += fabs(a[i]);
              else if (job == 5)
                dprod *= fabs(a[i]);
  ...
  if (!factored)
  {
...
#if (PRNTlevel >= 1)
    {
      ...
      MPI_Reduce(
        &num_mem_usage.for_lu, &for_lu, 1, MPI_FLOAT, MPI_SUM, 0, grid->comm);
      MPI_Reduce(
        &num_mem_usage.total, &total, 1, MPI_FLOAT, MPI_SUM, 0, grid->comm);
      temp = SUPERLU_MAX(symb_mem_usage.total,
                         symb_mem_usage.for_lu + (float)dist_mem_use +
      ...
      MPI_Allreduce(
        &stat->TinyPivots, &TinyPivots, 1, mpi_int_t, MPI_SUM, grid->comm);
  ...
  if (nrhs)
  {
    if (!(b_work = doubleMalloc_dist(n))) ABORT("Malloc fails for b_work[]");
    ...
    if (options->IterRefine)
    {
      ...
      t = SuperLU_timer_();
      pdgsrfs_ABXglobal(
        n, &AC, anorm, LUstruct, grid, B, ldb, X, ldx, nrhs, berr, stat, info);
