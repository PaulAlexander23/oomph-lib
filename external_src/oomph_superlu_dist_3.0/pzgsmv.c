...
)
{
  ...
  { /* Loop through each row */
    ...
    { /* Each nonzero in row i */
      ...
      { /* External */
        if (spa[jcol] == 0)
        { /* First time see this index */
          ++SendCounts[p];
          spa[jcol] = 1;
        }
      }
      else
      { /* Swap to beginning the part of A corresponding
to the local part of X */
  ...
  /* ------------------------------------------------------------
     ...
     ------------------------------------------------------------*/
  MPI_Alltoall(SendCounts, 1, MPI_INT, RecvCounts, 1, MPI_INT, grid->comm);
  ...
  for (p = 0; p < procs; ++p)
  {
    ...
    if (SendCounts[p])
    {
      MPI_Isend(&ind_tosend[ptr_ind_tosend[p]],
                SendCounts[p],
                mpi_int_t,
                p,
                iam,
    ...
    if (RecvCounts[p])
    {
      MPI_Irecv(&ind_torecv[ptr_ind_torecv[p]],
                RecvCounts[p],
                mpi_int_t,
                p,
                p,
  ...
  if (TotalIndSend && !(val_torecv = doublecomplexMalloc_dist(TotalIndSend)))
    ABORT("Malloc fails for val_torecv[].");
  if (TotalValSend && !(val_tosend = doublecomplexMalloc_dist(TotalValSend)))
...
)
{
  ...
  for (p = 0; p < procs; ++p)
  {
    if (RecvCounts[p])
    {
      MPI_Isend(&val_tosend[ptr_ind_torecv[p]],
                RecvCounts[p],
    ...
    if (SendCounts[p])
    {
      MPI_Irecv(&val_torecv[ptr_ind_tosend[p]],
                SendCounts[p],
                SuperLU_MPI_DOUBLE_COMPLEX,
                p,
                p,
