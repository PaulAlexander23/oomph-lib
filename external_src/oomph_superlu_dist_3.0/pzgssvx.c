 ...
 *permuted) o  all of ScalePermstruct o  all of LUstruct, including the actual
 *numerical values of L and U
 *
...
void pzgssvx(superlu_options_t* options,
             SuperMatrix* A,
...
structures are not used.                                  */
  ...
  *info = 0;
  ...
  if (!factored)
  {
    ...
    if (permc_spec != MY_PERMC && Fact == DOFACT)
    {
      if (permc_spec == PARMETIS)
      {
        ...
        flinfo = get_perm_c_parmetis(A,
                                     ...
                                     grid,
                                     &symb_comm);
        if (flinfo > 0) ABORT("ERROR in get perm_c parmetis.");
      }
      else
      {
    ...
    if (Fact != SamePattern_SameRowPerm)
    {
      ...
      { /* parallel symbolic factorization */
        t = SuperLU_timer_();
        flinfo = symbfact_dist(nprocs_num,
                               noDomains,
                               A,
                               perm_c,
                               perm_r,
      ...
      if (parSymbFact == NO) Destroy_CompCol_Permuted_dist(&GAC);
    ...
    if (parSymbFact == NO || Fact == SamePattern_SameRowPerm)
    {
      ...
      t = SuperLU_timer_();
      dist_mem_use =
        pzdistribute(Fact, n, A, ScalePermstruct, Glu_freeable, LUstruct, grid);
    ...
    else
    {
      ...
      t = SuperLU_timer_();
      dist_mem_use = zdist_psymbtonum(
        Fact, n, A, ScalePermstruct, &Pslu_freeable, LUstruct, grid);
      if (dist_mem_use > 0)
    ...
    if (options->PrintStat)
    {
     ...
     factorization */
        ...
        if (options->RowPerm != NO) temp = SUPERLU_MAX(temp, GA_mem_use);
      }
      else
      {
        temp =
          SUPERLU_MAX(symb_mem_usage.total + GA_mem_use, /* symbfact step */
  ...
  if (nrhs)
  {
    ...
    {
      zSolveInit(options, A, perm_r, perm_c, nrhs, LUstruct, grid, SOLVEstruct);
    }
    ...
    if (options->IterRefine)
    {
      ...
      if (options->RefineInitialized == NO || Fact == DOFACT)
      {
        ...
        if (options->RefineInitialized) pzgsmv_finalize(SOLVEstruct->gsmv_comm);
        pzgsmv_init(A, SOLVEstruct->row_to_proc, grid, SOLVEstruct->gsmv_comm);
...
So we use SOLVEstruct1 for the refinement step.
  */
        if (!(SOLVEstruct1 =
                (SOLVEstruct_t*)SUPERLU_MALLOC(sizeof(SOLVEstruct_t))))
        ...
        if (!(SOLVEstruct1->gstrs_comm =
                (pxgstrs_comm_t*)SUPERLU_MALLOC(sizeof(pxgstrs_comm_t))))
    ...
    pzPermute_Dense_Matrix(fst_row,
                           m_loc,
