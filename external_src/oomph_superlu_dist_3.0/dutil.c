...
int file_dPrint_CompRowLoc_Matrix_dist(FILE* fp, SuperMatrix* A)
{
  ...
  m_loc = Astore->m_loc;
  fprintf(
    fp, "nnz_loc %d, m_loc %d, fst_row %d\n", nnz_loc, m_loc, Astore->fst_row);
...
void dCopy_Dense_Matrix_dist(
  int_t M, int_t N, double* X, int_t ldx, double* Y, int_t ldy)
{
  /*! \brief
   *
   * <pre>
                   ...
                   int_t ldb)
{
  ...
  sp_dgemm_dist(
    trans, "N", A->nrow, nrhs, A->ncol, one, A, x, ldx, zero, rhs, ldb);
}
  ...
  for (lb = 0; lb < ncb; ++lb)
  {
    ...
    { /* Not an empty column */
      ...
      for (c = 0, k = BC_HEADER, r = 0; c < nb; ++c)
      {
        len = index[k + 1];
        printf(
          "(%d) row-block %d: block # %d\tlength %d\n", iam, c, index[k], len);
                     ...
                     gridinfo_t* grid)
{
  ...
  fprintf(fp,
          "TotalIndSend %d\tTotalValSend %d\n",
          gsmv_comm->TotalIndSend,
          gsmv_comm->TotalValSend);
  file_PrintInt10(fp, "extern_start", m_loc, gsmv_comm->extern_start);
  file_PrintInt10(
    fp, "ind_tosend", gsmv_comm->TotalIndSend, gsmv_comm->ind_tosend);
  file_PrintInt10(
    fp, "ind_torecv", gsmv_comm->TotalValSend, gsmv_comm->ind_torecv);
...
void GenXtrueRHS(int nrhs,
                 ...
                 int* ldb)
{
  int_t gb, gbrow, i, iam, irow, j, lb, lsup, myrow, n, nlrows, nsupr, nsupers,
    rel;
  ...
  *ldb = 0;
  ...
  if (!(lxsup = intMalloc_dist(lb))) ABORT("Malloc fails for lxsup[].");
  ...
  *ldx = n;
  ...
  if (!(bb = doubleCalloc_dist(*ldb * nrhs))) ABORT("Calloc fails for bb[].");
  for (j = 0; j < nrhs; ++j)
