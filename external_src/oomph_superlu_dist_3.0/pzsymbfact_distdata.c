 ...
 *        The type of A can be: Stype = SLU_NR_loc; Dtype = SLU_Z; Mtype =
 *SLU_GE.
 *
                     ...
                     int_t* ilsum_j)
{
  ...
  if (procs > 1)
  {
    ...
    if (!(aij_send =
            (doublecomplex**)SUPERLU_MALLOC(procs * sizeof(doublecomplex*))))
    {
  ...
  if (procs > 1)
  {
    ...
    memAux -= 2 * procs * sizeof(MPI_Request) + procs * sizeof(int_t*) +
              ...
              SendCnt * dword + procs * iword + 2 * maxnnzToRecv * iword +
              maxnnzToRecv * dword;
  }
  ...
  for (jb = 0; jb < nsupers; jb++)
  {
    ...
    { /* Block row jb in my process row */
      ...
      if (nrbu)
      {
        ...
        if (!(Unzval_br_ptr[ljb_i] = doublecomplexMalloc_dist(len)))
        {
          fprintf(stderr, "Malloc fails for Unzval_br_ptr[*][]");
          return (memDist + memNLU);
        }
    ...
    { /* Block column jb in my process column */
      ...
      { /* Do not ensure the blocks are sorted! */
        ...
        if (!(Lnzval_bc_ptr[ljb_j] = doublecomplexMalloc_dist(len * nsupc)))
        {
          fprintf(
