 ...
 * Internal prototypes
 */
static void pzgstrf2(superlu_options_t*,
                     int_t,
                     int_t,
                     int_t,
                     double,
...
static void pzgstrs2(
  int_t, int_t, int_t, Glu_persist_t*, gridinfo_t*, LocalLU_t*, SuperLUStat_t*);
#endif
              ...
              int* info)
{
  ...
  int_t fnz, i, ib, ijb, ilst, it, iukp, jb, jj, klst, knsupc, lb, lib, ldv,
    ljb, lptr, lptr0, lptrj, luptr, luptr0, luptrj, nlb, nub, nsupc, rel, rukp,
    il, iu;
  int_t Pc, Pr;
  ...
  int_t* iperm_u;
#endif
  int *msgcnt, **msgcnts, **msgcntsU, k0, kk0, kk1, kk2, jj0, iukp0, rukp0,
    flag0, flag1, *factored, *factoredU, nnodes, *sendcnts, *sdispls, *recvcnts,
    *rdispls, *srows, *rrows;
  ...
  *info = 0;
  ...
  if (!(Lval_buf_2 =
          SUPERLU_MALLOC((1 + num_look_aheads) * sizeof(doublecomplex*))))
  ...
  if (!(recv_reqs_u =
          SUPERLU_MALLOC((1 + num_look_aheads) * sizeof(MPI_Request*))))
   ...
   * ================================================== */
  if (options->lookahead_etree ==
        YES && /* use e-tree of symmetrized matrix, and      */
      (options->ParSymbFact ==
         NO || /* 1) symmetric fact with serial symbolic, or */
       (options->SymPattern == YES && /* 2) symmetric pattern, and */
        options->RowPerm ==
          NOROWPERM))) /*    no rowperm to destroy the symmetry      */
  {
    ...
    { /* ParSymbFACT==YES and SymPattern==YES  and RowPerm == NOROWPERM */
      ...
      etree_supno = SUPERLU_MALLOC(nsupers * sizeof(int_t));
      MPI_Allreduce(
        etree_supno_l, etree_supno, nsupers, mpi_int_t, MPI_MIN, grid->comm);
  ...
  { /* Unsymmetric pattern */
...
#else
        qsort(&(Lblock[nblocks - Lrows[ncb]]),
              (size_t)(Lrows[ncb]),
              sizeof(int_t),
              &superlu_sort_perm);
#endif
    ...
    MPI_Allreduce(
      sf_block_l, sf_block, nsupers, mpi_int_t, MPI_MIN, grid->comm);
    SUPERLU_FREE(sf_block_l);

    /* count number of nodes in DAG (i.e., the number of blocks on and above the
     * first match) */
  ...
  MPI_Allreduce(
    look_ahead_l, look_ahead, nsupers, mpi_int_t, MPI_MAX, grid->comm);
  ...
  if (!iam)
  {
    ...
    printf(".. Buffer size: Lsub %ld\tLval %ld\tUsub %ld\tUval %ld\tLDA %ld\n",
           (long int)Llu->bufmax[0],
           (long int)Llu->bufmax[1],
           (long int)Llu->bufmax[2],
           (long int)Llu->bufmax[3],
  ...
  if (!(indirect = intMalloc_dist(ldt))) ABORT("Malloc fails for indirect[].");
  ...
  if (mycol == kcol)
  {
    ...
    for (pj = 0; pj < Pc; ++pj)
    {
      if (ToSendR[lk][pj] != EMPTY)
      {
        ...
        TIC(t1);
#endif
        MPI_Isend(lsub, msgcnt[0], mpi_int_t, pj, 0, scp->comm, &send_req[pj]);
        MPI_Isend(lusup,
                  msgcnt[1],
...
#define IRECV_U
  ...
  for (k0 = 0; k0 < nsupers; ++k0)
  {
    ...
    /* tt1 = SuperLU_timer_(); */
    if (k0 == 0)
    { /* look-ahead all the column in the window */
      kk1 = k0 + 1;
      kk2 = SUPERLU_MIN(k0 + num_look_aheads, nsupers - 1);
    }
    else
    { /* look-ahead the new column */
    ...
    for (kk0 = kk1; kk0 <= kk2; kk0++)
    {
      ...
      if (look_ahead[kk] < k0)
      { /* dose not depends on the current column */
        ...
        { /* Post Recv of block column L(:,k+1). */
          /* double ttt1 = SuperLU_timer_(); */
          if (ToRecv[kk] >= 1)
          {
            scp = &grid->rscp; /* The scope of process row. */
...
#ifdef _CRAY
            pzgstrs2(
              n, kk0, kk, Glu_persist, grid, Llu, stat, ftcs1, ftcs2, ftcs3);
#else
...
#ifdef ISORT
        iperm_u[jj] =
          iperm_c_supno[usub[iukp]]; /* Global block number of block U(k,j). */
        perm_u[jj] = jj;
#else
        perm_u[2 * jj] =
          iperm_c_supno[usub[iukp]]; /* Global block number of block U(k,j). */
              ...
              for (jj = 0; jj < nsupc; ++jj)
              {
                ...
                if (segsize)
                {
                  ...
                  for (it = 0, i = 0; i < nbrow; ++i)
                  {
                    rel = lsub[lptr + i] - fnz;
                    z_sub(
                      &nzval[indirect[rel]], &nzval[indirect[rel]], &tempv[it]);
                    ++it;
                  }
                  tempv += ldt;
                }
            ...
            for (jj = 0; jj < nsupc; ++jj)
            {
              ...
              if (segsize)
              {
                ...
                for (i = 0; i < nbrow; ++i)
                {
                  rel = lsub[lptr + i] - fnz;
                  z_sub(
                    &nzval[indirect[rel]], &nzval[indirect[rel]], &tempv[i]);
                }
                tempv += ldt;
              }
              nzval += ldv;
  ...
  if (Pr * Pc > 1)
  {
    ...
    if (U_diag_blk_send_req[myrow])
    {
      ...
      for (krow = 0; krow < Pr; ++krow)
        if (krow != myrow) MPI_Wait(U_diag_blk_send_req + krow, &status);
    }
 ...
 * </pre>
 */
static void pzgstrf2(superlu_options_t* options,
                     int_t nsupers,
                     int_t k0,
                     int_t k,
                     double thresh,
  ...
  { /* diagonal process */
    ...
    { /* for each column in panel */
      ...
      if (options->ReplaceTinyPivot == YES)
      {
        ...
        { /* Diagonal */
#if (PRNTlevel >= 2)
          printf("(%d) .. col %d, tiny pivot %e  ", iam, jfst + j, lusup[i]);
#endif
 ...
 */
{
  ...
  {
    nsupr =
      Llu->Lsub_buf_2[k0 % (1 + stat->num_look_aheads)][1]; /* LDA of lusup[] */
...
#else
        ztrsv_(
          "L", "N", "U", &segsize, &lusup[luptr], &nsupr, &uval[rukp], &incx);
#endif
