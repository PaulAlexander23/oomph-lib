...
/*! \brief Deallocate the structure pointing to the actual storage of the
 * matrix. */
...
void LUstructInit(const int_t m, const int_t n, LUstruct_t* LUstruct)
{
  ...
  if (!(LUstruct->Glu_persist =
          (Glu_persist_t*)SUPERLU_MALLOC(sizeof(Glu_persist_t))))
    ABORT("Malloc fails for Glu_persist_t.");
  if (!(LUstruct->Llu = (LocalLU_t*)SUPERLU_MALLOC(sizeof(LocalLU_t))))
 ...
 */
void print_panel_seg_dist(
  int_t n, int_t w, int_t jcol, int_t nseg, int_t* segrep, int_t* repfnz)
{
  int_t j, k;
                ...
                gridinfo_t* grid)
{
  ...
  MPI_Reduce(&ops[FACT], &flopcnt, 1, MPI_FLOAT, MPI_SUM, 0, grid->comm);
  factflop = flopcnt;
  if (!iam && options->Fact != FACTORED)
  {
  ...
  MPI_Reduce(&ops[SOLVE], &flopcnt, 1, MPI_FLOAT, MPI_SUM, 0, grid->comm);
  solveflop = flopcnt;
  if (!iam)
  {
