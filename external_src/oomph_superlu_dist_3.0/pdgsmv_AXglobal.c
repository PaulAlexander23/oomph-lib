...
individual rows.  */
    ...
    for (p = 0; p < nprocs; ++p)
    {
      ...
      { /* First t2 processes will get one more row. */
        ++t1; /* Number of rows. */
      ...
      if (t1)
      {
        for (i = supno[t2]; i <= supno[t3]; ++i)
        {
          ...
          {
            fprintf(stderr, "mv_sup_to_proc conflicts at supno %d\n", i);
            exit(-1);
          }
#endif
        }
    ...
    for (i = Astore->colbeg[j]; i < Astore->colend[j]; ++i)
    {
      ...
      if (irow >= lo && irow <= hi)
      {
        if (irow != j) /* Exclude diagonal */
          ...
          ++nnz_diag; /* Count nonzero diagonal entries */
        ++nnz_local;
      }
    }
  }
 ...
 */
int pdgsmv_AXglobal(
  int_t m, int_t update[], double val[], int_t bindx[], double X[], double ax[])
{
  int_t i, j, k;
 ...
 */
static void dPrintMSRmatrix(int m, /* Number of rows of the submatrix. */
                            double val[],
                            int_t bindx[],
                            gridinfo_t* grid)
{
