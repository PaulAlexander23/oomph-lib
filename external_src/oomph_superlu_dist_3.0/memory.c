...
void superlu_free_dist(void* addr)
{
  ...
  if (!addr) ABORT("superlu_free: tried to free NULL pointer");

  if (!p) ABORT("superlu_free: tried to free NULL+DWORD pointer");

  {
    int_t n = ((int_t*)p)[0];

    if (!n) ABORT("superlu_free: tried to free a freed pointer");
    *((int_t*)p) = 0; /* Set to zero to detect duplicate free's. */
    ...
    if (superlu_malloc_total < 0) ABORT("superlu_malloc_total went negative");
  ...
  { /* MemModel == USER */
    ...
    else
    {
      ...
      if (type != USUB)
      {
        ...
        bytes_to_copy =
          (char*)stack.array + stack.top1 - (char*)expanders[type + 1].mem;
...
/************************************************************************/
{
  ...
  mem_usage->for_lu = (float)((4 * n + 3) * iword + lsub_size * iword);
  mem_usage->for_lu +=
    (float)((n + 1) * iword + Glu_freeable->xusub[n] * iword);
...
static int_t memory_usage(const int_t nzlmax, const int_t nzumax, const int_t n)
{
  register int_t iword = sizeof(int_t);
