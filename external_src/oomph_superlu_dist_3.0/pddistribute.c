                      ...
                      double* a[])
{
  ...
  if (k)
  { /* count can be zero. */
    if (!(ia = intMalloc_dist(2 * k))) ABORT("Malloc fails for ia[].");
    if (!(aij = doubleMalloc_dist(k))) ABORT("Malloc fails for aij[].");
  }
  ja = ia + k;
  ...
  if (!(*colptr = intCalloc_dist(n + 1))) ABORT("Malloc fails for *colptr[].");
  ...
  { /* nnz_loc can be zero */
    ...
    if (!(*a = doubleMalloc_dist(nnz_loc))) ABORT("Malloc fails for *a[].");
  }
 ...
 */
{
  ...
  int_t bnnz, fsupc, fsupc1, i, ii, irow, istart, j, jb, jj, k, len, len1,
    nsupc;
 ...
 the full array (local)                 */
  ...
  dReDistribute_A(
    A, ScalePermstruct, Glu_freeable, xsup, supno, grid, &xa, &asub, &a);
          ...
          for (i = xa[j]; i < xa[j + 1]; ++i)
          {
            ...
            if (myrow == PROW(gb, grid))
            {
              ...
              { /* in U */
                ...
                for (jj = 0; jj < k; ++jj) len += fsupc1 - index[istart++];
 ...
 ------------------------------------------------------------*/
    ...
    j = k * grid->npcol;
    if (!(index = intMalloc_dist(j))) ABORT("Malloc fails for index[].");
#if (PRNTlevel >= 1)
    ...
    if (!(ToSendD = intCalloc_dist(k))) ABORT("Malloc fails for ToSendD[].");
    if (!(ilsum = intMalloc_dist(k + 1))) ABORT("Malloc fails for ilsum[].");
    ...
    if (!(Ucbs = intCalloc_dist(k))) ABORT("Calloc fails for Ucbs[].");
#if (PRNTlevel >= 1)
    ...
    if (!(fmod = intCalloc_dist(k))) ABORT("Calloc fails for fmod[].");
    if (!(bmod = intCalloc_dist(k))) ABORT("Calloc fails for bmod[].");
      ...
      { /* Block column jb in my process column */
        ...
        for (j = fsupc; j < FstBlockC(jb + 1); ++j)
        {
          ...
          for (i = istart; i < xusub[j + 1]; ++i)
          {
            ...
            gb = BlockNum(irow);
            pr = PROW(gb, grid);
            if (pr != jbrow && myrow == jbrow && /* diag. proc. owning jb */
...
the block       */
                ...
                index[len - 1] = 0;
                for (k = 0; k < nsupc; ++k) index[len + k] = fsupc1;
        ...
        { /* Do not ensure the blocks are sorted! */
          ...
          if (!(Lnzval_bc_ptr[ljb] = doubleMalloc_dist(len * nsupc)))
          {
