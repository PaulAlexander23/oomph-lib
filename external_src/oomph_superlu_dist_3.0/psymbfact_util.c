   ...
   psymbfact_stat_t* PS)
{
  ...
  if (min_new_len > 0 && new_len < min_new_len) new_len = min_new_len;
  ...
  if (new_mem)
  {
    if (len_tcopy_fbeg != 0) copy_mem_int(len_tcopy_fbeg, prev_mem, new_mem);
    if (len_tcopy_fend != 0)
      copy_mem_int(len_tcopy_fend,
                   &(prev_mem[prev_len - len_tcopy_fend]),
   ...
   psymbfact_stat_t* PS)
{
  ...
  len_tcopy_fbeg = next;
  if (fstVtx_nextLvl == n) fstVtx_nextLvl_lid = nvtcs_loc;
  else
  ...
  if (mem_type == LSUB)
  {
    ...
    if (rout_type == DOMAIN_SYMB) prev_xsub_nextLvl = xsub[vtxXp_lid + 1];
    else
  ...
  {
    fprintf(stderr,
            "Pe[%d] Can't exp MemType %d: prv_len %d min_new %d new_l %d\n",
            iam,
            mem_type,
            prev_len,
            min_new_len,
  ...
  {
    if (rout_type == DOMAIN_SYMB) vtx_lid = vtxXp_lid + 1;
    else
    {
   ...
   psymbfact_stat_t* PS)
{
  ...
  vtxXp_lid = LOCAL_IND(globToLoc[vtxXp]);
  if (fstVtx_nextLvl == n) fstVtx_nextLvl_lid = VInfo->nvtcs_loc;
  else
    fstVtx_nextLvl_lid = LOCAL_IND(globToLoc[fstVtx_nextLvl]);
  ...
  else
  {
    ...
    if (nelts == 0) nelts = 1;
    len_texp = xsub[fstVtx_nextLvl_lid] - xsub[vtxXp_lid + 1] + 4 * nelts;
  ...
  if (prev_len + len_texp >= prev_xsub_nextLvl)
  {
    ...
    if (mem_error = psymbfact_LUXpandMem(iam,
                                         ...
                                         PS))
      return (mem_error);
    if (mem_type == LSUB) new_mem = Llu_symbfact->lsub;
    else if (mem_type == USUB)
  ...
  if (rout_type == LL_SYMB)
  {
    ...
    xsub[vtx_lid + 1] = i;
    k = *p_next;
    if (k < xsub[vtx_lid + 1]) new_mem[k] = EMPTY;
  }
  ...
  if (free_prev_mem && new_mem != prev_mem) SUPERLU_FREE(prev_mem);
   ...
   psymbfact_stat_t* PS)
{
  ...
  else if (mem_type == USUB)
  {
    ...
    ABORT("Tries to expand nonexisting memory type.\n");
  ...
  if (new_mem != prev_mem) SUPERLU_FREE(prev_mem);
  Llu_symbfact->no_expcp++;
   ...
   psymbfact_stat_t* PS)
{
  ...
  new_mem =
    expand(prev_len, min_new_len, prev_mem, &new_len, len_tcopy_fbeg, 0, PS);
