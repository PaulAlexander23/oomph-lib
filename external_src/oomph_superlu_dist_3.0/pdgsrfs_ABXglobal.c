...
static void gather_1rhs_diag_to_all(int_t,
                                    ...
                                    double[],
                                    double[]);
static void redist_all_to_diag(
  int_t, double[], Glu_persist_t*, LocalLU_t*, gridinfo_t*, int_t[], double[]);
...
#define ITMAX 20
  ...
  double *b, *ax, *R, *B_col, *temp, *work, *X_col, *x_trs, *dx_trs;
  int_t count, ii, j, jj, k, knsupc, lk, lwork, nprow, nsupers, nz, p;
  int i, iam, pkk;
  int_t *ilsum, *xsup;
  double eps, lstres;
  ...
  /*-- Function prototypes --*/
  extern void pdgstrs1(
    int_t, LUstruct_t*, gridinfo_t*, double*, int, SuperLUStat_t*, int*);
  extern double dlamch_(char*);

  /* Test the input parameters. */
  *info = 0;
  if (n < 0) *info = -1;
  else if (A->nrow != A->ncol || A->nrow < 0 || A->Stype != SLU_NCP ||
           A->Dtype != SLU_D || A->Mtype != SLU_GE)
    *info = -2;
  else if (ldb < SUPERLU_MAX(0, n))
    *info = -10;
  else if (ldx < SUPERLU_MAX(0, n))
    *info = -12;
  else if (nrhs < 0)
    *info = -13;
  if (*info != 0)
  ...
  pdgsmv_AXglobal_setup(
    A, Glu_persist, grid, &N_update, &update, &val, &bindx, mv_sup_to_proc);
  ...
  lwork = N_update /* For ax and R */
          ...
          + k /* For b */
          + jj; /* for temp */
  if (!(work = doubleMalloc_dist(lwork))) ABORT("Malloc fails for work[]");
  ax = R = work;
...
#if (DEBUGlevel >= 1)
  if (!iam)
    printf(".. eps = %e\tanorm = %e\tsafe1 = %e\tsafe2 = %e\n",
           eps,
           anorm,
           safe1,
           safe2);
  ...
  for (j = 0; j < nrhs; ++j)
  {
    ...
    { /* Loop until stopping criterion is satisfied. */
      ...
      if (berr[j] > eps && berr[j] * 2 <= lstres && count < ITMAX)
      {
        /* Compute new dx. */
        redist_all_to_diag(
          n, R, Glu_persist, Llu, grid, mv_sup_to_proc, dx_trs);
        ...
        for (p = 0; p < num_diag_procs; ++p)
          if (iam == diag_procs[p])
            for (k = p; k < nsupers; k += num_diag_procs)
            {
              ...
              ii = ilsum[lk] + (lk + 1) * XK_H;
              knsupc = SuperSize(k);
              for (i = 0; i < knsupc; ++i) x_trs[i + ii] += dx_trs[i + ii];
            }
        lstres = berr[j];
        ++count;
        /* Transfer x_trs (on diagonal processes) into X
           (on all processes). */
        gather_1rhs_diag_to_all(n,
                                x_trs,
                                Glu_persist,
                                Llu,
                                grid,
                               ...
                               double work[])
{
  ...
  for (k = 0; k < nsupers; ++k)
  {
    ...
    if (iam == psrc)
    {
      if (iam != pkk)
      { /* Send X component. */
        MPI_Send(&r[lr], knsupc, MPI_DOUBLE, pkk, Xk, grid->comm);
      }
      else
      { /* Local copy. */
        for (i = 0; i < knsupc; ++i) work[i + ii] = r[i + lr];
      }
      lr += knsupc;
    }
    else
    {
      if (iam == pkk)
      { /* Recv X component. */
        MPI_Recv(&work[ii], knsupc, MPI_DOUBLE, psrc, Xk, grid->comm, &status);
      }
    }
  }
} /* REDIST_ALL_TO_DIAG */
