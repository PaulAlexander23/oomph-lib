...
static void estimate_memUsage(int_t,
                              int,
                              mem_usage_t*,
                              float*,
                              float*,
...
static void symbfact_free(
  int, int, Llu_symbfact_t*, vtcsInfo_symbfact_t*, comm_symbfact_t*);

static int_t denseSep_symbfact(int,
                               int_t,
                               int,
                               int,
                               int,
                               ...
                               MPI_Comm*,
                               Llu_symbfact_t*,
                               Pslu_freeable_t* _freeable,
                               vtcsInfo_symbfact_t*,
                               comm_symbfact_t*,
                               psymbfact_stat_t*);

static int_t dnsUpSeps_symbfact(int_t,
                                int,
                                int,
                                int,
                                int,
...
static void intraLvl_symbfact(SuperMatrix*,
                              int,
                              int,
                              int,
                              int,
...
static void initLvl_symbfact(int_t,
                             int,
                             int_t,
                             int_t,
                             Pslu_freeable_t*,
...
static void domain_symbfact(SuperMatrix*,
                            int,
                            int,
                            int,
                            int,
...
static float allocPrune_domain(
  int_t, int_t, Llu_symbfact_t*, vtcsInfo_symbfact_t*, psymbfact_stat_t*);

static float allocPrune_lvl(Llu_symbfact_t*,
                            vtcsInfo_symbfact_t*,
                            psymbfact_stat_t*);

static int symbfact_alloc(int_t,
                          int,
                          Pslu_freeable_t*,
                          Llu_symbfact_t*,
                          vtcsInfo_symbfact_t*,
...
static float symbfact_mapVtcs(int,
                              int,
                              int,
                              SuperMatrix*,
                              int_t*,
...
static void symbfact_distributeMatrix(int,
                                      int,
                                      int,
                                      SuperMatrix*,
                                      int_t*,
...
static int_t interLvl_symbfact(SuperMatrix*,
                               int,
                               int,
                               int,
                               int,
   ...
   mem_usage_t* symb_mem_usage)
{
   ...
   *   for performing the supernodal Gaussian elimination with no pivoting
   *(GEPP). This routine computes the structure of one column of L and one row
   *of U at a time.  It uses: o distributed input matrix o supernodes o
  ...
  Llu_symbfact_t
    Llu_symbfact; /* local L and U and pruned L and U data structures */
  vtcsInfo_symbfact_t VInfo; /* local information on number of blocks,
        number of vertices in a block etc */
  matrix_symbfact_t
    AS; /* temporary storage for the input matrix after redistribution */
  ...
  {
    if (!(commLvls =
            (MPI_Comm*)SUPERLU_MALLOC(2 * nprocs_symb * sizeof(MPI_Comm))))
    {
...
#if (PROFlevel >= 1)
  time_lvlsT =
    (double*)SUPERLU_MALLOC(3 * nprocs_symb * (nlvls + 1) * sizeof(double));
  ...
  PS.allocMem +=
    (3 * nprocs_symb * (nlvls + 1) + 3 * (nlvls + 1)) * sizeof(double);
#endif
  ...
  if ((flinfo = symbfact_mapVtcs(iam,
                                 nprocs_num,
                                 nprocs_symb,
                                 A,
                                 fstVtxSep,
  ...
  if (iam < nprocs_symb)
  {
    ...
    if (iinfo = symbfact_alloc(
          n, nprocs_symb, Pslu_freeable, &Llu_symbfact, &VInfo, &CS, &PS))
      return (PS.allocMem);
    ...
    for (i = 0; i < n; i++) tempArray[i] = EMPTY;
      ...
      for (jSep = iSep; jSep < iSep + szSep; jSep++)
      {
        ...
        if (szSep == nprocs_symb)
        {
          ...
          if (fstP == iam)
          {
            ...
            t1 = SuperLU_timer_();
#endif
            if ((flinfo = allocPrune_domain(
                   fstVtx, lstVtx, &Llu_symbfact, &VInfo, &PS)) > 0)
              return (flinfo);
            if (fstVtx < lstVtx) VInfo.fstVtx_nextLvl = VInfo.begEndBlks_loc[2];
    ...
    fill_rcmd =
      SUPERLU_MAX(nextl / (nnz_ainf_loc + 1), nextu / (nnz_asup_loc + 1)) + 1;
    ...
    for (ind_blk = 0; ind_blk < VInfo.nblks_loc; ind_blk++)
    {
      ...
      szsn = 1;
      if (INT_MAX - nnzL <=
          Llu_symbfact.xlsub[fstVtx_lid + 1] - Llu_symbfact.xlsub[fstVtx_lid])
        printf("PE[%d] ERR nnzL %d\n", iam, nnzL);
      if (INT_MAX - nnzU <=
          Llu_symbfact.xusub[fstVtx_lid + 1] - Llu_symbfact.xusub[fstVtx_lid])
      ...
      for (vtx = fstVtx + 1, vtx_lid = fstVtx_lid + 1; vtx < lstVtx;
           vtx++, vtx_lid++)
      {
    ...
    MPI_Reduce(stat_loc, stat_glob, 23, MPI_FLOAT, MPI_SUM, 0, (*symb_comm));
    MPI_Reduce(
      &(stat_loc[5]), mem_glob, 14, MPI_FLOAT, MPI_MAX, 0, (*symb_comm));
...
#if (PROFlevel >= 1)
    MPI_Reduce(
      t_symbFact_loc, t_symbFact, 3, MPI_DOUBLE, MPI_MAX, 0, (*symb_comm));
    MPI_Gather(time_lvls,
               3 * nlvls,
               MPI_DOUBLE,
               time_lvlsT,
               3 * nlvls,
    ...
    if (PS.maxsz_msgSnd < PS.maxsz_msgCol) stat_msgs_l[1] = PS.maxsz_msgCol;
    stat_msgs_l[2] =
      PS.no_shmSnd + PS.no_msgsSnd + PS.no_shmRcvd + PS.no_msgsRcvd;
    ...
    MPI_Reduce(
      stat_msgs_l, stat_msgs_g, 4, MPI_FLOAT, MPI_MAX, 0, (*symb_comm));
    MPI_Reduce(&(stat_msgs_l[4]),
               &(stat_msgs_g[4]),
               6,
               MPI_FLOAT,
               MPI_SUM,
    ...
    if (!iam)
    {
      nnzL = (int_t)stat_glob[0];
      nnzU = (int_t)stat_glob[1];
      nsuper = (int_t)stat_glob[2];
      szLGr = (int_t)stat_glob[3];
      szUGr = (int_t)stat_glob[4];
      printf("\tMax szBlk          %ld\n", VInfo.maxSzBlk);
#if (PRNTlevel >= 2)
      ...
      printf("\tDNS ALLSEPS:\t MAX %d\tAVG %.2f\n",
             (int_t)mem_glob[4],
             stat_glob[9] / nprocs_symb);
      printf("\tDNS CURSEP:\t MAX %d\tAVG %.2f\n\n",
             (int_t)mem_glob[5],
             stat_glob[10] / nprocs_symb);

      printf("\t MAX FILL Mem(L+U) / Mem(A) per processor %ld\n", fill_rcmd);
      printf("\t      Per elt MAX %ld AVG %ld\n",
             (int_t)PS.fill_pelt[4],
             (int_t)(PS.fill_pelt[3] / PS.fill_pelt[5]));
      printf("\t      Per elt RL MAX %ld AVG %ld\n",
             (int_t)PS.fill_pelt[1],
             (int_t)(PS.fill_pelt[0] / PS.fill_pelt[2]));
      printf("\tM Nops:\t MAX %.2f\tAVG %.2f\n",
             mem_glob[11] * 1e-6,
             (stat_glob[16] / nprocs_symb) * 1e-6);


      printf("\tEXPANSIONS: MAX/AVG\n");
      printf("\tTOTAL: %d / %.2f\n",
             (int_t)mem_glob[6],
             stat_glob[11] / nprocs_symb);
      printf("\tREALLOC: %.f / %.2f RL_CP %.f / %.2f PR_CP %.f / %.2f\n",
             mem_glob[7],
             stat_glob[12] / nprocs_symb,
             mem_glob[8],
             stat_glob[13] / nprocs_symb,
             mem_glob[9],
             stat_glob[14] / nprocs_symb);

      printf("\n\tDATA MSGS  noMsgs*10^3 %.3f/%.3f size (MB) %.3f/%.3f \n",
             stat_msgs_g[2] * 1e-3,
             stat_msgs_g[4] / nprocs_symb * 1e-3,
             stat_msgs_g[0] * 1e-6,
             stat_msgs_g[8] / stat_msgs_g[6] * 1e-6);
      printf("\tTOTAL MSGS noMsgs*10^3 %.3f/%.3f size (MB) %.3f/%.3f \n",
             stat_msgs_g[3] * 1e-3,
             stat_msgs_g[5] / nprocs_symb * 1e-3,
             stat_msgs_g[1] * 1e-6,
             stat_msgs_g[9] / stat_msgs_g[7] * 1e-6);
#endif
      ...
      for (i = 0; i < nlvls; i++)
      {
        for (j = 1; j < 9; j++) time_lvlsg[j] = 0.;
        for (p = 0; p < nprocs_symb; p++)
        {
          k = p * 3 * nlvls;
          t = time_lvlsT[i * 3 + k] + time_lvlsT[i * 3 + k + 1] +
              time_lvlsT[i * 3 + k + 2];
          if (t > time_lvlsg[1])
          {
            time_lvlsg[1] = t;
        ...
        printf("%d \t%.3f/%.3f\t%.3f/%.3f\t%.3f/%.3f\t%.3f/%.3f\n",
               i,
               time_lvlsg[1],
               time_lvlsg[2] / nprocs_symb,
               time_lvlsg[3],
               time_lvlsg[4] / nprocs_symb,
               time_lvlsg[5],
               time_lvlsg[6] / nprocs_symb,
               time_lvlsg[7],
               time_lvlsg[8] / nprocs_symb);
      }
      printf("\t   %8.3f \n", time_lvlsg[0]);
#endif
    }
  ...
  else
  {
    ...
    Pslu_freeable->xlsub = NULL;
    Pslu_freeable->lsub = NULL;
    Pslu_freeable->xusub = NULL;
    Pslu_freeable->usub = NULL;
  ...
  if (commLvls != NULL) SUPERLU_FREE(commLvls);
...
static int_t initParmsAndStats(psymbfact_stat_t* PS /* Output -statistics*/
)
/*! \brief
 * <pre>
 * Purpose
 ...
 */
{
  ...
  for (i = 0; i < 6; i++) PS->fill_pelt[i] = 0.;
 ...
 */
{
  ...
  maxNvtcsPProc = Pslu_freeable->maxNvtcsPProc;
  if (Llu_symbfact->szLsub - VInfo->nnz_ainf_loc > n) minElt_vtx = lsub;
  else
  {
  ...
  for (ii = 0; ii < n; ii++) tempArray[ii] = n;
  for (ii = 0; ii < nvtcs_loc; ii++) cntelt_vtcs[ii] = 0;
  ...
  while (szSep >= 1)
  {
    ...
    for (j = i; j < i + szSep; j++)
    {
      ...
      if (fstP <= iam && iam < lstP)
      {
        ...
        if (szSep == nprocs_symb) vtx_lid = ii;
        else
        {
          MPI_Allreduce(&(tempArray[fstVtx]),
                        &(minElt_vtx[fstVtx]),
                        (int)(n - fstVtx),
                        mpi_int_t,
                        MPI_MIN,
          ...
          if (PS->maxsz_msgCol < n - fstVtx) PS->maxsz_msgCol = n - fstVtx;
#endif

          nelts = 0;
          for (ii = fstVtx; ii < lstVtx; ii++) tempArray[ii] = 0;
          for (ii = fstVtx; ii < n; ii++)
          {
            if (minElt_vtx[ii] != n)
            {
              if (minElt_vtx[ii] < fstVtx) nelts++;
              else
                tempArray[minElt_vtx[ii]]++;
              if (ii > lstVtx) tempArray[ii] = minElt_vtx[ii];
            }
          }
                 ...
                 ind_blk < 2 * VInfo->nblks_loc)
          {
            ...
            for (ii = lv; ii < fstVtx_blk; ii++) nelts += tempArray[ii];
            lv = lstVtx_blk;
...
)
{
  ...
  while (szSep >= 1)
  {
    ...
    for (jSep = iSep; jSep < iSep + szSep; jSep++)
    {
      ...
      else
      {
        ...
        for (ind_ap_s = (jSep - iSep) * npNode;
             ind_ap_s < (jSep - iSep + 1) * npNode;
             ind_ap_s++)
        {
...
#if (PRNTlevel >= 2)
  if (!iam) PrintInt10(" novtcs_pe", nprocs_symb, vtcs_pe);
#endif
  ...
  {
    if (maxNvtcsPProc < vtcs_pe[p]) maxNvtcsPProc = vtcs_pe[p];
    vtcs_pe[p] = 0;
  }
#if (PRNTlevel >= 2)
  if (!iam)
    printf("  MaxNvtcsPerProc %d MaxNvtcs/Avg %e\n\n",
           maxNvtcsPProc,
           ((float)maxNvtcsPProc * nprocs_symb) / (float)n);
#endif
  ...
  {
    p = globToLoc[k];
    if (p == iam) begEndBlks_loc[ind_blk] = k;
  ...
  if (iam < nprocs_symb) begEndBlks_loc[2 * nblks_loc] = n;
...
static void symbfact_distributeMatrix(
  int iam, /* Input - my processor number */
  int nprocs_num, /* Input - number of processors */
...
)
{
  ...
  for (i = 0; i < m_loc; i++)
  {
    ...
    if (neltsRow == 0) ptr_toSnd[p_irow] -= 2;
    else
    {
  ...
  if (iam < nprocs_symb)
  {
    ...
    for (i = 0, p = 0; p < nprocs_num; p++)
    {
      ...
      while (k < nnzToRecv[p])
      {
        ...
        else
        {
          ...
          k += nelts + 2;
          if (ainf_data) x_ainf[vtx_lid] += nelts;
          else
    ...
    {
      for (vtx = begEndBlks_loc[2 * i]; vtx < begEndBlks_loc[2 * i + 1];
           vtx++, vtx_lid++)
      {
    ...
    if (!(ind_ainf = intMalloc_symbfact(x_ainf[nvtcs_loc])) &&
        x_ainf[nvtcs_loc] != 0)
      ABORT("Malloc fails for ind_ainf[].");
    if (!(ind_asup = intMalloc_symbfact(x_asup[nvtcs_loc])) &&
        x_asup[nvtcs_loc] != 0)
...
static float allocPrune_lvl(Llu_symbfact_t* Llu_symbfact, /* Input/Output -
                                     local L, U data structures */
                            vtcsInfo_symbfact_t* VInfo, /* Input -local info on
 ...
 */
{
  ...
  if (Llu_symbfact->szLsubPr) SUPERLU_FREE(Llu_symbfact->lsubPr);
  if (Llu_symbfact->szUsubPr) SUPERLU_FREE(Llu_symbfact->usubPr);
  if (Llu_symbfact->xlsubPr) SUPERLU_FREE(Llu_symbfact->xlsubPr);
  if (Llu_symbfact->xusubPr) SUPERLU_FREE(Llu_symbfact->xusubPr);
  ...
  if (x_sz != 0)
  {
    ...
    while (!lsubPr || !usubPr)
    {
      ...
      nzlmaxPr /= 2;
      nzlmaxPr = alpha * (float)nzlmaxPr;
      nzumaxPr /= 2;
      nzumaxPr = alpha * (float)nzumaxPr;
...
static int_t symbfact_vtx(
  ...
  int domain_symb, /* Input - if TRUE, computation corresponds to the
    independent domain at the bottom of the separator tree */
  int_t fstVtx, /* Input - first vertex of current node */
  int_t lstVtx, /* Input - last vertex of current node */
  ...
  Llu_symbfact_t* Llu_symbfact, /* Input/Output - local L, U data structures */
  vtcsInfo_symbfact_t*
    VInfo, /* Input/Output - local info on vertices distribution */
  psymbfact_stat_t* PS,
  ...
  int_t* p_vtx_bel_mySn)
{
  ...
  while (k < x_aind_end)
  {
    if (sub[k] == EMPTY) k = x_aind_end;
    else
    {
      vtx_elt = sub[k];
      if (!computeL)
        if (marker[vtx_elt] == mark_vtx - 2)
          if (vtx_elt < prval_curvtx) prval_curvtx = vtx_elt;
      marker[vtx_elt] = mark_vtx;
      if (computeL && vtx_elt == vtx) diagind = TRUE;
      if (!computeL && vtx_elt == vtx)
        printf(
          "Pe[%d] ERROR diag elt in U part vtx %d dom_s %d fstV %d lstV %d\n",
          iam,
          vtx,
          domain_symb,
          fstVtx,
  ...
  if (domain_symb)
  {
    if (computeL) VInfo->nnz_ainf_loc -= x_aind_end - x_aind_beg;
    else
  ...
  ind = xsubPr[vtx_prid];
  if (vtx_bel_othSn == vtx) upd_lstSn = TRUE;
  ...
  while (ind != EMPTY || upd_lstSn)
  {
    ...
    if (!computeL) marker[vtx] = mark_vtx;
    ...
    for (; k < xsub_src[pr_lid + 1]; k++)
    {
      ...
      if (pr_elt >= vtx && marker[pr_elt] != mark_vtx)
      {
        ...
        if (next >= x_aind_end)
        {
          if (domain_symb)
          {
            if (mem_error = psymbfact_LUXpandMem(iam,
                                                 n,
                                                 vtx,
                                                 next,
                                                 0,
          ...
          else if (mem_error = psymbfact_LUXpand(iam,
                                                 n,
                                                 EMPTY,
                                                 vtx,
                                                 &next,
        ...
        if (computeL && pr_elt == vtx) diagind = TRUE;
        if (!computeL)
          if (marker[pr_elt] == mark_vtx - 2)
            if (pr_elt < prval_curvtx) prval_curvtx = pr_elt;
  ...
  if (vtx != fstVtx)
  {
    for (k = xsub[snrep_lid]; k < xsub[snrep_lid + 1]; k++)
    {
      ...
      if (vtx_elt >= vtx)
      {
        if ((vtx_elt > vtx && !computeL) || (vtx_elt >= vtx && computeL))
        {
          if (marker[vtx_elt] != mark_vtx) neltsZrVtx++;
          else
          {
            neltsMatched++;
          }
        }
        if (computeL && vtx_elt == vtx) *p_vtx_bel_mySn = vtx;
        if (!computeL && vtx_elt == vtx + 1) *p_vtx_bel_mySn = vtx + 1;
      }
    }
  }
  else
  {
    ...
    if (!computeL)
      ...
      {
        vtx_elt = sub[k];
        if (vtx_elt == vtx + 1) *p_vtx_bel_mySn = vtx + 1;
      }
  }
...
static int_t updateRcvd_prGraph(
  int_t n, /* Input - order of the matrix */
  int iam, /* Input - my processor number */
  int_t* sub_rcvd, /* elements of node */
...
)
{
  ...
  for (i = 0; i < nvtcs_toUpd; i++) marker[i] = 0;
  for (i = 0; i <= VInfo->maxSzBlk; i++) xsub_rcvd[i] = 0;
  ...
  if (ind == 0) /* quick return if no update */
    return 0;

  /* test if enough memory in usubPr array */
  if (ind >= szsubPr)
  {
    if (mem_error = psymbfact_prLUXpand(iam, ind, computeL, Llu_symbfact, PS))
      return (mem_error);
    if (computeL) subPr = Llu_symbfact->lsubPr;
    else
      subPr = Llu_symbfact->usubPr;
  }
  *p_indsubPr = ind;
      ...
      else
      {
        if (OWNER(globToLoc[vtx_elt]) == iam)
        {
          if (vtx_elt >= fstVtx_toUpd && vtx_elt < lstVtx_toUpd)
          {
            ...
            subPr[marker[vtx_elt_lid] + 1] =
              vtx - fstVtx_srcUpd + VInfo->nvtcs_loc;
  ...
  for (i = fstVtx_toUpd; i < nvtcs_toUpd; i++) marker[i] = 0;
}

static int_t update_prGraph(
  int iam,
  ...
  psymbfact_stat_t* PS)
{
  ...
  kmin = xsub[snrep_lid];
  kmax = xsub_snp1 - 1;
  if (prval_cursn != n) maxElt = prval_cursn;
  else
    maxElt = EMPTY;
  while (kmin <= kmax)
  {
    if (prval_cursn == n)
    {
      ...
      if (sub[kmin] > maxElt) maxElt = sub[kmin];
      kmin++;
    }
    else
    {
      /* Do a quicksort-type partition. */
      if (sub[kmax] > prval_cursn) kmax--;
      else if (sub[kmin] <= prval_cursn)
        kmin++;
      else
      { /* kmin does'nt belong to G^s(L), and kmax belongs:
  ...
  while (sub[k] <= prval_cursn && k < xsub_snp1)
  {
    ...
    if (sn_elt < lstVtx_blk)
    {
      ...
      {
        if (mem_error = psymbfact_prLUXpand(iam, 0, computeL, Llu_symbfact, PS))
          return (mem_error);
        if (computeL)
        {
          subPr = Llu_symbfact->lsubPr;
  ...
  int_t* p_nsuper_loc)
{
  ...
  int_t neltsZrSn_L, neltsZrSn_U, neltsZr, neltsTotal, neltsZr_tmp,
    neltsTotal_tmp, neltsZrSn, neltsVtxInit_l, neltsVtxInit_u;
  ...
  if (fstVtx_blk >= lstVtx_blk) return 0;
  ...
  if (ind_sizes1 == 0) domain_symb = TRUE;
  else
  {
    domain_symb = FALSE;
  ...
  for (vtx = fstVtx_blk; vtx < lstVtx_blk; vtx++, vtx_lid++, vtx_prid++)
  {
    ...
    markl1_vtx = marku2_vtx + 1;
    markl2_vtx = markl1_vtx + 1;
    marku1_vtx = markl2_vtx + 1;
    marku2_vtx = marku1_vtx + 1;

    prval_curvtx = n;
    /* Compute nonzero structure L(:,vtx) */
    if (mem_error = symbfact_vtx(n,
                                 iam,
                                 vtx,
                                 vtx_lid,
                                 vtx_prid,
    ...
    if (!domain_symb)
    {
      ...
      {
        stat = CEILING(
          stat, Llu_symbfact->cntelt_vtcsA_lvl[vtx_lid - fstVtx_loc_lid]);
      ...
      stat =
        CEILING(stat, Llu_symbfact->cntelt_vtcsA_lvl[vtx_lid - fstVtx_loc_lid]);
    ...
    neltsZrSn =
      neltsZrVtx_L + neltsZrVtx_U + (neltsZrSn_L + neltsZrSn_U) * szsn;
    neltsZr_tmp = neltsZr + neltsZrSn;
    neltsTotal_tmp = neltsTotal + neltsZrSn + neltsVtx_L + neltsVtx_U;
    if (neltsTotal_tmp == 0) neltsTotal_tmp = 1;
    ...
    else
    {
      ...
      if (relax_seps >= PS->relax_seps)
      {
        ...
        *p_nsuper_loc = nsuper_loc;
        if (mem_error = dnsUpSeps_symbfact(n,
                                           iam,
                                           szSep,
                                           ind_sizes1,
                                           ind_sizes2,
      ...
      else
      {
        if (relax_param >= PS->relax_gen)
        {
          ...
          if (neltsZrSn_L != 0)
          {
            ...
            {
              vtx_elt = lsub[k];
              if (vtx_elt >= vtx) marker[vtx_elt] = markl2_vtx;
            }
          ...
          if (neltsZrSn_U != 0)
          {
            ...
            if (marker[vtxp1] == marku2_vtx) vtx_bel_snU = vtxp1;
        ...
        VInfo->filledSep != FILLED_SEPS)
    {
      ...
      if (marker[vtxp1] == marku1_vtx) vtx_bel_snU = vtxp1;
      ...
      while (szLp > 0)
      {
        ...
        printf("Adr lsub %p usub %p lsub %p pos %d usub %p pos %d\n",
               ...
               xusub[snrep_lid]);
        PrintInt10(
          "Lsn", xlsub_snp1 - xlsub[snrep_lid], &(lsub[xlsub[snrep_lid]]));
        PrintInt10(
          "Usn", xusub_snp1 - xusub[snrep_lid], &(usub[xusub[snrep_lid]]));
#endif
    ...
    if (vtx == lstVtx_blk - 1) nsuper_loc++;
    ...
    if (!VInfo->filledSep)
    {
      ...
      if (relax_seps >= PS->relax_curSep) VInfo->filledSep = FILLED_SEP;
    }
    maxNeltsVtx--;
  }
  ...
  int_t nextu)
{
  ...
  int_t nelts, nelts_fill_l, nelts_fill_u, nelts_cnts, maxNvtcsPProc,
    *globToLoc;
  ...
  {
    nvtcsLvl_loc += begEndBlks_loc[ind_blk + 1] - begEndBlks_loc[ind_blk];
    ind_blk += 2;
  }
  ...
  if (use_fillcnts)
  {
    if (nextl + nelts_fill_l >= Llu_symbfact->szLsub - nelts_ainf)
      mem_error = psymbfact_LUXpandMem(iam,
                                       n,
                                       fstVtx,
                                       nextl,
                                       nextl + nelts_fill_l,
  ...
  while (begEndBlks_loc[ind_blk] < lstVtx && ind_blk < 2 * nblks_loc)
  {
    ...
    for (vtx = fstVtx_blk; vtx < lstVtx_blk; vtx++, vtx_lid++)
    {
      ...
      Llu_symbfact->cntelt_vtcsA_lvl[vtx_lid - fstVtx_loc_lid] = CEILING(
        (xlsub[vtx_lid + 1] - x_aind_beg_l + xusub[vtx_lid + 1] - x_aind_beg_u),
        2);
      ...
      if (nelts < cntelt_vtx_l) lsub[nextl] = EMPTY;
      ...
      if (nelts < cntelt_vtx_u) usub[nextu] = EMPTY;
  ...
  if (nvtcsLvl_loc == 0)
  {
    if (curblk_loc == 0) vtx_lid = 0;
    else
    {
      if (begEndBlks_loc[curblk_loc - 1] == 0) vtx_lid = 0;
      else
  ...
  else
  {
    ...
    xusub[vtx_lid] = nextu;
    if (PS->estimLSz < nextl) PS->estimLSz = nextl;
    if (PS->estimUSz < nextu) PS->estimUSz = nextu;
  ...
  psymbfact_stat_t* PS)
{
  ...
  for (i = vtx_upd_pr; i < lstVtx_upd_pr; i++, vtx_lid++)
  {
    ...
    if (xusubPr[i] != xusubPr[i + 1])
    {
      ...
      for (j = xusubPr[i] + 1; j < xusubPr[i + 1]; j++)
      {
        ...
        {
          vtx_elt_lid = LOCAL_IND(globToLoc[vtx_elt]);
          if (vtx_elt_lid == lvtx_lid) nelts = lsub_rcvd_sz - ii;
          else
            nelts = xlsub[vtx_elt_lid + 1] - xlsub[vtx_elt_lid];
          mpnelts = marker[vtx_elt] + nelts;
        }

        if (!computeL) marker[vtx] = markl;
      ...
      {
        /* reset marker array */
        for (j = fstVtx_toUpd; j < n; j++) marker[j] = EMPTY;
        markl = 0;
      }
    }
  ...
  *pmarkl = markl;
  ...
  nextl = xlsub[vtxXp_lid + 1];
  if (mem_error = psymbfact_LUXpand_RL(iam,
                                       n,
                                       vtxXp,
                                       nextl,
                                       len_texp,
...
)
{
  ...
  /* quick return */
  if (fstVtx_toUpd >= lstVtx_toUpd) return 0;
  ...
  while (i < usub_rcvd_sz)
  {
    ...
    {
      if (vtx_lid == lvtx_lid) nelts = usub_rcvd_sz - i;
      else
  ...
  for (i = 0; i < nvtcs_toUpd; i++)
  {
    ...
    {
      marker[i]++;
      if (fstVtx_upd == EMPTY) fstVtx_upd = i;
      lstVtx_upd = i;
    }
    xusubPr[i] = ind;
    ind += marker[i];
  ...
  {
    if (mem_error = psymbfact_prLUXpand(iam, ind, LSUB_PR, Llu_symbfact, PS))
      return (mem_error);
    usubPr = Llu_symbfact->lsubPr;
  }
  ...
  /* reset marker array */
  for (i = 0; i < nvtcs_toUpd; i++) marker[i] = EMPTY;
  if (fstVtx_srcUpd != EMPTY)
  {
    vtx_loc = fstVtx_srcUpd;
    vtx_lid = LOCAL_IND(globToLoc[vtx_loc]);
  ...
  while (i < lsub_rcvd_sz)
  {
    ...
    {
      vtx = vtx_loc;
      if (vtx_lid == lvtx_lid) nelts = lsub_rcvd_sz - i;
      else
  ...
  for (i = fstVtx_upd; i < lstVtx_upd; i++, vtx_lid++)
  {
    if (xusubPr[i] != xusubPr[i + 1])
    {
        ...
        for (ii; ii < mpnelts; ii++)
        {
          ...
          if (elt >= vtx)
          {
            if (marker[elt] != markl)
            {
              ...
              {
                if (mem_error = expand_RL(computeRcvd,
                                          n,
                                          iam,
                                          lsub_rcvd,
                                          lsub_rcvd_sz,
                ...
                if (computeL)
                {
                  ...
                  if (!computeRcvd)
                    lsub_rcvd =
                      &(Llu_symbfact
                          ->lsub[Llu_symbfact->xlsub[fstVtx_srcUpd_lid]]);
                }
                else
                {
                  marker[vtx] = markl;
                  ...
                  if (!computeRcvd)
                    lsub_rcvd =
                      &(Llu_symbfact
                          ->usub[Llu_symbfact->xusub[fstVtx_srcUpd_lid]]);
                }
              }
              lsub[nextl] = elt;
              nextl++;
      ...
      if (nextl < xlsub[vtx_lid + 1]) lsub[nextl] = EMPTY;
      markl++;
      if (markl == n)
      {
        /* reset marker array */
...
static int_t dnsUpSeps_symbfact(
  int_t n,
  ...
  int_t* p_nsuper_loc)
{
  ...
  if (fstVtx_dns == EMPTY) fstVtx_blk = VInfo->begEndBlks_loc[curblk_loc];
  else
    fstVtx_blk = fstVtx_dns;
  if (fstVtx_blk == n) return 0;
  ...
  while (szSep >= 1)
  {
    ...
    if (fstVtx_blk > fstVtx_lvl) vtx_elt = fstVtx_blk;
    else
      vtx_elt = fstVtx_lvl;
...
static int_t dnsCurSep_symbfact(
  int_t n, /* Input - order of the matrix */
  int iam, /* Input - my processor number */
  int ind_sizes1,
  int ind_sizes2,
  ...
  psymbfact_stat_t* PS)
{
  int_t fstVtx_blk, fstVtx_dns, fstVtx_dns_lid, lstVtx_blk, fstVtx, lstVtx,
    lstVtx_dns_lid;
  ...
  if (!rcvd_dnsSep) VInfo->curblk_loc += 2;
  ...
  if (CS->rcv_bufSz > n - fstVtx_dns) minElt_vtx = CS->rcv_buf;
  else
  {
  ...
  while (computeL || computeU)
  {
    ...
    for (i = 0; i < n - fstVtx_dns; i++) minElt_vtx[i] = n;
    ...
    MPI_Allreduce(&(minElt_vtx[lstVtx - fstVtx_dns]),
                  &(marker[lstVtx]),
                  n - lstVtx,
                  mpi_int_t,
                  MPI_MIN,
    ...
    if (PS->maxsz_msgCol < n - lstVtx) PS->maxsz_msgCol = n - lstVtx;
#endif

    /* use x_newelts to determine counts of elements starting in each vertex */
    for (vtx_lid = 0; vtx_lid < nvtcs_upd; vtx_lid++) x_newelts[vtx_lid] = 0;
  ...
  for (i = fstVtx_dns; i < n; i++) marker[i] = EMPTY;
  mark = 0;
         ...
         ind_blk < 2 * VInfo->nblks_loc)
  {
    ...
    for (vtx = fstVtx_blk; vtx < lstVtx_blk; vtx++, vtx_lid++)
    {
      ...
      if (vtx == fstVtx_blk ||
          x_newelts_L[vtx_lid_x + 1] != x_newelts_L[vtx_lid_x] ||
          x_newelts_U[vtx_lid_x + 1] != x_newelts_U[vtx_lid_x])
      {
        ...
        while (computeL || computeU)
        {
          ...
          for (i = 0; i < x_newelts[vtx_lid_x + 1]; i++)
          {
            ...
            sub[next] = vtx_elt;
            next++;
            if (computeU && vtx_elt < prval && marker[vtx_elt] == mark - 1)
              prval = vtx_elt;
            marker[vtx_elt] = mark;
          }
          if (computeL)
        ...
        if (vtx != fstVtx_blk) (*p_nsuper_loc)++;
      } /* a new supernode starts */
    ...
    if (VInfo->begEndBlks_loc[ind_blk - 1] == lstVtx && i > 1 && j > 0)
    {
      ...
      while (computeL || computeU)
      {
        ...
        if (prval != n)
        {
          ...
          while (kmin <= kmax)
          {
            ...
            if (sub[kmin - 1] == prval) prpos = kmin - 1;
          }
        }
        else
        {
      ...
      if (prval >= lstVtx)
      {
        ...
        while (prval >= lstVtx && szSep != 1)
        {
          ...
          if (CS->snd_vtxinter[lvl] == EMPTY) CS->snd_vtxinter[lvl] = snrep;
        }
      }
    }
  }
  ...
  *p_mark = mark;
  if (minElt_vtx != CS->rcv_buf) SUPERLU_FREE(minElt_vtx);
  SUPERLU_FREE(x_newelts_U);
  if (newelts_L) SUPERLU_FREE(newelts_L);
  if (newelts_U) SUPERLU_FREE(newelts_U);
  if (PS->szDnsSep < mem_dnsCS) PS->szDnsSep = mem_dnsCS;
}
...
static int_t denseSep_symbfact(
  ...
  psymbfact_stat_t* PS)
{
  ...
  if (iam == fstP && rcvd_dnsSep && nblk_loc == 1) nmsgsToRcv++;
  ...
  while ((nmsgsToSnd || nmsgsToRcv) && VInfo->begEndBlks_loc[ind_blk] < lstVtx)
  {
    ...
    if (nmsgsToSnd)
    {
      ...
      if (lstVtx_blk != lstVtx)
      {
        ...
        MPI_Send(
          &(rcv_intraLvl[fstP]), nprocsLvl, mpi_int_t, p, tag, (*symb_comm));
#if (PRNTlevel >= 1)
  ...
  {
    if (mem_error = dnsCurSep_symbfact(n,
                                       iam,
                                       ind_sizes1,
                                       ind_sizes2,
                                       sizes,
...
static int_t interLvl_symbfact(
  SuperMatrix* A, /* Input - input matrix A */
  int iam, /* Input - my processor number */
  int lvl, /* Input - current level in the separator tree */
  int szSep, /* Input - size of the current separator (node) */
...
)
{
  ...
  int_t* rcv_interLvl;
  int_t *snd_interLvl, *snd_interLvl1, *snd_interLvl2, snd_interLvlSz,
    snd_LinterLvlSz, snd_vtxLvl;
  int_t vtx_elt, update_loc, code_err;
  int_t *lsub, *xlsub, *usub, *xusub;
  int_t *lsub_rcvd, lsub_rcvd_sz, *usub_rcvd, usub_rcvd_sz;
  int_t n, mark, max_rcvSz;
  int_t nextl, nextu, ind_blk, vtx_lid, k, count, nelts, lstVtxLvl_loc,
    lstVtxLvl_loc_lid, mem_error;
  int_t fstVtx_blk, lstVtx_blk, i, j, vtx, prElt_L, prElt_U, snd_indBlk,
    prElt_ind;
  int_t fstVtxLvl_loc, nvtcsLvl_loc, maxNvtcsPProc, *globToLoc, fstVtx, lstVtx;
  ...
  if (snd_interLvlSz != 0)
  {
    if (snd_LinterLvlSz == 0) snd_interLvlSz = 0;
    if (snd_interLvlSz - snd_LinterLvlSz == 0) snd_interLvlSz = 0;
  }

  initInfo_loc[0] = snd_interLvlSz;
  initInfo_loc[1] = (int_t)VInfo->filledSep;
  MPI_Allreduce(initInfo_loc, initInfo_gl, 2, mpi_int_t, MPI_MAX, ndComm);
#if (PRNTlevel >= 1)
  PS->no_msgsCol += (float)(2 * (int_t)LOG2(nprocsLvl));
  PS->sz_msgsCol += 2;
  if (PS->maxsz_msgCol < 2) PS->maxsz_msgCol = 2;
#endif
  ...
  if (max_rcvSz == 0) /* quick return if no communication necessary */
    return 0;
  ...
  if (snd_interLvlSz)
    if (CS->snd_bufSz < snd_interLvlSz)
    {
      ...
      if (CS->snd_bufSz != 0) /* not first time allocate memory */
  ...
  for (p = 2 * fstP; p < 2 * lstP; p++) snd_interLvl[p] = EMPTY;

  if (snd_interLvlSz == 0 && nvtcsLvl_loc == 0)
  {
    code_err = MPI_Alltoall(&(snd_interLvl[2 * fstP]),
                            2,
                            mpi_int_t,
                            &(rcv_interLvl[2 * fstP]),
                            2,
  ...
  if (snd_interLvlSz != 0)
  {
    ...
    {
      fstVtx_blk = VInfo->begEndBlks_loc[ind_blk];
      if (ind_blk == snd_indBlk) fstVtx_blk = snd_vtxLvl;
      lstVtx_blk = VInfo->begEndBlks_loc[ind_blk + 1];
      for (vtx = fstVtx_blk; vtx < lstVtx_blk; vtx++, vtx_lid++)
      {
        toSendL = FALSE;
  ...
  MPI_Alltoall(&(snd_interLvl[2 * fstP]),
               2,
               mpi_int_t,
               &(rcv_interLvl[2 * fstP]),
               2,
  ...
  if (PS->maxsz_msgCol < 2 * nprocsLvl) PS->maxsz_msgCol = 2 * nprocsLvl;
#endif
  ...
  for (p = 2 * fstP; p < 2 * lstP; p += 2)
  {
    ...
    if (rcv_interLvl[p] != EMPTY) max_rcvSz += rcv_interLvl[p];
    CS->ptr_rcvBuf[p + 1] = max_rcvSz;
    if (rcv_interLvl[p + 1] != EMPTY) max_rcvSz += rcv_interLvl[p + 1];
    if (rcv_interLvl[p] != EMPTY || rcv_interLvl[p + 1] != EMPTY) nprocsToRcv++;
  }
  ...
  if (nprocsToSnd)
    if (!(request_snd = (MPI_Request*)SUPERLU_MALLOC(2 * nprocsToSnd *
                                                     sizeof(MPI_Request))))
      ABORT("Not enough memory when allocating MPI_Request");
  if (nprocsToRcv)
    if (!(request_rcv = (MPI_Request*)SUPERLU_MALLOC(2 * nprocsToRcv *
                                                     sizeof(MPI_Request))))
      ABORT("Not enough memory when allocating MPI_Request");
  if (nprocsToRcv || nprocsToSnd)
    if (!(status = (MPI_Status*)SUPERLU_MALLOC(2 * (lstP - fstP) *
                                               sizeof(MPI_Status))))
      ABORT("Not enough memory when allocating MPI_Request");
  ...
  if (nprocsToSnd) MPI_Waitall(2 * nprocsToSnd, request_snd, status);
  ...
  MPI_Comm* symb_comm)
{
  ...
  for (i = 0; i < 2 * nprocs; i++) commLvls[i] = MPI_COMM_NULL;
...
)
{
  ...
  {
    p = OWNER(globToLoc[k]);
    if (p == fstP) fstVtx_blkCyc = k;
    k += VInfo->maxSzBlk;
  }

  for (p = fstP; p < lstP; p++) rcv_intraLvl[p] = 0;
  ...
  fstVtx_loc_lid = LOCAL_IND(globToLoc[fstVtx_loc]);
  vtx = fstVtx_loc;
  if (fstVtx_loc >= fstVtx_blkCyc) nblk_loc = 1;
      ...
      while (!rcvd_prvP || nmsgsRcvd < nmsgsTRcv)
      {
        ...
        if (index_req[0] == 1)
        {
          ...
          if (nmsgsTRcv == nmsgsRcvd)
          {
            ...
            MPI_Send(&r, 1, mpi_int_t, iam, tag_intraLvl_szMsg, (*symb_comm));
        ...
        if (index_req[0] == 0)
        {
          ...
          if (PS->maxsz_msgRcvd < lsub_rcvd_sz)
            PS->maxsz_msgRcvd = lsub_rcvd_sz;
          if (PS->maxsz_msgRcvd < usub_rcvd_sz)
            PS->maxsz_msgRcvd = usub_rcvd_sz;
#endif
      ...
      if (lstVtx_blk != lstVtx)
      {
        ...
        if (VInfo->filledSep == FILLED_SEPS ||
            (VInfo->filledSep == FILLED_SEP &&
             ((lstVtx - lstVtx_blk > VInfo->maxSzBlk * nprocsLvl &&
               nblk_loc > 0) ||
              (lstVtx - fstVtx_blkCyc > VInfo->maxSzBlk * nprocsLvl &&
               nblk_loc == 0))))
        ...
        if (VInfo->filledSep == FILLED_SEPS)
        {
          for (p = fstP; p < lstP; p++)
            rcv_intraLvl[p] =
              maxNmsgsToRcv * VInfo->filledSep + rcv_intraLvl[p];
          denseSep_symbfact(0,
                            n,
                            iam,
                            ind_sizes1,
        ...
        else
        {
          ...
          for (p = fstP; p < lstP; p++) snd_intraLvl[p] = FALSE;
          nextl = 0;
          ...
          for (vtx = fstVtx_blk, vtx_lid = fstVtx_blk_lid; vtx < lstVtx_blk;
               vtx++, vtx_lid++)
          {
            toSend = FALSE;
            k = xlsub[vtx_lid];
          ...
          for (p = fstP; p < lstP; p++)
            if (snd_intraLvl[p]) rcv_intraLvl[p]++;
  ...
  if (VInfo->filledSep == FILLED_SEP) VInfo->filledSep = FALSE;
}
  ...
  comm_symbfact_t* CS)
{
  ...
  if (Llu_symbfact->szLsubPr != 0) SUPERLU_FREE(Llu_symbfact->lsubPr);
  if (Llu_symbfact->szUsubPr != 0) SUPERLU_FREE(Llu_symbfact->usubPr);
  if (Llu_symbfact->xlsubPr != NULL) SUPERLU_FREE(Llu_symbfact->xlsubPr);
  if (Llu_symbfact->xusubPr != NULL) SUPERLU_FREE(Llu_symbfact->xusubPr);

  if (Llu_symbfact->xlsub_rcvd != NULL) SUPERLU_FREE(Llu_symbfact->xlsub_rcvd);
  if (Llu_symbfact->xusub_rcvd != NULL) SUPERLU_FREE(Llu_symbfact->xusub_rcvd);
  ...
  if (CS->rcv_bufSz != 0) SUPERLU_FREE(CS->rcv_buf);
  if (CS->snd_bufSz != 0) SUPERLU_FREE(CS->snd_buf);
  ...
  psymbfact_stat_t* PS)
{
  ...
  overestimMem +=
    (float)(PS->estimUSz - Llu_symbfact->xusub[nvtcs_loc]) * lword;
