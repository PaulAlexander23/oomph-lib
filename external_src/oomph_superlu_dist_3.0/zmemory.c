                       ...
                       mem_usage_t* mem_usage)
{
  ...
  for (k = 0; k < nb; ++k)
  {
    ...
    if (gb < nsupers)
    {
      ...
      {
        mem_usage->for_lu +=
          (float)((BC_HEADER + index[0] * LB_DESCRIPTOR + index[1]) * iword);
  ...
  mem_usage->total += (float)(Llu->bufmax[0] * iword + Llu->bufmax[1] * dword);
 ...
 */
void zallocateA_dist(
  int_t n, int_t nnz, doublecomplex** a, int_t** asub, int_t** xa)
{
...
doublecomplex* doublecomplexMalloc_dist(int_t n)
{
  doublecomplex* buf;
  buf =
    (doublecomplex*)SUPERLU_MALLOC(SUPERLU_MAX(1, n) * sizeof(doublecomplex));
  return (buf);
}
