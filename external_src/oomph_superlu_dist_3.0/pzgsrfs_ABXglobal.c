...
static void gather_1rhs_diag_to_all(int_t,
                                    ...
                                    doublecomplex[],
                                    doublecomplex[]);
static void redist_all_to_diag(int_t,
                               doublecomplex[],
                               Glu_persist_t*,
                               LocalLU_t*,
                               gridinfo_t*,
...
#define ITMAX 20
  ...
  doublecomplex *b, *ax, *R, *B_col, *temp, *work, *X_col, *x_trs, *dx_trs;
  double* rwork;
  int_t notran;
  ...
  /*-- Function prototypes --*/
  extern void pzgstrs1(
    int_t, LUstruct_t*, gridinfo_t*, doublecomplex*, int, SuperLUStat_t*, int*);
        ...
        for (p = 0; p < num_diag_procs; ++p)
          if (iam == diag_procs[p])
            for (k = p; k < nsupers; k += num_diag_procs)
            {
              ...
              knsupc = SuperSize(k);
              for (i = 0; i < knsupc; ++i)
                z_add(&x_trs[i + ii], &x_trs[i + ii], &dx_trs[i + ii]);
            }
        lstres = berr[j];
        ++count;
                               ...
                               doublecomplex work[])
{
  ...
  for (k = 0; k < nsupers; ++k)
  {
    ...
    if (iam == psrc)
    {
      ...
      { /* Send X component. */
        MPI_Send(
          &r[lr], knsupc, SuperLU_MPI_DOUBLE_COMPLEX, pkk, Xk, grid->comm);
      }
      else
      { /* Local copy. */
