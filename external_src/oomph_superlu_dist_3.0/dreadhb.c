...
static void ReadVector(
  FILE* fp, int_t n, int_t* where, int_t perline, int_t persize)
{
...
void dReadValues(
  FILE* fp, int_t n, double* destination, int_t perline, int_t persize)
{
 ...
 */
void FormFullA(
  int_t n, int_t* nonz, double** nzval, int_t** rowind, int_t** colptr)
{
  ...
  for (j = 0; j < n; ++j)
  {
    for (i = al_colptr[j]; i < al_colptr[j + 1]; ++i) ++marker[al_rowind[i]];
  }
  t_colptr[0] = 0;
  for (i = 0; i < n; ++i)
  {
  ...
  for (j = 0; j < n; ++j)
  {
    for (i = t_colptr[j]; i < t_colptr[j + 1]; ++i)
    {
      ...
      { /* not diagonal */
        ...
        a_val[k] = t_val[i];
#ifdef DEBUG
        if (fabs(a_val[k]) < 4.047e-300) printf("%5d: %e\n", k, a_val[k]);
#endif
        ++k;
      }
    }
