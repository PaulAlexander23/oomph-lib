...
namespace CRBond_Bessel
{
                ...
                double& k1p)
  {
    ...
    if (x < 3.75)
    {
      ...
      t2 = t * t;
      i0 =
        (((((0.0045813 * t2 + 0.0360768) * t2 + 0.2659732) * t2 + 1.2067492) *
            t2 +
          3.0899424) *
           t2 +
         3.5156229) *
    ...
    if (x < 2.0)
    {
      ...
      dtmp =
        (((((0.0000074 * t2 + 0.0001075) * t2 + 0.00262698) * t2 + 0.0348859) *
            t2 +
          0.23069756) *
           t2 +
         0.4227842) *
    ...
    else
    {
      ...
      dtmp1 = exp(-x) / sqrt(x);
      dtmp =
        (((((0.00053208 * t - 0.0025154) * t + 0.00587872) * t - 0.01062446) *
            t +
          0.02189568) *
           t -
         0.07832358) *
      ...
      k0 = dtmp * dtmp1;
      dtmp =
        (((((-0.00068245 * t + 0.00325614) * t - 0.00780353) * t + 0.01504268) *
            t -
          0.0365562) *
           t +
         0.23498619) *
  ...
  int bessikna(
    int n, double x, int& nm, double* in, double* kn, double* inp, double* knp)
  {
                ...
                double& y1p)
  {
    ...
    if (x <= 4.0)
    {
      ...
      j1 =
        t *
        (((((((-0.1289769e-3 * t2 + 0.22069155e-2) * t2 - 0.0236616773) * t2 +
             0.1777582922) *
              t2 -
            0.8888839649) *
             t2 +
         ...
         1.9999999998);
      dtmp =
        (((((((-0.567433e-4 * t2 + 0.859977e-3) * t2 - 0.94855882e-2) * t2 +
             0.0772975809) *
              t2 -
            0.4261737419) *
             t2 +
  ...
  //
  int bessjyna(
    int n, double x, int& nm, double* jn, double* yn, double* jnp, double* ynp)
  {
    ...
    int n, double x, int& nm, double* jn, double* yn, double* jnp, double* ynp)
  {
    ...
    static double a[] = {-0.7031250000000000e-1,
                         0.1121520996093750,
                         -0.5725014209747314,
                         6.074042001273483};
    static double b[] = {0.7324218750000000e-1,
                         -0.2271080017089844,
                         1.727727502584457,
                         -2.438052969955606e1};
    static double a1[] = {
  ...
  int cbessik01(complex<double> z,
                complex<double>& ci0,
                complex<double>& ci1,
                complex<double>& ck0,
                complex<double>& ck1,
  ...
  int cbessjy01(complex<double> z,
                complex<double>& cj0,
                complex<double>& cj1,
                complex<double>& cy0,
                complex<double>& cy1,
                ...
                complex<double>* cyp)
  {
    ...
    if ((a0 <= 300.0) || (n > (int)(0.25 * a0)))
    {
      ...
      for (k = m; k >= 0; k--)
      {
        ...
        {
          csv +=
            (double)((-1) * ((k & 2) - 1) * k) * cf / (double)(k * k - 1.0);
        }
        cf2 = cf1;
        cf1 = cf;
      }
    ...
    if (abs(cj[0]) > 1.0) cy[1] = (cj[1] * cy[0] - 2.0 / (M_PI * z)) / cj[0];
    for (k = 2; k <= nm; k++)
    {
      if (abs(cj[k - 1]) >= abs(cj[k - 2]))
        ...
        cyy =
          (cj[k] * cy[k - 2] - 4.0 * (k - 1.0) / (M_PI * z * z)) / cj[k - 2];
      cy[k] = cyy;
    }
    cyp[0] = -cy[1];
    for (k = 1; k <= nm; k++)
                ...
                complex<double>* cyvp)
  {
    ...
    if ((lb > 4) && (imag(z) != 0.0))
    {
      while (lb != lb0)
      {
        ...
        cp11 = ch0;
        cp21 = ch2;
        if (lb == n) cjv[lb + 1] = 2.0 * (lb + v0) * cjv[lb] / z - cjv[lb - 1];
