...
printf("dgssvx: Fact=%4d, Trans=%4d, equed=%c\n",
  ...
  if (options->Fact != DOFACT && options->Fact != SamePattern &&
      ...
      options->Trans != CONJ && options->Equil != NO && options->Equil != YES)
    *info = -1;
  else if (A->nrow != A->ncol || A->nrow < 0 ||
           (A->Stype != SLU_NC && A->Stype != SLU_NR) || A->Dtype != SLU_S ||
           A->Mtype != SLU_GE)
    *info = -2;
  ...
  else
  {
    ...
    if (*info == 0)
    {
      ...
      { /* no checking if B->ncol=0 */
        if (Bstore->lda < SUPERLU_MAX(0, A->nrow) || B->Stype != SLU_DN ||
  ...
  if (nrhs > 0)
  {
    ...
    if (notran)
    {
      ...
      {
        for (j = 0; j < nrhs; ++j)
          for (i = 0; i < A->nrow; ++i) Bmat[i + j * ldb] *= R[i];
      }
    }
    else if (colequ)
    {
      for (j = 0; j < nrhs; ++j)
        for (i = 0; i < A->nrow; ++i) Bmat[i + j * ldb] *= C[i];
    }
    ...
    if (notran)
    {
      ...
      {
        for (j = 0; j < nrhs; ++j)
          for (i = 0; i < A->nrow; ++i) Xmat[i + j * ldx] *= C[i];
      }
    }
    else if (rowequ)
    {
      for (j = 0; j < nrhs; ++j)
        for (i = 0; i < A->nrow; ++i) Xmat[i + j * ldx] *= R[i];
    }
  } /* end if nrhs > 0 */
