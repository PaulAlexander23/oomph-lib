...
#define ITMAX 5
  ...
  extern int CSAXPY(
    int*, doublecomplex*, doublecomplex*, int*, doublecomplex*, int*);
#else
  ...
  *info = 0;
  ...
  for (j = 0; j < nrhs; ++j)
  {
...
#ifdef _CRAY
        CAXPY(&A->nrow, &done, work, &ione, &Xmat[j * ldx], &ione);
#else
        zaxpy_(&A->nrow, &done, work, &ione, &Xmat[j * ldx], &ione);
#endif
        lstres = berr[j];
        ++count;
      }
    ...
    do
    {
      zlacon_(&A->nrow, &work[A->nrow], work, &ferr[j], &kase);
      if (kase == 0) break;
