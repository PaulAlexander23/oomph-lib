...
void zCopy_Dense_Matrix(
  int M, int N, doublecomplex* X, int ldx, doublecomplex* Y, int ldy)
{
 ...
 */
void zprint_lu_col(
  char* msg, int jcol, int pivrow, int* xprune, GlobalLU_t* Glu)
{
  int i, k, fsupc;
  int *xsup, *supno;
  int *xlsub, *lsub;
...
/*! \brief Check whether tempv[] == 0. This should be true before and after
              ...
              SuperMatrix* B)
{
  ...
  sp_zgemm(transc, "N", A->nrow, nrhs, A->ncol, one, A, x, ldx, zero, rhs, ldc);
}
