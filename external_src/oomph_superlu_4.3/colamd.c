...
#define DEBUG1(params) \
  { \
    if (colamd_debug >= 1) (void)PRINTF params; \
  }
#define DEBUG2(params) \
  { \
    if (colamd_debug >= 2) (void)PRINTF params; \
  }
#define DEBUG3(params) \
  { \
    if (colamd_debug >= 3) (void)PRINTF params; \
  }
#define DEBUG4(params) \
  { \
    if (colamd_debug >= 4) (void)PRINTF params; \
  }
...
PRIVATE void debug_matrix(
  int n_row, int n_col, Colamd_Row Row[], Colamd_Col Col[], int A[]);
  ...
  switch (stats[COLAMD_STATUS])
  {
    ...
    case COLAMD_OK:
      ...
      PRINTF("%s: number of dense or empty rows ignored:           %d\n",
             method,
             stats[COLAMD_DENSE_ROW]);

      PRINTF("%s: number of dense or empty columns ignored:        %d\n",
             method,
             stats[COLAMD_DENSE_COL]);

      PRINTF("%s: number of garbage collections performed:         %d\n",
             method,
             stats[COLAMD_DEFRAG_COUNT]);
      break;
