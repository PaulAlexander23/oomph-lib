  ...
  *info = zLUMemInit(fact,
  ...
  for (jcol = 0; jcol < min_mn;)
  {
    ...
    { /* start of a relaxed snode */
      ...
      if ((*info = zsnode_dfs(
             jcol, kcol, asub, xa_begin, xa_end, xprune, marker, &Glu)) != 0)
        return;
    ...
    { /* Work on one panel of panel_size columns */
      ...
      for (jj = jcol; jj < jcol + panel_size; jj++)
      {
        ...
        if ((*info = zcopy_to_ucol(
               jj, nseg, segrep, &repfnz[k], perm_r, &dense[k], &Glu)) != 0)
          return;
        ...
        zpruneL(jj, perm_r, pivrow, nseg, segrep, &repfnz[k], xprune, &Glu);
