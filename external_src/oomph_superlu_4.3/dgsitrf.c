...
factorization, it is equal to perm_r. */
  ...
  *info = dLUMemInit(fact,
  ...
  if (drop_rule & DROP_SECONDARY) dwork2 = (double*)doubleMalloc(n);
  else
    dwork2 = NULL;
  ...
  i =
    mark_relax(m, relax_end, relax_fsupc, xa_begin, xa_end, asub, marker_relax);
#if (PRNTlevel >= 1)
  ...
  for (jcol = 0; jcol < min_mn;)
  {
    ...
    { /* start of a relaxed snode */
      ...
      if (jcol > 0 && jcol < last_drop)
      {
        ...
        if (drop_rule & DROP_PROWS)
          quota =
            gamma * Astore->nnz / m * (m - first) / m * (last - first + 1);
      ...
      if ((*info = ilu_dsnode_dfs(
             jcol, kcol, asub, xa_begin, xa_end, marker, &Glu)) != 0)
        return;
      ...
      {
        if ((*info = dLUMemXpand(jcol, nextlu, LUSUP, &nzlumax, &Glu))) return;
      }
    ...
    { /* Work on one panel of panel_size columns */
      ...
      dpanel_bmod(
        m, panel_size, jcol, nseg1, dense, tempv, segrep, repfnz, &Glu, stat);
      ...
      for (jj = jcol; jj < jcol + panel_size; jj++)
      {
        ...
        if (drop_sum != zero)
        {
          if (drop_sum > zero)
            omega = SUPERLU_MIN(
              2.0 * (1.0 - alpha) * amax[jj - jcol] / drop_sum, one);
          else
            omega = SUPERLU_MAX(
              2.0 * (1.0 - alpha) * amax[jj - jcol] / drop_sum, -one);
          drop_sum *= omega;
