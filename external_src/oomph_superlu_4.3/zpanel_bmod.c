...
)
{
...
#ifdef _CRAY
  _fcd ftcs1 = _cptofcd("L", strlen("L")), ftcs2 = _cptofcd("N", strlen("N")),
  ...
  { /* for each updating supernode */
    ...
    { /* 2-D block update */
      ...
      for (jj = jcol; jj < jcol + w;
           jj++, repfnz_col += m, dense_col += m, TriTmp += ldaTmp)
      {
...
#ifdef _CRAY
          CTRSV(
            ftcs1, ftcs2, ftcs3, &segsze, &lusup[luptr], &nsupr, TriTmp, &incx);
#else
          ztrsv_("L", "N", "U", &segsze, &lusup[luptr], &nsupr, TriTmp, &incx);
#endif
#else
      ...
      for (r_ind = 0; r_ind < nrow; r_ind += rowblk)
      {
             ...
             jj++, repfnz_col += m, dense_col += m, TriTmp += ldaTmp)
        {
...
#endif
#else
          zmatvec(nsupr, block_nrow, segsze, &lusup[luptr1], TriTmp, MatvecTmp);
#endif
          ...
          {
            irow = lsub[isub];
            z_sub(&dense_col[irow], &dense_col[irow], &MatvecTmp[i]);
            MatvecTmp[i] = zero;
            ++isub;
          }
...
#ifdef _CRAY
          CTRSV(
            ftcs1, ftcs2, ftcs3, &segsze, &lusup[luptr], &nsupr, tempv, &incx);
#else
          ztrsv_("L", "N", "U", &segsze, &lusup[luptr], &nsupr, tempv, &incx);
#endif
