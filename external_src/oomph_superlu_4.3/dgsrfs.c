 ...
 *count. See util.h for the definition of 'SuperLUStat_t'.
 *
 *   info    (output) int*
 *           = 0:  successful exit
...
#define ITMAX 5
  ...
  *info = 0;
  ...
  if (!notran && trans != TRANS && trans != CONJ) *info = -1;
    ...
    *info = -2;
  else if (L->nrow != L->ncol || L->nrow < 0 || L->Stype != SLU_SC ||
           L->Dtype != SLU_D || L->Mtype != SLU_TRLU)
    *info = -3;
  else if (U->nrow != U->ncol || U->nrow < 0 || U->Stype != SLU_NC ||
           U->Dtype != SLU_D || U->Mtype != SLU_TRU)
    *info = -4;
  else if (ldb < SUPERLU_MAX(0, A->nrow) || B->Stype != SLU_DN ||
           B->Dtype != SLU_D || B->Mtype != SLU_GE)
    *info = -10;
  else if (ldx < SUPERLU_MAX(0, A->nrow) || X->Stype != SLU_DN ||
  ...
  for (j = 0; j < nrhs; ++j)
  {
...
#ifdef _CRAY
        SAXPY(&A->nrow, &done, work, &ione, &Xmat[j * ldx], &ione);
#else
        daxpy_(&A->nrow, &done, work, &ione, &Xmat[j * ldx], &ione);
#endif
        lstres = berr[j];
        ++count;
      }
    ...
    do
    {
      dlacon_(&A->nrow, &work[A->nrow], work, &iwork[A->nrow], &ferr[j], &kase);
      if (kase == 0) break;
    ...
    {
      for (i = 0; i < A->nrow; ++i) lstres = SUPERLU_MAX(lstres, fabs(Xptr[i]));
    }
