  ...
  for (i = n; i <= m1;)
  {
    ...
    switch (nrm)
    {
      case ONE_NORM:
        ...
        break;
      case TWO_NORM:
        temp[i] = dnrm2_(&n, &lusup[xlusup_first + i], &m) / sqrt((double)n);
        break;
      case INF_NORM:
      default:
      ...
      if (r > 1) /* add to last row */
      {
        ...
        switch (milu)
        {
          ...
          case SMILU_2:
            daxpy_(&n,
                   &one,
                   &lusup[xlusup_first + i],
                   &m,
                   &lusup[xlusup_first + m - 1],
                   &m);
            break;
          case SMILU_3:
            for (j = 0; j < n; j++)
        ...
        dcopy_(&n, &lusup[xlusup_first + m1], &m, &lusup[xlusup_first + i], &m);
      } /* if (r > 1) */
      else /* move to last row */
      {
        dswap_(&n, &lusup[xlusup_first + m1], &m, &lusup[xlusup_first + i], &m);
        if (milu == SMILU_3)
          for (j = 0; j < n; j++)
          {
    ...
    for (i = n; i <= m1;)
    {
      if (temp[i] <= tol)
      {
        ...
        if (r > 1) /* add to last row */
        {
          ...
          dcopy_(
            &n, &lusup[xlusup_first + m1], &m, &lusup[xlusup_first + i], &m);
        } /* if (r > 1) */
        else /* move to last row */
        {
          dswap_(
