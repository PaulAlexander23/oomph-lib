...
factorization, it is equal to perm_r. */
  ...
  *info = sLUMemInit(fact,
  ...
  if (drop_rule & DROP_SECONDARY) swork2 = (float*)floatMalloc(n);
  else
    swork2 = NULL;
  ...
  for (jcol = 0; jcol < min_mn;)
  {
    ...
    { /* start of a relaxed snode */
      ...
      {
        if ((*info = sLUMemXpand(jcol, nextlu, LUSUP, &nzlumax, &Glu))) return;
      }
