...
#define TempSpace(m, w) \
  ((2 * w + 4 + NO_MARKER) * m * sizeof(int) + (w + 1) * m * sizeof(complex))
...
int cQuerySpace(SuperMatrix* L, SuperMatrix* U, mem_usage_t* mem_usage)
{
  ...
  /* For LU factors */
  mem_usage->for_lu =
    (float)((4.0 * n + 3.0) * iword + Lstore->nzval_colptr[n] * dword +
            Lstore->rowind_colptr[n] * iword);
  mem_usage->for_lu +=
    (float)((n + 1.0) * iword + Ustore->colptr[n] * (dword + iword));
...
int ilu_cQuerySpace(SuperMatrix* L, SuperMatrix* U, mem_usage_t* mem_usage)
{
  ...
  /* For LU factors */
  mem_usage->for_lu =
    (float)((4.0f * n + 3.0f) * iword + Lstore->nzval_colptr[n] * dword +
            Lstore->rowind_colptr[n] * iword);
  mem_usage->for_lu +=
    (float)((n + 1.0f) * iword + Ustore->colptr[n] * (dword + iword));
...
/*! \brief Allocate storage for the data structures common to all factor
 * routines.
 *
 * <pre>
               ...
               complex** dwork)
{
  ...
  if (!Glu->expanders)
    Glu->expanders = (ExpHeader*)SUPERLU_MALLOC(NO_MEMTYPE * sizeof(ExpHeader));
  ...
  if (fact != SamePattern_SameRowPerm)
  {
    ...
    while (!lusup || !ucol || !lsub || !usub)
    {
      ...
      {
        cuser_free(
          (nzlumax + nzumax) * dword + (nzlmax + nzumax) * iword, HEAD, Glu);
      }
      nzlumax /= 2;
      nzumax /= 2;
      nzlmax /= 2;
...
#if (PRNTlevel >= 1)
      printf(
        "cLUMemInit() reduce size: nzlmax %ld, nzumax %ld\n", nzlmax, nzumax);
      fflush(stdout);
#endif
      lusup = (complex*)cexpand(&nzlumax, LUSUP, 0, 0, Glu);
      ucol = (complex*)cexpand(&nzumax, UCOL, 0, 0, Glu);
                ...
                GlobalLU_t* Glu)
{
  ...
  complex* old_ptr;
  int maxsuper = SUPERLU_MAX(sp_ienv(3), sp_ienv(7)), rowblk = sp_ienv(4);

  isize = ((2 * panel_size + 3 + NO_MARKER) * m + n) * sizeof(int);
  dsize = (m * panel_size + NUM_TEMPV(m, panel_size, maxsuper, rowblk)) *
          sizeof(complex);
 ...
 */
void cSetRWork(
  int m, int panel_size, complex* dworkptr, complex** dense, complex** tempv)
{
...
)
{
  ...
  { /* MemModel == USER */
    if (Glu->num_expansions == 0)
    {
      ...
      if (NotDoubleAlign(new_mem) && (type == LUSUP || type == UCOL))
      {
        old_mem = new_mem;
                  ...
                  const int n)
{
  ...
  return (10 * n * iword + nzlmax * iword + nzumax * (iword + dword) +
          nzlumax * dword);
}
