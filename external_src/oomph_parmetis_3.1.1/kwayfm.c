               ...
               int npasses)
{
  ...
  tmp_edegrees =
    (EdgeType*)GKmalloc(sizeof(EdgeType) * nedges, "KWR: tmp_edegrees");
    ...
    for (c = 0; c < 2; c++)
    {
      ...
      for (iii = 0; iii < nmoved; iii++)
      {
        ...
        for (k = 0; k < rinfo[i].ndegrees; k++)
          if (rinfo[i].degrees[k].edge == to) break;
        if (rinfo[i].id == rinfo[i].degrees[k].ewgt) nzgswaps++;
