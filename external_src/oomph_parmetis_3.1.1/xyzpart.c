                          ...
                          WorkSpaceType* wspace)
{
  ...
  if (setup) SetUp(ctrl, graph, wspace);
  else
    graph->nrecv = 0;
  ...
  cand = (KeyValueType*)GKmalloc(nvtxs * sizeof(KeyValueType),
                                 "Coordinate_Partition: cand");
  ...
  for (i = 0; i < nvtxs; i++)
  {
    for (k = 0; k < ndims; k++)
    {
      if (xyz[i * ndims + k] < min[k]) min[k] = xyz[i * ndims + k];
      if (xyz[i * ndims + k] > max[k]) max[k] = xyz[i * ndims + k];
    }
  }
  ...
  for (k = 0; k < ndims; k++)
  {
     ...
     * gmax[k]-gmin[k]); */
    shift[k] = -gmin[k];
    if (gmax[k] != gmin[k]) scale[k] = 1.0 / (gmax[k] - gmin[k]);
    else
      scale[k] = 1.0;
  }
...
void PartSort(CtrlType* ctrl,
              ...
              WorkSpaceType* wspace)
{
  int i, j, k, nvtxs, nrecv, npes = ctrl->npes, mype = ctrl->mype, firstvtx,
                             lastvtx;
  ...
  mypicks = (KeyValueType*)GKmalloc(sizeof(KeyValueType) * (npes + 1),
                                    "ParSort: mypicks");
  allpicks = (KeyValueType*)GKmalloc(sizeof(KeyValueType) * npes * npes,
                                     "ParSort: allpicks");
  ...
  for (i = 1; i < npes; i++) mypicks[i] = allpicks[i * (npes - 1)];
  ...
  for (j = i = 0; i < nvtxs; i++)
  {
    if (elmnts[i].key < mypicks[j + 1].key ||
        (elmnts[i].key == mypicks[j + 1].key &&
         elmnts[i].val < mypicks[j + 1].val))
      scounts[j]++;
    else
      scounts[++j]++;
  }
  ...
  */
  ...
  nrecv = rcounts[npes];
  if (wspace->nlarge >= nrecv) relmnts = (KeyValueType*)wspace->pairs;
  else
    relmnts =
      (KeyValueType*)GKmalloc(sizeof(KeyValueType) * nrecv, "ParSort: relmnts");

  /* Issue the receives first */
  for (i = 0; i < npes; i++)
    MPI_Irecv((void*)(relmnts + rcounts[i]),
              2 * (rcounts[i + 1] - rcounts[i]),
              IDX_DATATYPE,
              i,
              1,
              ctrl->comm,
  ...
  for (i = 0; i < npes; i++)
    MPI_Isend((void*)(elmnts + scounts[i]),
              2 * (scounts[i + 1] - scounts[i]),
              IDX_DATATYPE,
              i,
              1,
              ctrl->comm,
  ...
  /* OK, now do the local sort of the relmnts. Use perm to keep track original
   * order */
  ...
  {
    if (vtxdist[i + 1] > firstvtx)
    { /* Found the first PE that is passed me */
  ...
  {
    ASSERTP(ctrl,
            relmnts[i].key >= 0 && relmnts[i].key < npes,
            (ctrl, "%d %d\n", i, relmnts[i].key));
  ...
  /* Issue the receives first */
  for (i = 0; i < npes; i++)
    MPI_Irecv((void*)(elmnts + scounts[i]),
              2 * (scounts[i + 1] - scounts[i]),
              IDX_DATATYPE,
              i,
              1,
  ...
  /* Issue the sends next */
  for (i = 0; i < npes; i++)
    MPI_Isend((void*)(relmnts + rcounts[i]),
              2 * (rcounts[i + 1] - rcounts[i]),
              IDX_DATATYPE,
              i,
              1,
  ...
  graph->where =
    idxmalloc(graph->nvtxs + graph->nrecv, "PartSort: graph->where");
  firstvtx = vtxdist[mype];
  for (i = 0; i < nvtxs; i++)
  {
    ASSERTP(ctrl,
            elmnts[i].key >= 0 && elmnts[i].key < npes,
            (ctrl, "%d %d\n", i, elmnts[i].key));
    ASSERTP(ctrl,
            elmnts[i].val >= vtxdist[mype] && elmnts[i].val < vtxdist[mype + 1],
            (ctrl,
             "%d %d %d %d\n",
             i,
             vtxdist[mype],
  ...
  if (wspace->nlarge < nrecv) free(relmnts);
}
