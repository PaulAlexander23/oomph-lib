 ...
 **************************************************************************/
void Mc_GlobalMatch_Balance(CtrlType* ctrl,
                            GraphType* graph,
                            WorkSpaceType* wspace)
{
  ...
  for (i = 0; i < nvtxs; i++) iperm[perm[i]] = i;

  nperm = iperm + nvtxs;
  for (i = 0; i < nnbrs; i++) nperm[i] = i;
  ...
  for (nmatched = pass = 0; pass < NMATCH_PASSES; pass++)
  {
    ...
    for (ii = nmatched; ii < nvtxs; ii++)
    {
      ...
      { /* Unmatched */
        ...
        for (j = xadj[i]; j < xadj[i + 1]; j++)
        {
          ...
          if (match[k] == UNMATCHED && myhome[k] == myhome[i] &&
              (maxi == -1 || adjwgt[maxi] < adjwgt[j] ||
               (maxidx < nvtxs && k < nvtxs && adjwgt[maxi] == adjwgt[j] &&
                BetterVBalance(ncon,
                               nvwgt + i * ncon,
                               nvwgt + maxidx * ncon,
                               nvwgt + k * ncon) >= 0)))
        ...
        if (maxi != -1)
        {
          k = ladjncy[maxi];
          if (k < nvtxs)
          { /* Take care the local vertices first */
            /* Here we give preference the local matching by granting it right
             * away */
            if (i <= k)
            {
          ...
          { /* Take care any remote boundary vertices */
    ...
    myprintf(
      ctrl, "[c: %2d] Nlocal: %d, Nrequests: %d\n", c, nlocal, nrequests);
#endif
    ...
    for (i = 0; i < nnbrs; i++)
    {
      MPI_Irecv((void*)(match_granted + recvptr[i]),
                2 * (recvptr[i + 1] - recvptr[i]),
                IDX_DATATYPE,
                peind[i],
                1,
                ctrl->comm,
    ...
    for (j = i = 0; i < nnbrs; i++)
    {
      ...
      MPI_Isend((void*)(match_requests + j),
                2 * (k - j),
                IDX_DATATYPE,
                peind[i],
                1,
    ...
    for (ii = 0; ii < nnbrs; ii++)
    {
      ...
      for (j = 0; j < nreqs_pe[i]; j++)
      {
        k = pe_requests[j].key;
        ASSERTP(ctrl,
                k >= firstvtx && k < lastvtx,
                (ctrl, "%d %d %d %d %d\n", firstvtx, lastvtx, k, j, peind[i]));
        /* myprintf(ctrl, "Requesting a match %d %d\n", pe_requests[j].key,
         * pe_requests[j].val); */
        if (match[k - firstvtx] == UNMATCHED)
        { /* Bingo, lets grant this request */
          changed[nchanged++] = k - firstvtx;
          if (nkept >= 0)
          { /* Flip a coin for who gets it */
           ...
           * pe_requests[j].val, pe_requests[j].key); */
        }
        else
        { /* We are not granting the request */
    ...
    for (i = 0; i < nnbrs; i++)
    {
      MPI_Irecv((void*)(match_requests + sendptr[i]),
                2 * (sendptr[i + 1] - sendptr[i]),
                IDX_DATATYPE,
                peind[i],
                1,
                ctrl->comm,
    ...
    for (i = 0; i < nnbrs; i++)
    {
      MPI_Isend((void*)(match_granted + recvptr[i]),
                2 * nreqs_pe[i],
                IDX_DATATYPE,
                peind[i],
                1,
  ...
  /* Traverse the vertices and those that were unmatched, match them with
   * themselves */
  cnvtxs = 0;
  for (i = 0; i < nvtxs; i++)
  {
    ...
    else if (match[i] >= KEEP_BIT)
    { /* A matched vertex which I get to keep */
      cnvtxs++;
    }
  }
 ...
 **************************************************************************/
void Mc_Global_CreateCoarseGraph(CtrlType* ctrl,
                                 GraphType* graph,
                                 WorkSpaceType* wspace,
                                 int cnvtxs)
{
  ...
  cmap = graph->cmap =
    idxmalloc(nvtxs + graph->nrecv, "CreateCoarseGraph: cmap");
   ...
   **************************************************************/
  cvtxdist = cgraph->vtxdist =
    idxmalloc(npes + 1, "CreateCoarseGraph: cvtxdist");
  cvtxdist[npes] =
    cnvtxs; /* Use last position in the cvtxdist as a temp buffer */

  MPI_Allgather((void*)(cvtxdist + npes),
                1,
                IDX_DATATYPE,
                (void*)cvtxdist,
                1,
  ...
  for (i = 0; i < nvtxs; i++)
  {
    if (match[i] >= KEEP_BIT)
    {
      ...
      if (k >= firstvtx && k < firstvtx + i)
        continue; /* Both (i,k) are local and i has been matched via the (k,i)
                     side */

      cmap[i] = cfirstvtx + cnvtxs++;
      if (k != firstvtx + i && (k >= firstvtx && k < lastvtx))
      { /* I'm matched locally */
        cmap[k - firstvtx] = cmap[i];
        match[k - firstvtx] +=
          KEEP_BIT; /* Add the KEEP_BIT to simplify coding */
      }
    }
  }
  ...
  {
    if (match[i] < KEEP_BIT)
    { /* Only vertices that go away satisfy this*/
   ...
   **************************************************************/
  /* Use wspace->pairs as the tmp space for the boundary vertices that are sent
   * and received */
  scand = wspace->pairs;
  rcand = graph->rcand = (KeyValueType*)GKmalloc(
    recvptr[nnbrs] * sizeof(KeyValueType), "CreateCoarseGraph: rcand");
  ...
  for (i = 0; i < nvtxs; i++)
  {
    ...
    else
    {
      ...
      { /* This is comming from afar */
        rcand[nrecv].key = k;
        rcand[nrecv].val =
          cmap[i] -
          cfirstvtx; /* Set it for use during the partition projection */
  ...
  rlens = graph->rlens =
    idxmalloc(nnbrs + 1, "CreateCoarseGraph: graph->rlens");
  slens = graph->slens =
    idxmalloc(nnbrs + 1, "CreateCoarseGraph: graph->slens");
  ...
  for (i = 0; i < nnbrs; i++)
  {
    if (rlens[i + 1] - rlens[i] > 0) MPI_Wait(ctrl->rreq + i, &ctrl->status);
  }
  for (i = 0; i < nnbrs; i++)
  {
    if (slens[i + 1] - slens[i] > 0) MPI_Wait(ctrl->sreq + i, &ctrl->status);
  }
  ...
  if ((4 + ncon) * (nrecv + nsend) + 2 * (nrecvsize + nsendsize) <=
      wspace->nlarge)
  {
  ...
  {
    rgraph = idxmalloc((4 + ncon) * nrecv + 2 * nrecvsize,
                       "CreateCoarseGraph: rgraph");
    sgraph = idxmalloc((4 + ncon) * nsend + 2 * nsendsize,
  ...
  for (l = i = 0; i < nnbrs; i++)
  {
    ...
    {
      MPI_Irecv((void*)(rgraph + l),
                (4 + ncon) * (rlens[i + 1] - rlens[i]) + 2 * rsizes[i],
                IDX_DATATYPE,
                peind[i],
                1,
                ctrl->comm,
  ...
  for (ll = l = i = 0; i < nnbrs; i++)
  {
    ...
    { /* Issue a send only if you are sending something */
      for (k = slens[i]; k < slens[i + 1]; k++)
      {
        ii = scand[k].val;
        sgraph[ll++] = firstvtx + ii;
        sgraph[ll++] = xadj[ii + 1] - xadj[ii];
        for (h = 0; h < ncon; h++) sgraph[ll++] = vwgt[ii * ncon + h];
      ...
      ASSERT(ctrl,
             ll - l == (4 + ncon) * (slens[i + 1] - slens[i]) + 2 * ssizes[i]);

      /* myprintf(ctrl, "Sending to pe:%d, %d lists of size %d\n", peind[i],
       * slens[i+1]-slens[i], ssizes[i]); */
      MPI_Isend((void*)(sgraph + l),
                ll - l,
                IDX_DATATYPE,
                peind[i],
                1,
  ...
  for (j = i = 0; i < nrecv; i++)
  {
     ...
     * BSearch(graph->nrecv, recvind, rgraph[j]), j+ncon); */
  ...
  cadjncy =
    idxmalloc(2 * (nkeepsize + nrecvsize), "CreateCoarserGraph: cadjncy");
  ...
  for (i = 0; i < nvtxs; i++)
  {
    if (match[i] >= KEEP_BIT)
    {
      ...
      for (h = 0; h < ncon; h++) cvwgt[cnvtxs * ncon + h] = vwgt[i * ncon + h];
      if (ctrl->partType == ADAPTIVE_PARTITION ||
          ctrl->partType == REFINE_PARTITION)
      {
      ...
      if (where != NULL) cwhere[cnvtxs] = where[i];
      nedges = 0;

      for (j = xadj[i]; j < xadj[i + 1]; j++)
      {
        k = cmap[ladjncy[j]];
        if (k != cfirstvtx + cnvtxs)
        { /* If this is not an internal edge */
          l = k & mask;
          if (htable[l] == -1)
          ...
          { /* Now you have to go and do a search. Expensive case */
            for (l = 0; l < nedges; l++)
            {
              if (cadjncy[cnedges + l] == k) break;
            }
            if (l < nedges)
            {
          ...
          for (j = 0; j < rgraph[u]; j++)
          {
            ...
            { /* If this is not an internal edge */
              ...
              { /* Now you have to go and do a search. Expensive case */
                ...
                {
                  cadjncy[cnedges + nedges] = k;
                  cadjwgt[cnedges + nedges++] =
                    rgraph[u + 3 + ncon + 2 * j + 1];
                }
              }
            }
          }
  ...
  for (j = 0; j < cnvtxs; j++)
    for (h = 0; h < ncon; h++)
      cgraph->nvwgt[j * ncon + h] =
        (float)(cvwgt[j * ncon + h]) / (float)(ctrl->tvwgts[h]);
