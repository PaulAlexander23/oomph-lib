 ...
 **************************************************************************/
GraphType* Mc_SetUpGraph(CtrlType* ctrl,
                         int ncon,
                         idxtype* vtxdist,
                         idxtype* xadj,
                         idxtype* vwgt,
                         ...
                         int* wgtflag)
{
  ...
  /* compute tvwgts */
  for (j = 0; j < ncon; j++) ltvwgts[j] = 0;

  for (i = 0; i < graph->nvtxs; i++)
    for (j = 0; j < ncon; j++) ltvwgts[j] += graph->vwgt[i * ncon + j];

  for (j = 0; j < ncon; j++) ctrl->tvwgts[j] = GlobalSESum(ctrl, ltvwgts[j]);
  ...
  {
    for (j = 0; j < ncon; j++)
      graph->nvwgt[i * ncon + j] =
        (float)(graph->vwgt[i * ncon + j]) / (float)(ctrl->tvwgts[j]);
  }
...
void SetUpCtrl(CtrlType* ctrl, int nparts, int dbglvl, MPI_Comm comm)
{
  ...
  ctrl->dbglvl = dbglvl;
  ctrl->nparts =
    nparts; /* Set the # of partitions is de-coupled from the # of domains */
                     ...
                     int from)
{
  ...
  { /* Change it from 1 to 0 */
    for (i = 0; i < npes + 1; i++) vtxdist[i]--;

    for (i = 0; i < nvtxs + 1; i++) xadj[i]--;
    for (i = 0; i < xadj[nvtxs]; i++) adjncy[i]--;
  }
  else
                         ...
                         int from)
{
  ...
  if (from == 1)
  { /* Change it from 1 to 0 */
    for (i = 0; i < npes + 1; i++) elmdist[i]--;

    for (i = 0; i < nelms + 1; i++) eptr[i]--;
    for (i = 0; i < eptr[nelms]; i++) eind[i]--;
  }
  else
  { /* Change it from 0 to 1 */
    for (i = 0; i < npes + 1; i++) elmdist[i]++;
    ...
    for (i = 0; i < xadj[nelms]; i++) adjncy[i]++;
    for (i = 0; i < nelms + 1; i++) xadj[i]++;

    if (part != NULL)
      for (i = 0; i < nelms; i++) part[i]++;
  }
}
  ...
  CtrlType* ctrl, GraphType* graph, int* nmoved, int* maxin, int* maxout)
{
  ...
  {
    lpvtxs[where[i]]++;
    if (where[i] != ctrl->mype) j++;
  }

  /* PrintVector(ctrl, ctrl->npes, 0, lpvtxs, "Lpvtxs: "); */

  MPI_Allreduce((void*)lpvtxs,
                (void*)gpvtxs,
                ctrl->nparts,
                IDX_DATATYPE,
                MPI_SUM,
