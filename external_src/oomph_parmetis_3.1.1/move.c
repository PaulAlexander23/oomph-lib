...
GraphType* Mc_MoveGraph(CtrlType* ctrl, GraphType* graph, WorkSpaceType* wspace)
{
  ...
  for (i = 0; i < nparts; i++) sinfo[i].key = sinfo[i].val = 0;
  ...
  for (i = 0; i < nparts; i++) lpwgts[i] = sinfo[i].key;

  MPI_Scan(
    (void*)lpwgts, (void*)gpwgts, nparts, IDX_DATATYPE, MPI_SUM, ctrl->comm);
  MPI_Allreduce(
    (void*)lpwgts, (void*)mvtxdist, nparts, IDX_DATATYPE, MPI_SUM, ctrl->comm);

  MAKECSR(i, nparts, mvtxdist);

  /* gpwgts[i] will store the label of the first vertex for each domain in each
   * processor */
  for (i = 0; i < nparts; i++) /* We were interested in an exclusive Scan */
    gpwgts[i] = mvtxdist[i] + gpwgts[i] - lpwgts[i];

  newlabel = idxmalloc(nvtxs + graph->nrecv, "MoveGraph: newlabel");

  for (i = 0; i < nvtxs; i++) newlabel[i] = gpwgts[where[i]]++;

  /* OK, now send the newlabel info to processors storing adjacent interface
   * nodes */
  CommInterfaceData(ctrl, graph, newlabel, wspace->indices, newlabel + nvtxs);

  /* Now lets tell everybody what and from where he will get it. Assume nparts
   * == npes */
  MPI_Alltoall(
    (void*)sinfo, 2, IDX_DATATYPE, (void*)rinfo, 2, IDX_DATATYPE, ctrl->comm);
  ...
  {
    /* Adjust core memory, incase the graph was originally very memory
     * unbalanced */
    GKfree((void**)&wspace->core, LTERM);
    wspace->maxcore =
      lpwgts[nparts] + 4 * gpwgts[nparts]; /* In spirit of the 8*nedges */
  ...
  {
    if (rinfo[i].key > 0)
      MPI_Irecv((void*)(rgraph + gpwgts[i]),
                gpwgts[i + 1] - gpwgts[i],
                IDX_DATATYPE,
                i,
                1,
  ...
  for (i = 0; i < nvtxs; i++)
  {
    ...
    for (h = 0; h < ncon; h++) sgraph[ii++] = vwgt[i * ncon + h];
  ...
  {
    if (sinfo[i].key > 0)
      MPI_Isend((void*)(sgraph + lpwgts[i]),
                lpwgts[i + 1] - lpwgts[i],
                IDX_DATATYPE,
                i,
                1,
  ...
  */
  ...
  for (i = 0; i < nparts; i++)
  {
    if (sinfo[i].key > 0) MPI_Wait(ctrl->sreq + i, &ctrl->status);
  }
  for (i = 0; i < nparts; i++)
  {
    if (rinfo[i].key > 0) MPI_Wait(ctrl->rreq + i, &ctrl->status);
  }
  ...
  {
    xadj[i] = rgraph[ii++];
    for (h = 0; h < ncon; h++) vwgt[i * ncon + h] = rgraph[ii++];
    for (j = 0; j < xadj[i]; j++)
    {
      adjncy[jj] = rgraph[ii++];
      adjwgt[jj++] = rgraph[ii++];
                     ...
                     WorkSpaceType* wspace)
{
  ...
  for (i = 0; i < nvtxs; i++) rinfo[where[i]]++;

  /* The rinfo are transposed and become the sinfo for the back-projection */
  MPI_Alltoall(
    (void*)rinfo, 1, IDX_DATATYPE, (void*)sinfo, 1, IDX_DATATYPE, ctrl->comm);
  ...
  for (i = 0; i < nparts; i++)
  {
    if (rinfo[i + 1] - rinfo[i] > 0)
      MPI_Irecv((void*)(auxinfo + rinfo[i]),
                rinfo[i + 1] - rinfo[i],
                IDX_DATATYPE,
                i,
                1,
  ...
  for (i = 0; i < nparts; i++)
  {
    if (rinfo[i + 1] - rinfo[i] > 0) MPI_Wait(ctrl->rreq + i, &ctrl->status);
  }
  for (i = 0; i < nparts; i++)
  {
    if (sinfo[i + 1] - sinfo[i] > 0) MPI_Wait(ctrl->sreq + i, &ctrl->status);
  }

  /* Scatter the info received in auxinfo back to info. */
  for (i = 0; i < nvtxs; i++) info[i] = auxinfo[rinfo[where[i]]++];
                 ...
                 WorkSpaceType* wspace)
{
  ...
  /* Here we care about the count and not total weight (diff since graph may be
   * weighted */
  idxset(nparts, 0, lpwgts);
  for (i = 0; i < nvtxs; i++) lpwgts[where[i]]++;
  ...
  for (i = 0; i < nparts; i++)
    gpwgts[i] = mvtxdist[i] + gpwgts[i] -
                lpwgts[i]; /* We were interested in an exclusive Scan */

  for (i = 0; i < nvtxs; i++) perm[i] = gpwgts[where[i]]++;
  ...
  for (i = 0; i < nvtxs; i++)
  {
    for (j = xadj[i]; j < xadj[i + 1]; j++)
    {
      ...
      if (adjncy[j] >= firstvtx && adjncy[j] < lastvtx)
      {
        ...
        {
          if (adjncy[jj] == firstvtx + i) break;
        }
        if (jj == xadj[k + 1])
          myprintf(ctrl,
                   "(%d %d) but not (%d %d) [%d %d] [%d %d]\n",
                   i,
                   k,
                   k,
                   i,
