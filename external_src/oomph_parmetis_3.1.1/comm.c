                       ...
                       idxtype* recvvector)
{
  ...
  for (i = 0; i < nnbrs; i++)
  {
    MPI_Irecv((void*)(recvvector + recvptr[i]),
  ...
  k = sendptr[nnbrs];
  for (i = 0; i < k; i++) sendvector[i] = data[sendind[i] - firstvtx];

  for (i = 0; i < nnbrs; i++)
  {
    MPI_Isend((void*)(sendvector + sendptr[i]),
                              ...
                              idxtype* psendptr)
{
  ...
  for (i = 0; i < nnbrs; i++)
  {
    MPI_Irecv((void*)(recvpairs + recvptr[i]),
  ...
  if (nchanged != 0)
  {
    ...
    for (i = 0; i < nnbrs; i++)
    {
      MPI_Isend((void*)(sendpairs + sendptr[i]),
                2 * (psendptr[i] - sendptr[i]),
                IDX_DATATYPE,
                peind[i],
                1,
                ctrl->comm,
  ...
  for (i = 0; i < nnbrs; i++)
  {
    ...
    if (n != 0)
    {
      ...
      for (k = 0; k < n; k++) data[pairs[k].key] = pairs[k].val;
    }
  }
