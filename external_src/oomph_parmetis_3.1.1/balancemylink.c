                  ...
                  float avgvwgt)
{
  ...
  float *nvwgt, lbvec[MAXNCON], pwgts[MAXNCON * 2], tpwgts[MAXNCON * 2],
    my_wgt[MAXNCON];
  ...
  queues =
    (FPQueueType*)(GKmalloc(sizeof(FPQueueType) * nqueues * 2, "queues"));

  for (i = 0; i < nvtxs; i++)
    hval[i] = Mc_HashVwgts(ncon, nvwgt + i * ncon) - minval;

  for (i = 0; i < nvtxs; i++) nvpq[hval[i]]++;

  ptr[0] = 0;
  for (i = 0; i < nqueues; i++) ptr[i + 1] = ptr[i] + nvpq[i];
  ...
  for (i = nqueues - 1; i > 0; i--) ptr[i] = ptr[i - 1];
  ptr[0] = 0;
  ...
  for (j = 0; j < nvtxs; j++)
    for (k = xadj[j]; k < xadj[j + 1]; k++)
      if (where[adjncy[k]] == where[j]) id[j] += adjwgt[k];
      else
        ed[j] += adjwgt[k];
  ...
  for (pass = 0; pass < N_MOC_BAL_PASSES; pass++)
  {
    ...
    for (j = 0; j < nvtxs; j++)
    {
      ...
      {
        if (where[j] == home[j]) newgain -= redist_factor * (float)vsize[j];
        else
    ...
    for (ii = 0; ii < nvtxs / 2; ii++)
    {
      ...
      if (from != -1 && qnum == -1)
      {
        ...
        {
          for (j = 0; j < ncon; j++)
            if (flows[j] > avgvwgt) break;
        }
        else
        {
          for (j = 0; j < ncon; j++)
            if (flows[j] < -1.0 * avgvwgt) break;
        }
      ...
      for (j = 0; j < ncon; j++)
        flows[j] +=
          (to == me) ? nvwgt[vtx * ncon + j] : -1.0 * nvwgt[vtx * ncon + j];
  ...
  for (i = 0; i < nvtxs; i++)
    if (where[i] == me)
      for (h = 0; h < ncon; h++) my_wgt[h] += nvwgt[i * ncon + h];

  for (i = 0; i < ncon; i++)
  {
    ftmp = (pwgts[i] + pwgts[ncon + i]) / 2.0;
    if (ftmp != 0.0) lbvec[i] = fabs(my_wgt[i] - tpwgts[i]) / ftmp;
    else
      lbvec[i] = 0.0;
  }
  ...
  *diff_lbavg = lbavg;
  ...
  for (i = 0; i < nvtxs; i++)
  {
    if (where[i] != home[i]) totalv += vsize[i];

    for (j = xadj[i]; j < xadj[i + 1]; j++)
      if (where[adjncy[j]] != where[i]) cut += adjwgt[j];
  }
  cut /= 2;
