                              ...
                              MPI_Comm* comm)
{
  ...
  if (npes == 1 && inparts > 1)
  {
    ...
    else
    {
      ...
      for (i = 0; i < inparts; i++) mytpwgts[i] = itpwgts[i * incon];

      moptions[7] = -1;
      METIS_mCPartGraphRecursive2(&nvtxs,
                                  &incon,
                                  xadj,
                                  adjncy,
                                  vwgt,
  ...
  graph =
    Mc_SetUpGraph(&ctrl, 1, vtxdist, xadj, uvwgt, adjncy, adjwgt, &uwgtflag);
  free(graph->nvwgt);
  graph->nvwgt = NULL;
  ...
  free(uvwgt);
  graph->vwgt =
    ((iwgtflag & 2) != 0) ? vwgt : idxsmalloc(graph->nvtxs * incon, 1, "vwgt");
  graph->ncon = incon;
  j = ctrl.nparts;
  ...
  /**********************************************************/
  /* compute tvwgts */
  for (j = 0; j < incon; j++) ltvwgts[j] = 0;

  for (i = 0; i < graph->nvtxs; i++)
    for (j = 0; j < incon; j++) ltvwgts[j] += mgraph->vwgt[i * incon + j];

  for (j = 0; j < incon; j++) ctrl.tvwgts[j] = GlobalSESum(&ctrl, ltvwgts[j]);
  ...
  for (i = 0; i < incon; i++)
  {
    ...
    {
      if (ctrl.mype == 0)
        printf("ERROR: sum weight for constraint %d is zero\n", i);
      MPI_Finalize();
      exit(-1);
    }
  }
  ...
  for (i = 0; i < mgraph->nvtxs; i++)
    for (j = 0; j < incon; j++)
      mgraph->nvwgt[i * incon + j] =
        (float)(mgraph->vwgt[i * incon + j]) / (float)(ctrl.tvwgts[j]);
      ...
      GlobalSESum(&ctrl, mgraph->nedges) == 0)
  {
    IFSET(ctrl.dbglvl,
          DBG_INFO,
          rprintf(&ctrl,
                  "Partitioning a graph of size %d serially\n",
                  vtxdist[npes]));
  ...
  *edgecut = mgraph->mincut;
  ...
  {
    rprintf(
      &ctrl, "Final %d-way CUT: %6d \tBalance: ", inparts, mgraph->mincut);
    avg = 0.0;
    for (h = 0; h < incon; h++)
    {
      maximb = 0.0;
      for (i = 0; i < inparts; i++)
        maximb =
          amax(maximb, mgraph->gnpwgts[i * incon + h] / itpwgts[i * incon + h]);
      avg += maximb;
