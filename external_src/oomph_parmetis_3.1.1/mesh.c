                           ...
                           MPI_Comm* comm)
{
  ...
  for (i = 0; i < eptr[nelms]; i++) eind[i] -= gminnode;
  ...
  nodelist =
    (KeyValueType*)GKmalloc(eptr[nelms] * sizeof(KeyValueType), "nodelist");
  ...
  {
    if (nodelist[i].key > nodelist[i - 1].key) count++;
  }
  ...
  for (i = 1; i < eptr[nelms]; i++)
  {
    ...
    nodelist[i].val =
      auxarray[nodelist[i].val]; /* Store the local element ID */
  }
  MPI_Barrier(*comm);
  ...
  {
    while (nodelist[i].key >= nodedist[pe + 1]) pe++;
    scounts[pe] += 2;
  }
  ASSERTS(pe < npes);
  ...
  nrecv = rdispl[npes] / 2;
  recvbuffer = (KeyValueType*)GKmalloc(amax(1, nrecv) * sizeof(KeyValueType),
                                       "recvbuffer");

  MPI_Alltoallv((void*)nodelist,
                scounts,
                sdispl,
                IDX_DATATYPE,
                (void*)recvbuffer,
  ...
  for (pe = 0; pe < npes; pe++)
  {
    for (j = rdispl[pe] / 2; j < rdispl[pe + 1] / 2; j++)
    {
      ...
      if (htable[lnode] == -1)
      {
        ...
        {
          if (k == gnptr[lnode]) sbuffer[count++] = -1 * (gnind[k] + 1);
          else
    ...
    for (i = 0; i < nelms; i++)
    {
      for (count = 0, j = eptr[i]; j < eptr[i + 1]; j++)
      {
        ...
        for (k = nptr[node]; k < nptr[node + 1]; k++)
        {
          ...
          /* Adjust the memory.
             This will be replaced by a idxrealloc() when GKlib will be
             incorporated */
          if (count == maxcount - 1)
          {
            idxtype* tmpptr;
  ...
  /*****************************************/
  for (i = 0; i < eptr[nelms]; i++) eind[i] = nmap[eind[i]] + gminnode;

  if (*numflag == 1)
    ChangeNumberingMesh(
      elmdist, eptr, eind, myxadj, myadjncy, NULL, npes, mype, 0);
