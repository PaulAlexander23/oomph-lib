...
float WavefrontDiffusion(CtrlType* ctrl, GraphType* graph, idxtype* home)
{
  ...
  perm =
    idxmalloc(2 * nvtxs + 2 * nparts + nedges + 1, "WavefrontDiffusion: perm");
  ed = perm + nvtxs;
  psize = perm + 2 * nvtxs;
  ...
  for (i = 0; i < nvtxs; i++) psize[where[i]]++;
  ...
  for (l = 0; l < npasses; l++)
  {
    ...
    for (ii = 0; ii < nvtxs / 3; ii++)
    {
      ...
      if (psize[from] == 1) continue;

      clean = (from == home[i]) ? 1 : 0;

      /* only move from top three or dirty vertices */
      if (from != first && from != second && from != third && clean) continue;
        ...
        if (from != to)
        {
          if (tmpvec[to] > (flowFactor * nvwgt[i]))
          {
            ...
            for (k = xadj[i]; k < xadj[i + 1]; k++)
            {
              ...
              if (where[edge] == from) ed[edge] += adjwgt[k];
              if (where[edge] == to) ed[edge] -= adjwgt[k];
            }
            break;
          }
        }
    ...
    if (l % 2 == 1)
    {
      ...
      if (balance < ubfactor + 0.035) done = 1;

      if (GlobalSESum(ctrl, done) > 0) break;

      noswaps = (nswaps > 0) ? 0 : 1;
      if (GlobalSESum(ctrl, noswaps) > ctrl->npes / 2) break;
    }
  }

  graph->mincut = ComputeSerialEdgeCut(graph);
  totalv = Mc_ComputeSerialTotalV(graph, home);
  cost = ctrl->ipc_factor * (float)graph->mincut +
         ctrl->redist_factor * (float)totalv;


CleanUpAndExit:
  GKfree(
    (void**)&solution, (void**)&perm, (void**)&workspace, (void**)&cand, LTERM);
