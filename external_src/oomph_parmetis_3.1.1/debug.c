...
void PrintVector(CtrlType* ctrl, int n, int first, idxtype* vec, char* title)
{
  ...
  for (penum = 0; penum < ctrl->npes; penum++)
  {
    if (ctrl->mype == penum)
    {
      if (ctrl->mype == 0) printf("%s\n", title);
      printf("\t%3d. ", ctrl->mype);
      for (i = 0; i < n; i++) printf("[%d %d] ", first + i, vec[i]);
      printf("\n");
      fflush(stdout);
    }
    ...
    if (ctrl->mype == penum)
    {
      ...
      for (i = 0; i < n; i++)
        printf("[%d %d.%d] ",
               first + i,
               (vec[i] >= KEEP_BIT ? 1 : 0),
               (vec[i] >= KEEP_BIT ? vec[i] - KEEP_BIT : vec[i]));
      printf("\n");
      fflush(stdout);
    }
    MPI_Barrier(ctrl->comm);
...
void PrintGraph(CtrlType* ctrl, GraphType* graph)
{
  ...
  for (penum = 0; penum < ctrl->npes; penum++)
  {
    if (ctrl->mype == penum)
    {
      ...
      {
        if (i == 0) printf("\t%2d %2d\t", firstvtx + i, graph->vwgt[i]);
        else
 ...
 **************************************************************************/
void WriteMetisGraph(
  int nvtxs, idxtype* xadj, idxtype* adjncy, idxtype* vwgt, idxtype* adjwgt)
{
  int i, j;
  FILE* fp;
