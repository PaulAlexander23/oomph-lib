 ...
 **************************************************************************/
void Mc_InitPartition_RB(CtrlType* ctrl,
                         GraphType* graph,
                         WorkSpaceType* wspace)
{
  ...
  if (lnparts == 1)
  {
    ...
    if (mype == fpe)
    {
      for (i = 0; i < agraph->nvtxs; i++) gwhere0[agraph->label[i]] = fpart;
    }
  }
  ...
  MPI_Allreduce(
    (void*)gwhere0, (void*)gwhere1, gnvtxs, IDX_DATATYPE, MPI_SUM, ipcomm);
  ...
  if (ngroups > 1)
  {
    ...
    MPI_Allreduce(
      (void*)&edgecut, (void*)&max_cut, 1, MPI_INT, MPI_MAX, ctrl->gcomm);
    MPI_Allreduce(
      (void*)&lbsum, (void*)&min_lbsum, 1, MPI_FLOAT, MPI_MIN, ctrl->gcomm);
 ...
 **************************************************************************/
void Mc_KeepPart(GraphType* graph,
                 WorkSpaceType* wspace,
                 idxtype* part,
                 int mypart)
{
  ...
  {
    if (part[i] == mypart) rename[i] = mynvtxs++;
  }
  ...
  for (mynvtxs = 0, mynedges = 0, j = xadj[0], i = 0; i < nvtxs; i++)
  {
    if (part[i] == mypart)
    {
      ...
      for (h = 0; h < ncon; h++) vwgt[mynvtxs * ncon + h] = vwgt[i * ncon + h];
