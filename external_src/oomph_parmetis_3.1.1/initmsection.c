...
/************************************************************************************/
/*!
 The entry point of the algorithm that finds the separators of the coarsest
 graph. This algorithm first assembles the graph to all the processors, it then
 splits the processors into groups depending on the number of partitions for
 which we want to compute the separator. The processors of each group compute
 the separator of their corresponding graph and the smallest separator is
 selected.
 ...
 graph->where[] = 0 for all vertices. Similarly, if nparts==4, it indicates that
 we have a graph that is already partitioned into two parts (as specified in
 graph->where) and we need to find the separator of each one of these parts.

 The final partitioning is encoded in the graph->where vector as follows. If
 nparts is the number of partitions, the left, right, and separator
 subpartitions of the original partition i will be labeled 2*i, 2*i+1, and
 nparts+2*i, respectively. Note that in the above expressions, i goes from
 [0...nparts/2]. As a result of this encoding, the left (i.e., 0th) partition of
 a node \c i on the separator will be given by where[i]%nparts.
...
void InitMultisection(CtrlType* ctrl, GraphType* graph, WorkSpaceType* wspace)
{
  ...
  switch (ctrl->ipart)
  {
    case ISEP_EDGE:
      ...
      agraph->where =
        idxmalloc(agraph->nvtxs, "InitMultisection: agraph->where");

      METIS_EdgeComputeSeparator(&agraph->nvtxs,
                                 agraph->xadj,
                                 agraph->adjncy,
                                 agraph->vwgt,
                                 agraph->adjwgt,
    ...
    case ISEP_NODE:
      ...
      METIS_NodeComputeSeparator(&agraph->nvtxs,
                                 agraph->xadj,
                                 agraph->adjncy,
                                 agraph->vwgt,
                                 agraph->adjwgt,
  ...
  for (i = 0; i < agraph->nvtxs; i++)
  {
    ...
    if (agraph->where[i] == 2) agraph->where[i] = ctrl->nparts + 2 * mypart;
    else
  ...
  if (lpecut[1] == 0)
  {
    ...
    for (i = 0; i < agraph->nvtxs; i++) gwhere[label[i]] = agraph->where[i];
  }
  ...
  GKfree((void**)&graph->where,
         LTERM); /* Remove the propagated down where info */
  ...
  MPI_Scatterv((void*)agraph->where,
               sendcounts,
               displs,
               IDX_DATATYPE,
               (void*)graph->where,
                                    ...
                                    WorkSpaceType* wspace)
{
  ...
  MPI_Allgather(
    (void*)(&mysize), 1, MPI_INT, (void*)recvcounts, 1, MPI_INT, ctrl->comm);
