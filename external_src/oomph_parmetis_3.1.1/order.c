...
/*!
  ...
  \param sizes is the 2*nparts array that will store the sizes of each
  subdomains and the sizes of the separators at each level. Note that the
                     ...
                     WorkSpaceType* wspace)
{
  ...
  for (i = 0; i < nvtxs; i++) perm[i] = i;
  lastnode[2] = graph->gnvtxs;

  idxset(nvtxs, -1, order);

  /* This is used as a pointer to the end of the sizes[] array (i.e., >=nparts)
     that has not yet been filled in so that the separator sizes of the
     succesive levels will be stored correctly. It is used in LabelSeparatos()
   */
  sizes[0] = 2 * npes - 1;
  ...
  /* Fill in the sizes[] array for the local part. Just the vtxdist of the
   * mgraph */
  for (i = 0; i < npes; i++)
  ...
  LocalNDOrder(ctrl,
               mgraph,
               morder,
               lastnode[2 * (ctrl->npes + ctrl->mype)] - mgraph->nvtxs,
               wspace);
  ...
  FreeGraph(mgraph);
  GKfree(
    (void**)&perm, (void**)&lastnode, (void**)&porder, (void**)&morder, LTERM);
...
/*********************************************************************************/
/*!
  This function is used to assign labels to the nodes in the separators It uses
  the appropriate entry in the lastnode array to select label boundaries and
  adjusts it for the next level.
                     ...
                     WorkSpaceType* wspace)
{
  ...
  for (i = nparts - 2; i >= 0; i -= 2) sizes[--sizes[0]] = gpwgts[nparts + i];
  ...
  for (i = 0; i < 2 * nparts; i++) sizescan[i] -= lpwgts[i];
  ...
  for (i = 0; i < nparts; i += 2)
  {
    lastnode[2 * nparts + 2 * i] =
      sizescan[nparts + i] - gpwgts[nparts + i] - gpwgts[i + 1];
    lastnode[2 * nparts + 2 * (i + 1)] =
      sizescan[nparts + i] - gpwgts[nparts + i];
  }
                  ...
                  WorkSpaceType* wspace)
{
  ...
  for (i = 0; i < nvtxs; i++)
  {
    if (where[i] < nparts)
    {
      ...
      {
        ASSERT(ctrl,
               where[i] == where[adjncy[j]] || where[adjncy[j]] >= nparts);
...
void Order_Partition(CtrlType* ctrl, GraphType* graph, WorkSpaceType* wspace)
{
       ...
       graph->gnvtxs > graph->finer->gnvtxs * COARSEN_FRACTION))
  {
    ...
    if (graph->finer == NULL)
    { /* Do that only if no-coarsening took place */
  ...
  { /* Coarsen it and then partition it */
