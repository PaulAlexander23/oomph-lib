...
void ikeysort(int total_elems, KeyValueType* pbase)
{
  ...
  if (total_elems > MAX_THRESH)
  {
    ...
    while (STACK_NOT_EMPTY)
    {
      ...
      if (mid->key < lo->key) QSSWAP(*mid, *lo, stmp);
      if (hi->key < mid->key) QSSWAP(*mid, *hi, stmp);
      else
        goto jump_over;
      if (mid->key < lo->key) QSSWAP(*mid, *lo, stmp);
   ...
   that this algorithm runs much faster than others. */
      do
      {
        while (left_ptr->key < pivot.key) left_ptr++;

        while (pivot.key < right_ptr->key) right_ptr--;
...

  {
    ...
    KeyValueType* thresh =
      (end_ptr < pbase + MAX_THRESH ? end_ptr : pbase + MAX_THRESH);
    ...
    for (run_ptr = tmp_ptr + 1; run_ptr <= thresh; run_ptr++)
      if (run_ptr->key < tmp_ptr->key) tmp_ptr = run_ptr;

    if (tmp_ptr != pbase) QSSWAP(*tmp_ptr, *pbase, stmp);
    ...
    {
      tmp_ptr = run_ptr - 1;
      while (run_ptr->key < tmp_ptr->key) tmp_ptr--;
      ...
      if (tmp_ptr != run_ptr)
      {
        ...
        for (mptr = run_ptr; mptr > tmp_ptr; mptr--) *mptr = *(mptr - 1);
        *mptr = elmnt;
      }
    }
  }
