 ...
 **************************************************************************/
void Mc_ComputeSerialBalance(CtrlType* ctrl,
                             GraphType* graph,
                             idxtype* where,
                             float* ubvec)
{
  ...
  /* The +1 in the following code is to deal with bad cases of tpwgts[i*ncon+j]
   * == 0 */
  for (j = 0; j < ncon; j++)
  {
    maximb = 0.0;
    for (i = 0; i < nparts; i++)
      maximb = amax(maximb,
                    (1.0 + (float)pwgts[i * ncon + j]) /
                      (1.0 + (tpwgts[i * ncon + j] * (float)tvwgts[j])));
    ubvec[j] = maximb;
  }
 ...
 * This function computes the balance of the partitioning
 **************************************************************************/
void Mc_ComputeParallelBalance(CtrlType* ctrl,
                               GraphType* graph,
                               idxtype* where,
                               float* ubvec)
{
  ...
  for (i = 0; i < nvtxs; i++)
  {
    for (j = 0; j < ncon; j++)
    {
      ...
      /* The following is to deal with tpwgts[] that are 0.0 for certain
       * partitions/constraints */
      lminvwgts[j] =
        (nvwgt[i * ncon + j] > 0.0 && lminvwgts[j] > nvwgt[i * ncon + j] ?
           nvwgt[i * ncon + j] :
           lminvwgts[j]);
    }
  }
  ...
  /* The +gminvwgts[j] in the following code is to deal with bad cases of
   * tpwgts[i*ncon+j] == 0 */
  for (j = 0; j < ncon; j++)
  {
    maximb = 0.0;
...
void Mc_PrintThrottleMatrix(CtrlType* ctrl, GraphType* graph, float* matrix)
{
  ...
  for (i = 0; i < ctrl->npes; i++)
  {
    if (i == ctrl->mype)
    {
      for (j = 0; j < ctrl->npes; j++) printf("%.3f ", matrix[j]);
      printf("\n");
      fflush(stdout);
    }
...
void Mc_ComputeRefineStats(CtrlType* ctrl, GraphType* graph, float* ubvec)
{
  ...
  if (mype == 0)
  {
    ...
    for (h = 0; h < ncon; h++)
    {
      for (i = 0; i < nparts; i++) printf("%9.3f ", gnpwgts[i * ncon + h]);
      printf("\n");
    }
    printf("\n");
  }
  ...
  /***************************************/
  for (i = 0; i < nparts; i++) connect[i * nparts + i] = -1;
  ...
  if (mype == 0)
  {
    ...
    {
      printf("%d: ", i);
      for (j = 0; j < nparts; j++) printf("%9d ", gconnect[i * nparts + j]);
      printf("\n");
    }
    printf("\n");
  }
  ...
  if (mype == 0)
  {
    ...
    for (i = 0; i < nparts; i++) printf("%9d ", gborder[i]);
    printf("\n\n");
  }
  ...
  for (i = 0; i < nvtxs; i++)
  {
    ...
    for (j = 0; j < rinfo[i].ndegrees; j++)
    {
      ...
      for (h = 0; h < ncon; h++)
        if (gnpwgts[other * ncon + h] + nvwgt[h] > badmaxpwgt[h]) break;
  ...
  MPI_Reduce(
    (void*)from, (void*)gfrom, nparts, IDX_DATATYPE, MPI_SUM, 0, ctrl->comm);
  MPI_Reduce(
    (void*)to, (void*)gto, nparts, IDX_DATATYPE, MPI_SUM, 0, ctrl->comm);
  ...
  MPI_Reduce(
    (void*)gain, (void*)ggain, 20, IDX_DATATYPE, MPI_SUM, 0, ctrl->comm);
  if (mype == 0)
  {
