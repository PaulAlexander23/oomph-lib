                         ...
                         WorkSpaceType* wspace)
{
  ...
  graph->where =
    idxmalloc(graph->nvtxs + graph->nrecv, "PartitionSmallGraph: where");
  ...
  else
  {
    ...
    for (i = 0; i < nparts; i++) mytpwgts[i] = ctrl->tpwgts[i * ncon];
                 ...
                 MPI_Comm* comm)
{
  ...
  if (wgtflag == NULL)
  {
    *iwgtflag = 0;
    IFSET(dbglvl,
          DBG_INFO,
          printf("WARNING: wgtflag is NULL.  Using a value of 0.\n"));
  }
  else
  {
    *iwgtflag = *wgtflag;
  ...
  if (numflag == NULL)
  {
    *inumflag = 0;
    IFSET(dbglvl,
          DBG_INFO,
          printf("WARNING: numflag is NULL.  Using a value of 0.\n"));
  }
  else
  {
    if (*numflag != 0 && *numflag != 1)
    {
      IFSET(dbglvl,
            DBG_INFO,
            printf("WARNING: bad value for numflag %d.  Using a value of 0.\n",
                   *numflag));
      *inumflag = 0;
    }
    else
  ...
  if (ncon == NULL)
  {
    *incon = 1;
    IFSET(dbglvl,
          DBG_INFO,
          printf("WARNING: ncon is NULL.  Using a value of 1.\n"));
  }
  else
  {
    if (*ncon < 1 || *ncon > MAXNCON)
    {
      IFSET(dbglvl,
            DBG_INFO,
            printf("WARNING: bad value for ncon %d.  Using a value of 1.\n",
                   *ncon));
      *incon = 1;
    }
    else
  ...
  if (nparts == NULL)
  {
    *inparts = npes;
    IFSET(dbglvl,
          DBG_INFO,
          printf("WARNING: nparts is NULL.  Using a value of %d.\n", npes));
  }
  else
  {
    if (*nparts < 1 || *nparts > MAX_NPARTS)
    {
      IFSET(dbglvl,
            DBG_INFO,
            printf("WARNING: bad value for nparts %d.  Using a value of %d.\n",
                   *nparts,
                   npes));
      *inparts = npes;
    }
  ...
  if (tpwgts == NULL)
  {
    ...
    IFSET(
      dbglvl,
      DBG_INFO,
      printf("WARNING: tpwgts is NULL.  Setting all array elements to %.3f.\n",
             1.0 / (float)(*inparts)));
  }
  else
  {
    for (i = 0; i < *incon; i++)
    {
      ...
      if (fabs(1.0 - tsum) < SMALLFLOAT) tsum = 1.0;
      for (j = 0; j < *inparts; j++)
  ...
  else
  {
    for (i = 0; i < *incon; i++)
    {
      if (ubvec[i] < 1.0 || ubvec[i] > (float)(*inparts))
      {
        ...
        IFSET(dbglvl,
              DBG_INFO,
              printf("WARNING: bad value for ubvec[%d]: %.3f.  Setting value "
                     "to 1.05.[%d]\n",
                     i,
                     ubvec[i],
                     *inparts));
  ...
  if (partType == ADAPTIVE_PARTITION)
  {
    if (ipc2redist != NULL)
    {
      if (*ipc2redist < SMALLFLOAT || *ipc2redist > 1000000.0)
      {
        IFSET(
          ...
          DBG_INFO,
          printf(
            "WARNING: bad value for ipc2redist %.3f.  Using a value of 1000.\n",
            *ipc2redist));
        *iipc2redist = 1000.0;
      }
      else
    ...
    else
    {
      IFSET(dbglvl,
            DBG_INFO,
            printf("WARNING: ipc2redist is NULL.  Using a value of 1000.\n"));
      *iipc2redist = 1000.0;
    }
  }
  /**************************************/
  ...
  if (options == NULL)
  {
    ioptions[0] = 0;
    IFSET(
      dbglvl, DBG_INFO, printf("WARNING: options is NULL.  Using defaults\n"));
  }
  else
  {
  ...
  {
    MPI_Allreduce(
      (void*)&doiabort, (void*)&doweabort, 1, MPI_INT, MPI_MAX, *comm);
    if (doweabort > 0) abort();
  }
  /**************************************/
}
