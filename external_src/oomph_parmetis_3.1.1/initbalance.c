...
void Balance_Partition(CtrlType* ctrl, GraphType* graph, WorkSpaceType* wspace)
{
  ...
  xadj = cgraph.xadj =
    idxmalloc(nvtxs * (5 + ncon) + 1 + nedges * 2, "U_IP: xadj");
  ...
  /* ADD: this is necessary because serial metis does not support the general
   * case */
  mytpwgts = fsmalloc(ctrl->nparts, 0.0, "mytpwgts");
  for (i = 0; i < ctrl->nparts; i++)
    for (j = 0; j < ncon; j++) mytpwgts[i] += ctrl->tpwgts[i * ncon + j];
  for (i = 0; i < ctrl->nparts; i++) mytpwgts[i] /= (float)ncon;
  ...
  if (ctrl->ps_relation == DISCOUPLED)
  {
    ...
    MPI_Allgatherv((void*)graph->home,
                   graph->nvtxs,
                   IDX_DATATYPE,
                   (void*)part,
                   rcounts,
    ...
    for (i = 0; i < agraph->nvtxs; i++) home[i] = part[i];
  ...
  {
    for (i = 0; i < ctrl->npes; i++)
      for (j = vtxdist[i]; j < vtxdist[i + 1]; j++) part[j] = home[j] = i;
  }

  /* Ensure that the initial partitioning is legal */
  for (i = 0; i < agraph->nvtxs; i++)
  {
    if (part[i] >= ctrl->nparts) part[i] = home[i] = part[i] % ctrl->nparts;
    if (part[i] < 0) part[i] = home[i] = (-1 * part[i]) % ctrl->nparts;
  }
  /****************************************/
  /****************************************/
  ...
  IFSET(ctrl->dbglvl,
        DBG_REFINEINFO,
        Mc_ComputeSerialBalance(ctrl, agraph, agraph->where, lbvec));
  IFSET(
    ctrl->dbglvl,
    DBG_REFINEINFO,
    rprintf(ctrl, "input cut: %d, balance: ", ComputeSerialEdgeCut(agraph)));
  ...
  if (sr == 1)
  {
    ...
    while (lnpes > 1 && lnparts > 1)
    {
      ...
      if (agraph->ncon == 1)
      {
        METIS_WPartGraphKway2(&agraph->nvtxs,
                              agraph->xadj,
                              agraph->adjncy,
                              agraph->vwgt,
                              agraph->adjwgt,
      ...
      {
        METIS_mCPartGraphRecursive2(&agraph->nvtxs,
                                    &ncon,
                                    agraph->xadj,
                                    agraph->adjncy,
                                    agraph->vwgt,
      ...
      sscale(lnparts - lnparts / 2,
             1.0 / (1.0 - wsum),
             mytpwgts + fpart + lnparts / 2);
    ...
    if (lnparts == 1)
    {
      ...
      if (srmype == fpe)
      {
        for (i = 0; i < agraph->nvtxs; i++) lwhere[agraph->label[i]] = fpart;
      }
    }
    ...
    MPI_Allreduce(
      (void*)lwhere, (void*)part, nvtxs, IDX_DATATYPE, MPI_SUM, srcomm);

    edgecut = ComputeSerialEdgeCut(&cgraph);
    Mc_ComputeSerialBalance(ctrl, &cgraph, part, lbvec);
    lbsum = ssum(ncon, lbvec);
    MPI_Allreduce(
      (void*)&edgecut, (void*)&max_cut, 1, MPI_INT, MPI_MAX, ipcomm);
    MPI_Allreduce(
      (void*)&lbsum, (void*)&min_lbsum, 1, MPI_FLOAT, MPI_MIN, ipcomm);
    ...
    MPI_Allreduce(
      (void*)&lpecost, (void*)&gpecost, 1, MPI_FLOAT_INT, MPI_MINLOC, ipcomm);
    ...
    {
      MPI_Recv(
        (void*)part, nvtxs, IDX_DATATYPE, gpecost.rank, 1, ctrl->comm, &status);
    }
  ...
  else
  {
    ...
    if (ncon == 1)
    {
      ...
      MPI_Allreduce(
        (void*)&rating, (void*)&max_rating, 1, MPI_FLOAT, MPI_MAX, ipcomm);
      MPI_Allreduce(
        (void*)&lbsum, (void*)&min_lbsum, 1, MPI_FLOAT, MPI_MIN, ipcomm);
      ...
      if (min_lbsum < UNBALANCE_FRACTION * (float)(ncon))
      {
        if (lbsum < UNBALANCE_FRACTION * (float)(ncon)) lpecost.cost = rating;
        else
  ...
  if (graph->ncon <= MAX_NCON_FOR_DIFFUSION)
  {
    if (ctrl->mype == sr_pe || ctrl->mype == gd_pe)
    {
      ...
      my_cost =
        ctrl->ipc_factor * my_cut + REDIST_WGT * ctrl->redist_base * my_totalv;

      IFSET(ctrl->dbglvl,
            DBG_REFINEINFO,
            printf("%s initial cut: %.1f, totalv: %.1f, balance: %.3f\n",
  ...
  /* For whatever reason, FreeGraph crashes here...so explicitly free the
    memory. FreeGraph(agraph);
  */
...
/* NOTE: this subroutine should work for static, adaptive, single-, and
 * multi-contraint */
/*************************************************************************
 * This function assembles the graph into a single processor
 **************************************************************************/
GraphType* Mc_AssembleAdaptiveGraph(CtrlType* ctrl,
                                    GraphType* graph,
                                    WorkSpaceType* wspace)
{
  ...
  MPI_Allgather(
    (void*)(&mysize), 1, MPI_INT, (void*)rcounts, 1, MPI_INT, ctrl->comm);
  ...
  mygraph =
    (mysize <= wspace->maxcore ? wspace->core :
                                 idxmalloc(mysize, "AssembleGraph: mygraph"));
  for (k = i = 0; i < nvtxs; i++)
  {
    mygraph[k++] = xadj[i + 1] - xadj[i];
    for (j = 0; j < ncon; j++) mygraph[k++] = vwgt[i * ncon + j];
  ...
  ggraph = (gsize <= wspace->maxcore - mysize ?
              wspace->core + mysize :
              idxmalloc(gsize, "AssembleGraph: ggraph"));
  MPI_Allgatherv((void*)mygraph,
                 mysize,
                 IDX_DATATYPE,
                 (void*)ggraph,
                 rcounts,
  ...
  GKfree((void**)&rcounts, (void**)&rdispls, LTERM);
  if (mysize > wspace->maxcore) free(mygraph);
  ...
  {
    axadj[i] = ggraph[k++];
    for (l = 0; l < ncon; l++) avwgt[i * ncon + l] = ggraph[k++];
    if (ctrl->partType == ADAPTIVE_PARTITION ||
        ctrl->partType == REFINE_PARTITION)
      avsize[i] = ggraph[k++];
    for (l = 0; l < axadj[i]; l++)
  ...
  for (i = 0; i < gnvtxs; i++)
    for (j = 0; j < ncon; j++)
      anvwgt[i * ncon + j] =
        (float)(agraph->vwgt[i * ncon + j]) / (float)(ctrl->tvwgts[j]);

  for (i = 0; i < gnvtxs; i++) alabel[i] = i;

  if (gsize > wspace->maxcore - mysize) free(ggraph);
