...
{
  int i, j, k, islocal, penum, gnvtxs, nvtxs, nlocal, firstvtx, lastvtx, nsend,
    nrecv, nnbrs, nadj;
  ...
  idxcopy(nrecv,
          receive,
          recvind); /* Copy the vertices to be received into recvind */
  ...
  for (penum = 0; penum < npes; penum++)
  {
    for (j = i; j < nrecv; j++)
    {
      if (recvind[j] >= vtxdist[penum + 1]) break;
    }
    if (j > i)
    {
      peind[nnbrs] = penum;
  ...
  MPI_Alltoall((void*)recvrequests,
               2,
               IDX_DATATYPE,
               (void*)sendrequests,
               2,
  ...
  for (i = 0; i < nnbrs; i++)
  {
    MPI_Irecv((void*)(sendind + sendptr[i]),
  ...
  for (i = 0; i < nnbrs; i++)
  {
    MPI_Isend((void*)(recvind + recvptr[i]),
  ...
  {
    ASSERTP(ctrl,
            sendind[i] >= firstvtx && sendind[i] < lastvtx,
  ...
  for (i = nvtxs; i > 0; i--) pexadj[i] = pexadj[i - 1];
  pexadj[0] = 0;
  ...
  for (i = 0; i < nvtxs; i++) imap[i] = firstvtx + i;
  for (i = 0; i < nrecv; i++) imap[nvtxs + i] = recvind[i];
  ...
  if (wspace->nlarge < nrecv + nsend)
  {
    ...
    wspace->pairs = (KeyValueType*)GKmalloc(
      sizeof(KeyValueType) * wspace->nlarge, "SetUp: wspace->pairs");
  }
  ...
  rprintf(ctrl,
          ...
          GlobalSEMin(ctrl, nvtxs),
          GlobalSEMax(ctrl, nvtxs),
          GlobalSEMin(ctrl, nlocal),
          GlobalSEMax(ctrl, nlocal),
          GlobalSEMin(ctrl, nsend),
          GlobalSEMax(ctrl, nsend),
          GlobalSEMin(ctrl, nrecv),
          GlobalSEMax(ctrl, nrecv));
