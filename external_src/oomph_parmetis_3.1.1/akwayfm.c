 ...
 **************************************************************************/
void Mc_KWayAdaptiveRefine(CtrlType* ctrl,
                           GraphType* graph,
                           ...
                           int npasses)
{
  ...
  tmp_rinfo =
    (RInfoType*)GKmalloc(sizeof(RInfoType) * nvtxs, "AKWR: tmp_rinfo");
  tmp_edegrees =
    (EdgeType*)GKmalloc(sizeof(EdgeType) * nedges, "AKWR: tmp_edegrees");
  ...
  {
    oldcut = graph->mincut;
    if (mype == 0) RandomPermute(nparts, pperm, 1);
    ...
    ndirty = 0;
    for (i = 0; i < nvtxs; i++)
      if (where[i] != mype) ndirty++;

    dptr = 0;
    for (i = 0; i < nvtxs; i++)
      if (where[i] != mype) perm[dptr++] = i;
      else
        perm[ndirty++] = i;
    ...
    for (c = 0; c < 2; c++)
    {
      ...
      for (iii = 0; iii < nvtxs; iii++)
      {
        ...
        for (h = 0; h < ncon; h++)
        {
          if (fabs(nvwgt[h] - gnpwgts[from * ncon + h]) < SMALLFLOAT) break;
        }
        if (h < ncon) continue;

        /* only check border vertices */
        if (tmp_rinfo[i].ed <= 0) continue;
        ...
        for (k = 0; k < tmp_rinfo[i].ndegrees; k++)
        {
          ...
          if (ProperSide(c, pperm[from], pperm[to]))
          {
            for (h = 0; h < ncon; h++)
            {
              if (gnpwgts[to * ncon + h] + nvwgt[h] >
                    badmaxpwgt[to * ncon + h] &&
                  nvwgt[h] > 0.0)
                break;
            }
            if (h == ncon) break;
        ...
        if (k < tmp_rinfo[i].ndegrees)
        {
          ...
          for (j = k + 1; j < tmp_rinfo[i].ndegrees; j++)
          {
            ...
            if (ProperSide(c, pperm[from], pperm[to]))
            {
              ...
              gain =
                ipc_factor * (float)(my_edegrees[j].ewgt - tmp_rinfo[i].id);
          ...
          if (gain > 0.0 || (gain > -1.0 * SMALLFLOAT &&
                                                ...
                                                ubvec)))
          {
            ...
            {
              INC_DEC(
                lnpwgts[to * ncon + h], lnpwgts[from * ncon + h], nvwgt[h]);
              INC_DEC(
                gnpwgts[to * ncon + h], gnpwgts[from * ncon + h], nvwgt[h]);
              INC_DEC(
                movewgts[to * ncon + h], movewgts[from * ncon + h], nvwgt[h]);
            }
              ...
              if (mydomain != from)
              {
                for (k = 0; k < myrinfo->ndegrees; k++)
                {
                  if (your_edegrees[k].edge == from)
                  {
                    ...
                    {
                      myrinfo->ndegrees--;
                      your_edegrees[k].edge =
                        your_edegrees[myrinfo->ndegrees].edge;
                      your_edegrees[k].ewgt =
                        your_edegrees[myrinfo->ndegrees].ewgt;
                    }
                    else
                    {
      ...
      for (j = 0; j < nparts; j++)
      {
        for (h = 0; h < ncon; h++)
        {
          ...
          if (overfill[j * ncon + h] > 0.0) overweight = 1;

          ASSERTP(ctrl,
                  ognpwgts[j * ncon + h] <= badmaxpwgt[j * ncon + h] ||
                    pgnpwgts[j * ncon + h] <= ognpwgts[j * ncon + h],
                  (ctrl,
                   "%.4f %.4f %.4f\n",
                   ognpwgts[j * ncon + h],
      ...
      if (overweight == 1)
      {
        for (iii = 0; iii < nmoved; iii++)
        {
          ...
          for (k = 0; k < tmp_rinfo[i].ndegrees; k++)
            if (my_edegrees[k].edge == where[i]) break;
          ...
          if (k != tmp_rinfo[i].ndegrees && h != ncon)
          {
            ...
            for (h = 0; h < ncon; h++)
              overfill[oldto * ncon + h] =
                amax(overfill[oldto * ncon + h] - nvwgt[h], 0.0);
  ...
  GKfree((void**)&badmaxpwgt,
         ...
         LTERM);
  GKfree(
    (void**)&changed, (void**)&pperm, (void**)&perm, (void**)&moved, LTERM);
