                           ...
                           MPI_Comm* comm)
{
  ...
  MPI_Scatterv((void*)iperm,
               sendcount,
               displs,
               IDX_DATATYPE,
               (void*)order,
               vtxdist[mype + 1] - vtxdist[mype],
               IDX_DATATYPE,
               0,
               ctrl.gcomm);
                               ...
                               idxtype* adjncy)
{
  ...
  for (i = 0; i < nvtxs; i++) xadj[i] = xadj[i + 1] - xadj[i];
  ...
  MPI_Allgather(
    (void*)(&nedges), 1, MPI_INT, (void*)recvcounts, 1, MPI_INT, ctrl->comm);

  displs[0] = 0;
  for (i = 1; i < npes + 1; i++) displs[i] = displs[i - 1] + recvcounts[i - 1];
  gnedges = displs[npes];

  aadjncy = idxmalloc(gnedges, "AssembleEntireGraph: aadjncy");

  /* Assemble the xadj and then the adjncy */
  MPI_Gatherv((void*)adjncy,
              nedges,
              IDX_DATATYPE,
              aadjncy,
              recvcounts,
