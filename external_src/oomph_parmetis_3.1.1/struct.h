...
struct workspacedef
{
  ...
  idxtype *pv1, *pv2, *pv3,
    *pv4; /* Vectors of npes+1 size used in various places */
...
struct graphdef
{
  ...
  idxtype* lperm; /* lperm[0:nlocal] points to interior vertices, the rest are
                     interface */
   ...
   */
  idxtype *rlens, *slens; /* Arrays of size nnbrs of how many vertices you are
...
struct controldef
{
  ...
  MPI_Request sreq[MAX_PES], rreq[MAX_PES]; /* MPI send and receive requests */
  MPI_Status statuses[MAX_PES];
  MPI_Status status;

  /* Various Timers */
  timer TotalTmr, InitPartTmr, MatchTmr, ContractTmr, CoarsenTmr, RefTmr,
    SetupTmr, ColorTmr, ProjectTmr, KWayInitTmr, KWayTmr, MoveTmr, RemapTmr,
    AuxTmr1, AuxTmr2, AuxTmr3, AuxTmr4, AuxTmr5, AuxTmr6;
};
