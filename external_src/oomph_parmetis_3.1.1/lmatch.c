...
void Mc_LocalMatch_HEM(CtrlType* ctrl, GraphType* graph, WorkSpaceType* wspace)
{
  ...
  idxset(
    graph->nrecv, 0, match + nvtxs); /* Easy way to handle remote vertices */
  ...
  for (ii = 0; ii < nvtxs; ii++)
  {
    ...
    if (match[i] == UNMATCHED)
    {
      ...
      for (h = 0; h < ncon; h++)
        if (nvwgt[i * ncon + h] > maxnvwgt) break;
      ...
      if (h == ncon && ii < nvtxs)
      {
        for (j = xadj[i]; j < xadj[i + 1]; j++)
        {
          ...
          if (myhome[edge] != myhome[i] || edge >= nvtxs) continue;

          for (h = 0; h < ncon; h++)
            if (nvwgt[edge * ncon + h] > maxnvwgt) break;
                                ...
                                int cnvtxs)
{
  ...
  for (i = 0; i < nvtxs; i++)
  {
    ...
    if (v > u) continue; /* I have already collapsed it as (u,v) */
    ...
    if (ctrl->partType == ADAPTIVE_PARTITION ||
        ctrl->partType == REFINE_PARTITION)
      chome[cnvtxs] = home[i];
    if (vsize != NULL) cvsize[cnvtxs] = vsize[i];
    ...
    {
      u -= firstvtx;
      for (h = 0; h < ncon; h++) cvwgt[cnvtxs * ncon + h] += vwgt[u * ncon + h];
      if (vsize != NULL) cvsize[cnvtxs] += vsize[u];
      if (where != NULL && cwhere[cnvtxs] != where[u])
        myprintf(ctrl,
                 "Something went wrong with the where local matching! %d %d\n",
                 cwhere[cnvtxs],
                 where[u]);
