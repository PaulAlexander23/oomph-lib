...
int sfamax(int n, float* x)
{
  ...
  for (i = 1; i < n; i++) max = (fabs(x[i]) > fabs(x[max]) ? i : max);
...
int samin_strd(int n, float* x, int incx)
{
  ...
  n *= incx;
  for (i = incx; i < n; i += incx) min = (x[i] < x[min] ? i : min);
...
float idxavg(int n, idxtype* x)
{
  ...
  for (i = 0; i < n; i++) retval += (float)(x[i]);
...
float BetterVBalance(int ncon, float* vwgt, float* u1wgt, float* u2wgt)
{
  ...
  if (ncon == 1) return u1wgt[0] - u1wgt[0];
...
int IsHBalanceBetterFT(
  int ncon, float* pfrom, float* pto, float* nvwgt, float* ubvec)
{
  int i;
...
int IsHBalanceBetterTT(
  int ncon, float* pt1, float* pt2, float* nvwgt, float* ubvec)
{
  int i;
...
void GetThreeMax(int n, float* x, int* first, int* second, int* third)
{
  ...
  *first = 0;
  ...
  for (i = 1; i < n; i++)
  {
    ...
    if (*third == -1 || x[i] > x[*third]) *third = i;
  }
