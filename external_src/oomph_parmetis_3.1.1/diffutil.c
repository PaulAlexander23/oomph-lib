...
void SetUpConnectGraph(GraphType* graph, MatrixType* matrix, idxtype* workspace)
{
  ...
  for (i = 0; i < nvtxs; i++) pcounts[where[i]]++;
  MAKECSR(i, nrows, pcounts);

  for (i = 0; i < nvtxs; i++) perm[pcounts[where[i]]++] = i;

  for (i = nrows; i > 0; i--) pcounts[i] = pcounts[i - 1];
  pcounts[0] = 0;
 ...
 * schemes
 **************************************************************************/
void Mc_ComputeMoveStatistics(
  CtrlType* ctrl, GraphType* graph, int* nmoved, int* maxin, int* maxout)
{
  ...
  MPI_Allreduce(
    (void*)lstart, (void*)gstart, nparts, IDX_DATATYPE, MPI_SUM, ctrl->comm);
  MPI_Allreduce(
    (void*)lleft, (void*)gleft, nparts, IDX_DATATYPE, MPI_SUM, ctrl->comm);
  MPI_Allreduce(
    (void*)lend, (void*)gend, nparts, IDX_DATATYPE, MPI_SUM, ctrl->comm);

  *nmoved = idxsum(nparts, gleft);
  *maxout = gleft[idxamax(nparts, gleft)];
  for (i = 0; i < nparts; i++) lstart[i] = gend[i] + gleft[i] - gstart[i];
...
int Mc_ComputeSerialTotalV(GraphType* graph, idxtype* home)
{
  ...
  for (i = 0; i < graph->nvtxs; i++)
  {
    if (graph->where[i] != home[i])
      totalv +=
        (graph->vsize == NULL) ? graph->vwgt[i * graph->ncon] : graph->vsize[i];
  }
 ...
 **************************************************************************/
void ComputeLoad(
  GraphType* graph, int nparts, float* load, float* tpwgts, int index)
{
  int i;
  int nvtxs, ncon;
  idxtype* where;
  ...
  for (i = 0; i < nvtxs; i++) load[where[i]] += nvwgt[i * ncon + index];
...
void ConjGrad2(MatrixType* A, float* b, float* x, float tol, float* workspace)
{
  ...
  {
    x[i] = 0.0;
    if (values[rowptr[i]] != 0.0) M[i] = 1.0 / values[rowptr[i]];
    else
      M[i] = 0.0;
  }
 ...
 **************************************************************************/
void ComputeTransferVector(
  int ncon, MatrixType* matrix, float* solution, float* transfer, int index)
{
  int j, k;
  int nrows;
  idxtype *rowptr, *colind;
