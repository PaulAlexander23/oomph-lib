...
/*!
  ...
  The only refinement-related information that is has is the \c graph->where
  vector and allocates the memory for the remaining of the refinement-related
  data-structures.

*/
/************************************************************************************/
void AllocateNodePartitionParams(CtrlType* ctrl,
                                 GraphType* graph,
                                 WorkSpaceType* wspace)
{
  ...
  graph->nrinfo = (NRInfoType*)GKmalloc(sizeof(NRInfoType) * nvtxs,
  ...
  vwgt = graph->vwgt;
  graph->vwgt =
    idxmalloc(nvtxs + graph->nrecv, "AllocateNodePartitionParams: graph->vwgt");
                                ...
                                WorkSpaceType* wspace)
{
  ...
  for (nsep = i = 0; i < nvtxs; i++)
  {
    ...
    if (me >= nparts)
    { /* If it is a separator vertex */
      sepind[nsep++] = i;
      lpwgts[2 * nparts - 1] +=
        vwgt[i]; /* Keep track of total separator weight */
      ...
      for (j = xadj[i]; j < xadj[i + 1]; j++)
      {
        ...
        if (me != other) myrinfo->edegrees[other % 2] += vwgt[adjncy[j]];
      }
    }
  }
  graph->nsep = nsep;
...
/*!
  ...
  partitions is disconnected from each other due to the removal of the previous
  level separators.
                    ...
                    float ubfraction)
{
  ...
  for (i = 0; i < nparts; i += 2)
  {
    badminpwgt[i] = badminpwgt[i + 1] =
      (1.0 / ubfraction) * (gpwgts[i] + gpwgts[i + 1]) / 2;
    badmaxpwgt[i] = badmaxpwgt[i + 1] =
      ubfraction * (gpwgts[i] + gpwgts[i + 1]) / 2;
  }
  // myprintf(ctrl, "%6d %6d %6d %6d %6d %6d %6d\n", lpwgts[0], lpwgts[1],
  // lpwgts[2], gpwgts[0], gpwgts[1], gpwgts[2], badmaxpwgt[0]);
  ...
  for (pass = 0; pass < npasses; pass++)
  {
    ...
    for (c = 0; c < 2; c++)
    {
      ...
      for (ii = 0; ii < nsep; ii++)
      {
        ...
        /* Go through the loop to see if gain is possible for the separator
         * vertex */
        if (rinfo[i].edegrees[(c + 1) % 2] <= vwgt[i])
        {
          ...
          /* myprintf(ctrl, "Vertex %d [%d %d] is moving to %d from %d [%d]\n",
                  i+firstvtx, vwgt[i], rinfo[i].edegrees[(c+1)%2], to, from,
             where[i]); */
          ...
          /* Put the vertices adjacent to i that belong to either the separator
             or the (c+1)%2 partition into the update array */
          for (j = xadj[i]; j < xadj[i + 1]; j++)
          {
            k = adjncy[j];
      ...
      for (i = 0; i < nnbrs; i++)
      {
        MPI_Irecv((void*)(rupdate + sendptr[i]),
                  sendptr[i + 1] - sendptr[i],
                  IDX_DATATYPE,
                  peind[i],
                  1,
                  ctrl->comm,
      ...
      /* OK, now get into the loop waiting for the send/recv operations to
       * finish */
      ...
      for (ii = 0; ii < nlupd; ii++)
      {
        ...
        { /* This vertex is pulled into the separator */
          ...
          lpwgts[2 * nparts - 1] += vwgt[i];
          /* myprintf(ctrl, "Vertex %d moves into the separator from %d to
      ...
      IFSET(
        ctrl->dbglvl,
        DBG_REFINEINFO,
        PrintNodeBalanceInfo(ctrl, nparts, gpwgts, badminpwgt, badmaxpwgt, 0));
    }
