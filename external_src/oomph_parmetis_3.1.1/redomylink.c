                ...
                float* sr_lbavg)
{
  ...
  /*****************************/
  bestcost = (float)idxsum(nedges, adjwgt) * ipc_factor +
             (float)idxsum(nvtxs, vsize) * redist_factor;
  best_lbavg = 10.0;
  ...
  for (pass = N_MOC_REDO_PASSES; pass > 0; pass--)
  {
    ...
    for (i = 0; i < nvtxs; i++) where[i] = (where[i] == 0) ? me : you;

    for (i = 0; i < ncon; i++)
    {
      ftmp = (pwgts[i] + pwgts[ncon + i]) / 2.0;
      if (ftmp != 0.0) lbvec[i] = fabs(npwgts[i] - tpwgts[i]) / ftmp;
      else
        lbvec[i] = 0.0;
    }
    ...
    mycost =
      (float)(graph->mincut) * ipc_factor + (float)totalv * redist_factor;
