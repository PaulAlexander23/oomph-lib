 ...
 **************************************************************************/
void Mc_DynamicSelectQueue(int nqueues,
                           int ncon,
                           int subdomain1,
                           int subdomain2,
                           idxtype* currentq,
                           ...
                           float maxdiff)
{
  ...
  *qnum = -1;
  ...
  current = 0;
  for (i = 0; i < ncon - 1; i++)
    if (array[i + 1].val - array[i].val < maxdiff * MC_FLOW_BALANCE_THRESHOLD &&
        dont_cares[current] < ncon - 1)
    {
      dont_cares[current]++;
      dont_cares[i + 1] = 0;
    }
  ...
  switch (ncon)
  {
    ...
    case 4:
      ...
      if (dont_cares[0] == 1 && dont_cares[1] == 1 && dont_cares[2] == 2 &&
          dont_cares[3] == 0)
      {
        nperms = 14;
        perm[0][0] = 0;
      ...
      if (dont_cares[0] == 2 && dont_cares[1] == 0 && dont_cares[2] == 2 &&
          dont_cares[3] == 0)
      {
        nperms = 14;
        perm[0][0] = 0;
      ...
      if (dont_cares[0] == 3 && dont_cares[1] == 0 && dont_cares[2] == 0 &&
          dont_cares[3] == 1)
      {
        nperms = 14;
        perm[0][0] = 0;
      ...
      if (dont_cares[0] == 1 && dont_cares[1] == 3 && dont_cares[2] == 0 &&
          dont_cares[3] == 0)
      {
        nperms = 14;
        perm[0][0] = 0;
  ...
  for (i = 0; i < nperms; i++)
  {
    for (j = 0; j < ncon; j++) cand[j] = array[perm[i][j]].key;

    for (j = 0; j < ncon; j++) rank[cand[j]] = j;
...
int Mc_HashVwgts(int ncon, float* nvwgt)
{
  ...
  qsort(array, ncon, sizeof(KVType), myvalkeycompare);
  for (i = 0; i < ncon; i++) rank[array[i].key] = i;
