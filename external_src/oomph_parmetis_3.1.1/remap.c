...
void ParallelReMapGraph(CtrlType* ctrl, GraphType* graph, WorkSpaceType* wspace)
{
  ...
  for (i = 0; i < nvtxs; i++) where[i] = map[where[i]];
                         ...
                         int ncon)
{
  ...
  for (pass = 0; pass < npasses; pass++)
  {
    ...
    MPI_Allgather(
      (void*)&send, 2, IDX_DATATYPE, (void*)recv, 2, IDX_DATATYPE, ctrl->comm);

    ikeysort(nparts, recv);
    if (recv[0].key == 0) break;
    ...
    for (ii = 0; ii < nparts; ii++)
    {
      ...
      if (map[j] == -1 && rowmap[k] == -1 &&
          SimilarTpwgts(ctrl->tpwgts, ncon, j, k))
      {
        map[j] = k;
        rowmap[k] = j;
        nmapped++;
  ...
  if (nmapped < nparts)
  {
    ...
    IFSET(ctrl->dbglvl,
          DBG_REMAP,
          rprintf(ctrl, "Savings from parallel remapping: %0\n"));
  }
  else
  {
    /* check for a savings */
    ...
    IFSET(
      ctrl->dbglvl,
      DBG_REMAP,
      rprintf(ctrl, "Savings from parallel remapping: %d\n", amax(0, gnsaved)));
  }
...
int SimilarTpwgts(float* tpwgts, int ncon, int s1, int s2)
{
  ...
  for (i = 0; i < ncon; i++)
    if (fabs(tpwgts[s1 * ncon + i] - tpwgts[s2 * ncon + i]) > SMALLFLOAT) break;
