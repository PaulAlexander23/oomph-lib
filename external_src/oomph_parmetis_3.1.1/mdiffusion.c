 ...
 * serially.
 **************************************************************************/
int Mc_Diffusion(CtrlType* ctrl,
                 GraphType* graph,
                 idxtype* vtxdist,
                 idxtype* where,
                 idxtype* home,
                 ...
                 int npasses)
{
  ...
  wsize = amax(sizeof(float) * nparts * 6,
               sizeof(idxtype) * (nvtxs + nparts * 2 + 1));
  workspace = (float*)GKmalloc(wsize, "Mc_Diffusion: workspace");
  degrees = GKmalloc(nedges * sizeof(EdgeType), "Mc_Diffusion: degrees");
  rinfo = graph->rinfo =
    GKmalloc(nvtxs * sizeof(RInfoType), "Mc_Diffusion: rinfo");
  ...
  for (pass = 0; pass < npasses; pass++)
  {
    ...
    for (h = 0; h < ncon; h++)
    {
      ...
      lbvec[h] =
        (load[samax(nparts, load)] + 1.0 / (float)nparts) * (float)nparts;
    ...
    while (nvisited < nlinks)
    {
      ...
      rdispl[0] = 0;
      for (i = 1; i < npes + 1; i++) rdispl[i] = rdispl[i - 1] + rcount[i - 1];
      ...
      for (i = npes; i > 0; i--) rdispl[i] = rdispl[i - 1];
      rdispl[0] = 0;
      ...
      if (proc2sub[mype * 2] != UNMATCHED)
      {
        ...
        for (i = 0; i < nvtxs; i++)
          if (where[i] == me || where[i] == you) sbuffer[i] = 1;
        ...
        if (egraph != NULL)
        {
          ...
          for (j = 0; j < egraph->nvtxs; j++) ehome[j] = home[map[j]];

          RedoMyLink(
            ctrl, egraph, ehome, me, you, sr_flows, &sr_cost, &sr_lbavg);
          ...
          if (ncon <= 4)
          {
            ...
            {
              for (i = 0; i < egraph->nvtxs; i++) where[map[i]] = sr_where[i];
            }
            else
            {
              for (i = 0; i < egraph->nvtxs; i++) where[map[i]] = diff_where[i];
            }
          }
          else
          {
      ...
      /***************************/
      MPI_Allgatherv((void*)&sbuffer[rdispl[mype]],
                     rcount[mype],
                     IDX_DATATYPE,
                     (void*)rbuffer,
                     rcount,
    ...
    {
      lbvec[h] =
        (float)(nparts)*npwgts[samax_strd(nparts, npwgts + h, ncon) * ncon + h];
    }
    lbavg = savg(ncon, lbvec);

    done = 0;
    if (tnswaps == 0 || lbavg >= oldlbavg || lbavg <= ubavg + 0.035) done = 1;
  ...
  me); you = idxamax(nparts, match); for (i=0; i<nvtxs; i++) { if (where[i] ==
  you) { where[i] = me; match[you]--; match[me]++; done = 0; break;
          }
        }
      }
    }
  */
  ...
  GKfree((void**)&load,
         ...
         LTERM);
  GKfree(
    (void**)&pack, (void**)&workspace, (void**)&degrees, (void**)&rinfo, LTERM);
                        ...
                        idxtype* rmap)
{
  ...
  for (i = 0; i < envtxs; i++)
  {
    ...
    ewhere[i] = where[vtx];
    for (h = 0; h < ncon; h++) envwgt[i * ncon + h] = nvwgt[vtx * ncon + h];
    ...
    for (j = xadj[vtx]; j < xadj[vtx + 1]; j++)
      if (indicator[adjncy[j]] == 1) exadj[i]++;
  }
  ...
  for (i = envtxs; i > 0; i--) exadj[i] = exadj[i - 1];
  exadj[0] = 0;
