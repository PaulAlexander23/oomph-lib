 ...
 **************************************************************************/
void Mc_SerialKWayAdaptRefine(
  GraphType* graph, int nparts, idxtype* home, float* orgubvec, int npasses)
{
  int i, ii, iii, j, k;
  ...
  for (i = 0; i < ncon; i++) ubvec[i] = amax(ubvec[i], orgubvec[i]);
  ...
  for (pass = 0; pass < npasses; pass++)
  {
    ...
    for (iii = 0; iii < nvtxs; iii++)
    {
      ...
      if (myrinfo->ed >= myrinfo->id)
      {
        ...
        if (myrinfo->id > 0 && AreAllHVwgtsBelow(ncon,
                                                 1.0,
                                                 npwgts + from * ncon,
                                                 -1.0,
                                                 nvwgt,
        ...
        for (k = 0; k < myndegrees; k++)
        {
          ...
          if (gain >= 0 &&
              (AreAllHVwgtsBelow(ncon,
                                 ...
                                 maxwgt + to * ncon) ||
               IsHBalanceBetterFT(
                 ncon, npwgts + from * ncon, npwgts + to * ncon, nvwgt, ubvec)))
          {
            break;
          }
        }
        ...
        for (j = k + 1; j < myndegrees; j++)
        {
          ...
          fit_in_to = AreAllHVwgtsBelow(
            ncon, 1.0, npwgts + to * ncon, 1.0, nvwgt, maxwgt + to * ncon);
          better_balance_ft = IsHBalanceBetterFT(
            ncon, npwgts + from * ncon, npwgts + to * ncon, nvwgt, ubvec);
          better_balance_tt =
            IsHBalanceBetterTT(ncon,
                               npwgts + mydegrees[k].edge * ncon,
                               npwgts + to * ncon,
                               nvwgt,
                               ubvec);

          if ((gain_is_greater && (fit_in_to || better_balance_ft)) ||
              (gain_is_same &&
               ((fit_in_to && going_home) || better_balance_tt)))
          {
            k = j;
          }
        }
        ...
        fit_in_from = AreAllHVwgtsBelow(ncon,
                                        1.0,
                                        npwgts + from * ncon,
                                        0.0,
                                        npwgts + from * ncon,
        ...
        if (zero_gain && !going_home && !better_balance_ft && fit_in_from)
          continue;
 ...
 **************************************************************************/
void Mc_ComputeSerialPartitionParams(GraphType* graph,
                                     int nparts,
                                     EdgeType* degrees)
{
  int i, j, k;
 ...
 **************************************************************************/
int AreAllHVwgtsBelow(
  int ncon, float alpha, float* vwgt1, float beta, float* vwgt2, float* limit)
{
  int i;

  for (i = 0; i < ncon; i++)
    if (alpha * vwgt1[i] + beta * vwgt2[i] > limit[i]) return 0;
                               ...
                               float* lbvec)
{
  ...
  for (i = 0; i < ncon; i++)
  {
    ...
    for (j = 0; j < nparts; j++)
    {
      if (npwgts[j * ncon + i] > max) max = npwgts[j * ncon + i];
    }
                 ...
                 float* tpwgts)
{
  ...
  sortvtx =
    (KeyKeyValueType*)GKmalloc(nvtxs * sizeof(KeyKeyValueType), "sortvtx");
  flowto = (KeyValueType*)GKmalloc(
    (nparts * max_mult + nparts) * sizeof(KeyValueType), "flowto");
  ...
  for (ii = 0; ii < nvtxs; ii++)
  {
    ...
    if (from > current_from)
    {
      ...
      for (j = 0; j < smallcount; j++) htable[flowto[j].val] = -1;
  ...
  for (ii = 0; ii < bigcount; ii++)
  {
    ...
    if (map[j] == -1 && rowmap[k] == -1 &&
        SimilarTpwgts(tpwgts, graph->ncon, j, k))
    {
      map[j] = k;
      rowmap[k] = j;
      nmapped++;
  ...
  if (nmapped < nparts)
    for (i = 0; i < nparts; i++) map[i] = i;

  for (i = 0; i < nvtxs; i++) remap[i] = map[remap[i]];
...
void Mc_Serial_FM_2WayRefine(GraphType* graph, float* tpwgts, int npasses)
{
  ...
  for (i = 0; i < nvtxs; i++) qnum[i] = samax(ncon, nvwgt + i * ncon);
  ...
  { /* Do a number of passes */
    ...
    for (i = 0; i < ncon; i++) mindiff[i] = fabs(tpwgts[i] - npwgts[i]);
      ...
      for (j = xadj[higain]; j < xadj[higain + 1]; j++)
      {
        ...
        { /* If k was a boundary vertex */
          ...
          { /* If it has not been moved, update its position in the queue */
            if (moved[k] == -1)
              FPQueueUpdate(&parts[qnum[k]][where[k]],
                            k,
                            (float)oldgain,
                            (float)(ed[k] - id[k]));
          }
        ...
        else
        {
          ...
          { /* It will now become a boundary vertex */
            ...
            if (moved[k] == -1)
              FPQueueInsert(
                &parts[qnum[k]][where[k]], k, (float)(ed[k] - id[k]));
          }
        }
      }
    }
     ...
     *****************************************************************/
    for (i = 0; i < nswaps; i++) moved[swaps[i]] = -1; /* reset moved array */
    for (nswaps--; nswaps > mincutorder; nswaps--)
    {
      ...
      for (j = xadj[higain]; j < xadj[higain + 1]; j++)
      {
        ...
        if (bndptr[k] != -1 && ed[k] == 0) BNDDelete(nbnd, bndind, bndptr, k);
        if (bndptr[k] == -1 && ed[k] > 0) BNDInsert(nbnd, bndind, bndptr, k);
      }
    }
                        ...
                        FPQueueType queues[MAXNCON][2])
{
  ...
  *cnum = -1;
  ...
  /* First determine the side and the queue, irrespective of the presence of
   * nodes */
  ...
  if (*from != -1 && FPQueueGetQSize(&queues[*cnum][*from]) == 0)
  {
    ...
    for (i++; i < ncon; i++)
    {
      if (npwgts[(*from) * ncon + i] > max &&
          FPQueueGetQSize(&queues[i][*from]) > 0)
      {
        max = npwgts[(*from) * ncon + i];
        *cnum = i;
      }
...
int Serial_BetterBalance(int ncon, float* npwgts, float* tpwgts, float* diff)
{
  ...
  for (i = 0; i < ncon; i++) ndiff[i] = fabs(tpwgts[i] - npwgts[i]);
...
void Mc_Serial_Balance2Way(GraphType* graph, float* tpwgts, float lbfactor)
{
  ...
  for (from = 0; from < 2; from++)
  {
    for (j = 0; j < ncon; j++)
    {
      if (qsizes[j][from] == 0)
      {
        for (i = 0; i < nvtxs; i++)
        {
          if (where[i] != from) continue;

          k = samax2(ncon, nvwgt + i * ncon);
          if (k == j && qsizes[qnum[i]][from] > qsizes[j][from] &&
  ...
  for (nswaps = 0; nswaps < nvtxs; nswaps++)
  {
    if (minbal < lbfactor) break;

    Serial_SelectQueue(ncon, npwgts, tpwgts, &from, &cnum, parts);
    to = (from + 1) % 2;

    if (from == -1 || (higain = FPQueueGetMax(&parts[cnum][from])) == -1) break;
    ...
    for (j = xadj[higain]; j < xadj[higain + 1]; j++)
    {
      ...
      if (ed[k] == 0 && bndptr[k] != -1) BNDDelete(nbnd, bndind, bndptr, k);
  ...
  for (nswaps--; nswaps > mincutorder; nswaps--)
  {
    ...
    saxpy2(ncon, 1.0, nvwgt + higain * ncon, 1, npwgts + to * ncon, 1);
    saxpy2(
      ncon, -1.0, nvwgt + higain * ncon, 1, npwgts + ((to + 1) % 2) * ncon, 1);
    for (j = xadj[higain]; j < xadj[higain + 1]; j++)
    {
      k = adjncy[j];
...
void Mc_Serial_Init2WayBalance(GraphType* graph, float* tpwgts)
{
  ...
  for (nswaps = 0; nswaps < nvtxs; nswaps++)
  {
    if (Serial_AreAnyVwgtsBelow(
          ncon, 1.0, npwgts + from * ncon, 0.0, nvwgt, tpwgts + from * ncon))
      break;

    if ((cnum = Serial_SelectQueueOneWay(ncon, npwgts, tpwgts, from, parts)) ==
        -1)
      break;
    ...
    for (j = xadj[higain]; j < xadj[higain + 1]; j++)
    {
      ...
      if (where[k] == from)
      {
        ...
        { /* It moves in boundary */
          FPQueueDelete(&parts[qnum[k]][1], k);
          FPQueueInsert(&parts[qnum[k]][0], k, (float)(ed[k] - id[k]));
        }
        else
        { /* It must be in the boundary already */
          FPQueueUpdate(
            &parts[qnum[k]][0], k, (float)(oldgain), (float)(ed[k] - id[k]));
        }
      }
 ...
 * a given set of values
 **************************************************************************/
int Serial_AreAnyVwgtsBelow(
  int ncon, float alpha, float* vwgt1, float beta, float* vwgt2, float* limit)
{
  int i;

  for (i = 0; i < ncon; i++)
    if (alpha * vwgt1[i] + beta * vwgt2[i] < limit[i]) return 1;
