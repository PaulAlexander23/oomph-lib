...
// LIC//
// LIC//====================================================================
// Header file for Volume elements
...
namespace oomph
{
  ...
  class VolumeEquations : public virtual FiniteElement
  {
  public:
    /// Function pointer to source function fct(x,f(x)) --
    /// x is a Vector!
    typedef void (*VolumeSourceFctPt)(const Vector<double>& x, double& f);
    ...
    VolumeEquations() : Source_fct_pt(0), Source_fct_gradient_pt(0) {}

    /// Broken copy constructor
    VolumeEquations(const VolumeEquations& dummy) = delete;

    /// Broken assignment operator
    void operator=(const VolumeEquations&) = delete;
    ...
    virtual inline unsigned u_index_volume() const
    {
      return 0;
    }
    ...
    void point_output_data(const Vector<double>& s, Vector<double>& data)
    {
      ...
      data[dim] = this->interpolated_u_volume(s);
    }
                               ...
                               const unsigned& nplot) const
    {
      ...
      if (i != 0)
      {
        ...
        error_stream
          << "Volume elements only store a single field so i must be 0 rather"
          << " than " << i << std::endl;
        throw OomphLibError(
      ...
      for (unsigned j = 0; j < local_loop; j++)
      {
        ...
        file_out << this->interpolated_u_volume(s) << std::endl;
      }
    }
      ...
      FiniteElement::SteadyExactSolutionFctPt exact_soln_pt) const
    {
      ...
      {
        std::stringstream error_stream;
        error_stream << "Volume equation has only one field. Can't call "
    ...
    {
      throw OomphLibError(
        "There is no time-dependent output_fct() for Volume elements ",
    ...
    {
      throw OomphLibError(
        "There is no time-dependent compute_error() for Volume elements",
        OOMPH_CURRENT_FUNCTION,
        OOMPH_EXCEPTION_LOCATION);
    }

    /// Access function: Pointer to source function
    VolumeSourceFctPt& source_fct_pt()
    {
      return Source_fct_pt;
    }

    /// Access function: Pointer to source function. Const version
    VolumeSourceFctPt source_fct_pt() const
    {
      return Source_fct_pt;
    }

    /// Access function: Pointer to gradient of source function
    VolumeSourceFctGradientPt& source_fct_gradient_pt()
    {
      return Source_fct_gradient_pt;
    }

    /// Access function: Pointer to gradient source function. Const version
    VolumeSourceFctGradientPt source_fct_gradient_pt() const
    {
      return Source_fct_gradient_pt;
    }
    ...
    inline virtual void get_source_gradient_volume(
      const unsigned& ipt,
      const Vector<double>& x,
      Vector<double>& gradient) const
    {
      ...
      if (Source_fct_gradient_pt == 0)
      {
        // Reference value
        double source = 0.0;
        get_source_volume(ipt, x, source);
        ...
        {
          x_pls[i] += eps_fd;
          get_source_volume(ipt, x_pls, source_pls);
    ...
    void fill_in_contribution_to_residuals(Vector<double>& residuals)
    {
      ...
      fill_in_generic_residual_contribution_volume(
                                          ...
                                          DenseMatrix<double>& jacobian)
    {
      ...
      fill_in_generic_residual_contribution_volume(residuals, jacobian, 1);
    }


    /// Return FE representation of function value u_volume(s)
    /// at local coordinate s
    virtual inline double interpolated_u_volume(const Vector<double>& s) const
    {
      // Find number of nodes
      const unsigned n_node = nnode();

      // Get the index at which the volume unknown is stored
      const unsigned u_nodal_index = u_index_volume();
  ...
  protected:
    ...
    virtual double dshape_and_dtest_eulerian_at_knot_volume(
      const unsigned& ipt,
      Shape& psi,
      DShape& dpsidx,
      Shape& test,
    ...
    /// Jacobian matrix
    virtual void fill_in_generic_residual_contribution_volume(
  ...
  //======================================================================
  /// QVolumeElement elements are linear/quadrilateral/brick-shaped
  /// Volume elements with isoparametric interpolation for the function.
  //======================================================================
  ...
  public:
    ...
    /// Volume equations
    QVolumeElement() : QElement<DIM, NNODE_1D>(), VolumeEquations<DIM>() {}

    /// Broken copy constructor
    QVolumeElement(const QVolumeElement<DIM, NNODE_1D>& dummy) = delete;

    /// Broken assignment operator
    void operator=(const QVolumeElement<DIM, NNODE_1D>&) = delete;
    ...
    void output(std::ostream& outfile)
    {
      VolumeEquations<DIM>::output(outfile);
    }
    ...
    void output(std::ostream& outfile, const unsigned& n_plot)
    {
      VolumeEquations<DIM>::output(outfile, n_plot);
    }
    ...
    void output(FILE* file_pt)
    {
      VolumeEquations<DIM>::output(file_pt);
    }
    ...
    void output(FILE* file_pt, const unsigned& n_plot)
    {
      VolumeEquations<DIM>::output(file_pt, n_plot);
    }
    ...
    void output_fct(std::ostream& outfile,
                    ...
                    FiniteElement::SteadyExactSolutionFctPt exact_soln_pt)
    {
      VolumeEquations<DIM>::output_fct(outfile, n_plot, exact_soln_pt);
    }
    ...
    void output_fct(std::ostream& outfile,
                    ...
                    FiniteElement::UnsteadyExactSolutionFctPt exact_soln_pt)
    {
      VolumeEquations<DIM>::output_fct(outfile, n_plot, time, exact_soln_pt);
    }
  ...
  protected:
    ...
    inline double dshape_and_dtest_eulerian_at_knot_volume(
      const unsigned& ipt,
      Shape& psi,
      DShape& dpsidx,
      Shape& test,
  ...
  //======================================================================
  template<unsigned DIM, unsigned NNODE_1D>
  double QVolumeElement<DIM, NNODE_1D>::dshape_and_dtest_eulerian_volume(
  ...
  template<unsigned DIM, unsigned NNODE_1D>
  double QVolumeElement<DIM, NNODE_1D>::
    dshape_and_dtest_eulerian_at_knot_volume(
      const unsigned& ipt,
      Shape& psi,
      DShape& dpsidx,
      RankFourTensor<double>& d_dpsidx_dX,
  ...
  //=======================================================================
  /// Face geometry for the QVolumeElement elements: The spatial
  /// dimension of the face elements is one lower than that of the
  /// bulk element but they have the same number of points
  /// along their 1D edges.
  //=======================================================================
  template<unsigned DIM, unsigned NNODE_1D>
  class FaceGeometry<QVolumeElement<DIM, NNODE_1D>>
  ...
  //=======================================================================
  /// Face geometry for the 1D QVolumeElement elements: Point elements
  //=======================================================================
  template<unsigned NNODE_1D>
  class FaceGeometry<QVolumeElement<1, NNODE_1D>> : public virtual PointElement
  {
  public:
    /// Constructor: Call the constructor for the
    /// appropriate lower-dimensional QElement
  ...
  //==========================================================
  /// Volume upgraded to become projectable
  //==========================================================
  template<class POISSON_ELEMENT>
  class ProjectableVolumeElement
  ...
  public:
    ...
    Vector<std::pair<Data*, unsigned>> data_values_of_field(const unsigned& fld)
    {
      ...
      {
        std::stringstream error_stream;
        error_stream << "Volume elements only store a single field so fld "
                        "must be 0 rather"
                     << " than " << fld << std::endl;
        throw OomphLibError(
          error_stream.str(), OOMPH_CURRENT_FUNCTION, OOMPH_EXCEPTION_LOCATION);
                                       ...
                                       Shape& psi)
    {
      ...
      double J =
        this->dshape_and_dtest_eulerian_volume(s, psi, dpsidx, test, dtestdx);
      return J;
    }
                     ...
                     const Vector<double>& s)
    {
      ...
      unsigned u_nodal_index = this->u_index_volume();
...
#endif
      const unsigned u_nodal_index = this->u_index_volume();
  ...
  class FaceGeometry<ProjectableVolumeElement<ELEMENT>>
  ...
  //=======================================================================
  template<class ELEMENT>
  class FaceGeometry<FaceGeometry<ProjectableVolumeElement<ELEMENT>>>
