...
// LIC//
// LIC//====================================================================
// Non-inline functions for Volume elements
#include "volume_elements.h"
...
namespace oomph
{
  ...
  const unsigned QVolumeElement<DIM, NNODE_1D>::Initial_Nvalue = 1;
  ...
  void VolumeEquations<DIM>::fill_in_generic_residual_contribution_volume(
    ...
    const unsigned& flag)
  {
    ...
    // Index at which the volume unknown is stored
    const unsigned u_nodal_index = u_index_volume();
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      double J = dshape_and_dtest_eulerian_at_knot_volume(
      ...
      {
        // Get the nodal value of the volume unknown
      ...
      //-------------------
      double source;
      get_source_volume(ipt, interpolated_x, source);
  ...
  //======================================================================
  template<unsigned DIM>
  void VolumeEquations<DIM>::get_dresidual_dnodal_coordinates(
    RankThreeTensor<double>& dresidual_dnodal_coordinates)
  {
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      const double J = dshape_and_dtest_eulerian_at_knot_volume(
      ...
      get_source_gradient_volume(ipt, interpolated_x, d_source_dx);
  ...
  //======================================================================
  template<unsigned DIM>
  unsigned VolumeEquations<DIM>::self_test()
  {
    bool passed = true;
                                     ...
                                     const unsigned& nplot)
  {
    ...
    for (unsigned iplot = 0; iplot < num_plot_points; iplot++)
    {
      ...
      outfile << interpolated_u_volume(s) << std::endl;
    }
  ...
  //======================================================================
  template<unsigned DIM>
  void VolumeEquations<DIM>::output(FILE* file_pt, const unsigned& nplot)
  {
    // Vector of local coordinates
    Vector<double> s(DIM);
  ...
  //======================================================================
  template<unsigned DIM>
  void VolumeEquations<DIM>::output_fct(
    std::ostream& outfile,
    const unsigned& nplot,
  ...
  //=======================================================================
  template<unsigned DIM>
  void VolumeEquations<DIM>::compute_norm(double& norm)
  {
    // Initialise
    norm = 0.0;
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      u = this->interpolated_u_volume(s);
  ...
  //======================================================================
  template<unsigned DIM>
  void VolumeEquations<DIM>::compute_error(
    std::ostream& outfile,
    FiniteElement::SteadyExactSolutionFctPt exact_soln_pt,
    double& error,
    double& norm)
  {
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      // Get FE function value
      double u_fe = interpolated_u_volume(s);
  ...
  //====================================================================
  template class QVolumeElement<1, 2>;
  template class QVolumeElement<1, 3>;
  template class QVolumeElement<1, 4>;

  template class QVolumeElement<2, 2>;
  template class QVolumeElement<2, 3>;
  template class QVolumeElement<2, 4>;

  template class QVolumeElement<3, 2>;
  template class QVolumeElement<3, 3>;
  template class QVolumeElement<3, 4>;
