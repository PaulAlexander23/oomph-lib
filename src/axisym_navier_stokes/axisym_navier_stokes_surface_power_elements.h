...
#ifndef OOMPH_AXISYM_NAVIER_STOKES_SURFACE_POWER_ELEMENTS_HEADER
#define OOMPH_AXISYM_NAVIER_STOKES_SURFACE_POWER_ELEMENTS_HEADER
...
namespace oomph
{
  ...
  class AxisymmetricNavierStokesSurfacePowerElement
  ...
  public:
    ...
    /// and its limit
    AxisymmetricNavierStokesSurfacePowerElement(
    ...
    //    bulk_el_pt->interpolated_u_axi_nst(s_bulk, veloc);
    ...
    // void get_rate_of_traction_work_components(double&
    ...
    //    for (unsigned i = 0; i < ndim + 1; i++)
    //    {
    //      kin_energy += veloc[i] * veloc[i] * x_bulk[0];
    //    }
    //    kin_energy *= 0.5;
    ...
    double get_volume_flux(std::ofstream& outfile)
    {
      ...
      for (unsigned ipt = 0; ipt < n_intpt; ipt++)
      {
        ...
        // Add to integral after multipling by r
        volume_flux_integral += volume_flux * W * x[0];
